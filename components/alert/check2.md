...
- [x] `ui` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（`components/alert/src` 已按 `mod.rs/logic.rs/view.rs/styles.rs/motion.rs` 分责；`mod.rs` 仅导出稳定 API（`Alert/AlertTone/AlertFill/AlertLayout/AlertVariant/AlertMotion`），未暴露 `web-sys`/DOM 细节类型；语义测试已迁移到 `components/alert/test/semantics.rs`，`components/alert/test/alert_semantics.rs` 仅保留转发入口。）
...
- [x] API 命名契约统一：公共 props/回调严格使用 `is_*`、`on_*`、`default_*` 前缀；同语义在全库同名，禁止别名漂移。（Alert 布尔轴以 `is_hide_icon` 为主命名，并保留 `hide_icon` 兼容输入；兼容优先级由 `logic::resolve_hide_icon` 统一管理且输出 `data-hide-icon-source` 来源标记。回归：`components/alert/test/semantics.rs::alert_api_naming_contract_keeps_is_prefix_with_legacy_alias_bridge`。）
...
- [x] 受控/非受控必须成对：每个可控状态轴都提供 `value + on_value_change + default_value`（如 `open/on_open_change/default_open`）；缺一项即不通过。（N/A：Alert 为状态展示组件，不暴露可控状态轴（无 `open/value/selected` 等写回模型）；当前 API 全部是静态配置输入 + 语义输出，无“半受控”行为。回归：`components/alert/test/semantics.rs::alert_has_no_controlled_or_uncontrolled_state_axes`。）
...
- [x] 状态归一化集中：状态输入先类型化，再在 `logic.rs` 统一派生；禁止在 `view.rs`、事件回调、样式分支中分散拼状态机。（`AlertStateInput -> resolve_state -> AlertState` 归一链路集中在 `components/alert/src/logic.rs`，`view.rs` 仅调用 `normalize_optional_text/resolve_hide_icon/resolve_icon_label/resolve_state/compose_class_name` 并挂载语义标记，不拼装状态机规则。回归：`components/alert/test/semantics.rs::alert_defaults_and_state_normalization_stay_in_logic_layer`、`components/alert/test/semantics.rs::alert_view_derives_state_from_logic_layer`。）
  - 输入边界统一进入 `logic.rs`，输出统一为可渲染语义状态与来源标记。
  - 事件处理器只触发状态变更，不重建状态机规则。
  - 样式层只消费状态标记，不承担状态判定职责。
- [x] 离散状态必须类型约束：`variant/size/mode/status` 等离散输入使用 `enum`；禁止用多个 `Option<bool>`/字符串自由组合表达互斥状态。（Alert 离散轴均为枚举：`AlertTone`、`AlertFill`、`AlertLayout`、`AlertVariant`；字符串仅作为最终 `data-*` 输出，不参与输入态决策。回归：`components/alert/test/logic.rs::variant_maps_to_expected_tone`、`components/alert/test/semantics.rs::alert_view_accepts_tone_fill_layout_and_slot_props`。）
  - 互斥状态优先用 `enum` 建模，利用编译器封住无效组合。
  - 字符串输入若需兼容外部配置，必须先映射到类型化枚举再进入逻辑层。
  - 布尔爆炸（多个 bool 表达一个状态机）应在设计评审阶段直接拦截。
- [x] 状态原语来源正确：组件层只消费 `status-primitives`（当前 `ui-state-primitives`）能力，不直接绑定业务 store；应用级全局状态必须经桥接层适配后再接入组件。（`components/alert/src/logic.rs` 通过 `pub use ui_state_primitives::alert_banner::{...}` 直接消费状态原语并做装配映射，不依赖业务 store 类型；回归：`components/alert/test/semantics.rs::alert_logic_uses_notification_primitives_and_variant_mapping`。）
  - 组件中出现可复用状态机实现（受控/非受控、展开规则、选择归一）即判应下沉。
  - 组件与业务全局状态之间必须有适配边界，禁止组件直接依赖业务 store 类型。
  - `logic.rs` 仅做装配与映射，不重新实现状态原语。
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。（N/A：Alert 组件无远程请求与异步状态轴；实现中无 `spawn_local/create_resource/use_async_action` 之类异步协议接入。回归：`components/alert/test/semantics.rs::alert_has_no_async_loading_protocol`。）
...
- [x] API 易用性验收标准（DX Paradox）：把复杂性留在内部，把简单留给用户。（`Alert` 除 `children` 外均为 `#[prop(optional)]`，可直接最小调用；`apps/docs-app/src/pages/components/pages/display.rs::alert` 的 Hello World 现为 3 行 `<Alert>...</Alert>` 示例，无需手动接线 `ui-state-primitives/ui-headless`。回归：`components/alert/test/semantics.rs::docs_page_hello_world_stays_minimal_and_copy_paste_ready`。）
...
- [x] 组合型组件主 API 必须“显示优于约定”：优先使用显式组合 `<Parent><Item ... /></Parent>`。（N/A：Alert 非集合/容器型组合组件，无 `Item` 子语义树与并行数组输入面，不存在 `labels + children`、`titles + panels` 类隐式配对风险。）
...
- [x] 组件层遵循 token-first 静态样式契约：样式通过 `styles.rs` 聚合注入；运行时仅传必要 CSS 变量；不把 Utility-First/CSS-in-Rust 当组件库默认范式。（样式集中在 `components/alert/src/styles.rs` 静态 CSS，聚合路径为 `crates/ui/src/css.rs` 的 `component-alert` feature 分支；组件未引入 Utility-First/CSS-in-Rust 作为默认实现。）
...
- [x] 级联层覆盖（`@layer ui`）：组件 CSS 默认聚合进 `@layer ui`；运行时数值调整仅通过 CSS Custom Properties（如 `style:--x=...`），禁止普通内联样式（如 `style=\"top: 10px\"`）。（`crates/ui/src/css.rs` 通过 `push_components_css` 统一以 `@layer ui` 聚合并在 `component-alert` feature 下注入 `crate::alert::styles::CSS`；`components/alert/src/view.rs` 未使用普通 `style=...` 内联样式。回归：`components/alert/test/semantics.rs::alert_css_is_aggregated_under_ui_layer_without_plain_inline_styles`。）
...
- [x] `ui` 固定入口文件落点正确。（`crates/ui/src/lib.rs` 保持 `component-alert` feature gate + `pub use ui_alert as alert`；`crates/ui/src/css.rs` 通过 `push_components_css` 在 `@layer ui` 下按 feature 注入；`crates/ui/src/root.rs` 集中注入 base/theme/components css 并提供 i18n/id provider；`crates/ui-visual-primitive/src/active_highlight.rs` 仅承载共享高亮动效能力；`crates/ui/src/overlay_open.rs`、`presence.rs`、`a11y.rs` 不存在。回归：`components/alert/test/semantics.rs::alert_ui_components_entrypoint_layout_and_headless_boundaries_are_correct`。）
...
- [x] Tree Shaking & 特性剪裁：组件必须注册到 `ui` 特性树（如 `component-accordion`）；`css.rs` 和 `lib.rs` 聚合必须受 feature 门控，禁止无条件全局依赖。（`crates/ui/Cargo.toml` 已注册 `component-alert = ["dep:ui-alert"]`；`crates/ui/src/lib.rs` 与 `crates/ui/src/css.rs` 均在 `#[cfg(feature = "component-alert")]` 下导出/聚合，无无条件 Alert 依赖。验证：`cargo tree -e features -p ui --no-default-features --features component-alert,inject-css --depth 2` 下仅出现 Alert 相关组件链；回归：`components/alert/test/semantics.rs::alert_tree_shaking_contract_is_feature_gated_for_module_and_css`。）
