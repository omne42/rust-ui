...
- [x] API 命名契约统一：公共 props/回调严格使用 `is_*`、`on_*`、`default_*` 前缀；同语义在全库同名，禁止别名漂移。（Alert 布尔轴以 `is_hide_icon` 为主命名，并保留 `hide_icon` 兼容输入；兼容优先级由 `logic::resolve_hide_icon` 统一管理且输出 `data-hide-icon-source` 来源标记。回归：`crates/ui-components/tests/alert_semantics.rs::alert_api_naming_contract_keeps_is_prefix_with_legacy_alias_bridge`。）
...
- [x] API 易用性验收标准（DX Paradox）：把复杂性留在内部，把简单留给用户。（`Alert` 除 `children` 外均为 `#[prop(optional)]`，可直接最小调用；docs 提供默认路径 `apps/docs-app/src/pages/components/pages/display.rs::alert()` 的 “Hello World” 示例。回归：`crates/ui-components/tests/alert_semantics.rs::docs_page_uses_unified_alert_api`。）
