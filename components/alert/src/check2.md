...
- [x] `ui` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（`components/alert` 目录已按 `mod.rs/logic.rs/view.rs/styles.rs/motion.rs` 分责；`mod.rs` 仅导出稳定 API（`Alert/AlertTone/AlertFill/AlertLayout/AlertVariant/AlertMotion`），未暴露 `web-sys` DOM 细节类型；装配链路由 `logic.rs + view.rs` 完成；语义测试主文件迁移到 `components/alert/test/semantics.rs`，`components/alert/test/alert_semantics.rs` 仅保留转发入口。）
