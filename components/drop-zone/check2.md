...
- [x] 受控/非受控必须成对：每个可控状态轴都提供 `value + on_value_change + default_value`（如 `open/on_open_change/default_open`）；缺一项即不通过。
  ...
  - 回归覆盖：`components/drop-zone/test/semantics.rs::drop_zone_has_no_controlled_uncontrolled_state_axis` 与 `components/drop-zone/test/drop_zone_semantics.rs::drop_zone_has_no_controlled_uncontrolled_state_axis`。
- [x] 默认值单一来源：默认值与优先级只在 `logic.rs` 归一化；`view.rs` 禁止二次兜底或隐式改写。
  ...
  - 回归覆盖：`components/drop-zone/test/logic.rs::resolve_props_keeps_default_source_in_logic`、`components/drop-zone/test/semantics.rs::drop_zone_defaults_are_normalized_in_logic_only`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_defaults_are_normalized_in_logic_only`。
- [x] 状态归一化集中：状态输入先类型化，再在 `logic.rs` 统一派生；禁止在 `view.rs`、事件回调、样式分支中分散拼状态机。
  ...
  - 回归覆盖：`components/drop-zone/test/logic.rs::reduce_drag_interaction_derives_drop_target_state_in_logic`、`components/drop-zone/test/semantics.rs::drop_zone_state_normalization_is_centralized_in_logic`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_state_normalization_is_centralized_in_logic`。
- [x] 离散状态必须类型约束：`variant/size/mode/status` 等离散输入使用 `enum`；禁止用多个 `Option<bool>`/字符串自由组合表达互斥状态。
  ...
  - 回归覆盖：`components/drop-zone/test/logic.rs::resolve_aria_label_source_is_closed_enum_mapping`、`components/drop-zone/test/semantics.rs::drop_zone_discrete_state_axes_use_typed_enums`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_discrete_state_axes_use_typed_enums`。
- [x] 状态原语来源正确：组件层只消费 `status-primitives`（当前 `ui-state-primitives`）能力，不直接绑定业务 store；应用级全局状态必须经桥接层适配后再接入组件。
  ...
  - 回归覆盖：`components/drop-zone/test/semantics.rs::drop_zone_consumes_state_primitives_without_business_store_binding`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_consumes_state_primitives_without_business_store_binding`。
...
- [x] API 易用性验收标准（DX Paradox）：把复杂性留在内部，把简单留给用户。
  ...
  - 回归覆盖：`components/drop-zone/test/semantics.rs::drop_zone_dx_paradox_keeps_default_api_simple`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_docs_page_includes_dx_quick_start_default_api`。
- [x] 组合型组件主 API 必须“显示优于约定”：优先使用显式组合 `<Parent><Item ... /></Parent>`。
  ...
  - 回归覆盖：`components/drop-zone/test/semantics.rs::drop_zone_composite_parent_item_api_is_not_applicable`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_composite_parent_item_api_is_not_applicable`。
...
- [x] 几何两段式渲染（Two-Pass Rendering）：`Tooltip/Popover/Menu` 等依赖 DOM 测量的组件必须走 `Intent -> Measure(view) -> Rectification(logic)`，并具备幂等收敛保护防死循环。
  ...
  - 回归覆盖：`components/drop-zone/test/semantics.rs::drop_zone_two_pass_rendering_is_not_applicable_without_geometry_measurement`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_two_pass_rendering_is_not_applicable_without_geometry_measurement`。
- [x] 集合注册协议（Registration Protocol）：`Accordion/Tabs/Menu` 动态子项必须通过 `RegistrationContext` 上报 `Register/Unregister`，逻辑层维护 `items_order`，禁止依赖 `HashSet` 迭代顺序做导航。
  ...
  - 回归覆盖：`components/drop-zone/test/semantics.rs::drop_zone_registration_protocol_is_not_applicable_without_dynamic_item_collections`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_registration_protocol_is_not_applicable_without_dynamic_item_collections`。
- [x] 插槽投影策略（Slot Projection）：容器组件明确 `Lazy/KeepAlive/Eager`；`KeepAlive` 隐藏时必须通过生命周期通知（如 `NotifyHidden`）暂停轮询/动画等高耗能副作用。
  ...
  - 回归覆盖：`components/drop-zone/test/semantics.rs::drop_zone_slot_projection_strategy_is_not_applicable_without_multi_slot_container`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_slot_projection_strategy_is_not_applicable_without_multi_slot_container`。
- [x] 环境订阅流（Env Streams）：`Resize/Theme/Intersection` 等环境变化在 `view.rs` 采样、防抖后转化为高层语义 `Action`（如 `BreakpointChanged`）推送到 `logic`；禁止原始事件洪泛。
  ...
  - 回归覆盖：`components/drop-zone/test/semantics.rs::drop_zone_environment_streams_are_not_applicable_without_env_subscriptions`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_environment_streams_are_not_applicable_without_env_subscriptions`。
- [x] 事件光锥（Event Light Cone）：`Table/Grid` 等大型集合批量操作必须走 `Context Bus + Selector` 与状态压缩表达（如 `SelectionState::All`），禁止 O(N) 级向下 prop drilling。
  ...
  - 回归覆盖：`components/drop-zone/test/semantics.rs::drop_zone_event_light_cone_is_not_applicable_without_large_collection_batch_ops`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_event_light_cone_is_not_applicable_without_large_collection_batch_ops`。
- [x] 统一因果总线（Causality Bus）：复杂派生总线操作必须支持透传 `TraceId`，确保“用户触发 -> 派生命令 -> 总线广播 -> 订阅者”因果链不断裂。
  ...
  - 回归覆盖：`components/drop-zone/test/semantics.rs::drop_zone_causality_bus_is_not_applicable_without_derived_bus_pipeline`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_causality_bus_is_not_applicable_without_derived_bus_pipeline`。
- [x] 存在 A11y 实现、国际化与本地化实现（至少具备接入点，不硬编码用户可见文本）。
  ...
  - 回归覆盖：`components/drop-zone/test/semantics.rs::drop_zone_has_a11y_i18n_l10n_contract_without_view_hardcoded_copy`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_has_a11y_i18n_l10n_contract_without_view_hardcoded_copy`。
- [x] 状态可观测、可检索、可验证：使用稳定 `data-*` 与 `aria-*` 标记表达状态和来源。
  ...
  - 回归覆盖：`components/drop-zone/test/semantics.rs::drop_zone_state_markers_are_observable_searchable_and_enumerable`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_state_markers_are_observable_searchable_and_enumerable`。
- [x] 样式依赖显式状态（`data-*`/class），而非脆弱 DOM 结构猜测。
  ...
  - 回归覆盖：`components/drop-zone/test/semantics.rs::drop_zone_styles_depend_on_explicit_state_markers_not_dom_structure_guessing`、`components/drop-zone/test/drop_zone_semantics.rs::drop_zone_styles_depend_on_explicit_state_markers_not_dom_structure_guessing`。
- [x] 测试验证“语义契约”而不只验证视觉快照。
  ...
  - 语义断言主路径：`components/drop-zone/test/semantics.rs` 与 `components/drop-zone/test/drop_zone_semantics.rs` 均直接断言 `role/aria/data-*` 与来源标记（如 `data-disabled-source`、`data-motion-source`、`data-aria-source`），而非依赖视觉快照。
