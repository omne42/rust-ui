...
### 1. 大骨架（架构边界与层职责）
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（`DisclosureStateInput/DisclosureState/resolve_state` 已下沉到 `crates/ui-state-primitives/src/disclosure.rs`，组件 `logic.rs` 仅做装配映射。）
...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。
  - N/A：`Disclosure` 无远程请求与异步状态轴，状态仅由同步交互切换驱动。
...
- [x] 级联层覆盖（`@layer ui`）：组件 CSS 默认聚合进 `@layer ui`；运行时数值调整仅通过 CSS Custom Properties（如 `style:--x=...`），禁止普通内联样式（如 `style="top: 10px"`）。
  - 通过：`crates/ui/src/css.rs` 在 `push_components_css` 中以 `@layer ui` 包裹组件样式聚合；`Disclosure` 运行时动效仅写入 `--ui-disclosure-*` 变量（`components/disclosure/src/motion.rs`），组件视图未使用普通 `style="..."` 内联样式。
...
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。
  - `Disclosure` 归类为 `Streaming Optional`，当前组件按 `fallback=snapshot` 稳定渲染。
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（组件范围已执行 `cargo test -p ui --test disclosure_semantics --no-default-features --features component-disclosure,inject-css`、`cargo clippy -p ui --test disclosure_semantics --no-default-features --features component-disclosure,inject-css -- -D warnings`、`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-disclosure,inject-css`。）
