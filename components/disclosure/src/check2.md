...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（组件范围已执行 `cargo test -p ui-components --test disclosure_semantics --no-default-features --features component-disclosure,inject-css`、`cargo clippy -p ui-components --test disclosure_semantics --no-default-features --features component-disclosure,inject-css -- -D warnings`、`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-disclosure,inject-css`。）
