...
fn load_source(rel_path: &str) -> String {
    if rel_path == "../../apps/docs-app/src/pages/components/pages/collections_extra.rs" {
        return docs_collections_extra_source().to_string();
    }
...
fn docs_collections_extra_source() -> &'static str {
    static SOURCE: OnceLock<String> = OnceLock::new();
    SOURCE
        .get_or_init(|| {
            let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
            let parent_path =
                manifest_dir.join("../../apps/docs-app/src/pages/components/pages/collections_extra.rs");
            let child_path = manifest_dir
                .join("../../apps/docs-app/src/pages/components/pages/collections_extra/disclosure_group.rs");
...
fn disclosure_group_composes_accordion_with_motion() {
    let mod_source = load_source("src/group/mod.rs");
    let view_source = load_source("src/group/view.rs");
...
fn disclosure_group_emits_baseline_style_state_data_attributes() {
    let source = load_source("src/group/view.rs");
...
fn disclosure_group_styles_include_state_markers() {
    let source = load_source("src/group/styles.rs");
