...
#### Label 本轮验收摘要（2026-02-19）
- 已修复：状态原语下沉（新增 `crates/ui-state-primitives/src/label.rs`，`crates/ui/src/label/logic.rs` 仅保留对 primitives 的 re-export）。
- 已修复：组件装配层纯化（`crates/ui/src/label/mod.rs` 改为导出 primitives 类型，不再内置状态原语定义）。
- 已修复：A11y locale 接入（`crates/ui/src/label/view.rs` 新增 `lang/dir` props 并挂载 `ui_headless::locale_attrs`）。
- 已补充：语义回归（`components/label/test/label_semantics.rs` 增加 primitives 来源断言与 check2 完成态断言）。
- 已验证：`CARGO_TARGET_DIR=target-codex-label cargo check -p ui --no-default-features --features component-label,inject-css`。
- 已验证：`CARGO_TARGET_DIR=target-codex-label cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-label,inject-css`。
- 已验证：`CARGO_TARGET_DIR=target-codex-label cargo test -p ui-state-primitives label`。
- 已验证：`CARGO_TARGET_DIR=target-codex-label cargo test -p ui --test label_semantics --no-default-features --features component-label,inject-css`。
...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - 本项按单组件职责执行：`label` 定向 compile/test 已通过；整仓全量 smoke 与多组件联动门禁由仓库级流水线统一裁决。
