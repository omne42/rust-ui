...
- 已执行验证：
  - `cargo tree -e features -p ui --no-default-features --features component-input_otp,inject-css --depth 3`（通过）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo check -p ui --no-default-features --features component-input_otp,inject-css`（通过）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-input_otp,inject-css`（通过，存在仓库既有 `observability.rs` dead_code warning，不阻断 check）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo test -p ui --test input_otp_semantics --no-default-features --features component-input_otp,inject-css`（通过，10/10）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo clippy -p ui --no-default-features --features component-input_otp,inject-css --all-targets -- -D warnings`（通过）
  ...
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo test -p ui --test input_otp_semantics --no-default-features --features component-input_otp,inject-css`（通过，10/10，复跑）
