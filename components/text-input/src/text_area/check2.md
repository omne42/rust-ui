...
### 本次逐项核验证据（text_area）
- 代码分层与 API：`components/text-input/src/text_area/mod.rs`、`components/text-input/src/text_area/logic.rs`、`components/text-input/src/text_area/view.rs`、`components/text-input/src/text_area/motion.rs`、`crates/ui-state-primitives/src/text_area.rs`。
- 样式与 token-first：`components/text-input/src/text_area/styles.rs`（仅消费 `var(--ui-*)` 与 motion css var）。
- 语义与文档回归：`components/text-input/test/text_area_semantics.rs`、`apps/docs-app/src/pages/components/pages/forms.rs`、`apps/web-demo/src/demos/text_area_demo.rs`、`e2e/tests/docs_app_text_area_contract.spec.mjs`。
