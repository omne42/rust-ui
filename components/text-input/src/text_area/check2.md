...
- 本地链路命令（`/root/.cargo/bin/cargo`，2026-02-18）：
  - 已通过：
    ...
    - `cargo test -p ui --no-default-features --features component-text_area,inject-css --test text_area_semantics`
    - `cargo check -p ui --no-default-features --features component-text_area,inject-css`
    - `cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-text_area,inject-css --offline`
    ...
    - `cargo clippy -p ui --no-default-features --features component-text_area,inject-css --test text_area_semantics -- -D warnings -A clippy::too_many_arguments -A clippy::clone_on_copy -A unused_macros -A unused_imports -A dead_code`
...
- 本轮复验通过的外部阻断点：
  ...
  - `crates/ui/src/button/copy/motion.rs`：`attach_motion` 参数改为 `Signal<bool>`，与 snippet logic 输出一致。
