...
- [x] 最小特性依赖链修复：`crates/ui/Cargo.toml` 中补齐 `component-number_field -> component-button`，确保 `NumberField` 在最小 feature 下可独立编译。
- [x] 最小特性构建通过（native + wasm）：
  - `CARGO_TARGET_DIR=target/number-field-check2 cargo check -p ui --no-default-features --features component-number_field,inject-css`
  - `CARGO_TARGET_DIR=target/number-field-check2 cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-number_field,inject-css`
- [x] 语义回归通过：
  - `CARGO_TARGET_DIR=target/number-field-check2 cargo test -p ui --test number_field_semantics --no-default-features --features component-number_field,inject-css`
- [x] 特性树确认（未被 `all-components` 隐式拉起）：
  - `cargo tree -e features -i ui -p ui --no-default-features --features component-number_field,inject-css`
