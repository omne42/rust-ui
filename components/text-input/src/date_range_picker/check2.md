...
- [x] `ui` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（`logic.rs` 负责文本/输入归一与 primitives 映射，`view.rs` 仅负责组合两个 `DatePicker` 与挂载语义标记。）
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（已执行：`rustfmt --edition 2024`（仅本次改动文件）；`cargo clippy -p ui-state-primitives -- -D warnings`；`cargo clippy -p ui --no-default-features --features component-date_range_picker,inject-css -- -D warnings`；`cargo test -p ui-state-primitives date_range_picker`；`cargo test -p ui --test date_range_picker_semantics --no-default-features --features component-date_range_picker,inject-css`；`cargo check` native/wasm 最小特性链。）
