...
### 8. 明确禁止的反模式
- [x] 在 `status-primitives`（当前 `ui-state-primitives`）写 DOM/样式逻辑。（`ui-logic-calendar/src/time_field.rs` 保持纯 Rust 状态与归一化函数，不引入 Leptos/DOM/CSS 依赖；`TimeField` 组件仅消费其输出。回归：`time_field_anti_pattern_status_primitives_remains_dom_and_style_free`。）
...
- [x] 明明是跨组件可复用状态原语，却长期留在某个组件 `logic.rs` 不下沉。（`TimeField` 可复用状态不变量位于 `ui-logic-calendar/src/time_field.rs` 与 `ui-headless/src/time_field.rs`，组件 `logic.rs` 仅做装配映射。回归：`time_field_anti_pattern_reusable_state_invariants_are_sunk_to_primitives_or_headless` + `time_field_check2_marks_forbidden_anti_patterns_complete`。）
...
- [x] 可测试（契约可断言）。（契约断言优先由 `time_field_semantics_suite_prioritizes_contract_assertions_over_snapshots` + `time_field_semantic_markers_changed_in_view_must_be_covered_by_semantics_checks` 保证。）
