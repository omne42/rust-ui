...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（组件范围已执行 `rustfmt --edition 2024 components/text-input/src/number/view.rs components/text-input/test/number_semantics.rs`、`cargo clippy -p ui --no-default-features --features component-number,inject-css -- -D warnings`、`cargo test -p ui --test number_semantics --no-default-features --features component-number,inject-css`、`cargo check -p ui --no-default-features --features component-number,inject-css`、`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-number,inject-css`。）
