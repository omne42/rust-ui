...
use crate::text_input::date_picker::{
...
pub fn DatePicker(
    ...
    view! {
        <div
            ...
            data-selected-day=move || state.get().selected_day.map(|day: u8| day.to_string())
            data-year=move || state.get().year
            data-month=move || state.get().month
