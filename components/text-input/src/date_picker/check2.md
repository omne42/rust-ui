...
- 代码分层修复：`DatePicker` 状态原语已下沉到 `crates/ui-logic-calendar/src/date_picker.rs`，并在 `crates/ui-state-primitives/src/lib.rs` 导出；`components/text-input/src/date_picker/logic.rs` 改为仅 re-export + 组件装配。
- A11y/i18n 修复：`components/text-input/src/date_picker/view.rs` 已接入 `lang/dir`（`ui_headless::a11y::locale_attrs`），不再假设单语言/单方向。
- 语义回归：`components/text-input/test/date_picker_semantics.rs` 新增/更新为 13 条契约测试（含 primitives 来源、feature 依赖、locale 接入、样式/状态标记、docs 覆盖、README 白名单与 workbench/comparison 文档约束）。
- docs shell 白名单：`apps/docs-app/src/pages/components/shell.rs` 已将 `"date-picker"` 映射到 `components/text-input/src/date_picker/README.md`，沿用静态 `include_str!` + `_ => None` 安全边界。
...
- 组件文档：`components/text-input/src/date_picker/README.md` 已补充 docs-playground 四区说明与多场景对比说明。
