...
- 组件语义回归：`cargo test -p ui --test date_picker_semantics --no-default-features --features component-date_picker,inject-css` 通过（13 tests）。
- 最小特性编译：`cargo check -p ui --no-default-features --features component-date_picker,inject-css` 通过。
- wasm 最小特性编译：`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-date_picker,inject-css` 通过。
- 最小特性静态检查：`cargo clippy -p ui --no-default-features --features component-date_picker,inject-css -- -D warnings` 通过。
- 特性树证据：`cargo tree -e features -p ui --no-default-features --features component-date_picker,inject-css -f "{p} {f}"` 显示 `component-date_picker` 正确拉起 `component-button/component-calendar/component-popover`。
- headless 互斥证据：
