...
- [x] `ui` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（`components/text-input/src/mod.rs` 保持 feature-gated 装配导出且不暴露 `web-sys` 细节；新增 `components/text-input/test/semantics.rs` 并由 `components/text-input/src/mod.rs` 挂载，回归锁定公共导出面不泄露 `view/logic` 内部模块与平台细节，同时保留旧语义入口到新目录的桥接样例。）
...
- [x] API 命名契约统一：公共 props/回调严格使用 `is_*`、`on_*`、`default_*` 前缀；同语义在全库同名，禁止别名漂移。（`text-input` 已补 canonical 命名：`input/input_group/input_otp/number_field/date_picker/date_range_picker/text` 增加 `is_*` 与 `on_*` 入口（如 `is_disabled`、`is_read_only`、`is_required`、`is_invalid`、`on_value_change`、`is_show_outside_days`、`is_truncated`）；旧命名（如 `disabled/on_change/required/invalid/show_outside_days/truncate`）保留为兼容别名并在组件内部显式归一，优先级为 canonical > alias，迁移路径为“文档与新调用只使用 canonical，旧别名后续窗口再移除”。回归：`components/text-input/test/semantics.rs::text_input_api_naming_contract_is_prefixed_with_compatible_alias_bridge`。）
...
- [x] 受控/非受控必须成对：每个可控状态轴都提供 `value + on_value_change + default_value`（如 `open/on_open_change/default_open`）；缺一项即不通过。（`Input`、`InputOtp`、`NumberField` 已从“强制 `value + set_value`”收敛为标准三元组，并统一接入 `use_controllable_state`；旧 `set_value/on_change` 保留为兼容别名并归一到 `on_value_change`，避免半受控。`DatePicker`/`DateRangePicker`/`DateField`/`TimeField`/`TextField`/`SearchField`/`Textarea` 现有状态轴保持三元组完整。回归：`components/text-input/test/semantics.rs::text_input_controlled_uncontrolled_triplets_are_complete_for_value_axes`。）
  - 受控模式：外部值是单一事实来源，内部不得偷偷写回本地状态。
  - 非受控模式：仅由默认值初始化一次，后续状态由内部原语管理。
  - 受控/非受控切换语义需稳定可测，避免“半受控”隐式行为。
- [x] 默认值单一来源：默认值与优先级只在 `logic.rs` 归一化；`view.rs` 禁止二次兜底或隐式改写。（`Input`/`InputOtp`/`NumberField` 已将 `default_value` 兜底从 `view.rs` 回收至各自 `logic.rs::normalize_default_value`，`view.rs` 仅消费归一化结果；回归：`components/text-input/test/semantics.rs::text_input_default_value_normalization_is_owned_by_logic_layer` + `components/text-input/test/{input,input_otp,number_field}/logic.rs`。）
  - 默认值优先级必须可读且可测试（显式规则而非分散 `unwrap_or`）。
  - `view.rs` 不允许再做默认值分支；仅消费 `logic.rs` 的归一化输出。
  - 一旦发现多处默认值来源，直接判不通过并回收至 `logic.rs`。
- [x] 状态归一化集中：状态输入先类型化，再在 `logic.rs` 统一派生；禁止在 `view.rs`、事件回调、样式分支中分散拼状态机。（`Input`/`InputOtp`/`NumberField`/`Text` 已补 `AccessibilityStateInput -> normalize_accessibility_state` typed 边界并由 `view.rs` 统一消费，移除同轴 `unwrap_or` 分散归一化；回归：`components/text-input/test/semantics.rs::text_input_state_normalization_is_centralized_in_logic_layer` + `components/text-input/test/{input,input_otp,number_field,text}/logic.rs`。）
  - 输入边界统一进入 `logic.rs`，输出统一为可渲染语义状态与来源标记。
  - 事件处理器只触发状态变更，不重建状态机规则。
  - 样式层只消费状态标记，不承担状态判定职责。
- [x] 离散状态必须类型约束：`variant/size/mode/status` 等离散输入使用 `enum`；禁止用多个 `Option<bool>`/字符串自由组合表达互斥状态。（`Text` 的 `tone/align/weight/element`、`Input` 的 `variant/size/label_placement`、`DateField` 的 `tone/data-state/control-mode` 等离散轴已为 enum；本轮把 `Input`/`TextField` 的兼容字符串 `input_type` 收敛为 `InputType`/`TextFieldInputType`，`view.rs` 仅消费 `as_html_attr()`，避免字符串直接驱动状态轴。回归：`components/text-input/test/semantics.rs::text_input_discrete_type_axes_are_enum_constrained`、`components/text-input/test/input/logic.rs::input_type_is_normalized_in_logic`、`components/text-input/test/text_field/logic.rs::normalize_input_type_maps_compat_strings_to_enum`。）
