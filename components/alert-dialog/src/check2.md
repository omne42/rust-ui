...
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（N/A：`AlertDialog` 本体不持有可复用受控/非受控状态机；`open` 由外部注入，组件内部 `logic.rs` 仅做 props 归一与语义来源标记派生（`resolve_state/compose_class_name`），未引入 list/selection/expansion 原语重写。回归：`components/alert-dialog/src/test/logic.rs`、`crates/ui-components/tests/alert_dialog_semantics.rs::alert_dialog_logic_exposes_state_helpers`。）
...
- [x] `ui-theme` 定义：唯一设计 token 与主题上下文层（system/color/scale + Light/Dark/OLED），负责 token 分类、主题映射与 CSS 变量生成。（`components/alert-dialog/src/styles.rs` 仅消费 `var(--ui-*)`（如 `--ui-overlay-panel-min-width`、`--ui-icon-size-200`、`--ui-space-2xs`、`--ui-heading-h5-font-size`），未引入组件私有 token 命名或硬编码 Hex。回归：`crates/ui-components/tests/alert_dialog_semantics.rs::alert_dialog_styles_include_state_and_source_markers`。）
...
- [x] `ui-components` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（目录职责完整：`mod.rs/logic.rs/view.rs/styles.rs/motion.rs/protocol.rs`；`mod.rs` 仅导出稳定 API，未暴露 `web-sys`/DOM 类型；`view.rs` 负责结构与语义挂载，`logic.rs` 负责归一派生，`motion.rs` 负责动效契约映射。回归：`crates/ui-components/tests/alert_dialog_semantics.rs` 全量 20 项。）
  - `logic.rs` 负责 props 归一与状态派生；`view.rs` 负责结构渲染与 headless 语义挂载；`styles.rs` 负责 token-first 静态样式；`motion.rs` 负责动效 attach。
  - 组件层不得重写 `status-primitives` 状态机或 `ui-headless` 交互契约；发现即判不通过并回迁到对应层。
  - 对外 API 禁止暴露 `web-sys`/DOM 细节类型；平台差异封装在内部模块。

### 2. API 设计与状态内核（Logic/Kernel）
- [x] API 命名契约统一：公共 props/回调严格使用 `is_*`、`on_*`、`default_*` 前缀；同语义在全库同名，禁止别名漂移。（已补齐 `is_confirm_disabled`/`is_secondary_disabled` 作为主命名，并保留 `confirm_disabled`/`secondary_disabled` 兼容别名；`logic::resolve_disabled_flag` 统一优先级，迁移策略为“`is_*` 优先，legacy 仅兼容”。回归：`components/alert-dialog/src/test/logic.rs::resolve_disabled_flag_prefers_is_prefix_and_keeps_legacy_alias`、`crates/ui-components/tests/alert_dialog_semantics.rs::alert_dialog_view_uses_logic_contracts_and_source_markers`。）
...
- [x] 状态原语来源正确：组件层只消费 `status-primitives`（当前 `ui-state-primitives`）能力，不直接绑定业务 store；应用级全局状态必须经桥接层适配后再接入组件。（N/A：`AlertDialog` 无可复用 selection/expansion/controllable-state 原语重写，组件不依赖业务 store；`logic.rs` 仅做装配映射与来源标记。）
...
- [x] API 易用性验收标准（DX Paradox）：把复杂性留在内部，把简单留给用户。（基础用法仅需 `open/id_base/title/on_close/confirm_label/on_confirm`，其余皆可选；docs-app 提供最小与状态矩阵示例：`apps/docs-app/src/pages/components/pages/overlays_alert_dialog.rs`。）
