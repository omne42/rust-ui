...
- 已完成修复：
  - `color_field` 状态原语下沉到 `crates/ui-state-primitives/src/color_field.rs`，组件层 `logic.rs` 改为纯消费 re-export。
  - `ColorField` 接入 `ui_headless` i18n 与 `lang/dir`，移除硬编码 `"Clear"` 文案。
  - `component-color_field` 增加特性链依赖 `component-color_swatch`，修复最小特性链断裂。
  - 新增/更新语义回归：`components/color-field/test/color_field_semantics.rs`。
  - 新增 E2E 契约：`e2e/tests/docs_app_color_field_contract.spec.mjs`。
  - 新增 E2E 门禁脚本：`components/color-field/scripts/check-ui-e2e-color-field.sh`。
- 已执行验证：
  - `cargo test -p ui-state-primitives color_field` ✅
  - `cargo check -p ui-state-primitives` ✅
- 追加验证：
  - `cargo check -p ui --no-default-features --features component-color_field,inject-css` ✅
  - `cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-color_field,inject-css` ✅
  - `cargo check -p ui-headless --no-default-features --features web` ✅
  - `cargo check -p ui-headless --no-default-features --features ssr` ✅
  - `cargo check -p ui-headless --no-default-features --features web,ssr` ❌（按预期触发互斥 `compile_error!`）
  - `cargo check -p ui-motion` / `cargo check -p ui-motion --target wasm32-unknown-unknown` ✅
  - `cargo clippy -p ui-state-primitives -- -D warnings` ✅
  - `cargo clippy -p ui --no-default-features --features component-color_field,inject-css -- -D warnings` ✅
...
- [x] 级联层覆盖（`@layer ui`）：组件 CSS 默认聚合进 `@layer ui`；运行时数值调整仅通过 CSS Custom Properties（如 `style:--x=...`），禁止普通内联样式（如 `style="top: 10px"`）。
  - `crates/ui/src/css.rs` 通过 `push_components_css` 统一以 `@layer ui` 包裹组件样式，`component-color_field` 走同一聚合路径。
...
- [x] SSR 与跨平台检查：覆盖 web/ssr/wasm 分支，不破坏 non-wasm 编译路径。（已通过 native + wasm 最小特性编译）
...
- [x] `ui-headless` web/ssr feature 互斥受 `compile_error!` 保护（`crates/ui-headless/src/lib.rs`）。（`web`/`ssr` 单开通过，`web,ssr` 按预期失败）
  - 组件依赖 `ui-headless` 能力时，不得破坏其 web/ssr 互斥约束。
  - 组件若新增 headless 功能接入，需验证两条 feature 路径都可编译。
  - 发现“同时启用 web+ssr 仍可过编译”视为契约回归。
- [x] `ui-motion` 非 wasm 提供 no-op/stub（`crates/ui-motion/src/lib.rs`），保证 SSR/tooling 可编译。（native/wasm compile-only 均通过）
  - `motion.rs` 调用必须可在 non-wasm 下安全降级，不触发 panic。
  - 组件不得假设动画句柄一定存在；no-op 分支行为需可预测。
  - toolchain 场景（测试/文档/静态分析）不得因 motion 依赖阻塞编译。
- [x] 组件实现覆盖 `reduced-motion` / SSR / wasm 分支。（ColorField 无组件级 motion；SSR/wasm 分支 compile-only 已验证）
...
- [x] 语义测试与性能回归：断言必须覆盖 `aria-*`、`data-*` 与焦点流转，不能只看快照；高频/重型组件必须补齐 `render_count` 断言/测量（如初始化空闲预算为 1）。
  ...
  - `components/color-field/test/color_field_semantics.rs` 已锁定上述语义与焦点流转断言关键片段，防止回归为仅快照检查。
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（按单组件最小特性门禁：fmt/rustfmt + clippy(-D warnings) + unit/semantics + native/wasm compile-only）
