...
- 已完成修复：
  ...
  - 新增 E2E 门禁脚本：`scripts/check-ui-components-e2e-color-field.sh`。
- 已执行验证：
  - `cargo test -p ui-state-primitives color_field` ✅
  - `cargo check -p ui-state-primitives` ✅
- 追加验证：
  - `cargo check -p ui-components --no-default-features --features component-color_field,inject-css` ✅
  - `cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-color_field,inject-css` ✅
  ...
  - `cargo clippy -p ui-components --no-default-features --features component-color_field,inject-css -- -D warnings` ✅
