...
- [x] 宏观/微观双状态机（Macro/Micro Duality）：拖拽等高频交互在 `Dragging` 期间由 `view/motion` 本地循环执行；禁止每帧穿越回 `logic.rs`，必须在结束时通过 `Action::DragEnd` 回流收敛。（N/A：`PressableFeedback` 仅覆盖按压反馈（pointer down/up/cancel + keyboard press），不包含拖拽交互，也不存在 `Dragging` 中间态或 `Action::DragEnd` 回流动作；当前实现仅做 press 状态同步与动效映射。防回归：`components/pressable-feedback/test/semantics.rs::pressable_feedback_has_no_dragging_macro_micro_state_machine_contract`。）
- [x] 几何两段式渲染（Two-Pass Rendering）：`Tooltip/Popover/Menu` 等依赖 DOM 测量的组件必须走 `Intent -> Measure(view) -> Rectification(logic)`，并具备幂等收敛保护防死循环。（N/A：`PressableFeedback` 不是 overlay/tooltip/popover 类几何定位组件；当前实现只处理按压态映射与反馈动效，不进行 DOM 几何测量，也不存在 `Intent -> Measure -> Rectification` 收敛环。防回归：`components/pressable-feedback/test/semantics.rs::pressable_feedback_has_no_two_pass_geometry_measure_rectification_contract`。）
- [x] 集合注册协议（Registration Protocol）：`Accordion/Tabs/Menu` 动态子项必须通过 `RegistrationContext` 上报 `Register/Unregister`，逻辑层维护 `items_order`，禁止依赖 `HashSet` 迭代顺序做导航。（N/A：`PressableFeedback` 不是集合容器组件，不管理动态子项注册与导航顺序；组件仅渲染单槽 `children` 并提供按压反馈，不存在 `RegistrationContext/Register/Unregister/items_order` 语义。防回归：`components/pressable-feedback/test/semantics.rs::pressable_feedback_has_no_collection_registration_protocol_contract`。）
- [x] 插槽投影策略（Slot Projection）：容器组件明确 `Lazy/KeepAlive/Eager`；`KeepAlive` 隐藏时必须通过生命周期通知（如 `NotifyHidden`）暂停轮询/动画等高耗能副作用。（N/A：`PressableFeedback` 非投影型容器组件，渲染路径为单槽 `children` 直出，不提供 `Lazy/KeepAlive/Eager` 投影策略，也不存在 `NotifyHidden` 生命周期回调来暂停后台轮询/动画。防回归：`components/pressable-feedback/test/semantics.rs::pressable_feedback_has_no_slot_projection_keepalive_contract`。）
- [x] 环境订阅流（Env Streams）：`Resize/Theme/Intersection` 等环境变化在 `view.rs` 采样、防抖后转化为高层语义 `Action`（如 `BreakpointChanged`）推送到 `logic`；禁止原始事件洪泛。（N/A：`PressableFeedback` 为本地按压反馈组件，不依赖窗口尺寸/交叉观察/主题变化订阅来驱动交互状态；当前实现仅消费 pointer/keyboard/focus 输入并映射到 press 状态与动效，不存在 `Resize/Theme/Intersection -> debounce -> Action` 回流链路。防回归：`components/pressable-feedback/test/semantics.rs::pressable_feedback_has_no_env_stream_sampling_to_action_contract`。）
- [x] 事件光锥（Event Light Cone）：`Table/Grid` 等大型集合批量操作必须走 `Context Bus + Selector` 与状态压缩表达（如 `SelectionState::All`），禁止 O(N) 级向下 prop drilling。（N/A：`PressableFeedback` 不是 `Table/Grid` 一类大型集合批处理组件，不存在批量选择、压缩选择态或集合总线广播需求；当前交互仅面向单表面按压反馈。防回归：`components/pressable-feedback/test/semantics.rs::pressable_feedback_has_no_event_light_cone_collection_bus_contract`。）
- [x] 统一因果总线（Causality Bus）：复杂派生总线操作必须支持透传 `TraceId`，确保“用户触发 -> 派生命令 -> 总线广播 -> 订阅者”因果链不断裂。（N/A：`PressableFeedback` 无复杂派生总线，不存在“命令广播 -> 订阅者”拓扑；交互链路为本地输入事件直连 press 状态更新，无 `TraceId` 透传需求。防回归：`components/pressable-feedback/test/semantics.rs::pressable_feedback_has_no_causality_bus_trace_id_contract`。）
...
### 4. DOM/环境边界治理
- [x] 焦点全局栈（Focus Stack & GC）：层叠 `Overlay` 禁止私存 `NodeRef` 作为恢复目标；必须依赖全局 Focus Manager（如 `FallbackTo/Selector`）防止焦点坠落到 `document.body`。（N/A：`PressableFeedback` 为单表面按压反馈组件，不是层叠 `Overlay`，不存在“关闭时恢复焦点目标”语义与焦点栈 GC 责任；组件中的 `NodeRef` 仅用于本地动效节点挂载（root/ripple），不保存“返回焦点”目标。防回归：`components/pressable-feedback/test/semantics.rs::pressable_feedback_has_no_overlay_focus_stack_gc_contract`。）
- [x] 受控外交特区（Escape Hatches）：集成 ECharts/Map 等命令式第三方库时必须处于 `Foreign Zone`（`YieldControl/CleanupForeign`）；第三方实例不得暴露为组件公共 API 或反向污染状态机。（N/A：`PressableFeedback` 为基础按压反馈组件，不集成 ECharts/Map 等命令式第三方实例；组件公共 API 仅暴露按压语义 props，不对外暴露第三方实例句柄，也不存在第三方实例反向驱动 `ui-state-primitives` 状态机的链路。防回归：`components/pressable-feedback/test/semantics.rs::pressable_feedback_has_no_foreign_zone_escape_hatch_contract`。）
- [x] SSR 时空断裂治理（Hydration Discontinuity）：逻辑初始化禁止依赖 `now()` 或原生随机 UUID；必须通过 `IdProvider` 注入确定性种子，确保 SSR/Hydration 间 ID 稳定。（N/A：`PressableFeedback` 当前不生成运行时动态 ID，也不依赖 `now()/random/uuid` 初始化路径；组件语义定位依赖稳定 `data-slot/data-*` 标记而非随机标识符，因此不存在 SSR/Hydration 间 ID 漂移面。防回归：`components/pressable-feedback/test/semantics.rs::pressable_feedback_has_no_hydration_discontinuity_nondeterministic_init_contract`。）
- [x] SSR 与跨平台检查：覆盖 web/ssr/wasm 分支，不破坏 non-wasm 编译路径。（`PressableFeedback` 平台差异已显式封装在 `components/pressable-feedback/src/motion.rs`：`#[cfg(target_arch = "wasm32")]` 使用浏览器对象驱动动效，`#[cfg(not(target_arch = "wasm32"))]` 提供 no-op `attach_motion`，non-wasm 分支不触达 `web-sys`/浏览器对象；并在门禁命令矩阵保留 compile-only 证据 `cargo check -p ui --target wasm32-unknown-unknown`、`cargo check -p ui-headless --no-default-features --features ssr`、`cargo test --workspace`。防回归：`components/pressable-feedback/test/semantics.rs::pressable_feedback_platform_matrix_keeps_cfg_boundaries_and_non_wasm_browser_free`。）
...
- [x] `ui-headless` web/ssr feature 互斥受 `compile_error!` 保护（`crates/ui-headless/src/lib.rs`）。（`crates/ui-headless/src/lib.rs` 已存在互斥保护：`#[cfg(all(feature = "web", feature = "ssr"))] compile_error!(...)`；`PressableFeedback` 仅通过 `ui-headless = { path = "../../crates/ui-headless" }` 依赖 headless 能力，未强行同时开启 `web+ssr`。compile-only 证据命令：`cargo check -p ui-headless --no-default-features --features web`、`cargo check -p ui-headless --no-default-features --features ssr`；冲突路径应失败：`cargo check -p ui-headless --no-default-features --features web,ssr`。防回归：`components/pressable-feedback/test/semantics.rs::pressable_feedback_preserves_ui_headless_web_ssr_feature_mutex_contract`。）
  - 组件依赖 `ui-headless` 能力时，不得破坏其 web/ssr 互斥约束。
  - 组件若新增 headless 功能接入，需验证两条 feature 路径都可编译。
  - 发现“同时启用 web+ssr 仍可过编译”视为契约回归。
- [x] `ui-motion` 非 wasm 提供 no-op/stub（`crates/ui-motion/src/lib.rs`），保证 SSR/tooling 可编译。（`crates/ui-motion/src/lib.rs` 已提供 `#[cfg(not(target_arch = "wasm32"))] pub mod web` no-op/stub：`prefers_reduced_motion() -> true` 与 `animate(..) {}`，且含 `non_wasm_web_backend_is_predictable_noop` 测试；组件 `components/pressable-feedback/src/motion.rs` 同步提供 `#[cfg(not(target_arch = "wasm32"))] attach_motion(..)` 空实现，`view.rs` 统一调用 `motion::attach_motion`，因此 non-wasm/SSR/tooling 路径安全降级且行为可预测，不依赖动画句柄存在。防回归：`components/pressable-feedback/test/semantics.rs::pressable_feedback_motion_contract_keeps_non_wasm_noop_stub_predictable`。）
