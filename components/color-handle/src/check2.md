...
2. 组件职责落位：已补 `components/color-handle/src/motion.rs`，并在 `view.rs` 挂载 `motion::attach_motion` 与 `data-motion-source`；`styles.rs` 改为消费 `--ui-color-handle-motion-duration`。
3. 语义契约回归：`components/color-handle/test/color_handle_semantics.rs` 新增 motion 契约挂载断言与 `check2` 无未勾选项断言，确保后续改动不回退。
