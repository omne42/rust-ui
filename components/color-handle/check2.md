...
- [x] 级联层覆盖（`@layer ui`）：组件 CSS 默认聚合进 `@layer ui`；运行时数值调整仅通过 CSS Custom Properties（如 `style:--x=...`），禁止普通内联样式（如 `style=\"top: 10px\"`）。
  - 已满足：`crates/ui/src/css.rs` 通过 `push_components_css` 将组件样式聚合进 `@layer ui`；`components/color-handle/src/view.rs` 的运行时 `style` 仅来自 `motion::attach_motion` 输出的 `--ui-color-handle-motion-duration` 自定义属性。
...
- [x] `ui` 固定入口文件落点正确。
  ...
  - 已满足：`crates/ui/src/lib.rs` 与 `crates/ui/src/css.rs` 均按 `component-*` 做 feature gate；`crates/ui/src/root.rs` 集中注入 base css/theme vars/components css 并 `provide_ui_i18n`；`crates/ui-visual-primitive/src/active_highlight.rs` 承载通用高亮样式与 motion；且 `crates/ui/src/overlay_open.rs`、`crates/ui/src/presence.rs`、`crates/ui/src/a11y.rs` 均不存在。
- [x] 组件目录标准文件落点正确。
...
- [x] Tree Shaking & 特性剪裁：组件必须注册到 `ui` 特性树（如 `component-accordion`）；`css.rs` 和 `lib.rs` 聚合必须受 feature 门控，禁止无条件全局依赖。
  - 已满足：`crates/ui/Cargo.toml` 已定义 `component-color_handle = ["component-color_thumb"]`，并纳入 `all-components` 与 `component-domain-color` 特性树。
  - 已满足：`crates/ui/src/lib.rs` 对 `color_handle` 模块导出使用 `#[cfg(feature = "component-color_handle")]` 门控，不会在未启用特性时进入可达路径。
  - 已满足：`crates/ui/src/css.rs` 仅在 `#[cfg(feature = "component-color_handle")]` 下聚合 `crate::color::handle::styles::CSS`，无条件全局聚合被避免。
  - 验证：`cargo tree -e features -i ui -p ui --no-default-features --features component-color_handle,inject-css` 输出仅包含 `component-color_handle -> component-color_thumb -> component-color_swatch` 与 `inject-css`，未出现 `all-components`。
- [x] 语义测试与性能回归：断言必须覆盖 `aria-*`、`data-*` 与焦点流转，不能只看快照；高频/重型组件必须补齐 `render_count` 断言/测量（如初始化空闲预算为 1）。
  ...
  - render_count 跟踪：断言 `scripts/check-ui-performance.sh` 持续执行 `perf_render_count_follow_up_is_tracked_in_plan`，并校验 `docs/plan/TODO.md` 仍保留 `render_count` 自动化补齐任务，符合“当前框架未提供通用精确计数时采用等价证据 + 跟踪闭环”。
...
- [x] Source-first 文档必须 Copy-Paste Ready：提供一键复制组件源码或最小可用片段能力。
  ...
  - 已满足：`e2e/tests/docs_app_color_handle_contract.spec.mjs` 新增 `docs-app color-handle playground source is copy-paste ready`，断言 workbench 代码区存在复制入口（`Show/Hide code` + `data-copyable="true"` + Copy 按钮 `aria-label`），并校验输出代码包含可运行 imports（`use leptos::prelude::*;`、`use ui::*;`）与 `<ColorHandle` 片段。
