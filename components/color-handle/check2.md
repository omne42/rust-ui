...
- [x] 受控/非受控必须成对：每个可控状态轴都提供 `value + on_value_change + default_value`（如 `open/on_open_change/default_open`）；缺一项即不通过。
  - N/A（本组件无内部可控状态轴）：`ColorHandle` 为展示型装配组件，`is_*` 状态均由上层传入并直接消费，不维护内部状态，也不存在受控/非受控切换路径。
...
- [x] 默认值单一来源：默认值与优先级只在 `logic.rs` 归一化；`view.rs` 禁止二次兜底或隐式改写。
  ...
  - 已满足：`components/color-handle/src/logic.rs` 新增 `ColorHandlePropsInput -> ColorHandleResolvedProps` 归一化链路，默认值集中在 `resolve_props`（`DEFAULT_IS_LOUPE_VISIBLE/DEFAULT_X_PERCENT/DEFAULT_Y_PERCENT` 与 `motion.unwrap_or_default()`）。
  - 已满足：`components/color-handle/src/view.rs` 改为消费 `logic::resolve_props(...)` 输出，不再使用 `#[prop(default=...)]` 或 `unwrap_or*` 做二次兜底。
  - 回归：`components/color-handle/test/semantics.rs` 新增 `color_handle_default_values_are_normalized_in_logic_only`，锁定默认值集中归一与 view 层无兜底分支契约。
...
- [x] 离散状态必须类型约束：`variant/size/mode/status` 等离散输入使用 `enum`；禁止用多个 `Option<bool>`/字符串自由组合表达互斥状态。
  ...
  - 已满足：`components/color-handle/src/logic.rs` 新增 `ColorHandleInteractionState`（`Disabled/Dragging/Focused/Idle`）并通过 `from_flags` 统一收敛互斥交互状态轴。
  - 已满足：`components/color-handle/src/view.rs` 不再直接拼接互斥布尔逻辑，统一消费 `props.interaction_state.is_*()` 映射到 `ColorHandleStateInput` 与 `ColorThumb`。
  - 回归：`components/color-handle/test/semantics.rs` 新增 `color_handle_discrete_status_axis_is_type_constrained_with_enum`，锁定 enum 建模与 view 消费路径。
...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。
  - N/A（本组件无异步交互）：`ColorHandle` 不发起远程请求，不维护加载/错误状态，输入均为同步 props 映射。
...
- [x] 组合型组件主 API 必须“显示优于约定”：优先使用显式组合 `<Parent><Item ... /></Parent>`。
  - N/A（本组件非组合容器）：`ColorHandle` 是单节点装配组件，不提供 `Parent/Item` 或并行槽位 API。
...
- [x] 宏观/微观双状态机（Macro/Micro Duality）：拖拽等高频交互在 `Dragging` 期间由 `view/motion` 本地循环执行；禁止每帧穿越回 `logic.rs`，必须在结束时通过 `Action::DragEnd` 回流收敛。
  - N/A（本组件不承载拖拽状态机）：`ColorHandle` 仅接收并透传 `is_dragging` 语义，不实现拖拽每帧循环；拖拽交互由上层/子能力负责。
- [x] 几何两段式渲染（Two-Pass Rendering）：`Tooltip/Popover/Menu` 等依赖 DOM 测量的组件必须走 `Intent -> Measure(view) -> Rectification(logic)`，并具备幂等收敛保护防死循环。
  - N/A（本组件无几何测量闭环）：`ColorHandle` 不负责 tooltip/popover/menu 定位，不读取 DOM 几何信息，也不存在 `Intent -> Measure -> Rectification` 回路。
...
- [x] 事件光锥（Event Light Cone）：`Table/Grid` 等大型集合批量操作必须走 `Context Bus + Selector` 与状态压缩表达（如 `SelectionState::All`），禁止 O(N) 级向下 prop drilling。
  - N/A（本组件非大型集合组件）：`ColorHandle` 不管理批量集合状态，不涉及 `Context Bus + Selector` 或 `SelectionState::All` 压缩表达。
- [x] 统一因果总线（Causality Bus）：复杂派生总线操作必须支持透传 `TraceId`，确保“用户触发 -> 派生命令 -> 总线广播 -> 订阅者”因果链不断裂。
  - N/A（本组件无复杂总线派生链）：`ColorHandle` 无命令总线广播/订阅模型，不涉及 `TraceId` 透传因果链。
...
- [x] 焦点全局栈（Focus Stack & GC）：层叠 `Overlay` 禁止私存 `NodeRef` 作为恢复目标；必须依赖全局 Focus Manager（如 `FallbackTo/Selector`）防止焦点坠落到 `document.body`。
  - N/A（本组件非层叠 Overlay）：`ColorHandle` 不管理 overlay 焦点恢复链路，不持有用于恢复焦点的 `NodeRef`，无 `FallbackTo/Selector` 全局焦点管理需求。
- [x] 受控外交特区（Escape Hatches）：集成 ECharts/Map 等命令式第三方库时必须处于 `Foreign Zone`（`YieldControl/CleanupForeign`）；第三方实例不得暴露为组件公共 API 或反向污染状态机。
  - N/A（本组件无第三方命令式集成）：`ColorHandle` 未接入 ECharts/Map 等外部实例，不存在 `Foreign Zone` 生命周期托管与实例泄漏到公共 API 的风险面。
- [x] SSR 时空断裂治理（Hydration Discontinuity）：逻辑初始化禁止依赖 `now()` 或原生随机 UUID；必须通过 `IdProvider` 注入确定性种子，确保 SSR/Hydration 间 ID 稳定。
  - 本组件初始化不使用 `now()`/随机 UUID；DOM 标识由外部显式传入 `id_base` 并派生，SSR/Hydration 间可保持稳定。
...
- [x] 级联层覆盖（`@layer ui`）：组件 CSS 默认聚合进 `@layer ui`；运行时数值调整仅通过 CSS Custom Properties（如 `style:--x=...`），禁止普通内联样式（如 `style=\"top: 10px\"`）。
  - 已满足：`crates/ui-components/src/css.rs` 通过 `push_components_css` 将组件样式聚合进 `@layer ui`；`components/color-handle/src/view.rs` 的运行时 `style` 仅来自 `motion::attach_motion` 输出的 `--ui-color-handle-motion-duration` 自定义属性。
- [x] Motion 合同化：`stiffness`/`damping` 等参数在 `motion.rs` 内置为组件 Contract，并通过 `attach_motion` 挂载；必须尊重 `prefers-reduced-motion` 且在 non-wasm/SSR 安全降级（no-op）。
  - 已满足：`components/color-handle/src/motion.rs` 以 `ColorHandleMotion + sanitize_motion + attach_motion` 固化组件 motion contract 并输出 CSS 变量；`components/color-handle/src/styles.rs` 新增 `@media (prefers-reduced-motion: reduce)` 将 `--ui-color-handle-motion-duration` 降为 `0ms`；挂载链路不依赖 `web-sys`，SSR/non-wasm 仅字符串变量拼装，行为可预测。
- [x] `ui-components` 固定入口文件落点正确。
  ...
  - 已满足：`crates/ui-components/src/lib.rs` 与 `crates/ui-components/src/css.rs` 均按 `component-*` 做 feature gate；`crates/ui-components/src/root.rs` 集中注入 base css/theme vars/components css 并 `provide_ui_i18n`；`crates/ui-visual-primitive/src/active_highlight.rs` 承载通用高亮样式与 motion；且 `crates/ui-components/src/overlay_open.rs`、`crates/ui-components/src/presence.rs`、`crates/ui-components/src/a11y.rs` 均不存在。
- [x] 组件目录标准文件落点正确。
  ...
  - 已满足：`components/color-handle/src/` 存在 `mod.rs/logic.rs/styles.rs/view.rs/motion.rs`；`mod.rs` 仅导出 `ColorHandle` 与必要状态/motion 类型；`logic.rs` 仅消费 `ui_state_primitives::color_handle` 做归一化与 class 组装；目录中不存在 `render.rs` 与 `spec.rs`。
...
- [x] 文件落点纪律：组件目录严格由 `mod.rs`（导出）、`logic.rs`（归一派生）、`styles.rs`（Token 样式）、`view.rs`（渲染）、`motion.rs`（动效）组成；复杂组件可选 `spec.rs`；禁止 `render.rs`。
  - 已满足：`components/color-handle/src/` 包含 `mod.rs/logic.rs/styles.rs/view.rs/motion.rs`；复杂组件可选文件 `spec.rs` 在本组件未引入；目录内不存在 `render.rs`。
- [x] Hyper-Structure Builder（`spec.rs`）：复杂组件必须提供 AI 友好的 `*Spec::new()...render()` 建造者 API。
  - N/A（本组件非复杂建造器组件）：`ColorHandle` 为颜色句柄原语，当前能力边界聚焦在 `logic/view/motion/styles` 装配与语义输出；仓库规则已约束 `spec.rs` 仅在少数复杂组件引入，本组件不应为形式统一新增 `*Spec::new()...render()`。
- [x] 上下文压缩协议（Manifest + RBI）：新增/大改组件必须同步维护组件目录下 `Component.toml`（能力清单）和 `.rbi`（接口签名投影），避免 AI 检索工具箱过时。
  - 已满足：已新增 `components/color-handle/src/Component.toml`（能力清单）与 `components/color-handle/src/color_handle.rbi`（接口签名投影），并对齐当前 `ColorHandle` props、语义输出与依赖边界。
- [x] 语义标记统一升级为 Agent Contract（Schema 化），让 Agent 不依赖 DOM 猜测理解组件状态与意图。
  - 关键交互组件必须输出稳定机器可读语义（至少 `data-*` + 状态来源标记；复杂组件建议补 `data-ui-schema`）。
  - Agent 消费字段应来自类型化 schema 生成，不允许散落字符串拼接。
  - 契约字段需可追溯到组件状态轴与动作语义（intent/action/state/source）。
  - 配置到组件的渲染链路必须走白名单能力边界，禁止任意脚本注入。
  - 已满足：`components/color-handle/src/logic.rs` 新增 `ColorHandleAgentContract` 与 `resolve_agent_contract`（typed schema/intent/action/state/source）；`components/color-handle/src/view.rs` 挂载 `data-ui-schema/schema-version/intent/action/state/source` 与 stream/output/capability 标记；`components/color-handle/test/semantics.rs` 补充了 schema 字段挂载与白名单渲染路径（无 `inner_html`/`<script`/`javascript:`）回归。
- [x] 流式在这里仅指 LLM 输出渲染（只看两种显示模式）。
  - `Streaming`：LLM 还在生成，界面边生成边显示。
  - `Snapshot`：LLM 全部生成完成后，一次性显示。
  - 已满足：`ColorHandle` 作为非正文阅读面按 `Snapshot` 渲染链路工作，并通过 `data-ui-stream-support` / `data-ui-stream-fallback` / `data-ui-stream-mode` 标记明确流式语义边界。
- [x] `Snapshot` 是所有组件的基础能力（默认必须支持）。
  - 所有组件都应能消费“完整生成结果”并稳定渲染。
  - 即使组件不直接展示正文，也应能在接收上层完整配置后正常渲染。
  - 已满足：`components/color-handle/src/view.rs` 固定输出 `data-ui-stream-mode="snapshot"`，并配套 `data-ui-stream-support` / `data-ui-stream-fallback` / `data-ui-output-status`，确保组件按完整结果输入稳定渲染。
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。
  - `Streaming Required`：组件本体就是正文阅读面，用户需要边生成边看。
  - `Streaming Optional`：组件不是正文阅读面，可以只消费 `Snapshot`；若不支持流式，必须明确 `fallback=snapshot`。
  - 无论是否支持 `Streaming`，都要显式标识当前输出状态（草稿/已验证/可提交），并保持 `role`/`aria-*`/`data-*` 连续可读。
  - 数据校验、断线恢复、重试策略由上层负责，组件层只负责稳定渲染。
  - 已满足：`ColorHandle` 非正文阅读面，当前按 `Streaming Optional` 输出 `data-ui-stream-support=\"optional\"` 与 `data-ui-stream-fallback=\"snapshot\"`；并通过 `data-ui-output-status` 显式标识输出状态，保留 `role`/`aria-*`/`data-*` 连续可读。

### 7. 测试、门禁与交付
- [x] 代码卫生（Rust Hygiene）：非测试代码中完全禁止 `unwrap/expect`，禁止无处理的 `let _ = ...`；字符串复制热点收敛为 `Cow<'static, str>`（执行 `./scripts/check-rust-hygiene.sh` 验证）。
  - 已满足（组件范围）：`components/color-handle/src/*.rs` 已清理 `unwrap/expect`、`let _ =` 与 `to_string/to_owned/String::from` 字符串复制热点。
  - 验证说明：`./scripts/check-rust-hygiene.sh` 在当前环境受仓库级前置约束影响未能给出可用通过结果（`check-api-contracts` 基线漂移且 `rg` 缺少 PCRE2）；本次以组件内等价扫描作为本条验收依据。
- [x] Tree Shaking & 特性剪裁：组件必须注册到 `ui-components` 特性树（如 `component-accordion`）；`css.rs` 和 `lib.rs` 聚合必须受 feature 门控，禁止无条件全局依赖。
  - 已满足：`crates/ui-components/Cargo.toml` 已定义 `component-color_handle = ["component-color_thumb"]`，并纳入 `all-components` 与 `component-domain-color` 特性树。
  - 已满足：`crates/ui-components/src/lib.rs` 对 `color_handle` 模块导出使用 `#[cfg(feature = "component-color_handle")]` 门控，不会在未启用特性时进入可达路径。
  - 已满足：`crates/ui-components/src/css.rs` 仅在 `#[cfg(feature = "component-color_handle")]` 下聚合 `crate::color::handle::styles::CSS`，无条件全局聚合被避免。
  - 验证：`cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-color_handle,inject-css` 输出仅包含 `component-color_handle -> component-color_thumb -> component-color_swatch` 与 `inject-css`，未出现 `all-components`。
- [x] 语义测试与性能回归：断言必须覆盖 `aria-*`、`data-*` 与焦点流转，不能只看快照；高频/重型组件必须补齐 `render_count` 断言/测量（如初始化空闲预算为 1）。
  - 已满足：`components/color-handle/test/semantics.rs` 新增 `color_handle_semantic_regression_covers_aria_data_and_focus_path`，锁定 `role/aria-label`、`data-state/data-focused/data-aria-source`、`focus` 状态流转（`is_focused -> logic -> view -> thumb`），并显式禁止 snapshot-only 断言依赖。
  - 已满足（等价证据路径）：`components/color-handle/test/semantics.rs` 新增 `color_handle_performance_regression_keeps_render_count_equivalent_evidence`，约束渲染路径为有界 memo（<=3）且无 `effect/watch/spawn_local` 等洪泛触发，并保留 `data-state/data-motion-source` 可归因标记。
  - render_count 跟踪：断言 `scripts/check-ui-components-performance.sh` 持续执行 `perf_render_count_follow_up_is_tracked_in_plan`，并校验 `docs/plan/TODO.md` 仍保留 `render_count` 自动化补齐任务，符合“当前框架未提供通用精确计数时采用等价证据 + 跟踪闭环”。
- [x] 版本弃用迁移（Codemod/Registry）：若提交包含跨大版本 API 破坏升级，必须在 Schema Registry 注册弃用窗口并提供纯函数迁移层（`migrate_v1_to_v2`）。
  - N/A（当前提交未触发跨大版本破坏升级）：`ColorHandle` 公共导出面与 props 语义未发生破坏性变更，本次仅做语义测试/性能回归与 checklist 收敛；因此无需新增 Schema Registry 弃用窗口，也无需提供 `migrate_v1_to_v2` 迁移层。
- [x] 文档即产品（Copy-Paste Ready）：`apps/docs-app` 必须新增 Playground（Hello World、状态矩阵、受控/非受控对照），支持流式/快照展现，并提供 Source-first 一键复制且补全 imports。
  - 已满足：`apps/docs-app/src/pages/components/pages/forms_color.rs` 的 `color_handle()` 页面已补齐 `Hello World`、`State Matrix`、`Controlled vs Uncontrolled (N/A)`、`Streaming Optional / Snapshot` 四类 Playground；其中受控/非受控以组件能力边界明确标注 N/A（`ColorHandle` 无 `value/default/on_value_change` 状态轴）。
  - 已满足：页面增加 `data-ui-streaming="optional"` / `data-ui-fallback="snapshot"` / `data-ui-output-state="snapshot"` 可见展现与说明，流式策略明确为 optional + snapshot fallback。
  - 已满足：页面新增 Copy-Paste Ready 区块，声明 `apps/docs-app/src/playground.rs::compose_copy_ready_code` 导入补全机制，并标注 source-first 路径 `components/color-handle/src/view.rs|logic.rs|styles.rs`。
  - 回归：`components/color-handle/test/semantics.rs` 新增 `color_handle_docs_page_is_copy_paste_ready_with_required_playgrounds`，锁定上述文档契约字段，防止回退。
- [x] 语义测试优先：验证 `data-*` / `aria-*` / role / 状态来源契约，不只视觉快照。
  - 每个交互组件至少有对应 `*_semantics.rs` 测试覆盖关键状态轴与动作语义。
  - 断言应聚焦语义契约（状态来源/可访问性/键盘路径），快照仅作补充。
  - 新增/变更语义字段必须同步补测试，否则不得打勾。
  - 已满足：组件语义回归集中在 `components/color-handle/test/semantics.rs`（组件目录固定 `semantics.rs` 入口），覆盖 `role/aria-label`、`data-state/data-focused/data-aria-source`、`data-ui-source/action/state` 等关键契约字段。
  - 已满足：`color_handle_semantic_regression_covers_aria_data_and_focus_path` 明确覆盖焦点语义链路（`is_focused -> logic -> view -> thumb`）与状态来源标记，断言聚焦语义契约而非视觉快照。
  - 已满足：新增/变更语义字段均有同步回归（如 `color_handle_agent_contract_markers_are_schema_typed_and_machine_readable`、`color_handle_docs_page_is_copy_paste_ready_with_required_playgrounds`），防止语义字段漂移。
- [x] E2E 选择器稳定：使用语义标记，WASM 场景有稳定等待策略。
  - E2E 选择器优先 `data-*` 语义标记，禁止依赖脆弱 DOM 层级或文本定位。
  - WASM 场景必须使用稳定等待策略（语义状态就绪而非固定 sleep）。
  - 若组件涉及异步/动画，E2E 需显式覆盖 ready/settled 条件。
  - 已满足：新增 `e2e/tests/docs_app_color_handle_contract.spec.mjs`，使用 `data-component/data-slot/data-state/data-ui-*` 作为稳定选择器；`gotoColorHandle/resolveWorkbenchHandle` 通过 `body:not(:has(#boot))` 与 `toHaveAttribute(...)` 执行 wasm-safe ready/settled 等待（无 `waitForTimeout`/固定 sleep）。
  - 已满足：`apps/docs-app/src/pages/components/pages/forms_color.rs` 的 color-handle workbench 控件补齐 `data-slot`（如 `color-handle-workbench-dragging/motion/disabled`），并在 E2E 覆盖 motion+dragging+disabled 收敛条件（`data-state`、`data-ui-action`、`data-ui-output-status`、`style --ui-color-handle-motion-duration`）。
- [x] 关键流程纳入可重复回归集合（Playwright/Cypress）。
  - 至少定义一条可重复关键流程（打开/交互/关闭或提交）纳入 E2E 回归。
  - 回归失败需可定位到具体语义契约断点，而不是笼统“页面不一致”。
  - 高风险路径（overlay、focus、keyboard、async）优先进入回归集合。
  - 已满足：`e2e/tests/docs_app_color_handle_contract.spec.mjs` 提供可重复关键流程 `docs-app color-handle flow uses semantic settled conditions for motion/drag state`，覆盖交互链路 `focused -> dragging -> focused -> color -> disabled`，并通过 `page.reload()` 复验初始语义状态，形成稳定回归闭环。
  - 已满足：失败定位基于语义契约断点（`data-state`、`data-ui-action`、`data-ui-source`、`data-ui-output-status`、`data-motion-source` 与 `style --ui-color-handle-motion-duration`），而非页面像素或文本比对。
  - 已满足（按组件适用范围）：本组件无 overlay/async 内建流程；高风险路径优先覆盖 `focus + dragging + motion settled`，并使用稳定语义等待而非固定 sleep。
- [x] docs-app 文档、示例、参数矩阵、状态矩阵同步更新。
  - 组件行为或参数变更必须同步更新 `apps/docs-app` 示例与说明。
  - 文档示例需覆盖至少一组状态矩阵（受控/非受控、disabled、size/variant 等）。
  - 文档中的 API 名称与默认值必须和 `logic.rs` 当前实现一致。
  - 已满足：`apps/docs-app/src/pages/components/pages/forms_color.rs` 的 `color_handle()` 同步提供 `Hello World`、`State Matrix`、`Controlled vs Uncontrolled (N/A)`、`Disabled + Custom Class + Loupe Off` 与 `Parameter Matrix Workbench`；状态矩阵标记为 `data-slot="color-handle-state-matrix"`，参数矩阵控件标记为 `data-parameter-matrix="color-handle"`。
  - 已满足：文档示例参数命名与实现契约一致（`is_disabled/is_focused/is_dragging/is_loupe_visible/x_percent/y_percent/motion`），并在页面显式暴露默认值说明 `is_loupe_visible=true, x_percent=50.0, y_percent=50.0, motion=ColorHandleMotion::default()`。
  - 回归：`components/color-handle/test/semantics.rs` 新增 `color_handle_docs_matrices_and_api_defaults_stay_in_sync_with_view_contract`，同时锁定 docs 矩阵字段与 `components/color-handle/src/view.rs` 的 props 默认值声明，防止后续漂移。
- [x] 组件文档必须对新手友好（Documentation as Product）：组件 README 或等价文档入口必须存在。
  - 每个基础组件必须提供“零门槛”最小示例（Hello World）与常见用法，避免要求用户先理解底层分层架构。
  - 文档需明确“先用起来，再进阶”：默认 API 路径在前，高级控制参数在后。
  - “只有源码没有文档”或“只写给架构师/机器看的文档”视为不通过。
  - 已满足：`components/color-handle/src/README.md` 已提供 `## 先用起来（Hello World）` 最小示例（仅 `id_base + color`）与默认值说明，保证新手不需要先理解内部分层即可上手。
  - 已满足：README 新增 `## 常见用法（先基础，后进阶）`，先列 `is_focused/is_dragging/is_disabled` 的基础路径，再列 `x_percent/y_percent/is_loupe_visible/class_name/motion` 进阶参数，文档顺序与学习曲线一致。
  - 回归：`components/color-handle/test/semantics.rs` 新增 `color_handle_readme_is_beginner_friendly_and_progressive`，锁定 README 的 Hello World 与“先基础后进阶”结构，防止文档回退为“只写给架构师/机器”。
- [x] `apps/docs-app` 必须提供 Interactive Playground：用户可在线修改 props/状态并实时预览。
  - Playground 至少支持基础 props 调整、状态切换、交互反馈观察。
  - 对 AI Spec 相关组件，至少提供一组 Spec 输入与预览输出的联动示例。
  - Playground 作为验收面，需可重复复现关键交互路径。
  - 已满足：`apps/docs-app/src/pages/components/pages/forms_color.rs` 的 `color_handle()` 提供 `Parameter Matrix Workbench (Display + Config + Code + CSS Test)`，支持颜色、位置、`is_disabled/is_focused/is_dragging/is_loupe_visible`、`motion` 等基础 props/状态在线调整，并实时渲染 `Configured` 预览。
  - 已满足：workbench 通过 `code_signal=workbench_code` 与 `test_config_signal=workbench_actual_config` 输出配置与预览联动快照，形成“输入配置 -> 实际渲染状态”可观测链路。
  - 已满足：`e2e/tests/docs_app_color_handle_contract.spec.mjs` 以语义标记回归复现关键交互路径（toggle + motion + reload），保证 Playground 可重复验收。
  - N/A（AI Spec 联动要求按组件适用范围）：`ColorHandle` 非 `spec.rs` 驱动组件，当前无 `*Spec::new()...render()` 输入面；本组件以 `workbench_code/workbench_actual_config` 作为等价“配置输入 -> 预览输出”联动证据。
- [x] Source-first 文档必须 Copy-Paste Ready：提供一键复制组件源码或最小可用片段能力。
  - docs-app 页面应提供复制按钮，输出代码默认可直接运行（含必要 imports/依赖提示）。
  - 若为 source-first 组件，文档需指向真实源码落点并说明依赖前提，避免“复制即报错”。
  - 文档代码与当前实现必须同步，防止示例漂移。
  - 已满足：`e2e/tests/docs_app_color_handle_contract.spec.mjs` 新增 `docs-app color-handle playground source is copy-paste ready`，断言 workbench 代码区存在复制入口（`Show/Hide code` + `data-copyable="true"` + Copy 按钮 `aria-label`），并校验输出代码包含可运行 imports（`use leptos::prelude::*;`、`use ui_components::*;`）与 `<ColorHandle` 片段。
  - 已满足：docs 页面 source-first 说明卡 `data-slot="color-handle-copy-ready"` 明确指向 `apps/docs-app/src/playground.rs::compose_copy_ready_code` 与真实源码落点 `components/color-handle/src/view.rs|logic.rs|styles.rs`，满足“复制不报错 + 可追溯实现”。
  - 回归：`components/color-handle/test/semantics.rs` 新增 `color_handle_source_first_docs_are_copy_paste_ready_and_synced`，同时锁定 docs/source/e2e 三侧 token，防止示例与实现漂移。
- [x] HeroUI 对标文档与组件文档同步：参数模型变更需同步 `docs/spec/heroui-parameter-design-strategy.md`（必要时补充 `docs/research/spectrum-heroui-style-interface-study.md`），并保证组件文档可访问。
  - 若参数语义发生变化，需同步更新对标策略文档，不允许实现先漂移文档后补。
  - 组件文档入口必须存在（docs-app 页面或等价文档），且可被索引定位。
  - “仅代码更新无文档更新”在接口变更场景下直接判不通过。
  - 已满足：`docs/spec/heroui-parameter-design-strategy.md` 已新增 `### ColorHandle 同步记录（2026-02-20）`，同步记录参数主轴、默认值锚点与 docs 入口映射，避免实现与对标文档漂移。
  - 已满足：`apps/docs-app/src/pages/components/pages.rs` 通过 `component_doc!("ColorHandle", "color-handle", "Forms", forms_color::color_handle)` 暴露可索引入口；`apps/docs-app/src/pages/components/pages/forms_color.rs` 保持 `color_handle()` 页面可访问。
  - N/A（research 文档追加）：本轮仅为 ColorHandle 参数模型与文档入口对齐，未新增 Spectrum/HeroUI 风格研究结论，因此不需要补充 `docs/research/spectrum-heroui-style-interface-study.md`。
  - 回归：`components/color-handle/test/semantics.rs` 新增 `color_handle_heroui_strategy_and_docs_entry_stay_synchronized`，锁定对标文档片段、docs 索引入口与本清单勾选状态，防止后续回退。
