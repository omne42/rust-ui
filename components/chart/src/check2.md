...
### 执行记录（2026-02-18）
- `component-chart` 最小特性链已修复为 `component-chart = ["dep:ui-chart"]`（`crates/ui-components/Cargo.toml`）。
...
- 验证命令（逐项）：
  - `rustfmt --edition 2024 --check crates/ui-state-primitives/src/chart.rs crates/ui-headless/src/chart.rs components/chart/src/logic.rs components/chart/src/motion.rs components/chart/src/view.rs crates/ui-components/tests/chart_semantics.rs` ✅
  ...
  - `cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-chart,inject-css` ✅（显示 `component-chart -> dep:ui-chart`）
