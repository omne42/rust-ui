...
### 执行记录（2026-02-18）
- `component-chart` 最小特性链已修复为 `component-chart = ["dep:ui-chart"]`（`crates/ui/Cargo.toml`）。
...
- 验证命令（逐项）：
  ...
  - `cargo tree -e features -i ui -p ui --no-default-features --features component-chart,inject-css` ✅（显示 `component-chart -> dep:ui-chart`）
  - `cargo tree -e features -i ui -p web-demo` ✅（`all-components: ABSENT`，仅 `web-demo-components` 链路）
