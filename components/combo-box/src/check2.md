...
- [x] Tree Shaking 是一等能力：package 模式支持组件级 feature；source 模式天然裁剪；样式层同步裁剪，禁止无条件聚合全部 CSS，禁止破坏 DCE/LTO 的全量中央注册表。（已修复 `component-combo_box` 的特性图，补齐对 `component-active_highlight` 与 `component-popover` 的显式依赖，避免最小特性构建缺模块。）
...
- [x] SSR 与跨平台检查：覆盖 web/ssr/wasm 分支，不破坏 non-wasm 编译路径。（本轮已实测组件最小特性路径：`cargo test -p ui --test combo_box_semantics --no-default-features --features component-combo_box,inject-css` 通过。）
...
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。（`ComboBox` 非 LLM 正文阅读面，归类为 `Streaming Optional`；当前组件以稳定 `Snapshot` 渲染为基线，上层若启用流式输出需走 `fallback=snapshot`。）
...
- [x] E2E 选择器稳定：使用语义标记，WASM 场景有稳定等待策略。（组件页回归纳入 `e2e/tests/docs_app_components_coverage.spec.mjs`，通过 `data-slot="combo-box"` 与 `body:not(:has(#boot))` 就绪条件验证，未依赖固定 sleep。）
  - E2E 选择器优先 `data-*` 语义标记，禁止依赖脆弱 DOM 层级或文本定位。
  - WASM 场景必须使用稳定等待策略（语义状态就绪而非固定 sleep）。
  - 若组件涉及异步/动画，E2E 需显式覆盖 ready/settled 条件。
- [x] 关键流程纳入可重复回归集合（Playwright/Cypress）。（关键流程“进入组件页 -> Playground 可见 -> `data-slot=combo-box` 可见”已进入 docs-app 组件覆盖回归集合。）
...
- [x] HeroUI 对标文档与组件文档同步：参数模型变更需同步 `docs/spec/heroui-parameter-design-strategy.md`（必要时补充 `docs/research/spectrum-heroui-style-interface-study.md`），并保证组件文档可访问。（本轮 `ComboBox` 未发生参数语义变更，策略文档同步要求按 N/A 记录；docs 入口与示例矩阵保持可访问且与实现一致。）
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（组件作用域门禁已通过记录：`rustfmt components/combo-box/test/combo_box/semantics.rs`、`cargo test -p ui --test combo_box_semantics --no-default-features --features component-combo_box,inject-css`、`cargo clippy -p ui --test combo_box_semantics --no-default-features --features component-combo_box,inject-css -- -D warnings`；仓库默认特性全量编译当前被并行改动中的其他模块阻塞，已观测错误位于 `ui-headless/color_area` 与 `ui/color_slider/dropdown`，非本组件改动引入。）
