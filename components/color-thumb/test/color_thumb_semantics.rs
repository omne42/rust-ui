...
fn color_thumb_uses_logic_state_model() {
    ...
    let primitive_source = load_source("../ui-state-primitives/src/color_thumb.rs");
    let primitive_lib_source = load_source("../ui-state-primitives/src/lib.rs");
    let headless_source = load_source("../ui-headless/src/color_thumb.rs");
    ...
    for needle in [
        ...
        "pub const DEFAULT_ARIA_VALUE_TEXT",
        ...
        assert!(
            ...
            "ColorThumb logic should bridge ui-state-primitives and include `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "ColorThumb state primitive should define `{needle}` in ui-state-primitives."
        );
    }

    assert!(
        primitive_lib_source.contains("pub mod color_thumb;"),
        "ui-state-primitives should export `color_thumb` module."
    );
    ...
    for needle in [
        ...
        "pub fn use_color_thumb(",
    ] {
        assert!(
            headless_source.contains(needle),
            "ColorThumb headless contract should include `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "ui-headless color_thumb should not include style/motion token `{forbidden}`."
        );
    }

    assert!(
        headless_lib_source.contains("pub mod color_thumb;"),
        "ui-headless should export `color_thumb` module."
    );
    ...
    for forbidden in ["role=\"slider\"", "tabindex=if disabled { -1 } else { 0 }"] {
        assert!(
            !view_source.contains(forbidden),
            "ColorThumb view should consume headless attrs/handlers; found direct semantics `{forbidden}`."
        );
    }
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "ColorThumb view should not rebuild state normalization rule `{forbidden}`."
        );
    }
}
...
#[test]
fn color_thumb_theme_contract_consumes_ui_theme_variables() {
    let source = load_source("../../components/color-thumb/src/styles.rs");
    ...
    for needle in [
        ...
        assert!(
            source.contains(needle),
            "ColorThumb styles should consume ui-theme variables via `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "ColorThumb styles should not rebuild theme/private palette token `{forbidden}`."
        );
    }
}

#[test]
fn color_thumb_motion_contract_stays_component_mapping_only() {
    let source = load_source("../../components/color-thumb/src/motion.rs");
    ...
    for needle in [
        ...
        assert!(
            source.contains(needle),
            "ColorThumb motion contract should include `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !source.contains(forbidden),
            "ColorThumb motion layer should not implement shared engine/runtime token `{forbidden}`."
        );
    }
}
...
fn color_thumb_component_files_keep_single_responsibility() {
    let css_registry_source = load_source("../ui/src/css.rs");
    let ui_root_source = load_source("../ui/src/root.rs");
    let lib_source = load_source("../../components/color-thumb/src/lib.rs");
    let mod_source = load_source("../../components/color-thumb/src/mod.rs");
    let logic_source = load_source("../../components/color-thumb/src/logic.rs");
    let styles_source = load_source("../../components/color-thumb/src/styles.rs");
    let view_source = load_source("../../components/color-thumb/src/view.rs");
    let motion_source = load_source("../../components/color-thumb/src/motion.rs");
    ...
    for required in [
        ...
        assert!(
            ...
            "ColorThumb mod.rs should keep export boundary marker `{required}`."
        );
    }
    for forbidden in [
        ...
        assert!(
            ...
            "ColorThumb mod.rs should not carry implementation details like `{forbidden}`."
        );
    }
    for forbidden in ["mod spec;", "pub mod spec;", "spec::"] {
        assert!(
            !lib_source.contains(forbidden),
            "ColorThumb lib.rs should not expose spec module token `{forbidden}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "ColorThumb logic.rs should not include DOM/view/style detail `{forbidden}`."
        );
    }
    ...
    assert!(
        ...
        "ColorThumb styles.rs should remain token-first static css contract."
    );
    for forbidden in [
        "fn ",
        "use ui_headless::",
        ...
        assert!(
            ...
            "ColorThumb styles.rs should not include runtime logic token `{forbidden}`."
        );
    }
    ...
    for required in [
        ...
        assert!(
            ...
            "ColorThumb view.rs should mount structure + headless contract via `{required}`."
        );
    }
    for forbidden in [
        "pub const CSS: &str",
        ...
        assert!(
            !view_source.contains(forbidden),
            "ColorThumb view.rs should not include style-engine detail `{forbidden}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "ColorThumb motion.rs should not include shared engine/view token `{forbidden}`."
        );
    }
...
fn color_thumb_docs_are_copy_paste_ready_with_hello_world_state_matrix_and_streaming_snapshot() {
    ...
    let check2_source = load_source("../../components/color-thumb/check2.md");
    let check2_src_source = load_source("../../components/color-thumb/src/check2.md");
...
fn color_thumb_docs_entry_exists_as_readme_or_equivalent_docs_app_page() {
    let readme_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../components/color-thumb/src/README.md");
    let docs_source = load_source("../../apps/docs-app/src/pages/components/pages/forms_color.rs");
...
fn color_thumb_docs_are_beginner_friendly_with_default_then_advanced_path() {
    let docs_source = load_source("../../apps/docs-app/src/pages/components/pages/forms_color.rs");
    let readme_source = load_source("../../components/color-thumb/src/README.md");
...
fn color_thumb_docs_app_provides_interactive_playground_for_props_state_and_preview() {
    ...
    let section_start = docs_source
        .find("pub(super) fn color_thumb() -> AnyView {")
        .expect("forms_color docs should contain color_thumb section");
    let section_tail = &docs_source[section_start..];
    let section_end_rel = section_tail
        .find("\npub(super) fn color_editor() -> AnyView {")
        .expect("forms_color docs should contain color_editor section after color_thumb");
    let section = &section_tail[..section_end_rel];
...
fn color_thumb_interactive_playground_reuses_repeatable_semantic_e2e_flow() {
    let e2e_source = load_source("../../e2e/tests/docs_app_color_thumb_contract.spec.mjs");
...
fn color_thumb_e2e_check_script_covers_interactive_playground_contract() {
    let script_source = load_source("../../components/color-thumb/scripts/check-ui-e2e-color-thumb.sh");
...
#[test]
fn color_thumb_semantic_regression_matrix_covers_interaction_and_platform_paths() {
    let headless_tests = load_source("../ui-headless/src/test/color_thumb.rs");
    let motion_tests = load_source("../../components/color-thumb/test/motion.rs");
    ...
    for required in [
        ...
        assert!(
            headless_tests.contains(required) || motion_tests.contains(required),
            "ColorThumb semantic regression matrix should include `{required}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "ColorThumb semantic verification should not be snapshot-only via `{forbidden}`."
        );
    }
}

#[test]
fn color_thumb_performance_governance_contract_is_budgeted_traceable_and_blocking() {
    let check2 = load_source("../../components/color-thumb/check2.md");
...
fn color_thumb_semantics_and_performance_regression_cover_aria_data_focus_and_render_count_measurement()
 {
    let local_semantics = include_str!("../../../components/color-thumb/test/semantics.rs");
...
fn color_thumb_check2_documents_e2e_selector_and_stable_wait_rules() {
    let check2 = load_source("../../components/color-thumb/check2.md");
    let check2_src = load_source("../../components/color-thumb/src/check2.md");
    let script_source = load_source("../../components/color-thumb/scripts/check-ui-e2e-color-thumb.sh");
...
fn color_thumb_view_macro_complexity_is_split_into_semantic_subrenders() {
    ...
    for needle in [
        "- [x] `view!` 宏复杂度受控",
        "`render_decorative_swatch(color, class_name)`",
        "`if let Some(color) = color.get_value()`",
        "color_thumb_view_macro_complexity_is_split_into_semantic_subrenders",
        "`scripts/check-ui-view-macro.sh`",
    ] {
        assert!(
            check2.contains(needle),
            "color-thumb check2 should include view-macro governance token `{needle}`.",
        );
    }
...
fn color_thumb_inner_html_usage_is_forbidden_in_component_and_docs_examples() {
    ...
    assert!(
        script_source.contains(script_needle),
        "inner-html check script should enforce ColorThumb contract marker `{script_needle}`.",
    );
}
...
fn color_thumb_agent_contract_is_schema_typed_and_machine_readable() {
    ...
    let logic_source = load_source("../../components/color-thumb/src/logic.rs");
    let view_source = load_source("../../components/color-thumb/src/view.rs");
    let component_manifest = load_source("../../components/color-thumb/src/Component.toml");
    let component_rbi = load_source("../../components/color-thumb/src/color_thumb.rbi");
...
fn color_thumb_hyper_structure_builder_spec_is_not_applicable_for_simple_component() {
    let lib_source = load_source("../../components/color-thumb/src/lib.rs");
    let mod_source = load_source("../../components/color-thumb/src/mod.rs");
    let protocol_source = load_source("../../components/color-thumb/src/protocol.rs");
    let check2_source = load_source("../../components/color-thumb/check2.md");
    let check2_src_source = load_source("../../components/color-thumb/src/check2.md");
    let script_source = load_source("../../scripts/check-ui-component-files.sh");
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
...
fn color_thumb_version_deprecation_migration_registry_is_explicitly_na_without_major_breaking_upgrade()
 {
    ...
    for source in [
        load_source("../../components/color-thumb/check2.md"),
        load_source("../../components/color-thumb/src/check2.md"),
    ] {
        for needle in [
...
#[test]
fn color_thumb_check2_marks_api_naming_contract_completed() {
    let check2 = load_source("../../components/color-thumb/check2.md");
    assert!(
    ...
    assert!(
        ...
        "color-thumb check2 should mark controlled/uncontrolled pairing contract as completed.",
    );
    assert!(
        check2.contains("N/A（已论证）"),
        "color-thumb check2 should include explicit N/A rationale for controlled/uncontrolled pairing.",
    );
    assert!(
    ...
    assert!(
        check2
            .contains("`components/color-thumb/src/*.rs` 未引用 `web-sys`/`wasm_bindgen`/`js_sys`"),
        "color-thumb check2 should include explicit non-wasm no-web-sys evidence.",
    );
    assert!(
        check2.contains("`crates/ui-motion/src/lib.rs` 的 `wasm32/non-wasm cfg` 与 non-wasm no-op"),
        "color-thumb check2 should include explicit wasm/non-wasm cfg evidence for motion backend.",
    );
    assert!(
        check2.contains("`crates/ui-headless/src/lib.rs` 的 `web/ssr` 互斥 `compile_error!`"),
        "color-thumb check2 should include explicit web/ssr mutual-exclusion evidence.",
    );
    assert!(
        check2.contains("`cargo check -p ui-color-thumb --target wasm32-unknown-unknown` 与 `cargo check -p ui-headless --no-default-features --features ssr`"),
    ...
    assert!(
        check2.contains("`ColorThumb` 仅在 `components/color-thumb/src/view.rs` 消费 `ui_headless::{use_color_thumb, ColorThumbOptions}`"),
        "color-thumb check2 should include explicit headless-consumption-only integration evidence.",
    );
    assert!(
        check2.contains("`cargo check -p ui-headless --no-default-features --features ssr` 与 `cargo check -p ui-headless --no-default-features --features web,ssr`"),
    ...
    assert!(
        check2.contains("`#[cfg(not(target_arch = \"wasm32\"))] pub mod web { ... }`"),
    ...
    assert!(
        check2.contains("`non_wasm_web_backend_is_predictable_noop` 与 `components/color-thumb/test/motion.rs` 的 `attach_motion_exports_css_variables`"),
    ...
    assert!(
        check2.contains("`resolve_runtime_motion` 在 `ui_motion::web::prefers_reduced_motion()` 为真时将时长降级到 `1ms`"),
        "color-thumb check2 should include explicit reduced-motion downgrade evidence.",
    );
    assert!(
        check2.contains("`crates/ui-motion/src/web.rs` 在 wasm 分支 `prefers-reduced-motion` 命中时直接跳过 `animate(...)`"),
        "color-thumb check2 should include explicit wasm reduced-motion skip evidence.",
    );
    assert!(
        check2.contains("`role/aria/data-*` 语义全部来自 `use_color_thumb`，不依赖 wasm 专属分支"),
    ...
    assert!(
        ...
        ) && check2.contains("mount-only 等价证据"),
        "color-thumb check2 should include explicit render_count follow-up tracking evidence.",
    );
    assert!(
        check2.contains("color_thumb_performance_governance_budget_is_defined_and_blocking"),
    ...
    assert!(
        check2.contains("- [x] 代码卫生（Rust Hygiene）：非测试代码中完全禁止 `unwrap/expect`，禁止无处理的 `let _ = ...`；字符串复制热点收敛为 `Cow<'static, str>`"),
    ...
    assert!(
        check2
            .contains("color_thumb_static_fragments_are_constantized_or_absent_for_simple_layout"),
        "color-thumb check2 should include explicit static-fragment regression test evidence.",
    );
    assert!(
        check2.contains("color_thumb_inner_html_usage_is_forbidden_in_component_and_docs_examples"),
    ...
    assert!(
        check2.contains("color_thumb_styles_use_defensive_variable_fallback_chain"),
        "color-thumb check2 should include explicit defensive-variable regression test evidence.",
    );
    assert!(
        check2.contains("color_thumb_cascade_layer_and_runtime_style_contract_is_enforced"),
        "color-thumb check2 should include explicit cascade-layer regression test evidence.",
    );
    assert!(
        check2
            .contains("color_thumb_motion_contract_is_builtin_and_attached_with_reduced_motion_and_non_wasm_noop"),
        "color-thumb check2 should include explicit motion-contract regression test evidence.",
    );
    assert!(
        check2.contains("color_thumb_ui_components_fixed_entry_files_follow_layered_boundaries"),
    ...
    assert!(
        check2.contains("color_thumb_component_files_script_covers_standard_layout_contract"),
    ...
    assert!(
        check2.contains("color_thumb_agent_contract_is_schema_typed_and_machine_readable"),
    ...
    assert!(
        check2
            .contains("color_thumb_snapshot_baseline_consumes_complete_result_and_renders_stably"),
    ...
    assert!(
        check2.contains("color_thumb_rust_hygiene_script_enforces_repo_level_hygiene_guards"),
    ...
    assert!(
        check2.contains("`SLOT_COLOR_THUMB*`、`CLASS_COLOR_THUMB*`、`BOOL_TRUE`"),
    ...
    assert!(
        check2.contains(
            "`cargo tree -e features -i ui -p web-demo` 输出未出现 `all-components`"
        ),
    ...
    assert!(
        check2.contains("- [x] 类型系统 + 语义标记共同提供机器可读状态"),
        "color-thumb check2 should mark typed machine-readable state contract item as completed.",
    );
    assert!(
        check2.contains(
            "`ColorThumbInteractionState` / `ColorThumbInputSource` / `ColorThumbAriaValueTextSource`"
        ),
        "color-thumb check2 should include explicit enum-backed state-axis rationale.",
    );
    assert!(
        check2.contains("`interaction_state_from_flags + resolve_component_state`"),
        "color-thumb check2 should include explicit logic normalization rationale.",
    );
    assert!(
        check2
            .contains("`data-state`、`data-*-source`、`data-x-bucket`、`data-y-bucket`、`aria-*`"),
        "color-thumb check2 should include explicit machine-readable semantic marker rationale.",
    );
    assert!(
        check2.contains(
            "`components/color-thumb/test/logic.rs`、`components/color-thumb/test/semantics.rs` 与 `components/color-thumb/test/color_thumb_semantics.rs`"
        ),
    ...
    assert!(
        check2
            .contains("`component-color_thumb` 已注册在 `crates/ui/Cargo.toml` 特性树"),
