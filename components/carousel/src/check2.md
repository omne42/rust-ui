...
### 10. 本轮验收记录（Carousel）
- 关键修复：`component-carousel` 最小特性编译原先失败（缺少 `active_highlight` 特性依赖），已修复为 `component-carousel = ["component-active_highlight"]`（`crates/ui/Cargo.toml`）。
- 语义契约回归：`cargo test -p ui --test carousel_semantics`（隔离 target 复验）通过，14/14 用例通过。
- 最小特性编译：`cargo check -p ui --no-default-features --features component-carousel,inject-css` 通过。
- wasm 路径编译：`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-carousel,inject-css` 通过。
- 静态检查：`cargo clippy -p ui --no-default-features --features component-carousel,inject-css -- -D warnings` 通过。
