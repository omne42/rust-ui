...
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。
  ...
  - StepList 归类为 `Streaming Optional` 且当前实现为 `N/A`（snapshot-only，`fallback=snapshot`）。
...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - 说明：本次按 `step_list` 负责范围已执行并通过 `rustfmt + clippy(lib/test-step_list_semantics) + test(step_list_semantics) + check(native/wasm)`；`docs-app` smoke 当前受仓库其他文件编译错误阻塞（`apps/docs-app/src/pages/components/pages/layout_extra.rs`），未在本任务范围内修复。
  - 逐项证据：`step_list_semantics` 已按 `--exact` 单条串行逐个执行，结果 `ALL_STEP_LIST_CHECKS_INDIVIDUAL_PASS=46`。
