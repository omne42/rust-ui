...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - `popover` 组件最小门禁已验证：`cargo test -p ui --test popover_semantics --no-default-features --features component-popover,inject-css`、`cargo check -p ui --no-default-features --features component-popover,inject-css`、`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-popover,inject-css`、`cargo clippy -p ui --no-default-features --features component-popover,inject-css --tests -- -D warnings`。
