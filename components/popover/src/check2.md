...
已核验（popover，2026-02-18）：本组件是 overlay 容器，核心职责是 open/close、anchor 定位、modal/non-modal 语义与退出动效契约映射，不承载远程异步流程与流式正文渲染职责；相关条目按 N/A 语义核验并保持契约可追溯。
...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - `popover` 组件最小门禁已验证：`cargo test -p ui-components --test popover_semantics --no-default-features --features component-popover,inject-css`、`cargo check -p ui-components --no-default-features --features component-popover,inject-css`、`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-popover,inject-css`、`cargo clippy -p ui-components --no-default-features --features component-popover,inject-css --tests -- -D warnings`。
