...
- 语义契约回归：`cargo test -p ui --test field_error_semantics --no-default-features --features component-field_error,inject-css` 通过，6/6 用例通过。
- 最小特性编译（native）：`cargo check -p ui --no-default-features --features component-field_error,inject-css` 通过。
- 最小特性编译（wasm）：`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-field_error,inject-css` 通过。
- 静态检查：`cargo clippy -p ui --no-default-features --features component-field_error,inject-css -- -D warnings` 通过。
- Tree Shaking 约束：`component-field_error` 为独立 feature（`crates/ui/Cargo.toml`），`lib.rs` 与 `css.rs` 均按 feature 条件导出/聚合。
