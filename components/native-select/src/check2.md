...
### 10. NativeSelect 组件核验证据（本次）
- 架构边界：`crates/ui-components/src/native_select/` 维持 `mod.rs + logic.rs + styles.rs + view.rs` 分层；`logic.rs` 负责归一化与状态派生，`view.rs` 负责渲染与挂载，不在组件层重写可复用状态原语。
- 状态与所有权：离散输入由 `NativeSelectSize` 枚举约束；`NativeSelectStateInput/NativeSelectState` 统一建模 `selected/invalid/disabled/empty` 等状态轴与来源标记；选项通过 `normalize_options/resolve_options/sanitize_selected_index` 在逻辑层集中归一。
- 语义与可观测：`view.rs` 输出稳定 `data-*`（`data-state/data-size/data-selected-index/data-selected-value/data-aria-source/...`）与 `aria-*`（`aria-label/aria-invalid`），并暴露 `data-slot` 供自动化选择器使用。
- 样式契约：`styles.rs` 仅使用稳定 class 状态标记（size/invalid/disabled/empty/selected）驱动样式，不依赖脆弱 DOM 结构选择器。
- 文档与示例：`apps/docs-app/src/pages/components/pages/forms_native.rs` 提供 `NativeSelect` 页面三个 Playground（`Controlled + Placeholder`、`Required + Invalid + Disabled`、`Interactive Playground`）；Interactive 覆盖展示区 / Config 区 / Code 区 / CSS Test 区，并提供 `Show compare matrix` 多场景对比。
- README：`crates/ui-components/src/native_select/README.md` 已补齐，包含展示区 / Config 区 / Code 区 / CSS Test 区结构与对比矩阵说明。
- 语义测试：`cargo test -p ui-components --test native_select_semantics --no-default-features --features component-native_select,inject-css` 通过（7/7）。
- 最小特性编译：`cargo check -p ui-components --no-default-features --features component-native_select,inject-css` 通过。
- wasm 路径编译：`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-native_select,inject-css` 通过（存在 `observability.rs` 的既有 dead_code warning，非本组件新增）。
- Clippy 门禁：`cargo clippy -p ui-components --no-default-features --features component-native_select,inject-css -- -D warnings` 通过。
- N/A 明确项：异步协议、`inner_html` 注入、`Streaming Required`（`NativeSelect` 为表单选择控件，按 Snapshot 稳定渲染）；组件内未引入独立 `motion.rs`，无组件级动效引擎重写风险。
