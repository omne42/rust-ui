...
- 架构边界：`crates/ui/src/native_select/` 维持 `mod.rs + logic.rs + styles.rs + view.rs` 分层；`logic.rs` 负责归一化与状态派生，`view.rs` 负责渲染与挂载，不在组件层重写可复用状态原语。
...
- README：`crates/ui/src/native_select/README.md` 已补齐，包含展示区 / Config 区 / Code 区 / CSS Test 区结构与对比矩阵说明。
- 语义测试：`cargo test -p ui --test native_select_semantics --no-default-features --features component-native_select,inject-css` 通过（7/7）。
- 最小特性编译：`cargo check -p ui --no-default-features --features component-native_select,inject-css` 通过。
- wasm 路径编译：`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-native_select,inject-css` 通过（存在 `observability.rs` 的既有 dead_code warning，非本组件新增）。
- Clippy 门禁：`cargo clippy -p ui --no-default-features --features component-native_select,inject-css -- -D warnings` 通过。
