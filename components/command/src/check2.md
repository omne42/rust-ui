...
- [x] 组件内导入路径去根重导出耦合：`crates/ui-components/src/command_dialog/view.rs` 与 `components/modal/src/view.rs` 改为直接引用 `crate::modal` / `crate::overlay`，避免最小 feature 下因 root re-export 缺失而构建失败。
