...
- [x] 级联层覆盖（`@layer ui`）：组件 CSS 默认聚合进 `@layer ui`；运行时数值调整仅通过 CSS Custom Properties（如 `style:--x=...`），禁止普通内联样式（如 `style=\"top: 10px\"`）。（适用项已验证：`crates/ui/src/css.rs` 在 `@layer ui` 聚合 Link CSS；`Link` 运行时未使用普通内联样式）
...
- [x] `ui` 固定入口文件落点正确。（适用项已验证：`crates/ui/src/lib.rs`、`crates/ui/src/css.rs`、`crates/ui/src/root.rs` 落点与职责符合清单约束）
...
- [x] Tree Shaking & 特性剪裁：组件必须注册到 `ui` 特性树（如 `component-accordion`）；`css.rs` 和 `lib.rs` 聚合必须受 feature 门控，禁止无条件全局依赖。（适用项已验证：`component-link` 在 `ui` 特性树中受门控导出与 CSS 聚合）
