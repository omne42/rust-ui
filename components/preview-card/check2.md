...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。
  ...
  - N/A 理由：`PreviewCard` 仅包含本地 hover/focus/overlay 交互与静态文案归一，不发起远程请求，也不维护 `is_loading/error/retry` 异步状态轴（`img loading="lazy"` 不属于组件异步业务协议）。
...
- [x] 组合型组件主 API 必须“显示优于约定”：优先使用显式组合 `<Parent><Item ... /></Parent>`。
  - N/A 理由：`PreviewCard` 为单体 overlay 组件，不存在多 item 的组合容器语义；当前 API 为 `trigger + title/description/url/site_label/image_src`，不存在 `labels + children` 或 `titles + panels` 并行数组入口。
...
- [x] 宏观/微观双状态机（Macro/Micro Duality）：拖拽等高频交互在 `Dragging` 期间由 `view/motion` 本地循环执行；禁止每帧穿越回 `logic.rs`，必须在结束时通过 `Action::DragEnd` 回流收敛。
  - N/A 理由：`PreviewCard` 不包含拖拽/手势跟随类高频交互，不存在 `Dragging` 本地循环与 `Action::DragEnd` 回流场景；当前仅为 hover/focus 打开关闭与 popover 定位/动效挂载。
- [x] 几何两段式渲染（Two-Pass Rendering）：`Tooltip/Popover/Menu` 等依赖 DOM 测量的组件必须走 `Intent -> Measure(view) -> Rectification(logic)`，并具备幂等收敛保护防死循环。
  - `PreviewCard` 流程已对齐：`Intent` 来自 `logic.rs` 归一化后的 `placement`（`components/preview-card/src/logic.rs`），`Measure` 在 `view.rs` 通过 `anchor_ref/panel_ref` 调用 `use_popover_position`（`components/preview-card/src/view.rs`），`Rectification` 由 `ui-headless` 的 `compute_popover_position` 回写 `top/left/placement`（`crates/ui-headless/src/popover_position.rs`）。
  - 幂等收敛保护已存在：位置更新使用 `should_update_scalar` 的 epsilon 门槛避免抖动回写，调度侧使用 `raf_pending` 合并同帧重复测量，避免测量-回写死循环（`crates/ui-headless/src/popover_position.rs`；回归见 `crates/ui-headless/src/test/popover_position.rs`）。
- [x] 集合注册协议（Registration Protocol）：`Accordion/Tabs/Menu` 动态子项必须通过 `RegistrationContext` 上报 `Register/Unregister`，逻辑层维护 `items_order`，禁止依赖 `HashSet` 迭代顺序做导航。
  - N/A 理由：`PreviewCard` 是单触发器 + 单面板的 overlay，不存在动态子项集合、顺序导航或 `items_order` 维护需求；组件内也未使用 `RegistrationContext`/`Register`/`Unregister` 或依赖 `HashSet` 迭代顺序实现导航。
- [x] 插槽投影策略（Slot Projection）：容器组件明确 `Lazy/KeepAlive/Eager`；`KeepAlive` 隐藏时必须通过生命周期通知（如 `NotifyHidden`）暂停轮询/动画等高耗能副作用。
  - N/A 理由：`PreviewCard` 不是多子项/多插槽容器组件，不提供 `Lazy/KeepAlive/Eager` 可选投影模式；当前仅单面板通过 `presence + Show` 显隐挂载（`components/preview-card/src/view.rs`），且无隐藏态轮询/长动画等需要 `NotifyHidden` 协议暂停的高耗能副作用。
- [x] 环境订阅流（Env Streams）：`Resize/Theme/Intersection` 等环境变化在 `view.rs` 采样、防抖后转化为高层语义 `Action`（如 `BreakpointChanged`）推送到 `logic`；禁止原始事件洪泛。
  - `PreviewCard` 在 `view.rs` 通过 `use_popover_position` 消费环境流（`ResizeObserver + window resize/scroll`），而不是直接绑定原始事件（`components/preview-card/src/view.rs`）。
  - 事件防洪与语义化在 `ui-headless` 内完成：`raf_pending` 合并同帧更新、防止高频洪泛；输出为类型化位置语义 `top_px/left_px/placement`，组件仅消费该语义状态并映射到样式变量（`crates/ui-headless/src/popover_position.rs`）。
- [x] 事件光锥（Event Light Cone）：`Table/Grid` 等大型集合批量操作必须走 `Context Bus + Selector` 与状态压缩表达（如 `SelectionState::All`），禁止 O(N) 级向下 prop drilling。
  - N/A 理由：`PreviewCard` 是单触发器 + 单预览面板组件，不是大型集合容器，不存在批量选择/全选语义与 `SelectionState::All` 状态压缩需求；组件实现中也无 `Context Bus + Selector` 或 O(N) 向下 prop drilling 路径。
- [x] 统一因果总线（Causality Bus）：复杂派生总线操作必须支持透传 `TraceId`，确保“用户触发 -> 派生命令 -> 总线广播 -> 订阅者”因果链不断裂。
  - N/A 理由：`PreviewCard` 不存在复杂派生总线或多订阅者广播链路；当前仅为 trigger/panel 的本地回调与 `ui-headless` 语义契约消费，没有需要跨总线透传 `TraceId` 的因果追踪路径。
- [x] 存在 A11y 实现、国际化与本地化实现（至少具备接入点，不硬编码用户可见文本）。
  - A11y 语义由 `ui-headless` 契约驱动：触发器/面板交互走 `use_hover_card_*`，面板语义改为 `tooltip_panel_attrs(TooltipPanelA11yOptions)` 输出 `role/id/lang/dir`，组件层只挂载，不重复发明同名工具（`components/preview-card/src/view.rs`，`crates/ui-headless/src/a11y.rs`）。
  - i18n/l10n 接入点已具备：新增 `lang`/`dir` props，并通过 `locale_attrs` 归一后挂载到 root + portal panel，避免 Portal 场景丢失 locale 继承（`components/preview-card/src/view.rs`）。
  - 用户可见文本未在 `view.rs` 硬编码：标题/描述/链接/站点文案优先来自 props，经 `logic.rs` 归一后渲染；仅在 `logic.rs` 提供默认兜底常量（`components/preview-card/src/logic.rs`）。
...
- [x] 状态可观测、可检索、可验证：使用稳定 `data-*` 与 `aria-*` 标记表达状态和来源。
  - 已覆盖稳定状态轴：`open/closed`（`data-state` + `data-open` + `data-closed`）、`disabled/enabled`、`content(media/text)`、`placement`、`focus-a11y-managed`，并挂载 `role/aria-keyshortcuts`（`components/preview-card/src/view.rs`）。
  - 已覆盖来源标记：`data-*-source`（class/delay/id/title/description/url/site-label/motion）+ `data-custom-*`，值域为封闭集合（如 `custom|default`、`derived|default|custom`），无自由文本漂移（`components/preview-card/src/logic.rs` + `components/preview-card/src/view.rs`）。
  - 自动化可检索性已锁定：语义回归测试与样式契约均以稳定语义标记断言/选择，不依赖 DOM 顺序与临时 class（`components/preview-card/test/semantics.rs`、`components/preview-card/src/styles.rs`）。
  - `受控/非受控` 与 `默认值/外部值` 的来源轴在 `PreviewCard` 当前 API 中为单一内部 open 来源（未暴露受控 open 轴），该子项对 open 轴记为 N/A；现有可配置来源轴均已显式标记。
