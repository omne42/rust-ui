...
- [x] 存在 A11y 实现、国际化与本地化实现（至少具备接入点，不硬编码用户可见文本）。
  - A11y 语义由 `ui-headless` 契约驱动：`use_hover_card_*` + `tooltip_panel_attrs(TooltipPanelA11yOptions)` 输出 `role/id/lang/dir`，组件层仅挂载。
  - i18n/l10n 接入点已具备：新增 `lang`/`dir` props，并经 `locale_attrs` 归一后挂载 root 与 portal panel，避免方向/语言上下文丢失。
  - 用户可见文本优先来自 props，`logic.rs` 仅提供默认兜底；`view.rs` 未硬编码业务文案。
...
- [x] 状态可观测、可检索、可验证：使用稳定 `data-*` 与 `aria-*` 标记表达状态和来源。
  - 已覆盖 `open/closed`、`disabled/enabled`、`content`、`placement` 等状态轴，且挂载 `role/aria-keyshortcuts`。
  - 已覆盖 `data-*-source` + `data-custom-*` 来源标记，值域封闭（`custom|default|derived`），避免自由文本漂移。
  - 测试与样式均基于稳定语义标记进行断言/选择（非 DOM 结构猜测）。
  - `open` 轴当前为单一内部来源（未暴露受控 open API），该子轴按 N/A 处理；其余来源轴均显式可读。
