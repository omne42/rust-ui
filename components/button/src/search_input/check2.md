...
- [x] `spec.rs` 只用于少数复杂组件（如 button），避免泛滥。（`crates/ui-components/src/button/search_input/` 仅保留 `mod.rs/logic.rs/styles.rs/view.rs/motion.rs`，无 `spec.rs`；`mod.rs` 导出面也未引入 schema/spec 额外层。）
...
- [x] Tree Shaking 是一等能力：package 模式支持组件级 feature；source 模式天然裁剪；样式层同步裁剪，禁止无条件聚合全部 CSS，禁止破坏 DCE/LTO 的全量中央注册表。（证据：`cargo check -p ui-components --no-default-features --features component-button_search_input,inject-css` 通过；`cargo tree -e features -p ui-components --no-default-features --features component-button_search_input,inject-css -f "{p} {f}"` 根节点仅启用 `component-button_search_input` 及其必要依赖 `component-button`；`crates/ui-components/src/css.rs` 通过 `#[cfg(feature = "component-button_search_input")]` 条件注入 `search_input` CSS。）
...
- [x] SSR 与跨平台检查：覆盖 web/ssr/wasm 分支，不破坏 non-wasm 编译路径。（compile-only 证据：`cargo check -p ui-components`、`cargo check -p ui-components --no-default-features --features component-button_search_input,inject-css`、`cargo check -p ui-headless --no-default-features --features ssr`、`cargo check -p ui-headless --target wasm32-unknown-unknown --no-default-features --features web`、`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-button_search_input,inject-css` 全部通过。）
...
### 5. 文件落点检查（必须提及）
- [x] `ui-components` 固定入口文件落点正确。（核验结果：`lib.rs/css.rs/root.rs/active_highlight.rs` 均存在；`overlay_open.rs/presence.rs/a11y.rs` 均不存在；`lib.rs` 通过 `css::push_components_css` 暴露聚合入口并导出 `UiRoot`，`root.rs` 负责注入 `crate::css::push_components_css`。）
...
- [x] 组件目录标准文件落点正确。（`crates/ui-components/src/button/search_input/` 当前为 `mod.rs + logic.rs + styles.rs + view.rs + motion.rs` 的标准五件套；`mod.rs` 导出最小公共面（`SearchInputButton` 与 `SearchInputButtonMotion`），其余文件职责已由语义测试锁定。）
...
### 7. 测试与文档（验证闭环）
- [x] 语义测试优先：验证 `data-*` / `aria-*` / role / 状态来源契约，不只视觉快照。（`search_input_button_semantics.rs` 与 `button_search_input_semantics.rs` 以语义契约断言为主，覆盖 `role/aria/data-*`、状态来源、motion/source 标记、docs 契约；本轮回归 `cargo test -p ui-components --test search_input_button_semantics`（29/29）与 `cargo test -p ui-components --test button_search_input_semantics`（10/10）均通过。）
