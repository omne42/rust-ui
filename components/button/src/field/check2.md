...
- [x] 在 `view` 层隐藏关键状态决策。（`FieldButton` 的 `mod.rs` 仅做 `resolve_props` 委托并透传 `resolved.*` 给 `Button`；状态归一与分支保留在 `logic.rs`，由 `components/button/test/field_button_semantics.rs` 新增用例 `field_button_view_layer_only_consumes_resolved_state_payload` 约束）
  - `view.rs` 只消费归一化结果；关键业务分支若散落在 view，必须回收至 `logic.rs`。
- [x] 新增参数但不纳入统一命名与契约。（`FieldButton` 参数已固定走 `mod.rs props -> FieldButtonResolveInput -> normalize_input/resolve_state -> docs 示例` 同名链路；`components/button/test/field_button_semantics.rs` 新增 `field_button_new_params_must_follow_naming_and_contract_pipeline` 约束新增参数必须进入统一命名与归一契约）
  - 新参数必须进入命名体系、类型约束、默认值归一和语义测试；缺任一项不得合并。
- [x] 用并行数组/隐式约定替代显式语义结构（如 `labels + children`）。（`FieldButton` API 固定为显式组合 `children: Children` 并委托 `<Button>{children()}</Button>`，语义测试 `components/button/test/field_button_semantics.rs` 中 `field_button_api_is_explicit_composition_without_parallel_arrays` 已约束禁止 `labels/titles/panels/items: Vec/item_specs` 这类并行数组输入）
  - 标题、语义、内容必须显式绑定在同一 item 结构；依赖位置索引配对视为反模式。
  - 发现“少写几行但语义变弱”的接口设计，默认拒绝合入。
- [x] 公共 API 泄露底层实现细节类型。（`FieldButton` 对外仅暴露组件级抽象输入并复用 `Button`；`components/button/test/field_button_semantics.rs` 的 `field_button_source_keeps_platform_agnostic_contract` 与相关平台契约用例已约束 `mod/logic/styles` 不得出现 `web_sys/wasm_bindgen/js_sys` 等底层平台细节泄露）
  - 公共接口不得暴露 `web-sys`/运行时私有类型；平台细节只允许存在于内部模块。
- [x] 用临时补丁破坏跨组件一致性。（`FieldButton` 继续通过 `<Button ...>` 透传统一契约字段，不引入临时分叉；`components/button/test/field_button_semantics.rs` 新增 `field_button_avoids_temporary_patches_and_keeps_button_contract_consistency`，约束禁止 `legacy/compat/workaround/hotfix/TODO/FIXME` 等临时补丁标记并校验关键透传字段）
  - 临时 patch 若绕开统一契约（命名/状态/语义），必须在同 PR 里修正或显式回退计划。
- [x] 明明是跨组件可复用状态原语，却长期留在某个组件 `logic.rs` 不下沉。（`FieldButton` 未自造状态机，已在 `logic.rs` 调用 `ui_state_primitives::button::{ButtonStateCoreInput, resolve_state_core}` 并仅做装配映射；`components/button/test/field_button_semantics.rs` 的 `field_button_consumes_state_primitives_instead_of_reimplementing_state_machine` 对该下沉契约做了回归约束）
...
- [x] 命名与全库一致。（`FieldButton` 公共 API 维持全库命名契约：布尔状态统一 `is_*`，事件统一 `on_press`，并禁止 legacy 别名；`components/button/test/field_button_semantics.rs` 的 `field_button_api_naming_uses_is_prefix_only` 与 `field_button_new_params_must_follow_naming_and_contract_pipeline` 持续约束）
- [x] 无效状态被限制或归一化。（`FieldButton` 通过 `normalize_input + resolve_state` 做集中归一，离散状态轴使用 `FieldButtonTone/FieldButtonValidation` 枚举并复用 `ui_state_primitives::button::resolve_state_core`；`components/button/test/field_button_semantics.rs` 的 `field_button_type_system_and_markers_form_machine_readable_contract`、`field_button_defaults_are_centralized_in_logic`、`field_button_state_normalization_is_not_scattered_in_view` 持续约束无效输入不可漂移）
- [x] 暴露必要语义标记。（`FieldButton` 持续暴露并消费稳定语义标记：`data-disabled/data-hovered/data-active/data-focus-visible` 与 `custom-handler/custom-aria-label` 来源标记，`components/button/test/field_button_semantics.rs` 中 `field_button_state_markers_are_observable_and_source_distinguishable`、`field_button_semantics_suite_covers_data_aria_role_and_source_contracts` 提供回归断言）
- [x] 覆盖 reduced-motion / SSR / wasm 分支。（`FieldButton` 通过复用 `Button`/`ui-motion` 覆盖分支契约：`components/button/test/field_button_semantics.rs` 中 `field_button_inherits_button_reduced_motion_ssr_and_wasm_branches`、`field_button_respects_ui_headless_web_ssr_mutex_contract`、`field_button_reuses_button_motion_and_ui_motion_has_non_wasm_stub`、`field_button_wasm_debug_contract_is_feature_gated_and_inherited_from_button` 持续回归）
- [x] 文档与示例同步更新。（`FieldButton` 的 docs-app 页面与示例矩阵已与实现同步，并维持 source-first/copy-paste 与 E2E 契约回归：`components/button/test/field_button_semantics.rs` 中 `field_button_docs_examples_are_synced_with_current_api_and_state_matrix`、`field_button_docs_page_covers_primary_playgrounds`、`field_button_docs_source_is_copy_paste_ready_with_imports_and_copy_control`，以及 `e2e/tests/docs_app_field_button_contract.spec.mjs`）
