...
- 本次按“一个一个 check”完成最小特性门禁验证：
  - `TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp CARGO_TARGET_DIR=target/modal-check2 cargo check -p ui --no-default-features --features component-modal,inject-css`
  - `TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp CARGO_TARGET_DIR=target/modal-check2 cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-modal,inject-css`
  - `TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp CARGO_TARGET_DIR=target/modal-check2 cargo test -p ui --no-default-features --features component-modal,inject-css --test modal_semantics`
  - `TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp CARGO_TARGET_DIR=target/modal-check2 cargo clippy -p ui --no-default-features --features component-modal,inject-css --test modal_semantics --no-deps -- -D warnings`
  - `TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp cargo tree -e features -i ui -p ui --no-default-features --features component-modal,inject-css`
