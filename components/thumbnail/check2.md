...
- [x] 宏观/微观双状态机（Macro/Micro Duality）：拖拽等高频交互在 `Dragging` 期间由 `view/motion` 本地循环执行；禁止每帧穿越回 `logic.rs`，必须在结束时通过 `Action::DragEnd` 回流收敛。（`N/A`：`thumbnail` 为非交互展示原语，无拖拽/手势状态机；`components/thumbnail/src/view.rs` 不含 `pointerdown/pointermove/pointerup` 或 `DragEnd` 回流语义，`components/thumbnail/src/motion.rs` 仅处理选中/聚焦态 spring attach。回归由 `components/thumbnail/test/semantics.rs::thumbnail_interaction_matrix_is_not_applicable_for_display_primitive`（禁止 pointer 事件）与相关 forbidden token 断言锁定。）
- [x] 几何两段式渲染（Two-Pass Rendering）：`Tooltip/Popover/Menu` 等依赖 DOM 测量的组件必须走 `Intent -> Measure(view) -> Rectification(logic)`，并具备幂等收敛保护防死循环。（`N/A`：`thumbnail` 为静态展示原语，不承担 overlay/定位类几何测量职责；`components/thumbnail/src/*` 不含 `Tooltip/Popover/Menu` 或 `getBoundingClientRect/offsetWidth/ResizeObserver` 等 DOM 测量路径，`logic.rs` 也不存在 `Intent -> Measure -> Rectification` 的收敛状态机。）
- [x] 集合注册协议（Registration Protocol）：`Accordion/Tabs/Menu` 动态子项必须通过 `RegistrationContext` 上报 `Register/Unregister`，逻辑层维护 `items_order`，禁止依赖 `HashSet` 迭代顺序做导航。（`N/A`：`thumbnail` 为单槽位展示原语，不是动态集合容器，不存在 `Accordion/Tabs/Menu` 类子项注册、排序与导航语义；`components/thumbnail/src/view.rs` 仅暴露 `Thumbnail(..., children: Children)`，无 `RegistrationContext/Register/Unregister/items_order` 路径。回归由 `components/thumbnail/test/semantics.rs::thumbnail_is_not_collection_composition_api` 及禁止 `labels/titles/panels/ItemSpec/default_items` 断言锁定。）
- [x] 插槽投影策略（Slot Projection）：容器组件明确 `Lazy/KeepAlive/Eager`；`KeepAlive` 隐藏时必须通过生命周期通知（如 `NotifyHidden`）暂停轮询/动画等高耗能副作用。（`N/A`：`thumbnail` 是单槽位展示原语，不是容器组件；`components/thumbnail/src/view.rs` 仅 `render_thumbnail_content(children)` 渲染内容，不存在 `Lazy/KeepAlive/Eager` 策略分派，也无 `NotifyHidden` 生命周期通知与轮询/高耗能副作用暂停通道。语义回归由 `components/thumbnail/test/semantics.rs::thumbnail_is_not_collection_composition_api` 与 `components/thumbnail/test/semantics.rs::thumbnail_interaction_matrix_is_not_applicable_for_display_primitive` 约束其非容器、非复杂交互定位。）
- [x] 环境订阅流（Env Streams）：`Resize/Theme/Intersection` 等环境变化在 `view.rs` 采样、防抖后转化为高层语义 `Action`（如 `BreakpointChanged`）推送到 `logic`；禁止原始事件洪泛。（`N/A`：`thumbnail` 是静态展示原语，不承担响应式布局容器职责；`components/thumbnail/src/view.rs` 不订阅 `Resize/Intersection/Theme` 事件，不存在 `ResizeObserver/IntersectionObserver/matchMedia/window resize` 采样防抖链路，`components/thumbnail/src/logic.rs` 也无 `BreakpointChanged` 等环境流 Action。主题变化通过 `UiRoot` 注入 CSS 变量生效，不经过组件内事件洪泛路径。）
- [x] 事件光锥（Event Light Cone）：`Table/Grid` 等大型集合批量操作必须走 `Context Bus + Selector` 与状态压缩表达（如 `SelectionState::All`），禁止 O(N) 级向下 prop drilling。（`N/A`：`thumbnail` 为单节点展示原语，不是 `Table/Grid` 这类大型集合组件；`components/thumbnail/src/view.rs` 仅接收 `children: Children` 并渲染单个 `thumbnail` 容器，不存在批量选择、跨行广播或 O(N) 向下传递路径。`components/thumbnail/src/logic.rs` 仅有 `ThumbnailAgentAction::Inspect` 等单体语义，不存在 `SelectionState::All`/批处理命令模型。）
- [x] 统一因果总线（Causality Bus）：复杂派生总线操作必须支持透传 `TraceId`，确保“用户触发 -> 派生命令 -> 总线广播 -> 订阅者”因果链不断裂。（`N/A`：`thumbnail` 为静态展示原语，不存在复杂派生命令总线；`components/thumbnail/src/logic.rs` 仅暴露 `ThumbnailAgentAction::Inspect` 的静态语义映射，`components/thumbnail/src/view.rs` 仅挂载 `data-ui-*` 标记，不含命令分发/广播/订阅链路，因此不存在 `TraceId` 透传断链风险。）
