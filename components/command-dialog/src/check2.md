...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（本轮按组件最小范围执行并通过：`cargo test -p ui --test command_dialog_semantics --no-default-features --features component-command_dialog,inject-css`、`cargo check -p ui --no-default-features --features component-command_dialog,inject-css`、`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-command_dialog,inject-css`；全仓默认特性构建存在他人模块既有错误，不在本任务改动范围。）

### 10. 本轮验证证据
- 代码落点：
  - `crates/ui/src/command_dialog/motion.rs`（新增，`CommandDialogMotion + attach_motion`）
  - `crates/ui/src/command_dialog/view.rs`（接入 motion，补 `data-ui-*` / `data-stream-*` / `data-output-status`）
...
- 关键命令：
  ...
  - `cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-command_dialog,inject-css`
  - `cargo tree -e features -p ui --no-default-features --features component-command_dialog,inject-css`
