...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（组件范围已执行 `rustfmt --edition 2024 components/form-field/src/view.rs components/form-field/test/form_field_semantics.rs`、`cargo clippy -p ui --no-default-features --features component-form_field,inject-css -- -D warnings`、`cargo test -p ui --test form_field_semantics --no-default-features --features component-form_field,inject-css`、`cargo check -p ui --no-default-features --features component-form_field,inject-css`、`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-form_field,inject-css`。）
