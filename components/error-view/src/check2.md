...
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo test -p ui --test error_view_semantics --no-default-features --features component-error_view,inject-css`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo check -p ui --no-default-features --features component-error_view,inject-css`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo clippy -p ui --no-default-features --features component-error_view,inject-css -- -D warnings`
- `./components/error-view/scripts/check-ui-e2e-error-view.sh`
- `cargo tree -e features -p ui --no-default-features --features component-error_view,inject-css -f "{p} {f}"`
