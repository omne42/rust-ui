...
- [x] 文件落点纪律：组件目录严格由 `mod.rs`（导出）、`logic.rs`（归一派生）、`styles.rs`（Token 样式）、`view.rs`（渲染）、`motion.rs`（动效）组成；复杂组件可选 `spec.rs`；禁止 `render.rs`。（通过：`components/field-label/src` 现有 `mod.rs/logic.rs/styles.rs/view.rs` 与协议文件 `protocol.rs`，未出现 `render.rs`；`FieldLabel` 无动效语义，`motion.rs` 按适用范围 N/A；回归测试 `components/field-label/test/field_label_semantics.rs::field_label_component_files_follow_expected_layout_and_no_spec_file` 已锁定 `render.rs/spec.rs/motion.rs` 不得漂移）
- [x] Hyper-Structure Builder（`spec.rs`）：复杂组件必须提供 AI 友好的 `*Spec::new()...render()` 建造者 API。（N/A：`FieldLabel` 为简单表单标签原语，非复杂组合组件，不引入 `spec.rs` 建造者；目录中无 `spec.rs`，并由回归测试 `components/field-label/test/field_label_semantics.rs::field_label_component_files_follow_expected_layout_and_no_spec_file` 锁定）
...
- [x] `Snapshot` 是所有组件的基础能力（默认必须支持）。（通过：`FieldLabel` 的 Agent Contract 已固定 `action = "snapshot_render"`，并在 `view.rs` 暴露 `data-ui-action=snapshot_render`；组件仅消费上层完整 props（text/for_id/is_required/is_disabled/tone/required_indicator/aria_label/class_name/lang/dir）归一后稳定渲染，无增量 token 依赖。回归覆盖：`components/field-label/test/semantics.rs` 与 `components/field-label/test/field_label_semantics.rs` 对 `snapshot_render` 与 `data-ui-action` 均有断言）
  - 所有组件都应能消费“完整生成结果”并稳定渲染。
  - 即使组件不直接展示正文，也应能在接收上层完整配置后正常渲染。
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。（通过：`FieldLabel` 非正文阅读面，明确采用 `Streaming Optional`；组件契约显式声明 `fallback=snapshot` 并在视图持续暴露 `data-ui-streaming=optional`、`data-ui-fallback=snapshot`、`data-ui-output-state=verified`（`components/field-label/src/view.rs`）。`components/field-label/src/Component.toml` 的 `agent_contract` 已标注 `streaming/fallback/output_state`，且 `agent_contract_whitelist` 将 `retry/reconnect/transport_validation` 列为上层职责边界；对应回归测试已覆盖（`components/field-label/test/semantics.rs`、`components/field-label/test/field_label_semantics.rs`））
