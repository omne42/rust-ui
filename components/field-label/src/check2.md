...
### 10. FieldLabel 本轮验收证据
- [x] 状态原语已下沉并由组件桥接消费：`crates/ui-state-primitives/src/field_label.rs` + `crates/ui-components/src/field_form/field_label/logic.rs`。
- [x] A11y 语义契约已通过 headless 层统一输出：`crates/ui-headless/src/field_label.rs`，组件 `view.rs` 只挂载 `attrs`。
- [x] 语义回归入口：`components/field-label/test/field_label_semantics.rs`，覆盖分层边界、稳定 `data-*`/`aria-*` 标记、docs 示例契约与清单完整性。
- [x] 定向门禁命令：
  - `CARGO_TARGET_DIR=target/field_label_check cargo test -p ui-state-primitives field_label --lib`
  - `CARGO_TARGET_DIR=target/field_label_check cargo test -p ui-headless field_label --lib`
  - `CARGO_TARGET_DIR=target/field_label_check cargo test -p ui-components --test field_label_semantics --no-default-features --features component-field_label,inject-css`
