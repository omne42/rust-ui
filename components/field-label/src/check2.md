...
- [x] 状态原语已下沉并由组件桥接消费：`crates/ui-state-primitives/src/field_label.rs` + `crates/ui/src/field_form/field_label/logic.rs`。
...
- [x] 定向门禁命令：
  ...
  - `CARGO_TARGET_DIR=target/field_label_check cargo test -p ui --test field_label_semantics --no-default-features --features component-field_label,inject-css`
