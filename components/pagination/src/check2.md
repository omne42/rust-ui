...
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-pagination,inject-css`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target ./components/pagination/scripts/check-ui-e2e-pagination.sh`
- `cargo tree -e features -p ui --no-default-features --features component-pagination,inject-css -f "{p} {f}"`
