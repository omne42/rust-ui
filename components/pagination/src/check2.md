...
- `rustfmt crates/ui-components/tests/pagination_semantics.rs`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo test -p ui-components --test pagination_semantics --no-default-features --features component-pagination,inject-css`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo check -p ui-components --no-default-features --features component-pagination,inject-css`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo clippy -p ui-components --no-default-features --features component-pagination,inject-css -- -D warnings`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-pagination,inject-css`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target ./scripts/check-ui-components-e2e-pagination.sh`
- `cargo tree -e features -p ui-components --no-default-features --features component-pagination,inject-css -f "{p} {f}"`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo check -p ui-headless --no-default-features --features web`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo check -p ui-headless --no-default-features --features ssr`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo check -p ui-headless --no-default-features --features web,ssr`（按预期触发 `compile_error!`）
