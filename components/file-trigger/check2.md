...
- [x] 组件层遵循 token-first 静态样式契约：样式通过 `styles.rs` 聚合注入；运行时仅传必要 CSS 变量；不把 Utility-First/CSS-in-Rust 当组件库默认范式。（适用：`FileTrigger` 样式常量位于 `styles.rs`，已通过 `ui/css.rs` 在 `component-file_trigger` feature 下聚合并由 `UiRoot` 注入；组件未采用 Utility-First/CSS-in-Rust 作为默认实现。）
...
- [x] 级联层覆盖（`@layer ui`）：组件 CSS 默认聚合进 `@layer ui`；运行时数值调整仅通过 CSS Custom Properties（如 `style:--x=...`），禁止普通内联样式（如 `style=\"top: 10px\"`）。（适用：`crates/ui/src/css.rs` 通过 `@layer ui` 聚合；`FileTrigger` 视图未注入普通 inline style。）
...
- [x] `ui` 固定入口文件落点正确。（适用：`crates/ui/src/lib.rs` + `css.rs` + `root.rs` 保持统一入口职责；`component-file_trigger` 受 feature gate 约束；未引入禁止文件 `overlay_open.rs/presence.rs/a11y.rs`。）
...
- [x] Tree Shaking & 特性剪裁：组件必须注册到 `ui` 特性树（如 `component-accordion`）；`css.rs` 和 `lib.rs` 聚合必须受 feature 门控，禁止无条件全局依赖。（已满足：`component-file_trigger` 已在 `Cargo.toml/lib.rs/css.rs` 形成 feature 闭环；无全量无条件聚合。）
