...
- 修复：`crates/ui/Cargo.toml` 中 `component-collapsible` 补齐依赖 `component-disclosure`，解决最小特性编译失败。
- 关键验证：
  - `cargo tree -e features -i ui -p ui --no-default-features --features component-collapsible,inject-css`
  - `cargo check -p ui --no-default-features --features component-collapsible,inject-css`
  - `cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-collapsible,inject-css`
  - `cargo clippy -p ui --no-default-features --features component-collapsible,inject-css -- -D warnings`
  - `cargo test -p ui --test collapsible_semantics --no-default-features --features component-collapsible,inject-css -- --nocapture`
