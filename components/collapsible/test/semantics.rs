...
#[test]
fn collapsible_public_api_keeps_dom_details_internal() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "Collapsible module should export `{needle}`.",
        );
    }
    ...
    for forbidden in ["web_sys", "HtmlElement", "NodeRef"] {
        assert!(
            ...
            "Collapsible public module should not expose DOM detail `{forbidden}`.",
        );
    }
    ...
    for forbidden in ["pub fn ", "impl ", "struct "] {
        assert!(
            !source.contains(forbidden),
            "Collapsible mod.rs should stay a thin export boundary; found implementation token `{forbidden}`.",
        );
    }
}

#[test]
fn collapsible_keeps_spec_rs_absent_for_simple_component_scope() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    ...
    assert!(
        !spec_path.exists(),
        "simple collapsible component should not introduce `spec.rs`; found unexpected file at {spec_path:?}.",
    );
    for forbidden in ["mod spec", "pub mod spec", "pub use spec::"] {
        assert!(
            !mod_source.contains(forbidden),
            "collapsible module should not wire optional spec module token `{forbidden}`.",
        );
    }
    assert!(
        readme.contains("## API"),
        "simple-component guidance should stay in README/checklist docs rather than a dedicated spec.rs.",
    );
}
...
fn collapsible_component_directory_standard_files_stay_in_canonical_layout() {
    ...
    for required in ["mod.rs", "logic.rs", "styles.rs", "view.rs", "motion.rs"] {
        let path = src_dir.join(required);
        assert!(
            path.exists(),
    ...
    for forbidden in ["render.rs", "spec.rs"] {
        let path = src_dir.join(forbidden);
        assert!(
            !path.exists(),
...
fn collapsible_context_compression_manifest_and_rbi_projection_are_present_and_current() {
    ...
    let rbi_source = load_source("src/collapsible.rbi");
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let component_toml_path = manifest_dir.join("src/Component.toml");
    let component_rbi_path = manifest_dir.join("src/collapsible.rbi");
...
fn collapsible_agent_contract_is_schema_typed_and_machine_readable() {
    ...
    let component_rbi = load_source("src/collapsible.rbi");
    let script_source = load_source("../../scripts/check-ui-contract-hygiene.sh");
    let check2_source = load_source("check2.md");
...
fn collapsible_view_mounts_headless_contract_and_semantic_markers() {
    ...
    for needle in [
        ...
        assert!(
            source.contains(needle),
            "Collapsible view should include headless semantics contract `{needle}`.",
        );
    }

    assert!(
        !source.contains("#[prop(optional)] state:"),
        "Collapsible public API should not expose an internal `state` object prop.",
    );
    for forbidden in [
        "\"Advanced options\"",
        "\"Disabled section\"",
...
#[test]
fn collapsible_layer_files_keep_role_boundaries() {
    ...
    assert!(
        logic.contains("pub use ui_state_primitives::collapsible::{"),
        "logic.rs should compose state primitives instead of redefining them.",
    );
    assert!(
        !logic.contains("view!"),
    ...
    for forbidden in ["NodeRef<", "web_sys", "color-mix("] {
        assert!(
            ...
            "logic.rs should not include DOM/style implementation detail `{forbidden}`.",
        );
    }
    assert!(
        !logic.contains("use_controlled_state("),
        "logic.rs should consume ui-state-primitives instead of rebuilding controlled-state primitives.",
    );
    assert!(
        logic.contains("pub fn normalize_dir(dir: Option<String>) -> Option<A11yDirection>"),
        "logic.rs should centralize `dir` normalization for lang/dir A11y integration.",
    );

    assert!(
        !view.contains("ui_state_primitives::"),
        "view.rs should consume primitives through logic.rs boundaries, not bind ui-state-primitives directly.",
    );
    assert!(
        view.contains("locale_attrs(lang, dir)"),
        "view.rs should consume shared locale helpers from ui-headless a11y contracts.",
    );
    assert!(
        view.contains("disclosure_trigger_attrs("),
        "view.rs should consume shared disclosure a11y attrs from ui-headless instead of rewriting aria logic.",
    );
    for forbidden in ["is_loading", "aria-busy", "retry", "use_async_action"] {
        assert!(
            !view.contains(forbidden),
            "collapsible is non-async; view.rs should not define async protocol token `{forbidden}`.",
        );
    }

    assert!(
        styles.contains("var(--ui-"),
        "styles.rs should consume ui-theme variables.",
    );
    for selector in [
        ...
        assert!(
            ...
            "styles.rs should branch visual states with explicit semantic selector `{selector}`.",
        );
    }
    for forbidden in [":nth-child(", ":nth-of-type(", ":has("] {
        assert!(
            !styles.contains(forbidden),
            "styles.rs should not use fragile structural selector `{forbidden}` for state styling.",
        );
    }
    assert!(
        !styles.contains("on:click"),
        "styles.rs should not contain event logic.",
    );
    assert!(
        !view.contains("style="),
        "view.rs should not embed business inline style; runtime style adjustments must stay in CSS variables/contracts.",
    );

    assert!(
        motion.contains("attach_indicator_motion"),
        "motion.rs should own animation attachment mapping.",
    );
    for needle in [
        ...
        assert!(
            ...
            "motion.rs should map component semantics to disclosure motion contract via `{needle}`.",
        );
    }
    assert!(
        !motion.contains("aria-"),
        "motion.rs should not encode A11y semantics.",
    );
    for forbidden in [
        "ui_motion::spring::SpringAnimator",
        "requestAnimationFrame",
        ...
        assert!(
            ...
            "motion.rs should not reimplement motion engine detail `{forbidden}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view.contains(forbidden),
            "collapsible has no drag interaction; view.rs should not include drag-loop token `{forbidden}`.",
        );
        assert!(
            !logic.contains(forbidden),
            "collapsible has no drag interaction; logic.rs should not include drag macro-state token `{forbidden}`.",
        );
        assert!(
            !motion.contains(forbidden),
            "collapsible has no drag interaction; motion.rs should not include drag-loop token `{forbidden}`.",
        );
    }
}

#[test]
fn collapsible_non_composite_api_rejects_parallel_item_inputs() {
    let view = load_source("src/view.rs");
    let docs = load_source("src/README.md");

    assert!(
        view.contains("children: Children"),
        "Collapsible should expose explicit parent + content composition through `children`.",
    );
    ...
    for forbidden in [
        ...
        assert!(
            !view.contains(forbidden),
            "Collapsible is not a composite collection component; found forbidden API token `{forbidden}` in view.rs.",
        );
        assert!(
            !docs.contains(forbidden),
            "Collapsible docs should not recommend parallel-slot API token `{forbidden}`.",
        );
    }
}
...
fn collapsible_reduced_motion_ssr_wasm_contracts_stay_convergent() {
    let view = load_source("src/view.rs");
    let styles = load_source("src/styles.rs");
    let disclosure_motion = load_source("../disclosure/src/motion.rs");
...
fn collapsible_motion_contract_is_builtin_and_attached_with_reduced_motion_and_non_wasm_noop() {
    ...
    let motion_source = load_source("src/motion.rs");
    let styles_source = load_source("src/styles.rs");
    let disclosure_motion_source = load_source("../disclosure/src/motion.rs");
    ...
    for required in [
        ...
        assert!(
            mod_source.contains(required) || view_source.contains(required),
            "collapsible public/view layer should expose built-in motion contract marker `{required}`.",
        );
    }
    ...
    for required in [
        ...
        assert!(
            motion_source.contains(required),
            "collapsible motion adapter should keep contract mapping marker `{required}`.",
        );
    }

    for required in ["SpringConfig {", "stiffness: 260.0,", "damping: 18.0,"] {
        assert!(
            disclosure_motion_source.contains(required),
            "shared disclosure motion contract should keep built-in spring marker `{required}`.",
        );
    }
    ...
    for required in [
        ...
        assert!(
            ...
            "collapsible styles should keep reduced-motion downgrade marker `{required}`.",
        );
    }
    ...
    for required in [
        ...
        assert!(
            ...
            "ui-motion should keep non-wasm safe no-op marker `{required}`.",
        );
    }
    ...
    for required in [
        ...
        assert!(
            check2_source.contains(required),
            "collapsible checklist should keep motion-contract governance marker `{required}`.",
        );
    }
}
...
fn collapsible_registration_protocol_is_not_applicable_for_single_item_disclosure() {
    ...
    for forbidden in [
        ...
        assert!(
            !view.contains(forbidden),
            "collapsible is not a dynamic item-collection; view.rs should not include `{forbidden}`.",
        );
        assert!(
            !logic.contains(forbidden),
            "collapsible is not a dynamic item-collection; logic.rs should not include `{forbidden}`.",
        );
        assert!(
            !motion.contains(forbidden),
            "collapsible is not a dynamic item-collection; motion.rs should not include `{forbidden}`.",
        );
        assert!(
            !docs.contains(forbidden),
...
fn collapsible_slot_projection_policy_is_not_applicable_for_single_panel_disclosure() {
    ...
    for forbidden in ["Lazy", "KeepAlive", "Eager", "NotifyHidden"] {
        assert!(
            !view.contains(forbidden),
            "collapsible has no slot-projection policy contract; view.rs should not include `{forbidden}`.",
        );
        assert!(
            !logic.contains(forbidden),
            "collapsible has no slot-projection lifecycle contract; logic.rs should not include `{forbidden}`.",
        );
        assert!(
            !motion.contains(forbidden),
            "collapsible has no slot-projection lifecycle contract; motion.rs should not include `{forbidden}`.",
        );
        assert!(
            !docs.contains(forbidden),
...
fn collapsible_env_streams_are_not_applicable_without_responsive_sampling_contract() {
    ...
    for forbidden in [
        ...
        assert!(
            !view.contains(forbidden),
            "collapsible has no env-stream sampling contract; view.rs should not include `{forbidden}`.",
        );
        assert!(
            !logic.contains(forbidden),
            "collapsible has no env-stream action projection; logic.rs should not include `{forbidden}`.",
        );
        assert!(
            !motion.contains(forbidden),
...
fn collapsible_event_light_cone_is_not_applicable_for_single_disclosure_state_axis() {
    ...
    for forbidden in [
        ...
        assert!(
            !view.contains(forbidden),
            "collapsible has no large-collection batch semantics; view.rs should not include `{forbidden}`.",
        );
        assert!(
            !logic.contains(forbidden),
            "collapsible has no large-collection state compression; logic.rs should not include `{forbidden}`.",
        );
        assert!(
            !motion.contains(forbidden),
...
fn collapsible_causality_bus_is_not_applicable_without_derived_broadcast_graph() {
    ...
    for forbidden in [
        ...
        assert!(
            !view.contains(forbidden),
            "collapsible has no derived causality bus graph; view.rs should not include `{forbidden}`.",
        );
        assert!(
            !logic.contains(forbidden),
            "collapsible has no derived causality bus graph; logic.rs should not include `{forbidden}`.",
        );
        assert!(
            !motion.contains(forbidden),
...
fn collapsible_semantics_matrix_prefers_contract_assertions_over_snapshots() {
    ...
    for needle in [
        ...
        assert!(
            e2e.contains(needle),
            "collapsible e2e contract should cover semantic matrix token `{needle}`.",
        );
    }
    ...
    for forbidden in ["toHaveScreenshot(", "toMatchSnapshot("] {
        assert!(
            ...
            "collapsible e2e should not rely on visual snapshot assertion `{forbidden}` as primary contract.",
        );
    }
}
...
fn collapsible_check2_marks_e2e_selector_stability_item_complete() {
    ...
    for marker in [
        "- [x] E2E 选择器稳定：使用语义标记，WASM 场景有稳定等待策略。",
        "collapsible_check2_documents_e2e_selector_and_stable_wait_rules",
        "collapsible_e2e_selector_contract_uses_semantic_markers_and_settled_waits",
        "collapsible_e2e_contract_covers_ready_and_settled_conditions_for_disclosure_paths",
...
fn collapsible_check2_marks_repeatable_keyflow_regression_item_complete() {
    ...
    for marker in [
        "- [x] 关键流程纳入可重复回归集合（Playwright/Cypress）。",
        "collapsible_check2_documents_repeatable_keyflow_regression_rules",
        "collapsible_e2e_regression_suite_contains_repeatable_disclosure_keyflow",
        "collapsible_e2e_regression_failures_map_to_semantic_contract_breakpoints",
        "collapsible_e2e_regression_prioritizes_focus_and_keyboard_risk_paths",
        "collapsible_e2e_check_script_covers_repeatable_keyflow_regression_contract",
...
fn collapsible_semantics_suite_is_contract_first_not_snapshot_only() {
    ...
    let module = load_source("src/mod.rs");
    let test_mod = load_source("test/mod.rs");
    ...
    assert!(
        !logic_tests_source.contains("assert_snapshot!(")
            && !logic_tests_source.contains("assert_debug_snapshot!(")
            && !logic_tests_source.contains(".to_match_snapshot("),
        "logic regression suite should avoid snapshot-only assertions.",
    );
}
...
fn collapsible_semantic_markers_changed_in_view_must_be_covered_by_semantics_checks() {
    ...
    let aggregated_semantics =
        include_str!("../../../components/collapsible/test/collapsible/semantics.rs");
...
#[test]
fn collapsible_token_first_css_contract_is_centrally_injected() {
    let styles = load_source("src/styles.rs");
    let view = load_source("src/view.rs");
    let css_aggregator = load_source("../../crates/ui/src/css.rs");
    let ui_root = load_source("../../crates/ui/src/root.rs");

    assert!(
        styles.contains("var(--ui-"),
        "styles.rs should consume ui-theme CSS variables via var(--ui-*).",
    );
    for forbidden in ["@apply", "tailwind", " tw-", "styled(", "style!"] {
        assert!(
            !styles.contains(forbidden),
            "component styles should not adopt utility-first/CSS-in-Rust default token `{forbidden}`.",
        );
    }
    assert!(
        !view.contains("style="),
        "view.rs should not push business style logic inline.",
    );

    assert!(
        css_aggregator.contains("#[cfg(feature = \"component-collapsible\")]"),
        "css.rs should feature-gate collapsible CSS aggregation.",
    );
    assert!(
        css_aggregator.contains("out.push_str(crate::collapsible::styles::CSS);"),
        "css.rs should aggregate collapsible styles from styles.rs.",
    );
    assert!(
        ui_root.contains("if inject_components_css.get_value()"),
        "UiRoot should guard component CSS injection by `inject_components_css`.",
    );
    assert!(
        ui_root.contains("crate::css::push_components_css(&mut out);"),
        "UiRoot should inject aggregated component CSS through crate::css::push_components_css.",
    );
}

#[test]
fn collapsible_visual_desire_contract_has_feedback_cues_and_theme_baseline_proofs() {
    let styles = load_source("src/styles.rs");
    let view = load_source("src/view.rs");
    ...
    for needle in [
        ...
        assert!(
            ...
            "collapsible styles should expose visual quality cue `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            view.contains(needle),
            "collapsible view should mount interaction feedback contract `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "collapsible docs page should expose visual/desire validation entry `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "shared docs visual baseline page should include `{needle}`.",
        );
    }
...
fn collapsible_styles_use_defensive_variable_fallback_chain() {
    ...
    for required in [
        ...
        "--ui-accent, var(--ui-fallback-accent)",
        "--ui-border, var(--ui-fallback-border)",
        "--ui-bg, var(--ui-fallback-bg)",
        "--ui-accent-soft, var(--ui-fallback-accent-soft)",
    ] {
        assert!(
            styles_source.contains(required),
            "collapsible styles should keep defensive fallback chain token `{required}`.",
        );
    }
    ...
    for required in [
        ...
        assert!(
            ...
            "ui-theme should stay SSOT for collapsible fallback token `{required}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !styles_source.contains(forbidden),
            "collapsible styles should not keep raw terminal token `{forbidden}`.",
        );
    }
    ...
    for required in [
        ...
        assert!(
            check2_source.contains(required),
            "collapsible checklist should keep defensive-variable governance marker `{required}`.",
        );
    }
}
...
fn collapsible_cascade_layer_and_runtime_style_contract_is_enforced() {
    ...
    for required in [
        ...
        assert!(
            check2_source.contains(required),
            "collapsible checklist should keep cascade-layer governance marker `{required}`.",
        );
    }
}
...
fn collapsible_tree_shaking_contract_keeps_feature_gates_explicit() {
    let cargo = load_source("../../crates/ui/Cargo.toml");
    let lib = load_source("../../crates/ui/src/lib.rs");
    let css = load_source("../../crates/ui/src/css.rs");
...
fn collapsible_performance_governance_contract_is_budgeted_traceable_and_blocking() {
    ...
    assert!(
        logic.contains("resolve_state(") || primitive.contains("pub fn resolve_state("),
        "state resolution contract should remain attributable through logic wiring or primitive definition.",
    );
    for needle in [
    ...
    assert!(
        ...
        "collapsible view reactive budget exceeded: expected <= 2 `Signal::derive`, found {view_signal_derive_count}.",
    );
...
fn collapsible_semantics_and_performance_regression_cover_aria_data_focus_and_render_count_measurement()
 {
    ...
    let aggregated_semantics =
        load_source("../../components/collapsible/test/collapsible/semantics.rs");
    let view = load_source("src/view.rs");
    let e2e = load_source("../../e2e/tests/docs_app_collapsible.spec.mjs");
    let todo = load_source("../../docs/plan/TODO.md");
    let script = load_source("../../scripts/check-ui-performance.sh");
...
#[test]
fn collapsible_view_macro_complexity_is_split_into_semantic_subblocks() {
    let view = load_source("src/view.rs");
...
fn collapsible_view_functional_split_prefers_plain_functions_over_local_components() {
    ...
    for needle in [
        ...
        assert!(
            view.contains(needle),
            "view function-first split should include `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view.contains(forbidden),
            "local fragment should remain plain function and avoid `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            view.contains(needle),
            "semantic markers should stay stable after function split `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "view-macro gate script should include collapsible function split test target.",
    );

    assert!(
        check2.contains("- [x] 函数式拆分优先：不涉及复杂状态与生命周期管理的 UI 片段，优先拆为普通 Rust 函数（返回 `impl IntoView`/`View`），而不是新增 `#[component]`。"),
        "check2 should mark function-first split gate complete.",
    );
}
...
fn collapsible_static_fragments_are_constantized_or_absent_for_simple_layout() {
    ...
    for needle in [
        ...
        assert!(
            view.contains(needle),
            "static fragment contract should keep constantized token `{needle}`.",
        );
    }
    ...
    assert_eq!(
        ...
        2,
        "indicator glyph should keep one constant source and one render usage.",
    );
    ...
    assert!(
        ...
        "view-macro gate script should include collapsible static fragment test target.",
    );

    assert!(
        check2.contains("- [x] 静态片段常量化：复杂 SVG、页脚、长说明文本等纯静态内容优先常量化/模板化，减少重复 `view!` 渲染指令生成。"),
        "check2 should mark static fragment gate complete.",
    );
}
...
fn collapsible_inner_html_usage_is_forbidden_in_component_and_docs_examples() {
    ...
    for rel_path in [
        ...
        for forbidden in [
            ...
            assert!(
                !source.contains(forbidden),
                "collapsible component source `{rel_path}` should forbid raw-html injection token `{forbidden}`.",
            );
        }
    }
    ...
    assert!(
        script.contains(
            "cargo test -p ui --test collapsible_semantics --no-default-features --features component-collapsible,inject-css collapsible_inner_html_usage_is_forbidden_in_component_and_docs_examples",
        ),
        "inner-html check script should include collapsible inner-html contract target.",
    );
...
fn collapsible_engineering_contract_keeps_tracing_semantics_unified_without_component_local_events()
{
    ...
    let button_view = load_source("../button/src/view.rs");
    let combined = [
...
fn collapsible_docs_are_source_first_copy_paste_ready_with_imports_copy_button_and_sync() {
    let docs_source =
        load_source("../../apps/docs-app/src/pages/components/pages/collections_groups.rs");
    let playground_source = load_source("../../apps/docs-app/src/playground.rs");
    let code_block_view_source = load_source("../code-block/src/view.rs");
...
fn collapsible_heroui_strategy_and_component_docs_are_synchronized_and_indexable() {
    ...
    for required in [
        ...
        assert!(
            ...
            "docs component registry should expose collapsible entrypoint marker `{required}`.",
        );
    }
...
fn collapsible_check2_marks_docs_sync_and_state_matrix_item_complete() {
    ...
    for required in [
        ...
        "motion",
        "aria_label/class_name/lang/dir",
        "components/collapsible/src/view.rs",
        "components/collapsible/src/logic.rs",
        "collapsible_check2_documents_docs_sync_and_state_matrix_rules",
...
fn collapsible_check2_marks_documentation_as_product_item_complete() {
    ...
    for required in [
        ...
        "collapsible_check2_documents_documentation_as_product_rules",
        "collapsible_documentation_entry_exists_with_beginner_first_progression",
        "collapsible_dx_check_script_covers_documentation_as_product_contract",
        "scripts/check-ui-dx.sh",
        "Invalid cross-device link (os error 18)",
    ] {
        assert!(
...
fn collapsible_wasm_debug_contract_reuses_global_trace_and_stays_feature_isolated() {
    ...
    let trace = load_source("../../crates/ui-headless/src/trace.rs");
    let view = load_source("src/view.rs");
    let logic = load_source("src/logic.rs");
    let motion = load_source("src/motion.rs");
    let readme = load_source("src/README.md");
