...
- [x] `ui-motion` 定义：动效能力与契约执行层（spring、keyframes、WAAPI/RAF backend），只负责时间函数、插值与运行时驱动，不承载组件业务语义与状态决策。（`sheet/motion.rs` 仅做 `open/placement -> ui_motion spring` 映射与 attach，包含 wasm/non-wasm 与 reduced-motion 分支；未在组件层自实现通用动画引擎。回归：`crates/ui-components/tests/sheet_semantics.rs::sheet_motion_contract_exposes_default_custom_and_direction_checks`、`sheet_reduced_motion_ssr_wasm_branches_are_covered_without_semantic_split`。）
