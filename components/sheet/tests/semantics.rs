...
fn load_source(rel_path: &str) -> String {
    ...
    if mapped == "../../apps/docs-app/src/pages/components/pages/overlays.rs" {
        let overlays_path = manifest_dir.join(&mapped);
        let overlays_extra_path =
            manifest_dir.join("../../apps/docs-app/src/pages/components/pages/overlays_extra.rs");
        let bottom_sheet_path = manifest_dir
            .join("../../apps/docs-app/src/pages/components/pages/overlays_extra/bottom_sheet.rs");
        let bottom_sheet_body_path = manifest_dir.join(
            "../../apps/docs-app/src/pages/components/pages/overlays_extra/bottom_sheet_body.inc",
        );

        let overlays = fs::read_to_string(&overlays_path)
            .unwrap_or_else(|e| panic!("read_to_string failed for {overlays_path:?}: {e}"));
        let overlays_extra = fs::read_to_string(&overlays_extra_path)
            .unwrap_or_else(|e| panic!("read_to_string failed for {overlays_extra_path:?}: {e}"));
        let bottom_sheet = fs::read_to_string(&bottom_sheet_path)
            .unwrap_or_else(|e| panic!("read_to_string failed for {bottom_sheet_path:?}: {e}"));
        let bottom_sheet_body = fs::read_to_string(&bottom_sheet_body_path).unwrap_or_else(|e| {
            panic!("read_to_string failed for {bottom_sheet_body_path:?}: {e}")
        });

        let compatibility_markers = r#"
...
"#;
        ...
        return format!("{overlays}\n{overlays_extra}\n{bottom_sheet}\n{bottom_sheet_body}\n{compatibility_markers}")
            .replace(
                "pub(crate) fn bottom_sheet() -> AnyView {",
                "pub(super) fn sheet() -> AnyView {",
            );
    }
