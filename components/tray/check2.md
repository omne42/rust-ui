...
- [x] 宏观/微观双状态机（Macro/Micro Duality）：拖拽等高频交互在 `Dragging` 期间由 `view/motion` 本地循环执行；禁止每帧穿越回 `logic.rs`，必须在结束时通过 `Action::DragEnd` 回流收敛。
  - N/A：`Tray` 当前不提供拖拽/连续手势交互，组件交互仅为离散 open/close；不存在 `Dragging` 本地循环与 `Action::DragEnd` 回流路径。回归：`components/tray/test/semantics.rs::tray_macro_micro_duality_is_not_applicable_without_dragging_loop`。
- [x] 几何两段式渲染（Two-Pass Rendering）：`Tooltip/Popover/Menu` 等依赖 DOM 测量的组件必须走 `Intent -> Measure(view) -> Rectification(logic)`，并具备幂等收敛保护防死循环。
  - N/A：`Tray` 当前为固定底部 Sheet 装配，不依赖 DOM 几何测量闭环；实现中无 `Intent -> Measure -> Rectification` 往返与幂等收敛循环。回归：`components/tray/test/semantics.rs::tray_two_pass_rendering_is_not_applicable_without_geometry_measurement_loop`。
- [x] 集合注册协议（Registration Protocol）：`Accordion/Tabs/Menu` 动态子项必须通过 `RegistrationContext` 上报 `Register/Unregister`，逻辑层维护 `items_order`，禁止依赖 `HashSet` 迭代顺序做导航。
  - N/A：`Tray` 为单面板 overlay，不维护动态子项集合与顺序，不存在 `RegistrationContext/Register/Unregister/items_order` 导航链路。回归：`components/tray/test/semantics.rs::tray_registration_protocol_is_not_applicable_without_dynamic_item_collection`。
- [x] 插槽投影策略（Slot Projection）：容器组件明确 `Lazy/KeepAlive/Eager`；`KeepAlive` 隐藏时必须通过生命周期通知（如 `NotifyHidden`）暂停轮询/动画等高耗能副作用。
  - N/A：`Tray` 当前为单面板 `Sheet` 装配，不暴露 `Lazy/KeepAlive/Eager` 投影策略轴；隐藏生命周期由底层 `Sheet` 管理，组件侧不存在 `KeepAlive` 隐藏态与 `NotifyHidden` 高耗能副作用暂停协议。回归：`components/tray/test/semantics.rs::tray_slot_projection_policy_is_not_applicable_without_keep_alive_axis`。
- [x] 环境订阅流（Env Streams）：`Resize/Theme/Intersection` 等环境变化在 `view.rs` 采样、防抖后转化为高层语义 `Action`（如 `BreakpointChanged`）推送到 `logic`；禁止原始事件洪泛。
  - N/A：`Tray` 当前无 Resize/Theme/Intersection 事件采样与订阅流，不存在 `BreakpointChanged` 等高层 Action 汇聚链路；组件仅消费离散 open/close 与语义映射。回归：`components/tray/test/semantics.rs::tray_env_streams_are_not_applicable_without_environment_sampling_pipeline`。
- [x] 事件光锥（Event Light Cone）：`Table/Grid` 等大型集合批量操作必须走 `Context Bus + Selector` 与状态压缩表达（如 `SelectionState::All`），禁止 O(N) 级向下 prop drilling。
  - N/A：`Tray` 为单面板 overlay，不承载 `Table/Grid` 大型集合批处理；不存在 `Context Bus + Selector` 广播压缩与 `SelectionState::All` 状态表达链路。回归：`components/tray/test/semantics.rs::tray_event_light_cone_is_not_applicable_without_large_collection_batch_ops`。
- [x] 统一因果总线（Causality Bus）：复杂派生总线操作必须支持透传 `TraceId`，确保“用户触发 -> 派生命令 -> 总线广播 -> 订阅者”因果链不断裂。
  - N/A：`Tray` 当前不承载复杂派生总线与多订阅者广播流，交互仅收敛于局部 open/close 状态切换；不存在跨总线 `TraceId` 透传链路。回归：`components/tray/test/semantics.rs::tray_causality_bus_is_not_applicable_without_cross_subscriber_derived_bus`。
- [x] 存在 A11y 实现、国际化与本地化实现（至少具备接入点，不硬编码用户可见文本）。
  - 已实现：`view.rs` 通过 `use_tray_a11y(TrayA11yOptions)` 挂载 `aria-*`，并透传 `lang/dir`；可见关闭文案由 `close_label` 输入并在 `logic.rs` 统一兜底，不在 `view.rs` 硬编码。回归：`components/tray/test/semantics.rs::tray_a11y_i18n_contract_keeps_text_source_out_of_view`。
...
- [x] 状态可观测、可检索、可验证：使用稳定 `data-*` 与 `aria-*` 标记表达状态和来源。
  - 已实现：`view.rs` 暴露稳定 `data-*`（open/closed/state/source/custom）与 `aria-*`（`aria-labelledby`/`aria-describedby`）标记，且通过 `ui-state-primitives` 派生的封闭状态枚举输出属性值；自动化可基于语义标记检索，不依赖 DOM 位置。回归：`components/tray/test/semantics.rs::tray_state_observability_contract_uses_stable_data_and_aria_markers`。
...
- [x] 样式依赖显式状态（`data-*`/class），而非脆弱 DOM 结构猜测。
  - 已实现：`styles.rs` 的状态分支基于稳定 `data-*`/`slot`/class 标记（如 `data-state`、`data-close-button`、`data-footer`），未使用 `:nth-child` 等结构猜测选择器；`view.rs` 不承载业务 `style=` 逻辑，视觉切换由语义标记直接驱动。回归：`components/tray/test/semantics.rs::tray_style_contract_uses_explicit_markers_without_fragile_dom_assumptions`。
...
- [x] 测试验证“语义契约”而不只验证视觉快照。
  - 已实现：`components/tray/test/semantics.rs` 以 `role/aria/data-state/source markers` 为断言主轴（含受控/非受控与来源标记覆盖）；`Tray` 当前无组件内指针手势状态机与独立 SSR 分支，相关路径按适用范围由 `Sheet/ui-headless` 契约覆盖。视觉快照未作为主验证方式。回归：`components/tray/test/semantics.rs::tray_semantic_contract_tests_exist_and_do_not_rely_on_visual_snapshots`。
