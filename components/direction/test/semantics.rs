fn load_source(path: &str) -> &'static str {
    match path {
        ...
        "performance_script" => include_str!("../../../scripts/check-ui-performance.sh"),
...
fn direction_docs_and_ui_components_exports_are_present() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui should expose direction through `{needle}`.",
        );
    }
}
...
fn direction_token_first_static_style_contract_is_enforced() {
    ...
    assert!(
        ...
        "ui css.rs should feature-gate and aggregate direction styles."
    );
...
fn direction_ui_components_fixed_entry_files_follow_layered_boundaries() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 ui fixed entry contract should keep `{needle}`.",
        );
    }
    ...
    for missing_file in [
        ...
        assert!(
            !path.exists(),
            "ui forbidden entry file should stay absent: {missing_file}",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored ui fixed-entry regression.",
    );
}
