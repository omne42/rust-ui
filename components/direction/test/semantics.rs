fn load_source(path: &str) -> &'static str {
    match path {
        ...
        "component_rbi" => include_str!("../src/direction.rbi"),
        "cargo" => include_str!("../Cargo.toml"),
        "primitive_direction" => {
            include_str!("../../../crates/ui-state-primitives/src/direction.rs")
        }
        "primitive_lib" => include_str!("../../../crates/ui-state-primitives/src/lib.rs"),
        "headless_direction" => include_str!("../../../crates/ui-headless/src/direction.rs"),
        "headless_lib" => include_str!("../../../crates/ui-headless/src/lib.rs"),
        "motion_lib" => include_str!("../../../crates/ui-motion/src/lib.rs"),
        ...
        "cross_crate_direction_semantics" => {
            include_str!("../../../crates/ui-components/tests/direction_semantics.rs")
        }
        ...
        "performance_script" => include_str!("../../../scripts/check-ui-components-performance.sh"),
        "engineering_script" => include_str!("../../../scripts/check-ui-components-engineering.sh"),
        "contract_hygiene_script" => {
            include_str!("../../../scripts/check-ui-components-contract-hygiene.sh")
        }
        "tree_shaking_budget_env" => include_str!("../../../scripts/tree_shaking_budget.env"),
        "button_check2" => include_str!("../../../components/button/check2.md"),
        "input_check2" => include_str!("../../../components/text-input/src/input/check2.md"),
        ...
        "direction_e2e_contract" => {
            include_str!("../../../e2e/tests/docs_app_direction_contract.spec.mjs")
        }
        "dev_docs_script" => include_str!("../../../scripts/dev-docs-app.sh"),
        "dev_web_script" => include_str!("../../../scripts/dev-web-demo.sh"),
        "dx_script" => include_str!("../../../scripts/check-ui-components-dx.sh"),
        "docs_app_playground" => include_str!("../../../apps/docs-app/src/playground.rs"),
        "docs_components_test_mod" => {
            include_str!("../../../apps/docs-app/src/pages/components/test/mod.rs")
        }
        "docs_components_pages_registry" => {
            include_str!("../../../apps/docs-app/src/pages/components/pages.rs")
        }
        "docs_app_lib" => include_str!("../../../apps/docs-app/src/lib.rs"),
        "docs_app_debug_overlay" => include_str!("../../../apps/docs-app/src/debug_overlay.rs"),
        "ui_headless_trace" => include_str!("../../../crates/ui-headless/src/trace.rs"),
        "protocol_test" => include_str!("../test/protocol.rs"),
        _ => panic!("unsupported source path: {path}"),
    }
}

#[test]
fn direction_component_module_boundaries_are_correct() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "Direction module should include `{needle}`."
        );
    }

    for forbidden in ["pub mod view", "pub mod logic"] {
        assert!(
            !source.contains(forbidden),
            "Direction internals should stay private; found `{forbidden}`."
        );
    }
}

#[test]
fn direction_assembly_consumes_primitives_and_headless_contracts() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let primitive_source = load_source("primitive_direction");
    let primitive_lib_source = load_source("primitive_lib");
    let headless_source = load_source("headless_direction");
    let headless_lib_source = load_source("headless_lib");

    assert!(
        logic_source.contains(
            "pub use ui_state_primitives::direction::{DirectionMode, normalize_optional_text};"
        ),
        "Direction logic should consume state primitive contract from ui-state-primitives."
    );
    for forbidden in [
        ...
        assert!(
            ...
            "Direction logic should not reimplement primitive `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "Direction primitive should define `{needle}`.",
        );
    }
    assert!(
        primitive_lib_source.contains("pub mod direction;"),
        "ui-state-primitives should export direction module."
    );
    ...
    for needle in [
        ...
        assert!(
            ...
            "Direction view should assemble headless semantics via `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-headless direction contract should define `{needle}`.",
        );
    }
    for needle in [
        "pub mod direction;",
        ...
        assert!(
            ...
            "ui-headless should export direction contract marker `{needle}`.",
        );
    }
}

#[test]
fn direction_public_api_is_platform_agnostic() {
    let sources = [
        load_source("mod"),
        load_source("logic"),
        load_source("view"),
        load_source("styles"),
        load_source("protocol"),
    ];
    ...
    for source in sources {
        for forbidden in ["web_sys", "wasm_bindgen", "HtmlElement"] {
            assert!(
                !source.contains(forbidden),
                "Direction public surface should not expose platform detail `{forbidden}`.",
            );
        }
    }
}
...
#[test]
fn direction_docs_and_ui_components_exports_are_present() {
    let docs_source = load_source("docs_page");
    let ui_components_source = load_source("ui_components_lib");
    ...
    for needle in [
        ...
        assert!(
            ...
            "Direction docs page should include `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ui_components_source.contains(needle),
            "ui-components should expose direction through `{needle}`.",
        );
    }
}

#[test]
fn direction_component_semantics_test_contract_exists() {
    ...
    assert!(
        ...
        "Direction component should mount component-level semantics.rs tests.",
    );
}

#[test]
fn direction_api_naming_contract_keeps_primary_name_and_alias_strategy() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let check_source = include_str!("../check2.md");
    ...
    for needle in [
        ...
        "DirectionPropSource::Direction",
        "DirectionPropSource::DirAlias",
        "DirectionPropSource::Default",
        ...
        assert!(
            found,
            "Direction API naming contract should include `{needle}`.",
        );
    }

    assert!(
        check_source.contains("兼容策略")
            && check_source.contains("迁移路径")
            && check_source.contains("N/A（direction）"),
        "direction/check2.md should document naming compatibility strategy, migration path, and N/A scope."
    );
}

#[test]
fn direction_has_no_controlled_uncontrolled_state_axes() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let check_source = include_str!("../check2.md");
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "Direction should stay stateless and must not introduce controlled/uncontrolled axis token `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "direction/check2 controlled contract should keep `{needle}`.",
        );
    }
}

#[test]
fn direction_default_value_resolution_is_single_sourced_in_logic() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let check_source = include_str!("../check2.md");
    ...
    for needle in [
        ...
        assert!(
            ...
            "Direction logic should keep single-source default rule fragment `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "Direction view should consume normalized direction from logic only."
    );
    for forbidden in [
        "unwrap_or(",
        "unwrap_or_else(",
        ...
        assert!(
            ...
            "Direction view should not perform fallback/default branching `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 default-source contract should keep `{needle}`.",
        );
    }
}

#[test]
fn direction_state_normalization_is_centralized_in_logic() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    ...
    for needle in [
        ...
        assert!(
            logic_source.contains(needle),
            "Direction logic should own typed normalization contract fragment `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "Direction view should only consume normalized state via `{needle}`.",
        );
    }
    for forbidden in [
        "match direction",
        ...
        assert!(
            !view_source.contains(forbidden),
            "Direction view should not rebuild state machine logic fragment `{forbidden}`.",
        );
    }

    assert!(
        styles_source.contains(".ui-direction-provider"),
        "Direction styles should keep static layout class contract."
    );
    for forbidden in ["data-direction", "if ", "match "] {
        assert!(
            !styles_source.contains(forbidden),
            "Direction styles should not contain state branching fragment `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 normalization contract should keep `{needle}`.",
        );
    }
}

#[test]
fn direction_discrete_state_inputs_are_enum_constrained() {
    let primitive_source = load_source("primitive_direction");
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let check_source = include_str!("../check2.md");
    ...
    for needle in [
        ...
        assert!(
            primitive_source.contains(needle),
            "Direction primitive should define enum-constrained discrete state fragment `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            found,
            "Direction inputs should remain enum-typed via `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden) && !view_source.contains(forbidden),
            "Direction should not regress to boolean/string state combination `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 discrete-state contract should keep `{needle}`.",
        );
    }
}

#[test]
fn direction_state_primitive_source_boundary_is_enforced() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    ...
    assert!(
        ...
        "Direction logic should consume state primitives from ui-state-primitives only.",
    );
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden) && !view_source.contains(forbidden),
            "Direction component should not reimplement primitives or bind business store token `{forbidden}`.",
        );
    }

    assert!(
        cargo_source.contains("ui-state-primitives"),
        "Direction component should declare ui-state-primitives dependency.",
    );
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 primitive-source contract should keep `{needle}`.",
        );
    }
}

#[test]
fn direction_has_no_async_interaction_protocol_surface() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let check_source = include_str!("../check2.md");
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden) && !view_source.contains(forbidden),
            "Direction should remain async-free and must not include token `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 async contract should keep `{needle}`.",
        );
    }
}

#[test]
fn direction_api_dx_stays_simple_and_minimal() {
    let view_source = load_source("view");
    let docs_source = load_source("docs_page");
    let check_source = include_str!("../check2.md");
    ...
    assert!(
        ...
        "Direction docs should expose simple component-level imports only."
    );
    for forbidden in ["ui_state_primitives", "ui_headless::", "use_direction("] {
        assert!(
            !docs_source.contains(forbidden),
            "Direction docs should not require primitive/headless wiring token `{forbidden}`.",
        );
    }

    assert!(
        view_source.contains("pub fn DirectionProvider("),
        "Direction should expose direct component API entry."
    );
    for forbidden in ["state:", "state =", "state: RwSignal", "state: ReadSignal"] {
        assert!(
            !view_source.contains(forbidden),
            "Direction should not require internal state object token `{forbidden}`.",
        );
    }
  ...
  <div class="docs-direction-demo">"Name → Value"</div>
    ...
    assert!(
        ...
        "Direction docs should keep runnable hello-world snippet."
    );
    ...
    assert!(
        hello_world_lines <= 5,
        "Direction hello-world snippet should stay within 5 lines, got {hello_world_lines}.",
    );
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 DX contract should keep `{needle}`.",
        );
    }
}

#[test]
fn direction_composition_api_rule_is_marked_na_with_guardrails() {
    let view_source = load_source("view");
    let docs_source = load_source("docs_page");
    let check_source = include_str!("../check2.md");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 composition contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "Direction should keep single-container provider shape."
    );
    for forbidden in [
        "ItemSpec",
        "labels",
        ...
        assert!(
            !view_source.contains(forbidden),
            "Direction view should not include composite item protocol token `{forbidden}`.",
        );
    }
    ...
    for forbidden in ["labels + children", "titles + panels", "ItemSpec"] {
        assert!(
            !docs_source.contains(forbidden),
            "Direction docs should not recommend composite sugar token `{forbidden}`.",
        );
    }
}

#[test]
fn direction_macro_micro_drag_state_machine_is_not_applicable() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let styles_source = load_source("styles");
    let cargo_source = load_source("cargo");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 macro-micro contract should keep `{needle}`.",
        );
    }

    assert!(
        !cargo_source.contains("ui-motion"),
        "Direction should remain motion-free in current static scope.",
    );
    for forbidden in [
        "Dragging",
        "Action::DragEnd",
        ...
        assert!(
            !logic_source.contains(forbidden)
                && !view_source.contains(forbidden)
                && !styles_source.contains(forbidden),
            "Direction should not contain macro/micro drag state token `{forbidden}`.",
        );
    }
}

#[test]
fn direction_two_pass_rendering_geometry_pipeline_is_not_applicable() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let styles_source = load_source("styles");
    let check_source = include_str!("../check2.md");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 two-pass contract should keep `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden)
                && !view_source.contains(forbidden)
                && !styles_source.contains(forbidden),
            "Direction should not contain two-pass geometry token `{forbidden}`.",
        );
    }
}

#[test]
fn direction_registration_protocol_is_not_applicable_for_single_provider() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let styles_source = load_source("styles");
    let check_source = include_str!("../check2.md");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 registration-protocol contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        view_source.contains("pub fn DirectionProvider(")
            && view_source.contains("children: Children"),
        "Direction should keep single-provider + children API shape."
    );
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden)
                && !view_source.contains(forbidden)
                && !styles_source.contains(forbidden),
            "Direction should not contain collection registration token `{forbidden}`.",
        );
    }
}

#[test]
fn direction_slot_projection_strategy_is_not_applicable_for_single_provider() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let styles_source = load_source("styles");
    let check_source = include_str!("../check2.md");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 slot-projection contract should keep `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden)
                && !view_source.contains(forbidden)
                && !styles_source.contains(forbidden),
            "Direction should not contain slot projection token `{forbidden}`.",
        );
    }
}

#[test]
fn direction_env_streams_pipeline_is_not_applicable_for_single_provider() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let styles_source = load_source("styles");
    let check_source = include_str!("../check2.md");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 env-streams contract should keep `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden)
                && !view_source.contains(forbidden)
                && !styles_source.contains(forbidden),
            "Direction should not contain env stream token `{forbidden}`.",
        );
    }
}

#[test]
fn direction_event_light_cone_is_not_applicable_for_single_provider() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let styles_source = load_source("styles");
    let check_source = include_str!("../check2.md");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 event-light-cone contract should keep `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden)
                && !view_source.contains(forbidden)
                && !styles_source.contains(forbidden),
            "Direction should not contain event light cone token `{forbidden}`.",
        );
    }
}

#[test]
fn direction_causality_bus_is_not_applicable_for_single_provider() {
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let styles_source = load_source("styles");
    let check_source = include_str!("../check2.md");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 causality-bus contract should keep `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden)
                && !view_source.contains(forbidden)
                && !styles_source.contains(forbidden),
            "Direction should not contain causality bus token `{forbidden}`.",
        );
    }
}

#[test]
fn direction_a11y_i18n_contract_is_present_and_text_is_not_hardcoded() {
    let view_source = load_source("view");
    let headless_direction_source = load_source("headless_direction");
    let check_source = include_str!("../check2.md");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 a11y-i18n contract should keep `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "Direction view should mount a11y/i18n contract via `{needle}`.",
        );
    }
    ...
    for forbidden in ["Name → Value", "الاسم ← القيمة", "aria-label=", "role="] {
        assert!(
            !view_source.contains(forbidden),
            "Direction view should not hardcode user text or fake interactive semantics `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "Headless direction contract should use shared a11y tool via `{needle}`.",
        );
    }
}
...
fn direction_state_markers_are_observable_queryable_and_enumerable() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 state-marker contract should keep `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "Direction should keep stable marker fragment `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "Direction view should not encode unrelated interactive marker token `{forbidden}`.",
        );
    }
}
...
fn direction_styles_depend_on_stable_markers_or_class_not_dom_shape() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 style-explicit-state contract should keep `{needle}`.",
        );
    }

    assert!(
        styles_source.contains(".ui-direction-provider"),
        "Direction styles should keep stable class selector contract."
    );
    ...
    for forbidden in [
        ...
        assert!(
            !styles_source.contains(forbidden),
            "Direction styles should not use fragile structural/state selector token `{forbidden}`.",
        );
    }
    ...
    for forbidden in ["style=", "style:", "style:--"] {
        assert!(
            !view_source.contains(forbidden),
            "Direction view should not carry runtime inline style token `{forbidden}`.",
        );
    }
}
...
fn direction_tests_prioritize_semantic_contracts_over_snapshots() {
    ...
    for needle in [
        ...
        "语义契约覆盖：`components/direction/test/semantics.rs` 与 `crates/ui-components/tests/direction_semantics.rs` 均断言 `lang/dir/data-direction/data-direction-source` 与来源封闭集合。",
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 semantic-test contract should keep `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "Direction semantic tests should cover contract marker `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "Direction tests should not rely on snapshot assertion token `{forbidden}`.",
        );
    }
}
...
fn direction_component_file_responsibilities_are_well_scoped() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 file-responsibility contract should keep `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "Direction mod.rs should include boundary fragment `{needle}`.",
        );
    }
    for forbidden in [
        "pub mod view",
        ...
        assert!(
            ...
            "Direction mod.rs should not expose implementation/detail token `{forbidden}`.",
        );
    }
    ...
    assert!(
        styles_source.contains(".ui-direction-provider"),
        "Direction styles.rs should keep static class rule."
    );
    for forbidden in ["if ", "match ", "var(--ui-", "color:", "content:"] {
        assert!(
            !styles_source.contains(forbidden),
            "Direction styles.rs should not contain style-branch/theme-copy token `{forbidden}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "Direction view.rs should not hide state/motion detail token `{forbidden}`.",
        );
    }
...
fn direction_spec_file_policy_stays_na_for_simple_component() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 spec-file governance should keep `{needle}`.",
        );
    }
    ...
    for forbidden in ["mod spec;", "pub mod spec;", "pub use spec::"] {
        assert!(
            !mod_source.contains(forbidden),
            "Direction mod.rs should not wire spec module token `{forbidden}`.",
        );
    }

    assert!(
        !spec_path.exists(),
        "Direction is a simple semantic-provider component and should not define `src/spec.rs`.",
    );
    ...
    for needle in [
        ...
        assert!(
            ...
            "Direction protocol.rs should keep minimal versioned schema token `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            docs_source.contains(needle),
            "Direction docs-app page should keep primary documentation token `{needle}`.",
        );
    }
}
...
fn direction_token_first_static_style_contract_is_enforced() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 token-first style contract should keep `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "Direction styles.rs should keep static style fragment `{needle}`.",
        );
    }
    ...
    for forbidden in ["var(--ui-direction-", "@apply", "tw-", "style!(", "css!"] {
        assert!(
            !styles_source.contains(forbidden),
            "Direction styles.rs should not include utility/css-in-rust/private-token token `{forbidden}`.",
        );
    }
    ...
    assert!(
        ...
        "ui-components css.rs should feature-gate and aggregate direction styles."
    );
    ...
    assert!(
        ...
        "Direction view.rs should not include inline style business logic."
    );

    for forbidden in ["stylist", "tailwind", "css-in-rust"] {
        assert!(
            !cargo_source.contains(forbidden),
            "Direction Cargo manifest should not wire utility/css-in-rust dependency token `{forbidden}`.",
        );
    }
}
...
fn direction_visual_desire_reuses_global_theme_baseline_and_stays_non_visual() {
    let check_source = include_str!("../check2.md");
    let styles_source = load_source("styles");
    let baseline_page_source = load_source("theme_visual_baseline_page");
    let baseline_registry_source = load_source("theme_visual_baseline_registry");
    let baseline_e2e_source = load_source("theme_visual_baseline_e2e");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 visual-desire contract should keep `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !styles_source.contains(forbidden),
            "Direction should stay non-visual utility and avoid style token `{forbidden}`.",
        );
    }
}
...
fn direction_tree_shaking_contract_is_feature_gated_and_budgeted() {
    ...
    let tree_shaking_budget_env = load_source("tree_shaking_budget_env");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 tree-shaking contract should keep `{needle}`.",
        );
    }
...
fn direction_type_system_and_semantic_markers_form_machine_readable_contract() {
    ...
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let cross_crate_semantics_source = load_source("cross_crate_direction_semantics");
    ...
    for needle in [
        ...
        "可持续反馈闭环：`components/direction/test/semantics.rs` 与 `crates/ui-components/tests/direction_semantics.rs` 对类型轴、归一路径与语义标记进行回归锁定，破坏契约可直接定位到具体层（primitive/logic/view）。",
    ] {
        assert!(
            check_source.contains(needle),
            "direction/check2 typed-machine-readable contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        cross_crate_semantics_source.contains(
            "fn direction_type_system_and_semantic_markers_form_machine_readable_contract()"
        ),
        "cross-crate semantics suite should keep mirrored machine-readable contract regression.",
    );
}
...
fn direction_focus_stack_contract_is_not_applicable_and_private_node_refs_are_forbidden() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 focus-stack contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored focus-stack contract regression.",
    );
}
...
fn direction_escape_hatches_contract_is_not_applicable_and_foreign_zone_is_forbidden() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 escape-hatches contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored escape-hatches contract regression.",
    );
}
...
fn direction_ssr_hydration_discontinuity_contract_uses_deterministic_id_provider_path() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 hydration-discontinuity contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored hydration-discontinuity contract regression.",
    );
}
...
fn direction_ssr_and_cross_platform_compile_contract_is_documented_and_non_wasm_safe() {
    ...
    let logic_source = load_source("logic");
    let view_source = load_source("view");
    let platform_script_source = load_source("platform_script");
    let cross_crate_semantics_source = load_source("cross_crate_direction_semantics");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 cross-platform contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored cross-platform contract regression.",
    );
}
...
fn direction_ui_headless_web_ssr_mutex_is_compile_error_guarded() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 ui-headless mutex contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        cross_crate_semantics_source
            .contains("fn direction_ui_headless_web_ssr_mutex_is_compile_error_guarded()",),
        "cross-crate semantics suite should keep mirrored ui-headless mutex contract regression.",
    );
}
...
fn direction_ui_motion_non_wasm_stub_contract_keeps_ssr_tooling_buildable() {
    let check_source = include_str!("../check2.md");
    let motion_lib_source = load_source("motion_lib");
    let platform_script_source = load_source("platform_script");
    let cargo_source = load_source("cargo");
    let view_source = load_source("view");
    let cross_crate_semantics_source = load_source("cross_crate_direction_semantics");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 ui-motion non-wasm contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored ui-motion non-wasm contract regression.",
    );
}
...
fn direction_reduced_motion_ssr_wasm_contract_is_explicitly_na_and_semantics_stable() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 reduced-motion/ssr/wasm contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored reduced-motion/ssr/wasm contract regression.",
    );
}
...
fn direction_performance_governance_is_mount_only_traceable_and_blocking_via_global_gates() {
    let check_source = include_str!("../check2.md");
    let performance_script_source = load_source("performance_script");
    let button_check2_source = load_source("button_check2");
    let input_check2_source = load_source("input_check2");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 performance governance contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored performance governance contract regression.",
    );
}
...
fn direction_view_macro_complexity_stays_single_block_and_flat_structure() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 macro-complexity contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored macro-complexity contract regression.",
    );
}
...
fn direction_function_first_split_policy_keeps_single_component_entrypoint() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 function-first split contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored function-first split contract regression.",
    );
}
...
fn direction_static_fragments_are_consolidated_and_do_not_bloat_view_templates() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 static-fragment contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored static-fragment contract regression.",
    );
}
...
fn direction_inner_html_contract_forbids_dynamic_injection_and_keeps_semantics_stable() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 inner_html contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored inner_html contract regression.",
    );
}
...
fn direction_wasm_debug_contract_is_globally_traceable_feature_isolated_and_non_polluting() {
    ...
    let view_source = load_source("view");
    let logic_source = load_source("logic");
    let trace_source = load_source("ui_headless_trace");
    let docs_app_lib_source = load_source("docs_app_lib");
    let docs_app_debug_overlay_source = load_source("docs_app_debug_overlay");
    let ui_components_cargo_source = load_source("ui_components_cargo");
    let cross_crate_semantics_source = load_source("cross_crate_direction_semantics");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 wasm-debug contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored wasm-debug contract regression.",
    );
}
...
fn direction_dx_contract_uses_playground_hot_reload_and_marks_optional_persist_na() {
    ...
    let view_source = load_source("view");
    let logic_source = load_source("logic");
    let docs_page_source = load_source("docs_page");
    let playground_source = load_source("docs_app_playground");
    let docs_components_test_source = load_source("docs_components_test_mod");
    let dev_docs_script_source = load_source("dev_docs_script");
    let dev_web_script_source = load_source("dev_web_script");
    let dx_script_source = load_source("dx_script");
    let cross_crate_semantics_source = load_source("cross_crate_direction_semantics");
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored DX contract regression.",
    );
}
...
fn direction_engineering_contract_uses_serde_protocol_and_keeps_tracing_runtime_boundaries() {
    ...
    let logic_source = load_source("logic");
    let mod_source = load_source("mod");
    let engineering_script_source = load_source("engineering_script");
    let trace_source = load_source("ui_headless_trace");
    let cross_crate_semantics_source = load_source("cross_crate_direction_semantics");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 engineering contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored engineering contract regression.",
    );
}
...
fn direction_styles_use_defensive_variable_fallback_chain() {
    let check_source = include_str!("../check2.md");
    let styles_source = load_source("styles");
    let ui_theme_css_source = load_source("ui_theme_css");
    let contract_hygiene_script_source = load_source("contract_hygiene_script");
    let cross_crate_semantics_source = load_source("cross_crate_direction_semantics");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 defensive-variable contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored defensive-variable contract regression.",
    );
}
...
fn direction_cascade_layer_and_runtime_style_contract_is_enforced() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 cascade-layer contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored cascade-layer contract regression.",
    );
}
...
fn direction_motion_contract_is_explicitly_na_and_keeps_reduced_motion_noop_guards() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 motion-contract entry should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored motion-contract regression.",
    );
}
...
fn direction_ui_components_fixed_entry_files_follow_layered_boundaries() {
    let check_source = include_str!("../check2.md");
    let ui_components_lib_source = load_source("ui_components_lib");
    let ui_components_css_source = load_source("ui_components_css");
    let ui_components_root_source = load_source("ui_components_root");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 ui-components fixed entry contract should keep `{needle}`.",
        );
    }
    ...
    let workspace_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .and_then(std::path::Path::parent)
        .unwrap_or_else(|| panic!("workspace root should be three levels above direction crate"));
    ...
    for missing_file in [
        ...
        let path = workspace_root.join(missing_file);
        assert!(
            !path.exists(),
            "ui-components forbidden entry file should stay absent: {missing_file}",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored ui-components fixed-entry regression.",
    );
}
...
fn direction_component_directory_standard_files_are_correct() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 directory-standard-files contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored directory-standard-files regression.",
    );
}
...
fn direction_file_placement_discipline_is_enforced() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 file-placement discipline contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        cross_crate_semantics_source
            .contains("fn direction_file_placement_discipline_is_enforced()"),
        "cross-crate semantics suite should keep mirrored file-placement discipline regression.",
    );
}
...
fn direction_hyper_structure_builder_contract_is_explicitly_na_for_simple_provider() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 hyper-structure-builder contract should keep `{needle}`.",
        );
    }

    assert!(
        !component_root.join("src/spec.rs").exists(),
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored hyper-structure-builder regression.",
    );
}
...
fn direction_context_compression_manifest_and_rbi_projection_are_present_and_current() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 manifest+rbi contract should keep `{needle}`.",
        );
    }

    assert!(
        component_root.join("src/Component.toml").exists(),
        "direction should keep context-compression manifest file present.",
    );
    assert!(
        component_root.join("src/direction.rbi").exists(),
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored manifest+rbi regression.",
    );
}
...
fn direction_agent_contract_schema_markers_are_typed_traceable_and_whitelisted() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 agent-contract contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored agent-contract regression.",
    );
}
...
fn direction_streaming_term_scope_is_documented_and_stays_snapshot_oriented() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 streaming-term contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored streaming-term regression.",
    );
}
...
fn direction_snapshot_is_baseline_and_consumes_complete_config_stably() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 snapshot-baseline contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored snapshot-baseline regression.",
    );
}
...
fn direction_streaming_requiredness_is_optional_with_snapshot_fallback_and_stable_status_markers() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 streaming-requiredness contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored streaming-requiredness regression.",
    );
}
...
fn direction_rust_hygiene_contract_is_enforced_for_non_test_code_and_string_cow_paths() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 rust-hygiene contract should keep `{needle}`."
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored rust-hygiene regression.",
    );
}
...
fn direction_tree_shaking_feature_pruning_is_gated_in_ui_components() {
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored tree-shaking regression.",
    );
}
...
fn direction_semantic_and_performance_regression_contract_is_covered_without_snapshot_bias() {
    ...
    for needle in [
        ...
        "语义覆盖证据：`components/direction/test/semantics.rs` 与 `crates/ui-components/tests/direction_semantics.rs` 已断言 `lang/dir/data-slot/data-direction/data-direction-source` 与来源封闭集合，主路径基于语义标记而非视觉快照。",
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 semantic+performance contract should keep `{needle}`.",
        );
    }
    ...
    for mirrored in [
        ...
        assert!(
            cross_crate_semantics_source.contains(mirrored),
            "cross-crate semantics suite should keep mirrored contract `{mirrored}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored semantic+performance regression.",
    );
}
...
fn direction_version_deprecation_migration_contract_is_explicitly_na_without_breaking_upgrade() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 version-migration contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored version-migration regression.",
    );
}
...
fn direction_docs_copy_paste_ready_contract_is_documented_and_enforced() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 docs copy-ready contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored docs copy-ready regression.",
    );
}
...
fn direction_semantic_contract_priority_is_enforced_with_mirrored_semantics_tests() {
    ...
    for needle in [
        ...
        "语义测试落点：`components/direction/test/semantics.rs` 与 `crates/ui-components/tests/direction_semantics.rs` 均存在并持续镜像，保证组件内与聚合层对同一语义契约双向回归。",
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 semantic-priority contract should keep `{needle}`.",
        );
    }
...
fn direction_e2e_selector_contract_uses_semantic_markers_and_stable_readiness() {
    let check_source = include_str!("../check2.md");
    let direction_e2e_source = load_source("direction_e2e_contract");
    let cross_crate_semantics_source = load_source("cross_crate_direction_semantics");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 e2e-selector contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored e2e-selector regression.",
    );
}
...
fn direction_key_flow_is_repeatable_and_reports_semantic_breakpoints() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 key-flow regression contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored key-flow regression.",
    );
}
...
fn direction_docs_examples_and_matrices_are_synced_with_logic_contract() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 docs-matrix contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored docs-matrix regression.",
    );
}
...
fn direction_documentation_entry_is_beginner_friendly_and_ordered_for_progression() {
    let check_source = include_str!("../check2.md");
    let docs_page_source = load_source("docs_page");
    let docs_components_pages_source = load_source("docs_components_pages_registry");
    let docs_components_test_source = load_source("docs_components_test_mod");
    let cross_crate_semantics_source = load_source("cross_crate_direction_semantics");
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 documentation-product contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored documentation-product regression.",
    );
}
...
fn direction_interactive_playground_supports_realtime_props_and_repeatable_flow() {
    ...
    for needle in [
        ...
        assert!(
            check_source.contains(needle),
            "direction/check2 interactive-playground contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored interactive-playground regression.",
    );
}
...
fn direction_source_first_docs_are_copy_paste_ready_and_traceable() {
    ...
    for needle in [
        ...
        "同步策略已显式化：`data-slot=\"direction-source-sync-note\"` 约束 starter snippet 与 Hello World 同步更新；`components/direction/test/semantics.rs` 与 `crates/ui-components/tests/direction_semantics.rs` 镜像回归锁定该契约，防止示例漂移。",
    ] {
        assert!(
            check_source.contains(needle),
            "direction/check2 source-first docs contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored source-first regression.",
    );
}
...
fn direction_heroui_strategy_doc_and_docs_entry_are_synced_for_parameter_changes() {
    let check_source = include_str!("../check2.md");
    let heroui_strategy_source = load_source("heroui_strategy_doc");
    let heroui_research_source = load_source("heroui_research_doc");
    let docs_components_pages_source = load_source("docs_components_pages_registry");
    let docs_components_test_source = load_source("docs_components_test_mod");
    let cross_crate_semantics_source = load_source("cross_crate_direction_semantics");
    ...
    for needle in [
        ...
        "回归约束：`components/direction/test/semantics.rs` 与 `crates/ui-components/tests/direction_semantics.rs` 镜像断言该条勾选状态、HeroUI 对标文档段落和 docs 索引入口，防止“仅代码更新无文档更新”回归。",
    ] {
        assert!(
            check_source.contains(needle),
            "direction/check2 HeroUI-doc-sync contract should keep `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "cross-crate semantics suite should keep mirrored HeroUI-doc-sync regression.",
    );
}
