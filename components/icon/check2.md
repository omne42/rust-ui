...
- [x] 性能治理：关键路径有预算（首次渲染/更新耗时/内存），回归可检测、可归因、可阻断。（单组件结论：`icon` 为静态展示组件，`components/icon/src` 无 `create_signal/create_memo/create_effect` 等响应式更新路径、无动效循环与异步分支，关键路径预算为“初始化一次渲染 + props 变化触发重渲染”；等价可重复证据由 `components/icon/test/semantics.rs` 对稳定渲染语义与来源标记做固定断言。仓库级 `Button/Input` 预算与 `render_count` 自动化后续由 `crates/ui-components/tests/button_semantics.rs`、`crates/ui-components/tests/input_semantics.rs`、`crates/ui-components/tests/accordion_semantics.rs` 中的 performance governance / `render_count` follow-up 断言承接）
...
- [x] 级联层覆盖（`@layer ui`）：组件 CSS 默认聚合进 `@layer ui`；运行时数值调整仅通过 CSS Custom Properties（如 `style:--x=...`），禁止普通内联样式（如 `style=\"top: 10px\"`）。（实现证据：`crates/ui-components/src/css.rs` 的 `push_components_css` 以 `out.push_str(\"\\n@layer ui {\\n\")` 开启并在末尾闭合，`component-icon/component-iconset/component-icons/component-icons_ui/component-icons_workflow` 的 CSS 均通过该入口聚合进入 `@layer ui`；`components/icon/src` 与 `crates/ui-components/tests/icon*_semantics.rs`/`icons*_semantics.rs` 检索未发现 `style=\"...\"`、`style=...` 或 `style:<prop>=...` 普通内联样式绑定）
...
- [x] `Snapshot` 是所有组件的基础能力（默认必须支持）。（已满足：`icon` 家族为 snapshot-first 渲染路径，消费完整 props 一次性稳定渲染；`components/icon/src/Component.toml` 已声明 `snapshot_rendering=true`，`components/icon/src/icon.rbi` 对外投影 `Icon/Icons/Iconset/IconsUi/IconsWorkflow` 完整调用签名且未暴露 streaming 专属输入。回归：`crates/ui-components/tests/icon_semantics.rs::icon_snapshot_mode_is_baseline_and_streaming_protocol_is_not_exposed`）
  - 所有组件都应能消费“完整生成结果”并稳定渲染。
  - 即使组件不直接展示正文，也应能在接收上层完整配置后正常渲染。
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。（已落实为组件级 `Streaming Optional + fallback=snapshot`：`components/icon/src/protocol.rs` 新增类型化契约 `IconStreamingRequirement/IconOutputMode/IconOutputStatus/IconOutputDataAttrs + resolve_output_data_attrs()`，固定输出 `optional + snapshot + verified`；`components/icon/src/view.rs`、`components/icon/src/icons/view.rs`、`components/icon/src/set/view.rs`、`components/icon/src/ui/view.rs`、`components/icon/src/workflow/view.rs` 根节点统一挂载 `data-ui-streaming/data-ui-streaming-fallback/data-ui-output-mode/data-ui-output-status`，保证与现有 `role/aria/data-*` 连续可读；`components/icon/src/Component.toml` 同步声明 `[streaming_policy]` 与 `streaming_optional_with_snapshot_fallback` 能力；回归：`crates/ui-components/tests/icon_semantics.rs::icon_agent_contract_schema_is_typed_and_mounted_across_views` 与 `icon_snapshot_mode_is_baseline_and_streaming_protocol_is_not_exposed`）
