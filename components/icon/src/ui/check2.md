...
### 10. IconsUi 本轮验收证据
- [x] 最小特性依赖链修复：`crates/ui/Cargo.toml` 中补齐 `component-icons_ui -> component-iconset` 与 `component-iconset -> component-icon`，确保 source/package 模式下 `IconsUi` 可独立编译。
- [x] 组件导入去根重导出耦合：
  - `crates/ui/src/icon/ui/view.rs` 改为 `use crate::iconset::Iconset;`
  - `crates/ui/src/icon/set/view.rs` 改为 `use crate::icon::Icon;`
- [x] 最小特性构建通过（native + wasm）：
  - `CARGO_TARGET_DIR=target/icons-ui-check2 cargo check -p ui --no-default-features --features component-icons_ui,inject-css`
  - `CARGO_TARGET_DIR=target/icons-ui-check2 cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-icons_ui,inject-css`
- [x] 语义回归通过：
  - `CARGO_TARGET_DIR=target/icons-ui-check2 cargo test -p ui --test icons_ui_semantics --no-default-features --features component-icons_ui,inject-css`
- [x] 特性树确认（未被 `all-components` 隐式拉起）：
  - `cargo tree -e features -i ui -p ui --no-default-features --features component-icons_ui,inject-css`
