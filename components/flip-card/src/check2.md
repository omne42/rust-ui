...
- [x] 本次组件范围内核心结论：`flip-card` 分层与语义契约成立，`mod/logic/view/styles/motion` 职责清晰，最小组件 crate 可独立构建。
- [x] 本次迁移阻断项：无新增阻断；迁移后仅保留新组件 crate 门禁验证。
...
- [x] Tree-shaking 结论：`ui` 通过 `component-flip_card -> dep:ui-flip-card` 接线，最小特性链路无隐式全量组件拉起。
