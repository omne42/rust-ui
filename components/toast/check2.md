...
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（`Toast` 的 slot/state/source 原语已下沉到 `crates/ui-state-primitives/src/toast.rs`（新增 `ToastSlot/ToastPartState*/ToastViewportState*/ToastStoreSource/ToastVariant` 与 `resolve_state/resolve_viewport_state`）；组件层 `components/toast/src/toast/logic.rs` 改为委托 primitive；组件导出在 `components/toast/src/toast/mod.rs` 通过 `pub use ui_state_primitives::toast::{...}` 暴露。primitive 回归新增 `crates/ui-state-primitives/src/test/toast.rs::toast_state_markers_are_derived_from_input` 与 `::text_normalization_helpers_trim_and_default`。）
