...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（已执行 calendar 定向门禁：`cargo test -p ui-calendar --test calendar_semantics`、`cargo check -p ui-calendar`、`cargo check -p ui-calendar --target wasm32-unknown-unknown`。）
