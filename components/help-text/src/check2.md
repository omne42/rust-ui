...
- 语义测试：`cargo test -p ui --test help_text_semantics --no-default-features --features component-help_text,inject-css` 通过（6/6）。
- 最小特性编译：`cargo check -p ui --no-default-features --features component-help_text,inject-css` 通过。
- wasm 路径编译：`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-help_text,inject-css` 通过（存在 `observability.rs` 的既有 dead_code warning，非 `help_text` 组件新增）。
- Clippy 门禁：`cargo clippy -p ui --no-default-features --features component-help_text,inject-css -- -D warnings` 通过。
- 说明：默认特性全量编译当前被 `crates/ui/src/menu/dropdown/view.rs` 的既有错误阻塞（与本组件无关），本次按“最小特性链 + 组件边界”完成核验，避免干扰并行协作改动。
