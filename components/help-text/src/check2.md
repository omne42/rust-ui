...
- 架构边界：`components/help-text/src/` 维持 `mod.rs + logic.rs + styles.rs + view.rs + motion.rs` 分层；状态归一与来源标记集中在 `logic.rs`，`view.rs` 只负责渲染与挂载，未越层实现可复用状态原语。
...
- README：`components/help-text/src/README.md` 已补齐，包含展示区 / Config 区 / Code 区 / CSS Test 区结构与状态对比说明。
