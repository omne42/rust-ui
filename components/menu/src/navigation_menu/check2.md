...
2. 修复动作：在 `crates/ui/Cargo.toml` 将 `component-navigation_menu` 调整为依赖 `component-active_highlight`，保持最小特性链可编译。
...
6. 本轮执行门禁（定向）：
   - `CARGO_TARGET_DIR=target-check2-navigation-menu cargo test -p ui --no-default-features --features component-navigation_menu,inject-css --test navigation_menu_semantics`
   - `CARGO_TARGET_DIR=target-check2-navigation-menu cargo clippy -p ui --no-default-features --features component-navigation_menu,inject-css -- -D warnings`
   - `CARGO_TARGET_DIR=target-check2-navigation-menu cargo check -p ui --no-default-features --features component-navigation_menu,inject-css`
   - `CARGO_TARGET_DIR=target-check2-navigation-menu cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-navigation_menu,inject-css`
   - `cargo tree -e features -p ui --no-default-features --features component-navigation_menu,inject-css -f "{p} {f}" | head -n 1`
