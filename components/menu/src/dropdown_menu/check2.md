...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（本次已完成组件范围门禁与最小特性门禁：`TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp CARGO_TARGET_DIR=target/dropdown-menu-check2 cargo check -p ui-components --no-default-features --features component-dropdown_menu,inject-css`、`TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp CARGO_TARGET_DIR=target/dropdown-menu-check2 cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-dropdown_menu,inject-css`、`TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp CARGO_TARGET_DIR=target/dropdown-menu-check2 cargo clippy -p ui-components --no-default-features --features component-dropdown_menu,inject-css --test dropdown_menu_semantics --no-deps -- -D warnings`、`TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp CARGO_TARGET_DIR=target/dropdown-menu-check2 cargo test -p ui-components --no-default-features --features component-dropdown_menu,inject-css --test dropdown_menu_semantics`、`cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-dropdown_menu,inject-css`；结果全部通过。）
