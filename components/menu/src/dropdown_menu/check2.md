...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（本次已完成组件范围门禁与最小特性门禁：`TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp CARGO_TARGET_DIR=target/dropdown-menu-check2 cargo check -p ui --no-default-features --features component-dropdown_menu,inject-css`、`TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp CARGO_TARGET_DIR=target/dropdown-menu-check2 cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-dropdown_menu,inject-css`、`TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp CARGO_TARGET_DIR=target/dropdown-menu-check2 cargo clippy -p ui --no-default-features --features component-dropdown_menu,inject-css --test dropdown_menu_semantics --no-deps -- -D warnings`、`TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp CARGO_TARGET_DIR=target/dropdown-menu-check2 cargo test -p ui --no-default-features --features component-dropdown_menu,inject-css --test dropdown_menu_semantics`、`cargo tree -e features -i ui -p ui --no-default-features --features component-dropdown_menu,inject-css`；结果全部通过。）

### 10. DropdownMenu 本次逐项修复记录（2026-02-18）
- 已修复最小特性依赖链断裂：`crates/ui/Cargo.toml` 将 `component-dropdown_menu` 改为依赖 `component-button`、`component-menu`、`component-popover`，确保 `--no-default-features --features component-dropdown_menu,inject-css` 可独立编译。
- 已修复 `view` 层根导出耦合：`crates/ui/src/menu/dropdown_menu/view.rs` 改为模块路径导入 `button/menu/popover`，避免非 `all-components` 模式下符号缺失。
- 已修复依赖链阻塞点：`crates/ui/src/menu/motion.rs` 改为 `use ui_visual_primitive::active_highlight::ActiveHighlightMotion;`，确保 `component-menu` 在最小特性编译路径可用。
- 已新增回归测试锁定：`components/menu/test/dropdown_menu_semantics.rs` 增加 `dropdown_menu_minimal_feature_gate_keeps_dependency_chain_and_module_paths_wired`，防止后续再次破坏特性依赖和模块导入路径。
