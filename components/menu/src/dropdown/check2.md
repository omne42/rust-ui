...
1. 架构与职责：
   - 状态原语已下沉到 `crates/ui-state-primitives/src/dropdown.rs`，`crates/ui/src/menu/dropdown/logic.rs` 仅做归一化装配与 class 组合。
...
4. 文档与示例：
   ...
   - 组件文档新增 `crates/ui/src/menu/dropdown/README.md`，含 Streaming 策略与 Source-first 路径。
5. 平台与工程约束：
...
- `cargo test -p ui --test dropdown_semantics --no-default-features --features component-dropdown,inject-css` ✅
- `cargo clippy -p ui --test dropdown_semantics --no-default-features --features component-dropdown,inject-css -- -D warnings` ✅
