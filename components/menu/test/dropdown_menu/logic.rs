...
fn resolve_state_tracks_trigger_items_and_strategy_flags() {
    let state = resolve_state(DropdownMenuStateInput {
        ...
        action_mode: DropdownMenuActionMode::KeepOpenOnAction,
...
#[test]
fn dropdown_menu_event_decision_is_centralized() {
...
fn normalize_discrete_props_resolves_alias_priority() {
    assert_eq!(
        normalize_discrete_props(DropdownMenuDiscreteInput {
...
fn normalize_open_state_tracks_control_mode_and_forwards_triplet() {
    let state = normalize_open_state(DropdownMenuOpenStateInput {
        is_open: None,
        open: None,
