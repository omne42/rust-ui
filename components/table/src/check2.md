...
- [x] `ui` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。
  - 组件侧依据：`crates/ui` 以 `component-table` feature gate 暴露 `pub use ui_table as table` 并稳定导出 `Table*`；样式聚合仅在 `component-table` 下附加 `table::styles::CSS`。
  - 语义回归：`components/table/tests/semantics.rs::table_ui_aggregation_exports_feature_gated_public_api_without_dom_type_leaks` 锁定聚合导出与 feature 门控，并断言组件公共边界不泄露 `web-sys`/`wasm_bindgen` 类型。
  - `logic.rs` 负责 props 归一与状态派生；`view.rs` 负责结构渲染与 headless 语义挂载；`styles.rs` 负责 token-first 静态样式；`motion.rs` 负责动效 attach。
  - `table` 无组件级动效语义，`motion.rs` 按职责为 N/A（回归已锁定无 `ui-motion` 依赖且无 `src/motion.rs`）。
...
- [x] API 命名契约统一：公共 props/回调严格使用 `is_*`、`on_*`、`default_*` 前缀；同语义在全库同名，禁止别名漂移。
  - 组件侧依据：`Table` 布尔公开 props 已统一为 `is_striped` / `is_sticky_header`；docs 与语义回归已同步新命名。
  - 布尔状态统一 `is_*`（如 `is_open`/`is_disabled`），事件统一 `on_*`，默认值统一 `default_*`。
  - 同一语义 across 组件必须同名（如都用 `on_open_change`，禁止同义别名并存）。
  - 兼容/迁移策略：采用显式重命名（`striped -> is_striped`、`sticky_header -> is_sticky_header`），迁移路径为一对一替换，旧命名由语义回归持续禁止。
  - N/A：当前组件无受控状态回调，不暴露 `on_* / default_*`；新增可控状态轴时必须遵循命名契约。
- [x] 受控/非受控必须成对：每个可控状态轴都提供 `value + on_value_change + default_value`（如 `open/on_open_change/default_open`）；缺一项即不通过。
  - N/A（当前 `table` 不存在组件内部可变状态轴；`is_striped` / `is_sticky_header` 属于输入配置，不是组件自管理状态）。
  - 语义回归：`components/table/tests/semantics.rs::table_has_no_controlled_uncontrolled_state_axes_yet` 锁定“无半受控 API”。
  - 演进约束：新增可变状态轴时必须同次改动补齐 `value + on_value_change + default_value`，并覆盖受控/非受控切换语义测试。
...
- [x] 默认值单一来源：默认值与优先级只在 `logic.rs` 归一化；`view.rs` 禁止二次兜底或隐式改写。
  - 组件侧依据：`view.rs` 已移除单元格渲染中的 `unwrap_or*` 默认值兜底，改为消费归一化后的列元信息进行配对渲染。
  - 回归锁定：`components/table/tests/semantics.rs::table_default_resolution_stays_out_of_view_layer` 禁止 `view.rs` 回流默认值判定，并校验默认值能力集中在 `ui-state-primitives/src/table.rs`。
