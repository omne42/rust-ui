...
- [x] `ui-motion` 定义：动效能力与契约执行层（spring、keyframes、WAAPI/RAF backend），只负责时间函数、插值与运行时驱动，不承载组件业务语义与状态决策。
  - N/A（Table 当前为静态数据展示组件，不存在 open/close、enter/exit、active/inactive 动效语义；组件未依赖 `ui-motion`，也未自实现 spring/keyframe/driver 执行器）。
...
- [x] `ui-theme` 定义：唯一设计 token 与主题上下文层（system/color/scale + Light/Dark/OLED），负责 token 分类、主题映射与 CSS 变量生成。
  - 组件侧依据：`table` 样式仅消费 `var(--ui-*)` 变量，不在 `logic.rs/view.rs` 重建主题上下文或私有 token 体系；语义测试已锁定该约束。
...
- [x] `ui` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。
  - 组件侧依据：`crates/ui` 通过 `component-table` feature gate 暴露 `pub use ui_table as table`，并在 `all_components` 保持稳定 `Table*` 导出；`css.rs` 仅在 `component-table` 下聚合 `table::styles::CSS`。
  - 语义回归：`components/table/tests/semantics.rs::table_ui_aggregation_exports_feature_gated_public_api_without_dom_type_leaks` 锁定 `ui/Cargo.toml` 依赖组合、`ui/src/lib.rs` 稳定导出、`ui/src/css.rs` 样式聚合门控，以及组件公共 API 不泄露 `web-sys`/`wasm_bindgen` 细节。
  - `logic.rs` 负责 props 归一与状态派生；`view.rs` 负责结构渲染与 headless 语义挂载；`styles.rs` 负责 token-first 静态样式；`motion.rs` 负责动效 attach。
  - `table` 当前无组件级动效状态机，`motion.rs` 按组件职责记为 N/A（已由语义回归锁定无 `ui-motion` 依赖与无 `src/motion.rs`）。
  ...
  - `semantics` 回归已位于 `components/table/tests/semantics.rs`，并显式防回退到旧路径 `test/semantics.rs` / `test/table_semantics.rs`。

### 2. API 设计与状态内核（Logic/Kernel）
- [x] API 命名契约统一：公共 props/回调严格使用 `is_*`、`on_*`、`default_*` 前缀；同语义在全库同名，禁止别名漂移。
  - 组件侧依据：`Table` 布尔公开 props 已统一为 `is_striped` / `is_sticky_header`；docs playground 与源码片段同步使用新命名，语义回归已锁定。
  - 布尔状态统一 `is_*`（如 `is_open`/`is_disabled`），事件统一 `on_*`，默认值统一 `default_*`。
  - 同一语义 across 组件必须同名（如都用 `on_open_change`，禁止同义别名并存）。
  - 兼容/迁移策略：本次采用显式重命名（`striped -> is_striped`、`sticky_header -> is_sticky_header`），迁移路径为一对一机械替换；语义测试禁止旧命名回流。
  - N/A 说明：`table` 当前无受控状态轴与事件回调，不暴露 `on_* / default_*`；该约束在新增状态轴时强制执行。
- [x] 受控/非受控必须成对：每个可控状态轴都提供 `value + on_value_change + default_value`（如 `open/on_open_change/default_open`）；缺一项即不通过。
  - N/A（当前 `table` 不存在组件内部可变状态轴；`is_striped` / `is_sticky_header` 为一次性输入配置，组件不会在内部写回该值）。
  - 语义回归：`components/table/tests/semantics.rs::table_has_no_controlled_uncontrolled_state_axes_yet` 已锁定当前“无半受控 API”约束。
  - 演进约束：若后续引入可变状态轴（如 selection/sort/expand），必须同 PR 一次性补齐 `value + on_value_change + default_value` 与受控/非受控切换测试。
...
- [x] 默认值单一来源：默认值与优先级只在 `logic.rs` 归一化；`view.rs` 禁止二次兜底或隐式改写。
  - 组件侧依据：`view.rs` 已移除单元格渲染路径中的 `unwrap_or*` 兜底，行渲染改为使用已归一化列元信息 `zip` 配对输出。
  - 回归锁定：`components/table/tests/semantics.rs::table_default_resolution_stays_out_of_view_layer` 明确禁止 `view.rs` 出现默认值兜底，并断言默认值实现集中在 `ui-state-primitives/src/table.rs`。
