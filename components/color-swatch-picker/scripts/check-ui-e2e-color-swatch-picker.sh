...
cargo test -p ui --test color_swatch_picker_semantics --no-default-features --features component-color_swatch_picker,inject-css color_swatch_picker_check2_documents_e2e_selector_and_stable_wait_rules

echo "[e2e-color-swatch-picker] contract: semantic selectors + wasm-stable waits"
cargo test -p ui --test color_swatch_picker_semantics --no-default-features --features component-color_swatch_picker,inject-css color_swatch_picker_e2e_selector_contract_uses_semantic_markers_and_stable_waits

echo "[e2e-color-swatch-picker] contract: interaction path ready/settled semantic breakpoints"
cargo test -p ui --test color_swatch_picker_semantics --no-default-features --features component-color_swatch_picker,inject-css color_swatch_picker_e2e_animation_path_covers_ready_and_settled_semantic_breakpoints
