...
### 1. 大骨架（架构边界与层职责）
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（`ColorLoupe` 状态原语已下沉至 `ui-state-primitives/src/color_loupe.rs`，并在 `ui-state-primitives/src/lib.rs` 导出；组件侧 `components/color-loupe/src/logic.rs` 仅桥接消费，回归覆盖 `components/color-loupe/test/color_loupe_semantics.rs`）
...
- [x] `ui-theme` 定义：唯一设计 token 与主题上下文层（system/color/scale + Light/Dark/OLED），负责 token 分类、主题映射与 CSS 变量生成。（`components/color-loupe/src/styles.rs` 仅消费 `var(--ui-*)` 与 `color-mix(...)`，未引入组件私有 token 体系）
...
- [x] 测试验证“语义契约”而不只验证视觉快照。（覆盖：`components/color-loupe/test/color_loupe_semantics.rs`，断言 role/aria/data-state/slot/class 契约与 docs 状态矩阵）
