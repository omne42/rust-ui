...
- [x] `ui` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（`mod.rs / logic.rs / styles.rs / view.rs` 分工清晰；`view.rs` 只做结构与语义挂载，`logic.rs` 只做归一和 class 组装）
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（本次执行：`cargo test -p ui --test color_loupe_semantics --no-default-features --features component-color_loupe,inject-css`、`cargo test -p ui-state-primitives color_loupe --lib`、`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-color_loupe,inject-css`、`cargo tree -e features -p ui --no-default-features --features component-color_loupe,inject-css`）
