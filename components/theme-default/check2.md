...
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（N/A：`theme-default` 为主题导出 crate，仅导出 `Theme` 与 `default_theme()`，无状态轴与状态机实现）
...
- [x] `ui-headless` 定义：交互与 A11y 原语层（press/focus/hover/roving/listbox/menu/tooltip 等），把输入设备事件与状态语义标准化为可复用契约；输出必须是类型化 `attrs + handlers + state`。不做样式、不写组件 CSS、不做组件级动效编排。（N/A：`theme-default` 仅导出主题对象与 `default_theme()`，无输入事件、焦点模型或 ARIA 语义契约）
...
- [x] `ui-motion` 定义：动效能力与契约执行层（spring、keyframes、WAAPI/RAF backend），只负责时间函数、插值与运行时驱动，不承载组件业务语义与状态决策。（N/A：`theme-default` 仅导出主题对象与 `default_theme()`，无组件状态到动效 contract 的映射，也无 motion attach/runtime 驱动）
...
- [x] `ui-theme` 定义：唯一设计 token 与主题上下文层（system/color/scale + Light/Dark/OLED），负责 token 分类、主题映射与 CSS 变量生成。（N/A：`theme-default` 仅复用并导出 `ui-theme::Theme` 与 `default_theme()`，不在组件侧实现 token 分类/主题映射/CSS 变量生成）
...
- [x] `ui` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（N/A：`theme-default` 是主题兼容导出 crate，不承担 `crates/ui` 的 Leptos 组件装配职责；该组件语义回归由 `components/theme-default/test/semantics.rs` 覆盖）
...
- [x] API 命名契约统一：公共 props/回调严格使用 `is_*`、`on_*`、`default_*` 前缀；同语义在全库同名，禁止别名漂移。（N/A：`theme-default` 无组件 props/callback API，公开接口仅 `Theme` 与 `default_theme()`）
...
- [x] 受控/非受控必须成对：每个可控状态轴都提供 `value + on_value_change + default_value`（如 `open/on_open_change/default_open`）；缺一项即不通过。（N/A：`theme-default` 不暴露可控状态轴，公开接口仅 `Theme` 与 `default_theme()`，不存在受控/非受控切换语义）
  - 受控模式：外部值是单一事实来源，内部不得偷偷写回本地状态。
  - 非受控模式：仅由默认值初始化一次，后续状态由内部原语管理。
  - 受控/非受控切换语义需稳定可测，避免“半受控”隐式行为。
- [x] 默认值单一来源：默认值与优先级只在 `logic.rs` 归一化；`view.rs` 禁止二次兜底或隐式改写。（N/A：`theme-default` 不含 `logic.rs/view.rs`，默认值仅在 `default_theme()` 单点定义为 `Theme::light()`）
  - 默认值优先级必须可读且可测试（显式规则而非分散 `unwrap_or`）。
  - `view.rs` 不允许再做默认值分支；仅消费 `logic.rs` 的归一化输出。
  - 一旦发现多处默认值来源，直接判不通过并回收至 `logic.rs`。
- [x] 状态归一化集中：状态输入先类型化，再在 `logic.rs` 统一派生；禁止在 `view.rs`、事件回调、样式分支中分散拼状态机。（N/A：`theme-default` 不含 `logic.rs/view.rs` 与状态机输入通道，公开接口仅导出 `Theme` 与 `default_theme()`）
  - 输入边界统一进入 `logic.rs`，输出统一为可渲染语义状态与来源标记。
  - 事件处理器只触发状态变更，不重建状态机规则。
  - 样式层只消费状态标记，不承担状态判定职责。
- [x] 离散状态必须类型约束：`variant/size/mode/status` 等离散输入使用 `enum`；禁止用多个 `Option<bool>`/字符串自由组合表达互斥状态。（N/A：`theme-default` 不暴露离散输入状态轴，公开接口仅 `Theme` 与 `default_theme()`）
  - 互斥状态优先用 `enum` 建模，利用编译器封住无效组合。
  - 字符串输入若需兼容外部配置，必须先映射到类型化枚举再进入逻辑层。
  - 布尔爆炸（多个 bool 表达一个状态机）应在设计评审阶段直接拦截。
- [x] 状态原语来源正确：组件层只消费 `status-primitives`（当前 `ui-state-primitives`）能力，不直接绑定业务 store；应用级全局状态必须经桥接层适配后再接入组件。（N/A：`theme-default` 不包含组件状态原语消费与业务 store 接入路径，公开接口仅 `Theme` 与 `default_theme()`）
  - 组件中出现可复用状态机实现（受控/非受控、展开规则、选择归一）即判应下沉。
  - 组件与业务全局状态之间必须有适配边界，禁止组件直接依赖业务 store 类型。
  - `logic.rs` 仅做装配与映射，不重新实现状态原语。
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，无远程请求与异步状态）
...
- [x] API 易用性验收标准（DX Paradox）：把复杂性留在内部，把简单留给用户。（N/A：`theme-default` 无交互 props/state 接线面，默认调用路径仅 `default_theme()`；docs 侧通过 `UiRoot` 页面覆盖最小使用路径）
...
- [x] 组合型组件主 API 必须“显示优于约定”：优先使用显式组合 `<Parent><Item ... /></Parent>`。（N/A：`theme-default` 非组合型组件，无 `Parent/Item` 结构化子项 API，公开接口仅 `Theme` 与 `default_theme()`）
...
- [x] 宏观/微观双状态机（Macro/Micro Duality）：拖拽等高频交互在 `Dragging` 期间由 `view/motion` 本地循环执行；禁止每帧穿越回 `logic.rs`，必须在结束时通过 `Action::DragEnd` 回流收敛。（N/A：`theme-default` 不包含拖拽/高频交互状态机与 `logic.rs/view.rs/motion.rs` 执行链，公开接口仅 `Theme` 与 `default_theme()`）
- [x] 几何两段式渲染（Two-Pass Rendering）：`Tooltip/Popover/Menu` 等依赖 DOM 测量的组件必须走 `Intent -> Measure(view) -> Rectification(logic)`，并具备幂等收敛保护防死循环。（N/A：`theme-default` 不包含 `Tooltip/Popover/Menu` 这类依赖 DOM 测量的交互组件，也无 `Intent -> Measure -> Rectification` 渲染链）
- [x] 集合注册协议（Registration Protocol）：`Accordion/Tabs/Menu` 动态子项必须通过 `RegistrationContext` 上报 `Register/Unregister`，逻辑层维护 `items_order`，禁止依赖 `HashSet` 迭代顺序做导航。（N/A：`theme-default` 非 `Accordion/Tabs/Menu` 类集合交互组件，不包含 `RegistrationContext` 或动态子项导航逻辑）
- [x] 插槽投影策略（Slot Projection）：容器组件明确 `Lazy/KeepAlive/Eager`；`KeepAlive` 隐藏时必须通过生命周期通知（如 `NotifyHidden`）暂停轮询/动画等高耗能副作用。（N/A：`theme-default` 非容器型交互组件，不包含 slot 投影生命周期与副作用调度链路）
- [x] 环境订阅流（Env Streams）：`Resize/Theme/Intersection` 等环境变化在 `view.rs` 采样、防抖后转化为高层语义 `Action`（如 `BreakpointChanged`）推送到 `logic`；禁止原始事件洪泛。（N/A：`theme-default` 不含 `view.rs/logic.rs` 环境事件采样链路，也无 `Resize/Theme/Intersection` 订阅处理）
- [x] 事件光锥（Event Light Cone）：`Table/Grid` 等大型集合批量操作必须走 `Context Bus + Selector` 与状态压缩表达（如 `SelectionState::All`），禁止 O(N) 级向下 prop drilling。（N/A：`theme-default` 非 `Table/Grid` 大型集合组件，不包含批量选择、Context Bus 或 Selector 驱动链路）
- [x] 统一因果总线（Causality Bus）：复杂派生总线操作必须支持透传 `TraceId`，确保“用户触发 -> 派生命令 -> 总线广播 -> 订阅者”因果链不断裂。（N/A：`theme-default` 不包含交互总线与派生命令链路，公开接口仅 `Theme` 与 `default_theme()`，无 `TraceId` 透传场景）
- [x] 存在 A11y 实现、国际化与本地化实现（至少具备接入点，不硬编码用户可见文本）。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含交互元素渲染与用户可见文案输出面）
...
- [x] 状态可观测、可检索、可验证：使用稳定 `data-*` 与 `aria-*` 标记表达状态和来源。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不渲染 DOM 节点与交互状态轴，因此不存在 `data-*`/`aria-*` 状态来源标记面）
...
- [x] 样式依赖显式状态（`data-*`/class），而非脆弱 DOM 结构猜测。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，无 `styles.rs`、无 DOM 渲染与运行时样式分支，不存在基于结构猜测状态的选择器风险）
...
- [x] 测试验证“语义契约”而不只验证视觉快照。（N/A：`theme-default` 仅导出主题对象，不渲染交互 DOM 与 A11y 语义路径；不存在受控/非受控、键盘/指针、SSR/wasm 交互矩阵，且无视觉快照替代语义断言的问题）
...
- [x] 组件文件职责正确：`mod.rs`（导出边界）、`logic.rs`（归一/派生/来源标记）、`styles.rs`（静态 token-first CSS）、`view.rs`（Leptos 结构 + headless 挂载）、`motion.rs`（动效契约 + attach）。（N/A：`theme-default` 是主题导出 crate，当前仅有 `mod.rs` 暴露 `Theme` 与 `default_theme()`；不存在交互组件实现所需的 `logic.rs/styles.rs/view.rs/motion.rs` 职责分层面）
...
- [x] `spec.rs` 只用于少数复杂组件（如 button），避免泛滥。（N/A：`theme-default` 为轻量主题导出 crate，仅暴露 `Theme` 与 `default_theme()`，无外部 Schema 契约与复杂配置固化需求，不应引入 `spec.rs`）
  - 仅当组件存在稳定外部规范/Schema 契约或复杂配置固化需求时才引入 `spec.rs`。
  - 简单组件不得为了“形式统一”新增 `spec.rs`；说明文档应留在 `check2.md`/组件文档。
  - 新增 `spec.rs` 必须同步给出契约测试与版本演进说明。
- [x] 组件层遵循 token-first 静态样式契约：样式通过 `styles.rs` 聚合注入；运行时仅传必要 CSS 变量；不把 Utility-First/CSS-in-Rust 当组件库默认范式。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含组件 `styles.rs`、`UiRoot` 样式注入链路或 Utility/CSS-in-Rust 组件实现面）
  - 样式规则统一落在 `styles.rs`，由 `crates/ui/src/css.rs` 聚合并通过 `UiRoot` 注入。
  - 颜色/间距/圆角/阴影等视觉值必须来自 `var(--ui-*)`，禁止组件私有 token 体系。
  - Utility-First 仅作为 `apps/*` 应用层布局手段，不得反向污染组件库契约。
  - CSS-in-Rust 仅在有明确类型安全与构建成本净收益时作为例外采用。
- [x] 默认主题美学质量达标（Visual Desire）：以 HeroUI 现代审美为学习对标，默认主题不仅“可用”，还必须“第一眼可信”。（N/A：`theme-default` 仅提供 `default_theme()` 入口并返回 `Theme::light()`；视觉语言、基线页面与截图回归由 `crates/ui-theme` 与 `apps/docs-app` 侧承担）
...
- [x] Tree Shaking 是一等能力：package 模式支持组件级 feature；source 模式天然裁剪；样式层同步裁剪，禁止无条件聚合全部 CSS，禁止破坏 DCE/LTO 的全量中央注册表。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不承担 `ui` 包级组件 feature 编排、`css.rs` 聚合或全量注册表职责；相关 tree-shaking 验证应在 `crates/ui` 与应用集成层执行）
...
- [x] 类型系统 + 语义标记共同提供机器可读状态；关键输入空间受类型约束。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含组件级离散输入协议、`logic.rs` 状态归一化流程或 DOM 语义标记输出面）
...
### 4. DOM/环境边界治理
- [x] 焦点全局栈（Focus Stack & GC）：层叠 `Overlay` 禁止私存 `NodeRef` 作为恢复目标；必须依赖全局 Focus Manager（如 `FallbackTo/Selector`）防止焦点坠落到 `document.body`。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不实现 Overlay/Focus 管理，也无 `NodeRef` 焦点恢复路径）
- [x] 受控外交特区（Escape Hatches）：集成 ECharts/Map 等命令式第三方库时必须处于 `Foreign Zone`（`YieldControl/CleanupForeign`）；第三方实例不得暴露为组件公共 API 或反向污染状态机。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不集成命令式第三方运行时实例，也不存在 Foreign Zone 逃逸与状态机污染路径）
- [x] SSR 时空断裂治理（Hydration Discontinuity）：逻辑初始化禁止依赖 `now()` 或原生随机 UUID；必须通过 `IdProvider` 注入确定性种子，确保 SSR/Hydration 间 ID 稳定。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不含 SSR/Hydration 交互逻辑、随机 ID 生成或 `IdProvider` 注入链路）
- [x] SSR 与跨平台检查：覆盖 web/ssr/wasm 分支，不破坏 non-wasm 编译路径。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含 `web-sys`/浏览器对象访问与平台分支逻辑；跨平台编译检查由上层 `ui`/app 构建矩阵承担）
...
- [x] `ui-headless` web/ssr feature 互斥受 `compile_error!` 保护（`crates/ui-headless/src/lib.rs`）。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不接入 `ui-headless` 事件/A11y 语义能力，不会改变其 web/ssr feature 互斥编译契约）
  - 组件依赖 `ui-headless` 能力时，不得破坏其 web/ssr 互斥约束。
  - 组件若新增 headless 功能接入，需验证两条 feature 路径都可编译。
  - 发现“同时启用 web+ssr 仍可过编译”视为契约回归。
- [x] `ui-motion` 非 wasm 提供 no-op/stub（`crates/ui-motion/src/lib.rs`），保证 SSR/tooling 可编译。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含 `motion.rs` 或 `ui-motion` 运行时调用路径，不会引入 non-wasm 动效降级与 panic 风险）
  - `motion.rs` 调用必须可在 non-wasm 下安全降级，不触发 panic。
  - 组件不得假设动画句柄一定存在；no-op 分支行为需可预测。
  - toolchain 场景（测试/文档/静态分析）不得因 motion 依赖阻塞编译。
- [x] 组件实现覆盖 `reduced-motion` / SSR / wasm 分支。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含组件渲染、动效执行或 wasm 特化交互路径，因此不存在 `reduced-motion`/SSR/wasm 分支实现面）
  - `reduced-motion` 下动画应跳过或降级为最小必要反馈。
  - SSR 输出必须与客户端 hydration 兼容，避免首帧语义错位。
  - wasm 分支允许增强交互，但语义契约不得与 SSR 分支分裂。
- [x] 性能治理：关键路径有预算（首次渲染/更新耗时/内存），回归可检测、可归因、可阻断。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含组件渲染/更新/动效执行路径，不存在 `render_count` 与交互关键路径预算面）
...
- [x] `view!` 宏复杂度受控：单个 `view!` 块不得承载超长深嵌套结构；复杂布局按语义分块，避免一次性宏展开导致编译与 wasm 体积劣化。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不含 `view.rs` 与 `view!` 宏渲染实现，不存在宏深嵌套展开风险）
  - 复杂结构按语义子块拆分（header/body/item 等），避免巨型单块 `view!`。
  - `view.rs` 中若出现多层嵌套重复片段，应优先提取局部渲染函数。
  - 编译时间/产物体积异常增长时，优先排查宏展开体量。
- [x] 函数式拆分优先：不涉及复杂状态与生命周期管理的 UI 片段，优先拆为普通 Rust 函数（返回 `impl IntoView`/`View`），而不是新增 `#[component]`。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含 `view.rs`/`IntoView` 渲染片段与 `#[component]` 抽象拆分路径）
  - 纯静态或轻逻辑片段优先函数化；仅在需要独立 props 语义时升级为组件。
  - 禁止把所有局部片段都升格为 `#[component]` 导致抽象噪音。
  - 拆分后语义标记与测试定位仍需稳定。
- [x] 静态片段常量化：复杂 SVG、页脚、长说明文本等纯静态内容优先常量化/模板化，减少重复 `view!` 渲染指令生成。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含 `view!` 渲染片段、SVG 模板或静态文案拼装路径）
  - 可判定为纯静态的片段应避免重复动态构造。
  - 常量化后仍需维持可访问语义（title/aria-label/role 等）。
  - 静态资源变更路径要清晰，避免散落在多个 `view!` 片段中。
- [x] `inner_html` 使用约束：仅允许注入受信任静态常量，禁止拼接用户输入；使用处必须补充语义与安全回归测试。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含 DOM 渲染与 `inner_html` 注入路径，不存在用户输入拼接面）
  - 仅允许编译期常量或明确白名单内容进入 `inner_html`。
  - 严禁直接或间接注入用户输入、远端返回或未清洗模板字符串。
  - 使用 `inner_html` 的节点必须补语义测试与安全回归说明。
- [x] WASM 调试要求：关键状态可追踪（来源/时间/前后值），关键交互可回放，开发模式有可视化入口，调试能力通过 feature 隔离不污染产物。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不含 WASM 交互状态机、事件回放链路或调试可视化入口，亦无调试 feature 注入面）
  - 开发模式下至少能追踪关键状态变更来源与前后值。
  - 关键交互链路应支持最小可复现记录（事件顺序/状态转移）。
  - 调试开关默认不进入生产包体与公共 API。
- [x] DX 要求：样式热重载优先无需重编 wasm；组件热开发尽量保持上下文；提供可选状态保留；有 Workbench 隔离画布。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含组件热开发流程、状态保留机制或 workbench/demo 交互画布能力）
  - 常见样式调整应走快速反馈路径，不依赖完整 wasm 重编译。
  - 组件调试应尽量保持当前交互上下文，降低重复操作成本。
  - 复杂交互组件应有隔离演练入口（workbench/story/demo 之一）。
- [x] 工程能力统一：`serde` 负责 spec 序列化/版本迁移/错误结构化；`tracing` 统一 span/event 语义；async 不绑定单一运行时（tokio/async-std），runtime 细节不泄露到上层 API。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不处理 spec/config 序列化、tracing 流程埋点或异步 runtime 抽象边界）
  - 若组件涉及 spec/config 输入，序列化与错误输出应走统一结构化路径。
  - 关键流程埋点语义应与全库 tracing 约定一致，避免组件各说各话。
  - 异步边界不得把具体 runtime 类型暴露到组件公共接口。

### 5. 样式与动效（Theme & Motion）
- [x] 样式孤岛防御（Defensive Variables）：`styles.rs` 使用双层回退链 `var(--ui-*, var(--ui-fallback-*))`；禁止组件内硬编码 Hex 或裸尺寸终值，Fallback 终值由 `ui-theme` 统一输出（SSOT）。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含组件 `styles.rs` 实现与 CSS 变量回退链定义路径）
- [x] 级联层覆盖（`@layer ui`）：组件 CSS 默认聚合进 `@layer ui`；运行时数值调整仅通过 CSS Custom Properties（如 `style:--x=...`），禁止普通内联样式（如 `style=\"top: 10px\"`）。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含组件 CSS 聚合、`@layer ui` 注入或运行时样式写入路径）
- [x] Motion 合同化：`stiffness`/`damping` 等参数在 `motion.rs` 内置为组件 Contract，并通过 `attach_motion` 挂载；必须尊重 `prefers-reduced-motion` 且在 non-wasm/SSR 安全降级（no-op）。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不包含组件 `motion.rs`、`attach_motion` 挂载链路或运行时动效参数契约）
- [x] `ui` 固定入口文件落点正确。（N/A：`theme-default` 为组件分片中的主题导出 crate，仅暴露 `Theme` 与 `default_theme()`，不维护 `crates/ui` / `crates/ui-visual-primitive` 的入口文件布局与原语落点）
...
- [x] 组件目录标准文件落点正确。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不属于交互组件实现分层，当前仅需稳定 `mod.rs` 导出面，不要求 `logic.rs/styles.rs/view.rs/motion.rs/spec.rs` 文件族）
...
- [x] 文件落点纪律：组件目录严格由 `mod.rs`（导出）、`logic.rs`（归一派生）、`styles.rs`（Token 样式）、`view.rs`（渲染）、`motion.rs`（动效）组成；复杂组件可选 `spec.rs`；禁止 `render.rs`。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不属于交互组件实现分层，不存在 `logic.rs/styles.rs/view.rs/motion.rs/spec.rs` 与 `render.rs` 漂移面）
- [x] Hyper-Structure Builder（`spec.rs`）：复杂组件必须提供 AI 友好的 `*Spec::new()...render()` 建造者 API。（N/A：`theme-default` 仅导出 `Theme` 与 `default_theme()`，不属于复杂交互组件，不存在 `spec.rs` builder 与 `render()` 组装入口需求）
- [x] 上下文压缩协议（Manifest + RBI）：新增/大改组件必须同步维护组件目录下 `Component.toml`（能力清单）和 `.rbi`（接口签名投影），避免 AI 检索工具箱过时。（N/A：`theme-default` 为极简主题导出 crate，仅暴露 `Theme` 与 `default_theme()`，本轮未引入新增/大改组件接口面，不涉及 `Component.toml` 与 `.rbi` 维护）
- [x] 语义标记统一升级为 Agent Contract（Schema 化），让 Agent 不依赖 DOM 猜测理解组件状态与意图。（N/A：`theme-default` 为主题数据导出 crate，不渲染 DOM、无交互状态轴与动作语义输出面，当前不存在 `data-*`/`data-ui-schema` 合同化挂载点）
