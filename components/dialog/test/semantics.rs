...
fn dialog_docs_page_covers_primary_playgrounds() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "dialog docs page should include `{needle}`."
        );
    }
}
...
fn dialog_docs_playgrounds_lock_state_matrix_contract_values() {
    ...
    for needle in [
        "data-slot=\"dialog-state-matrix\"",
        "data-slot=\"dialog-controlled-uncontrolled\"",
        "data-slot=\"dialog-streaming-contract\"",
        ...
        assert!(
            docs_source.contains(needle),
            "dialog docs state-matrix contract should include `{needle}`."
        );
    }
}
...
fn dialog_logic_and_view_follow_layering_contract() {
    ...
    assert!(
        logic_source.contains("use ui_state_primitives::dialog as dialog_state;"),
        "Dialog logic should consume `ui-state-primitives` instead of reimplementing state primitives."
    );
    ...
    for needle in [
        ...
        assert!(
            ...
            "Dialog view should mount headless/motion contracts via `{needle}`."
        );
    }
}

#[test]
fn dialog_public_api_prefers_prefixed_names_with_alias_migration() {
    let view_source = load_source("src/view.rs");
    let logic_source = load_source("src/logic.rs");
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "Dialog API naming migration should keep `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "Dialog logic should consume primitive contracts via `{needle}`."
        );
    }
...
fn dialog_public_surface_avoids_web_sys_types() {
    ...
    assert!(
        !lib_source.contains("web_sys") && !mod_source.contains("web_sys"),
        "Dialog public module boundary should not leak web_sys platform details."
    );
    assert!(
        mod_source.contains("pub use view::Dialog;")
            && mod_source.contains("pub use motion::DialogMotion;"),
        "Dialog should export stable component API from mod boundary."
    );
}

#[test]
fn dialog_public_api_does_not_require_internal_state_object() {
    let view_source = load_source("src/view.rs");
    ...
    for forbidden in [
        ...
        "#[prop(into)] state:",
        "state: DialogState",
        "state: DialogOpenState",
    ] {
        assert!(
            !view_source.contains(forbidden),
            "Dialog should not force internal state object prop `{forbidden}`."
        );
    }
}
...
fn dialog_non_composite_api_keeps_explicit_single_tree_contract() {
    ...
    for needle in ["pub fn Dialog(", "title: String", "children: ChildrenFn"] {
        assert!(
            view_source.contains(needle),
            "Dialog should keep explicit single-tree API evidence `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "Dialog non-composite API should not expose parallel-array/config sugar `{forbidden}`."
        );
    }
}
...
fn dialog_has_no_dragging_macro_micro_state_machine_contracts() {
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "Dialog does not support dragging and should not expose macro/micro drag protocol `{forbidden}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "Dialog should keep open/close + overlay motion delegation contract `{needle}`."
        );
    }
}
...
fn dialog_has_no_collection_registration_protocol_contracts() {
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "Dialog should not expose collection registration protocol contract `{forbidden}`."
        );
    }
    ...
    for needle in ["pub fn Dialog(", "title: String", "children: ChildrenFn"] {
        assert!(
            view_source.contains(needle),
            "Dialog should stay as explicit single-tree container contract `{needle}`."
        );
    }
}
...
fn dialog_has_no_slot_projection_strategy_contracts() {
    ...
    for needle in ["pub fn Dialog(", "title: String", "children: ChildrenFn"] {
        assert!(
            view_source.contains(needle),
            "Dialog should keep explicit single-panel container contract `{needle}`."
        );
    }
}
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden)
                && !logic_source.contains(forbidden)
                && !motion_source.contains(forbidden),
            "Dialog should not expose environment stream subscription contract `{forbidden}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden)
                && !logic_source.contains(forbidden)
                && !readme_source.contains(forbidden),
            "Dialog should not expose event-light-cone bulk collection contract `{forbidden}`."
        );
    }
...
fn dialog_has_no_foreign_zone_escape_hatch_contracts() {
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "Dialog should keep declarative local contract `{needle}`."
        );
    }
}
...
fn dialog_motion_contract_is_component_scoped_reduced_motion_aware_and_non_wasm_safe() {
    let motion_source = load_source("src/motion.rs");
    let motion_unit_test_source = load_source("test/motion.rs");
    let overlay_motion_source = load_source("../overlay/src/motion.rs");
    ...
    for needle in [
        ...
        assert!(
            ...
            "dialog component-local motion regression should include `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "dialog view should sanitize and forward motion contract via overlay attach path `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "dialog checklist should keep motion-contract marker `{needle}`.",
        );
    }
}
...
fn dialog_ui_components_fixed_entry_files_follow_layered_boundaries() {
    ...
    for needle in [
        ...
        assert!(
            checklist_source.contains(needle),
            "dialog checklist should keep fixed-entry-files marker `{needle}`.",
        );
    }
}
...
fn dialog_tree_shaking_contract_registers_component_feature_and_gates_lib_css_aggregation() {
    ...
    assert!(
        ...
        "dialog CSS aggregation should be directly gated by `component-dialog` in css.rs.",
    );
}
...
fn dialog_tree_shaking_script_enforces_component_minimal_feature_tree_and_budget() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "tree-shaking gate script should enforce `{needle}`.",
        );
    }
}
...
fn dialog_check2_documents_tree_shaking_feature_pruning_requirements() {
    ...
    for needle in [
        ...
        assert!(
            source.contains(needle),
            "dialog check2 tree-shaking section should keep marker `{needle}`.",
        );
    }
}
...
fn dialog_component_directory_standard_files_follow_contract_and_na_paths() {
    ...
    for needle in [
        ...
        assert!(
            checklist_source.contains(needle),
            "dialog checklist should keep component-directory marker `{needle}`.",
        );
    }
}
...
fn dialog_file_placement_discipline_is_strict_for_component_scope() {
    ...
    for needle in [
        ...
        assert!(
            checklist_source.contains(needle),
            "dialog checklist should keep file-placement marker `{needle}`.",
        );
    }
}
...
fn dialog_hyper_structure_builder_spec_is_not_applicable_for_simple_component() {
    ...
    for forbidden in [
        ...
        assert!(
            !module_source.contains(forbidden) && !readme_source.contains(forbidden),
            "Dialog should not expose Hyper-Structure builder marker `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            checklist_source.contains(needle),
            "dialog checklist should keep Hyper-Structure builder N/A marker `{needle}`.",
        );
    }
}
...
fn dialog_context_compression_manifest_and_rbi_projection_are_present_and_current() {
    ...
    assert!(
        src_dir.join("dialog.rbi").exists(),
        "dialog context-compression contract requires `src/dialog.rbi`."
    );
    ...
    for needle in [
        ...
        "name = \"data-state\"",
        "name = \"data-open-mode\"",
        "name = \"data-open-source\"",
        "name = \"data-open-change-source\"",
        "name = \"data-size\"",
    ...
    for needle in [
        ...
        assert!(
            checklist_source.contains(needle),
            "dialog checklist should keep context-compression marker `{needle}`.",
        );
    }
}
...
fn dialog_check2_documents_streaming_definition_is_llm_output_only_with_two_modes() {
    ...
    for needle in [
        ...
        assert!(
            checklist_source.contains(needle),
            "dialog checklist should pin streaming two-mode definition marker `{needle}`.",
        );
    }
}
...
fn dialog_check2_documents_snapshot_as_default_baseline_capability() {
    ...
    for needle in [
        ...
        assert!(
            checklist_source.contains(needle),
            "dialog checklist should pin snapshot baseline marker `{needle}`.",
        );
    }
}
...
fn dialog_check2_documents_streaming_required_optional_classification_rules() {
    ...
    for needle in [
        ...
        assert!(
            checklist_source.contains(needle),
            "dialog checklist should keep streaming required/optional rule `{needle}`.",
        );
    }
}
...
fn dialog_semantics_and_performance_regression_cover_aria_data_focus_and_render_count_measurement()
{
    ...
    let aggregated_semantics = load_source("../../crates/ui-components/tests/dialog_semantics.rs");
...
fn dialog_check2_marks_e2e_repeatable_key_flow_contract_complete() {
    ...
    for needle in [
        ...
        "crates/ui-components/tests/dialog_semantics.rs::dialog_e2e_key_flow_is_repeatable_and_failure_points_are_semantic",
        "scripts/check-ui-components-e2e-dialog.sh",
        "Invalid cross-device link (os error 18)",
    ] {
        assert!(
...
fn dialog_engineering_contract_uses_serde_protocol_and_keeps_tracing_runtime_boundaries() {
    let protocol_source = load_source("src/protocol.rs");
    let protocol_test_source = load_source("test/protocol.rs");
    let mod_source = load_source("src/mod.rs");
    let logic_source = load_source("src/logic.rs");
    let view_source = load_source("src/view.rs");
    let styles_source = load_source("src/styles.rs");
