...
- [x] 本次发现并修复的阻断项：
  - `component-code_block` 最小特性无法单独编译，原因是缺少 `button` 与 `snippet` 依赖链；已修复 `crates/ui/Cargo.toml`：`component-code_block = ["component-button", "component-snippet"]`。
...
#### 10.1 门禁与验证命令（逐条执行）
- [x] `rustfmt --edition 2024 --check crates/ui/src/code_block/mod.rs crates/ui/src/code_block/logic.rs crates/ui/src/code_block/view.rs crates/ui/src/code_block/styles.rs crates/ui/src/code_block/motion.rs crates/ui/src/code_block/i18n.rs`
- [x] `CARGO_TARGET_DIR=target/code-block-check cargo check -p ui --no-default-features --features component-code_block,inject-css`
- [x] `CARGO_TARGET_DIR=target/code-block-check cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-code_block,inject-css`
- [x] `CARGO_TARGET_DIR=target/code-block-check cargo clippy -p ui --no-default-features --features component-code_block,inject-css --test code_block_semantics --no-deps -- -D warnings`
- [x] `CARGO_TARGET_DIR=target/code-block-check cargo test -p ui --no-default-features --features component-code_block,inject-css --test code_block_semantics`
- [x] `CARGO_TARGET_DIR=target/code-block-check cargo test -p ui --no-default-features --features component-code_block,inject-css --test code_block_semantics <single-test> -- --exact`（11 条语义测试逐条单独执行，全部通过）
- [x] `cargo tree -e features -i ui -p ui --no-default-features --features component-code_block,inject-css`
- [x] `cargo tree -e features -i ui -p web-demo`
