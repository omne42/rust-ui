...
- [x] Tree Shaking 是一等能力：package 模式支持组件级 feature；source 模式天然裁剪；样式层同步裁剪，禁止无条件聚合全部 CSS，禁止破坏 DCE/LTO 的全量中央注册表。（`component-list` 已显式声明依赖 `component-active_highlight` 与 `component-illustrated_message`，与 `view/motion` 引用路径一致。）
...
- [x] SSR 与跨平台检查：覆盖 web/ssr/wasm 分支，不破坏 non-wasm 编译路径。（组件代码未直接引用 `web-sys`；跨平台分支由 `ui-headless/ui-motion` 统一约束。）
...
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。（`List` 非 LLM 正文阅读面，归类为 `Streaming Optional`，以 `Snapshot` 渲染为基线，流式场景由上层编排 `fallback=snapshot`。）
...
- [x] E2E 选择器稳定：使用语义标记，WASM 场景有稳定等待策略。（组件页覆盖纳入 `e2e/tests/docs_app_components_coverage.spec.mjs`，采用 `data-slot` 选择器与 `body:not(:has(#boot))` 就绪等待。）
  - E2E 选择器优先 `data-*` 语义标记，禁止依赖脆弱 DOM 层级或文本定位。
  - WASM 场景必须使用稳定等待策略（语义状态就绪而非固定 sleep）。
  - 若组件涉及异步/动画，E2E 需显式覆盖 ready/settled 条件。
- [x] 关键流程纳入可重复回归集合（Playwright/Cypress）。（关键流程“进入 list 页面 -> playground 渲染 -> 语义节点可见”由 docs-app 组件覆盖回归持续执行。）
...
- [x] HeroUI 对标文档与组件文档同步：参数模型变更需同步 `docs/spec/heroui-parameter-design-strategy.md`（必要时补充 `docs/research/spectrum-heroui-style-interface-study.md`），并保证组件文档可访问。（本轮 `List` 未发生参数语义变更，按 N/A 处理；docs 入口与示例矩阵已同步可访问。）
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（组件作用域已完成 `list_module_semantics` 清单约束回归；仓库全量编译门禁当前受并行改动中的跨组件错误阻塞，非本组件引入。）
