...
1. 特性解耦修复（阻断项）：
   - 新增 `crates/ui/src/color/slider/motion.rs`，`ColorSliderMotion` 与 `attach_motion` 下沉到组件内，移除对 `crate::slider` 的隐式依赖。
   - `crates/ui/src/color/slider/logic.rs::sanitize_track_color` 改为 `ui_state_primitives::swatch::sanitize_color_value(...)`，移除对 `crate::color_swatch` 的 feature 依赖。
2. API/可访问性契约补齐：
   - `crates/ui/src/color/slider/view.rs` 新增 `is_disabled`（兼容 `disabled`）、`lang`、`dir` 接口。
...
7. 组件范围验证命令（通过）：
   - `CARGO_TARGET_DIR=target-color-slider-check cargo test -p ui --test color_slider_semantics --no-default-features --features component-color_slider,inject-css`
   - `CARGO_TARGET_DIR=target-color-slider-check cargo check -p ui --no-default-features --features component-color_slider,inject-css`
   - `CARGO_TARGET_DIR=target-color-slider-check cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-color_slider,inject-css`
   - `CARGO_TARGET_DIR=target-color-slider-check cargo clippy -p ui --no-default-features --features component-color_slider,inject-css -- -D warnings`
8. 并行协作阻断说明（不跨线修改）：
