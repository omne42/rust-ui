...
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。
  - 证据：`components/accordion/test/semantics.rs::status_primitives_remain_dom_and_style_free`、`accordion_reuses_state_primitives_instead_of_reimplementing_reusable_rules`。
...
- [x] `ui-headless` 定义：交互与 A11y 原语层（press/focus/hover/roving/listbox/menu/tooltip 等），把输入设备事件与状态语义标准化为可复用契约；输出必须是类型化 `attrs + handlers + state`。不做样式、不写组件 CSS、不做组件级动效编排。
  - 证据：`components/accordion/test/semantics.rs::accordion_uses_headless_hooks`、`accordion_mounts_headless_disclosure_locale_contract`、`ui_headless_remains_semantic_without_visual_or_motion_orchestration`。
...
- [x] `ui-motion` 定义：动效能力与契约执行层（spring、keyframes、WAAPI/RAF backend），只负责时间函数、插值与运行时驱动，不承载组件业务语义与状态决策。
  - 证据：`components/accordion/test/semantics.rs::accordion_motion_is_spring_driven`、`accordion_motion_respects_reduced_motion_preferences`、`ui_motion_non_wasm_stub_contract_is_explicit`。
...
- [x] `ui-theme` 定义：唯一设计 token 与主题上下文层（system/color/scale + Light/Dark/OLED），负责 token 分类、主题映射与 CSS 变量生成。
  - 证据：`components/accordion/test/semantics.rs::accordion_motion_defaults_come_from_ui_theme_tokens`、`accordion_token_first_css_is_injected_through_root_pipeline`。
...
- [x] `ui` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。
  - 证据：`components/accordion/test/semantics.rs::accordion_file_roles_follow_layer_contract`、`ui_components_entry_file_locations_follow_contract`、`accordion_directory_standard_files_follow_contract`。
...
- [x] API 命名契约统一：公共 props/回调严格使用 `is_*`、`on_*`、`default_*` 前缀；同语义在全库同名，禁止别名漂移。
  - 证据：`components/accordion/test/semantics.rs::accordion_supports_controlled_and_uncontrolled_open_state`、`accordion_docs_api_contract_aligns_with_logic_defaults`。
...
- [x] 受控/非受控必须成对：每个可控状态轴都提供 `value + on_value_change + default_value`（如 `open/on_open_change/default_open`）；缺一项即不通过。
  - 证据：`components/accordion/test/semantics.rs::accordion_supports_controlled_and_uncontrolled_open_state`。
...
- [x] 默认值单一来源：默认值与优先级只在 `logic.rs` 归一化；`view.rs` 禁止二次兜底或隐式改写。
  - 证据：`components/accordion/test/semantics.rs::accordion_uses_logic_state_model`。
...
- [x] 状态归一化集中：状态输入先类型化，再在 `logic.rs` 统一派生；禁止在 `view.rs`、事件回调、样式分支中分散拼状态机。
  - 证据：`components/accordion/test/semantics.rs::accordion_uses_logic_state_model`、`accordion_view_consumes_logic_outputs_without_hiding_state_machine_rules`。
...
- [x] 离散状态必须类型约束：`variant/size/mode/status` 等离散输入使用 `enum`；禁止用多个 `Option<bool>`/字符串自由组合表达互斥状态。
  - 证据：`components/accordion/test/semantics.rs::accordion_uses_typed_discrete_mode_from_state_primitives`。
...
- [x] 状态原语来源正确：组件层只消费 `status-primitives`（当前 `ui-state-primitives`）能力，不直接绑定业务 store；应用级全局状态必须经桥接层适配后再接入组件。
  - 证据：`components/accordion/test/semantics.rs::accordion_consumes_state_primitive_sources_only`。
...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。
  ...
  - 证据：`components/accordion/test/semantics.rs::accordion_has_no_async_loading_protocol_axis`。
...
- [x] API 易用性验收标准（DX Paradox）：把复杂性留在内部，把简单留给用户。
  - 证据：`components/accordion/test/semantics.rs::accordion_api_keeps_minimal_default_path_without_internal_state_objects`、`accordion_docs_hello_world_example_stays_under_five_lines`。
...
- [x] 组合型组件主 API 必须“显示优于约定”：优先使用显式组合 `<Parent><Item ... /></Parent>`。
  - 证据：`components/accordion/test/semantics.rs::accordion_primary_api_uses_explicit_item_composition_contract`。
...
- [x] 集合注册协议（Registration Protocol）：`Accordion/Tabs/Menu` 动态子项必须通过 `RegistrationContext` 上报 `Register/Unregister`，逻辑层维护 `items_order`，禁止依赖 `HashSet` 迭代顺序做导航。
  - 证据：`components/accordion/test/semantics.rs::accordion_collection_registration_protocol_is_context_driven_and_ordered_in_logic`。
- [x] 插槽投影策略（Slot Projection）：容器组件明确 `Lazy/KeepAlive/Eager`；`KeepAlive` 隐藏时必须通过生命周期通知（如 `NotifyHidden`）暂停轮询/动画等高耗能副作用。
  - 证据：`components/accordion/test/semantics.rs::accordion_slot_projection_contract_covers_lazy_keepalive_eager_and_notify_hidden`。
...
- [x] 存在 A11y 实现、国际化与本地化实现（至少具备接入点，不硬编码用户可见文本）。
  - 证据：`components/accordion/test/semantics.rs::accordion_panels_are_labeled_regions`、`accordion_mounts_headless_disclosure_locale_contract`、`accordion_localizable_copy_is_caller_owned`。
...
- [x] 状态可观测、可检索、可验证：使用稳定 `data-*` 与 `aria-*` 标记表达状态和来源。
  - 证据：`components/accordion/test/semantics.rs::accordion_emits_baseline_style_data_attributes`、`accordion_open_source_markers_use_closed_enums`、`accordion_agent_contract_is_schema_typed_and_mounted`。
...
- [x] 样式依赖显式状态（`data-*`/class），而非脆弱 DOM 结构猜测。
  - 证据：`components/accordion/test/semantics.rs::accordion_styles_depend_on_explicit_state_markers`。
...
- [x] 测试验证“语义契约”而不只验证视觉快照。
  - 证据：`components/accordion/test/semantics.rs`（语义契约主测试）与 `e2e/tests/docs_app_accordion.spec.mjs`（交互回归）。
...
- [x] 组件文件职责正确：`mod.rs`（导出边界）、`logic.rs`（归一/派生/来源标记）、`styles.rs`（静态 token-first CSS）、`view.rs`（Leptos 结构 + headless 挂载）、`motion.rs`（动效契约 + attach）。
  - 证据：`components/accordion/test/semantics.rs::accordion_file_roles_follow_layer_contract`。
...
- [x] `spec.rs` 只用于少数复杂组件（如 button），避免泛滥。
  - 证据：`components/accordion/test/semantics.rs::accordion_has_no_spec_rs_contract_file`。
...
- [x] 组件层遵循 token-first 静态样式契约：样式通过 `styles.rs` 聚合注入；运行时仅传必要 CSS 变量；不把 Utility-First/CSS-in-Rust 当组件库默认范式。
  - 证据：`components/accordion/test/semantics.rs::accordion_token_first_css_is_injected_through_root_pipeline`、`accordion_styles_depend_on_explicit_state_markers`。
...
- [x] 默认主题美学质量达标（Visual Desire）：以 HeroUI 现代审美为学习对标，默认主题不仅“可用”，还必须“第一眼可信”。
  - 证据：`components/accordion/test/semantics.rs::docs_app_exposes_default_theme_visual_baseline_page`、`docs_app_visual_baseline_has_screenshot_regression_contract`。
...
- [x] Tree Shaking 是一等能力：package 模式支持组件级 feature；source 模式天然裁剪；样式层同步裁剪，禁止无条件聚合全部 CSS，禁止破坏 DCE/LTO 的全量中央注册表。
  - 证据：`components/accordion/test/semantics.rs::accordion_tree_shaking_feature_wiring_is_component_scoped`。
...
- [x] 类型系统 + 语义标记共同提供机器可读状态；关键输入空间受类型约束。
  - 证据：`components/accordion/test/semantics.rs::accordion_uses_typed_discrete_mode_from_state_primitives`、`accordion_agent_contract_is_schema_typed_and_mounted`。
...
- [x] SSR 与跨平台检查：覆盖 web/ssr/wasm 分支，不破坏 non-wasm 编译路径。
  - 证据：`components/accordion/test/semantics.rs::accordion_explicitly_gates_browser_calls_for_non_wasm_paths`、`ui_motion_non_wasm_stub_contract_is_explicit`。
...
- [x] `ui-headless` web/ssr feature 互斥受 `compile_error!` 保护（`crates/ui-headless/src/lib.rs`）。
  - 证据：`components/accordion/test/semantics.rs::ui_headless_has_web_ssr_compile_error_mutex_guard`。
...
- [x] `ui-motion` 非 wasm 提供 no-op/stub（`crates/ui-motion/src/lib.rs`），保证 SSR/tooling 可编译。
  - 证据：`components/accordion/test/semantics.rs::ui_motion_non_wasm_stub_contract_is_explicit`。
...
- [x] 组件实现覆盖 `reduced-motion` / SSR / wasm 分支。
  - 证据：`components/accordion/test/semantics.rs::accordion_motion_respects_reduced_motion_preferences`、`accordion_explicitly_gates_browser_calls_for_non_wasm_paths`。
  - `reduced-motion` 下动画应跳过或降级为最小必要反馈。
  - SSR 输出必须与客户端 hydration 兼容，避免首帧语义错位。
  - wasm 分支允许增强交互，但语义契约不得与 SSR 分支分裂。
- [x] 性能治理：关键路径有预算（首次渲染/更新耗时/内存），回归可检测、可归因、可阻断。
  - 证据：`components/accordion/test/semantics.rs::docs_perf_probe_budgets_are_wired_for_component_pages`、`perf_render_count_follow_up_is_tracked_in_plan`。
...
- [x] `view!` 宏复杂度受控：单个 `view!` 块不得承载超长深嵌套结构；复杂布局按语义分块，避免一次性宏展开导致编译与 wasm 体积劣化。
  - 证据：`components/accordion/test/semantics.rs::accordion_view_macro_complexity_is_split_by_semantic_fragments`。
...
- [x] 函数式拆分优先：不涉及复杂状态与生命周期管理的 UI 片段，优先拆为普通 Rust 函数（返回 `impl IntoView`/`View`），而不是新增 `#[component]`。
  - 证据：`components/accordion/test/semantics.rs::accordion_view_macro_complexity_is_split_by_semantic_fragments`（局部渲染函数拆分与组件数量约束）。
...
- [x] 静态片段常量化：复杂 SVG、页脚、长说明文本等纯静态内容优先常量化/模板化，减少重复 `view!` 渲染指令生成。
  - 证据：`components/accordion/test/semantics.rs::accordion_static_fragments_are_constantized_for_stable_rendering`。
  - 可判定为纯静态的片段应避免重复动态构造。
...
- [x] `inner_html` 使用约束：仅允许注入受信任静态常量，禁止拼接用户输入；使用处必须补充语义与安全回归测试。
  - 证据：`components/accordion/test/semantics.rs::accordion_disallows_inner_html_in_runtime_markup_paths`。
  - 仅允许编译期常量或明确白名单内容进入 `inner_html`。
  - 严禁直接或间接注入用户输入、远端返回或未清洗模板字符串。
  - 使用 `inner_html` 的节点必须补语义测试与安全回归说明。
- [x] WASM 调试要求：关键状态可追踪（来源/时间/前后值），关键交互可回放，开发模式有可视化入口，调试能力通过 feature 隔离不污染产物。
  - 证据：`components/accordion/test/semantics.rs::accordion_wasm_debug_contract_is_feature_gated_and_dev_only`。
  - 开发模式下至少能追踪关键状态变更来源与前后值。
  - 关键交互链路应支持最小可复现记录（事件顺序/状态转移）。
  - 调试开关默认不进入生产包体与公共 API。
- [x] DX 要求：样式热重载优先无需重编 wasm；组件热开发尽量保持上下文；提供可选状态保留；有 Workbench 隔离画布。
  - 证据：`components/accordion/test/semantics.rs::accordion_docs_workbench_supports_live_style_tuning_and_optional_persist`。
  - 常见样式调整应走快速反馈路径，不依赖完整 wasm 重编译。
  - 组件调试应尽量保持当前交互上下文，降低重复操作成本。
  - 复杂交互组件应有隔离演练入口（workbench/story/demo 之一）。
- [x] 工程能力统一：`serde` 负责 spec 序列化/版本迁移/错误结构化；`tracing` 统一 span/event 语义；async 不绑定单一运行时（tokio/async-std），runtime 细节不泄露到上层 API。
  - 证据：`components/accordion/test/semantics.rs::accordion_engineering_contract_uses_tracing_and_avoids_runtime_leaks`；`components/accordion/src/protocol.rs` 使用 `serde` 结构化协议。
...
- [x] 样式孤岛防御（Defensive Variables）：`styles.rs` 使用双层回退链 `var(--ui-*, var(--ui-fallback-*))`；禁止组件内硬编码 Hex 或裸尺寸终值，Fallback 终值由 `ui-theme` 统一输出（SSOT）。
  - 证据：`components/accordion/test/semantics.rs::accordion_styles_define_motion_css_vars`。
- [x] 级联层覆盖（`@layer ui`）：组件 CSS 默认聚合进 `@layer ui`；运行时数值调整仅通过 CSS Custom Properties（如 `style:--x=...`），禁止普通内联样式（如 `style=\"top: 10px\"`）。
  - 证据：`components/accordion/test/semantics.rs::accordion_styles_depend_on_explicit_state_markers`（样式标记约束 + 组件 view 无普通业务内联样式）。
- [x] Motion 合同化：`stiffness`/`damping` 等参数在 `motion.rs` 内置为组件 Contract，并通过 `attach_motion` 挂载；必须尊重 `prefers-reduced-motion` 且在 non-wasm/SSR 安全降级（no-op）。
  - 证据：`components/accordion/test/semantics.rs::accordion_motion_is_spring_driven`、`accordion_motion_sanitizes_custom_contract_values`、`accordion_motion_respects_reduced_motion_preferences`。
- [x] `ui` 固定入口文件落点正确。
  - 证据：`components/accordion/test/semantics.rs::ui_components_entry_file_locations_follow_contract`。
...
- [x] 组件目录标准文件落点正确。
  - 证据：`components/accordion/test/semantics.rs::accordion_directory_standard_files_follow_contract`。
...
- [x] `Snapshot` 是所有组件的基础能力（默认必须支持）。
  ...
  - 即使组件不直接展示正文，也应能在接收上层完整配置后正常渲染。
    ...
    - 回归：`components/accordion/test/semantics.rs` 断言 `data-ui-stream-fallback=\"full-snapshot\"` 与 `data-ui-output-status`。
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。
  ...
  - 无论是否支持 `Streaming`，都要显式标识当前输出状态（草稿/已验证/可提交），并保持 `role`/`aria-*`/`data-*` 连续可读。
    - 证据：`components/accordion/src/view.rs` 挂载 `data-ui-output-status`；`components/accordion/src/README.md` 明确 `draft/verified/submittable` 输出集合；`components/accordion/test/semantics.rs` 覆盖对应语义字段断言。
...
- [x] 语义测试与性能回归：断言必须覆盖 `aria-*`、`data-*` 与焦点流转，不能只看快照；高频/重型组件必须补齐 `render_count` 断言/测量（如初始化空闲预算为 1）。
  - 语义契约覆盖证据：`components/accordion/test/semantics.rs` 对 `aria-expanded/aria-controls`、`data-open-state-source/data-open-init-source/data-open-last-change-source`、`data-focused/data-focus-visible` 及键盘/指针路径有显式断言。
  - 性能测量证据：`components/accordion/test/semantics.rs` 的 `docs_perf_probe_budgets_are_wired_for_component_pages` 断言 docs perf probe 预算接线（含 `accordion`），并要求 E2E 覆盖 `data-perf-mount-ms/data-perf-budget-ms/data-perf-observability/data-perf-violation`。
  - `render_count` 现状证据：`components/accordion/test/semantics.rs` 的 `perf_render_count_follow_up_is_tracked_in_plan` 对 `docs/plan/TODO.md` 中 `render_count` 自动化任务做回归跟踪（当前按“测量+计划跟踪”策略执行）。
...
- [x] 文档即产品（Copy-Paste Ready）：`apps/docs-app` 必须新增 Playground（Hello World、状态矩阵、受控/非受控对照），支持流式/快照展现，并提供 Source-first 一键复制且补全 imports。
  ...
  - 回归证据：`components/accordion/test/semantics.rs` 包含 `accordion_docs_page_covers_primary_playgrounds`、`accordion_docs_playgrounds_lock_state_matrix_contract_values`、`accordion_docs_examples_remain_copy_paste_ready`。
- [x] 语义测试优先：验证 `data-*` / `aria-*` / role / 状态来源契约，不只视觉快照。
  - 每个交互组件至少有对应 `*_semantics.rs` 测试覆盖关键状态轴与动作语义。
    - 证据：`components/accordion/test/semantics.rs` 已存在，覆盖 `aria-expanded/aria-controls/role`、`data-open-*source`、`data-ui-*`、键盘/指针路径与焦点语义。
  - 断言应聚焦语义契约（状态来源/可访问性/键盘路径），快照仅作补充。
    - 证据：测试以源码契约断言为主（`source.contains(\"data-*\"/\"aria-*\"/\"role=...\")`），并包含 E2E 语义选择器与键盘路径断言；未依赖纯视觉快照作为唯一验收。
  - 新增/变更语义字段必须同步补测试，否则不得打勾。
    - 证据：`semantics.rs` 对 `data-ui-schema/data-ui-source/data-ui-stream-*` 等字段有显式字符串守卫，字段漂移会直接触发测试失败。
- [x] E2E 选择器稳定：使用语义标记，WASM 场景有稳定等待策略。
  ...
  - 若组件涉及异步/动画，E2E 需显式覆盖 ready/settled 条件。
    - 证据：同文件对展开/收起后的 `data-open` 与 `aria-expanded`、`data-ui-output-status`、`data-ui-action` 进行阶段性断言，覆盖交互 settle 条件；语义护栏在 `components/accordion/test/semantics.rs` 的 `accordion_e2e_uses_semantic_selectors_and_stable_waits` 同步校验。
- [x] 关键流程纳入可重复回归集合（Playwright/Cypress）。
  ...
  - 高风险路径（overlay、focus、keyboard、async）优先进入回归集合。
    - 证据：Accordion 已覆盖适用高风险路径中的 `focus + keyboard + async(streaming status)`；`overlay` 对本组件为 N/A。语义护栏由 `components/accordion/test/semantics.rs::accordion_e2e_uses_semantic_selectors_and_stable_waits` 同步校验。
- [x] docs-app 文档、示例、参数矩阵、状态矩阵同步更新。
  ...
  - 文档示例需覆盖至少一组状态矩阵（受控/非受控、disabled、size/variant 等）。
    - 证据：同文件覆盖 `受控/非受控 + disabled + selection_mode(single/multiple) + variant(light/shadow/bordered/splitted)` 的状态矩阵；语义护栏由 `components/accordion/test/semantics.rs::accordion_docs_playgrounds_lock_state_matrix_contract_values` 锁定。
  - 文档中的 API 名称与默认值必须和 `logic.rs` 当前实现一致。
    - 证据：`components/accordion/test/semantics.rs::accordion_docs_api_contract_aligns_with_logic_defaults` 同时断言 docs 代码片段中的 `open/on_open_change/default_open`、`selection_mode` 等命名与 `components/accordion/src/logic.rs` 的归一化/初始化契约一致，并拦截 `open_indices/default_open_indices` 等旧别名回流。
- [x] 组件文档必须对新手友好（Documentation as Product）：组件 README 或等价文档入口必须存在。
  ...
  - “只有源码没有文档”或“只写给架构师/机器看的文档”视为不通过。
    - 证据：组件同时具备 README 与 docs-app 可访问入口，且 `components/accordion/test/semantics.rs` 中 `accordion_docs_page_covers_primary_playgrounds`、`accordion_docs_hello_world_example_stays_under_five_lines` 对新手入口与最小示例提供回归护栏。
- [x] `apps/docs-app` 必须提供 Interactive Playground：用户可在线修改 props/状态并实时预览。
  ...
  - Playground 作为验收面，需可重复复现关键交互路径。
    - 证据：`e2e/tests/docs_app_accordion.spec.mjs` 覆盖进入页面、打开/关闭、键盘 roving、disabled 分支的可重复流程；`components/accordion/test/semantics.rs::accordion_e2e_uses_semantic_selectors_and_stable_waits` 对该回归入口做语义护栏。
- [x] Source-first 文档必须 Copy-Paste Ready：提供一键复制组件源码或最小可用片段能力。
  ...
  - 文档代码与当前实现必须同步，防止示例漂移。
    - 证据：`components/accordion/test/semantics.rs::accordion_docs_examples_remain_copy_paste_ready`、`accordion_docs_api_contract_aligns_with_logic_defaults`、`accordion_docs_playgrounds_lock_state_matrix_contract_values` 对 docs 代码片段、API 命名与状态矩阵进行回归约束。
- [x] HeroUI 对标文档与组件文档同步：参数模型变更需同步 `docs/spec/heroui-parameter-design-strategy.md`（必要时补充 `docs/research/spectrum-heroui-style-interface-study.md`），并保证组件文档可访问。
  - 若参数语义发生变化，需同步更新对标策略文档，不允许实现先漂移文档后补。
    - 证据：`docs/spec/heroui-parameter-design-strategy.md` 已包含 `### Accordion 对齐记录（2026-02）`，明确 `selection_mode/open/on_open_change/default_open` 与 docs 对齐入口；`components/accordion/test/semantics.rs::accordion_parameter_strategy_doc_tracks_current_model` 对该同步关系做回归约束。
  ...
  - “仅代码更新无文档更新”在接口变更场景下直接判不通过。
    - 证据：`components/accordion/test/semantics.rs` 同时校验参数对标文档与 docs 页面契约（`accordion_parameter_strategy_doc_tracks_current_model` + docs 相关测试），可拦截“实现更新但文档未同步”的回归。
