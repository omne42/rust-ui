...
- [x] 默认主题美学质量达标（Visual Desire）：以 HeroUI 现代审美为学习对标，默认主题不仅“可用”，还必须“第一眼可信”。
  - N/A（已论证）：`example-theme` 仅导出 `Theme` 快照（`example_theme() -> Theme`），不渲染 Button/Input/Overlay 等可视组件，也不承载 docs 基线页面；该视觉质量回归应在 `ui` + `apps/docs-app` 维度验证。
...
- [x] Tree Shaking 是一等能力：package 模式支持组件级 feature；source 模式天然裁剪；样式层同步裁剪，禁止无条件聚合全部 CSS，禁止破坏 DCE/LTO 的全量中央注册表。
  - N/A（已论证）：`example-theme` 为独立主题导出 crate（仅 `src/mod.rs` + `ui-theme` 依赖，`default = []`），不属于 `ui` 聚合层，不存在 `lib.rs/css.rs` 组件全量聚合或 `component-*` 特性树注册面，因此该条在本组件维度不适用。
...
- [x] DX 要求：样式热重载优先无需重编 wasm；组件热开发尽量保持上下文；提供可选状态保留；有 Workbench 隔离画布。
  - N/A（已论证）：`example-theme` 仅导出 `example_theme() -> Theme`，无组件样式编辑回路、无交互上下文保持需求，也无 workbench/story/demo 可视演练面；该 DX 约束应在 `ui` 与 `apps/docs-app` 维度验收。
...
- [x] `ui` 固定入口文件落点正确。
  - N/A（已论证）：`example-theme` 为独立主题导出 crate（仅 `src/mod.rs` + `ui-theme` 依赖），不属于 `crates/ui` 聚合实现层；`lib.rs/css.rs/root.rs` 与 `overlay_open.rs/presence.rs/a11y.rs` 的入口落点治理应在 `ui` 仓库层检查。
