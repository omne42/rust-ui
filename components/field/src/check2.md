...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - `field` 组件最小门禁已验证：`cargo test -p ui --test field_semantics --no-default-features --features component-field,inject-css`、`cargo check -p ui --no-default-features --features component-field,inject-css`、`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-field,inject-css`、`cargo clippy -p ui --no-default-features --features component-field,inject-css --tests -- -D warnings`。
  - 仓库当前存在与本任务无关的全量构建阻塞（`crates/ui/src/color/slider/view.rs` 缺失）；本任务按组件最小特性集完成并验证通过，不干扰并行协作分支。
