...
- [x] API 易用性验收标准（DX Paradox）：把复杂性留在内部，把简单留给用户。（`Legend` 基础调用保持单行默认路径（仅 `text`），不要求用户接线 `ui-state-primitives`/`ui-headless` 状态机，也不暴露 `state=...` 必填入口；docs-app 新增 `Hello World` playground 作为最小可用示例。回归：`components/legend/test/legend_semantics.rs::legend_dx_paradox_keeps_simple_default_api_and_docs_path`。）
...
- [x] 组合型组件主 API 必须“显示优于约定”：优先使用显式组合 `<Parent><Item ... /></Parent>`。（N/A：`Legend` 为单语义节点组件，不是组合型容器；因此不引入 `labels/titles/panels` 并行数组、`ItemSpec` 语法糖或 `Parent/Item` 隐式配对接口。docs 仅展示显式 `<Legend ... />` 调用路径。回归：`components/legend/test/legend_semantics.rs::legend_non_composite_api_avoids_parallel_array_conventions`。）
...
- [x] 测试验证“语义契约”而不只验证视觉快照。（`Legend` 语义测试以 `role/aria/data-state/source markers` 为主：`components/legend/test/legend_semantics.rs` 已覆盖 `aria-disabled`、`data-state`、`data-required-source`、`data-disabled-source`、`data-text-source` 等契约断言；矩阵分支中 `disabled` 已覆盖，受控/非受控与键盘/指针路径对 `Legend` 作为非交互静态语义节点判定为 N/A 并有对应回归约束，SSR/wasm 差异由 `components/legend/test/motion.rs` 的 `cfg!(target_arch = \"wasm32\")` 分支断言覆盖；未使用视觉快照断言替代语义契约。回归：`components/legend/test/legend_semantics.rs::legend_semantic_contract_tests_cover_matrix_without_snapshot_lock_in`。）
...
- [x] `spec.rs` 只用于少数复杂组件（如 button），避免泛滥。（`Legend` 作为简单语义组件不引入 `src/spec.rs`；组件契约说明留在 `check2.md`/`README.md`，版本化协议通过 `src/protocol.rs`（`LegendComponentSpec`）承载且有 `components/legend/test/protocol.rs` 回归。回归：`components/legend/test/legend_semantics.rs::legend_avoids_spec_rs_sprawl_and_keeps_contract_in_protocol_module`。）
...
- [x] E2E 选择器稳定：使用语义标记，WASM 场景有稳定等待策略。（已新增 `e2e/tests/docs_app_legend_contract.spec.mjs`，路由就绪统一使用 `await page.locator("body:not(:has(#boot))").waitFor();`，并以语义选择器为主：`[data-component="legend"]` + `[data-slot="legend"]` + `data-ui-output-status/data-ui-stream-*` 作为 ready/settled 断点；断言覆盖 `default/required/custom/disabled` 多状态语义标记（`data-required-source`、`data-disabled-source` 等），未使用固定 sleep 或脆弱 DOM 层级选择器。源码回归锁定：`components/legend/test/legend_semantics.rs::legend_e2e_contract_uses_semantic_selectors_and_wasm_stable_waits`。）
  - E2E 选择器优先 `data-*` 语义标记，禁止依赖脆弱 DOM 层级或文本定位。
  - WASM 场景必须使用稳定等待策略（语义状态就绪而非固定 sleep）。
  - 若组件涉及异步/动画，E2E 需显式覆盖 ready/settled 条件。
- [x] 关键流程纳入可重复回归集合（Playwright/Cypress）。（`e2e/tests/docs_app_legend_contract.spec.mjs` 已定义可重复关键流程：`runLegendControlledRequiredFlow` 覆盖“打开 docs -> 聚焦 `switch` -> 键盘 `Space` 触发 required 状态切换 -> 语义断点断言 `data-state/data-required/data-required-source` -> `page.reload()` 后重复同流程”；失败定位可直接落到语义契约断点而非笼统页面差异。回归锁定：`components/legend/test/legend_semantics.rs::legend_e2e_key_flow_is_repeatable_with_semantic_breakpoints`。高风险优先项中，`Legend` 非 overlay/async 组件，适用路径为 keyboard + focus，已纳入同一 E2E 回归。）
  - 至少定义一条可重复关键流程（打开/交互/关闭或提交）纳入 E2E 回归。
  - 回归失败需可定位到具体语义契约断点，而不是笼统“页面不一致”。
  - 高风险路径（overlay、focus、keyboard、async）优先进入回归集合。
- [x] docs-app 文档、示例、参数矩阵、状态矩阵同步更新。（`apps/docs-app/src/pages/components/pages/forms_groups_extra.rs::legend()` 已同步更新并覆盖 `Hello World`、`Required Legend`、`Tone + Custom Indicator + Disabled`（状态矩阵）与 `Controlled vs Default (Comparison)`（受控/非受控）；示例参数使用当前 API 命名（`is_required`/`is_disabled`/`tone`/`required_indicator`）且与 `components/legend/src/logic.rs` 默认归一化契约一致（`DEFAULT_IS_REQUIRED=false`、`DEFAULT_IS_DISABLED=false`，默认值集中在 `logic.rs`）。回归锁定：`components/legend/test/legend_semantics.rs::legend_docs_page_covers_primary_playgrounds`、`components/legend/test/legend_semantics.rs::legend_docs_playgrounds_lock_state_matrix_contract_values`、`components/legend/test/legend_semantics.rs::legend_docs_api_names_and_default_contracts_stay_in_sync_with_logic`。）
  - 组件行为或参数变更必须同步更新 `apps/docs-app` 示例与说明。
  - 文档示例需覆盖至少一组状态矩阵（受控/非受控、disabled、size/variant 等）。
  - 文档中的 API 名称与默认值必须和 `logic.rs` 当前实现一致。
- [x] 组件文档必须对新手友好（Documentation as Product）：组件 README 或等价文档入口必须存在。（`components/legend/src/README.md` 已补齐新手路径结构：`先用起来（默认路径） -> Hello World（最小可用） -> 常见用法 -> 再进阶（高级控制）`，并明确“默认 API 路径优先、不需要手动接线底层分层”；等价 docs 入口 `apps/docs-app/src/pages/components/pages/forms_groups_extra.rs::legend()` 持续提供 `Hello World/Required/状态矩阵/Controlled vs Default`。回归锁定：`components/legend/test/legend_semantics.rs::legend_readme_is_copy_paste_ready`、`components/legend/test/legend_semantics.rs::legend_documentation_is_beginner_friendly_with_readme_or_equivalent_entry`。）
  - 每个基础组件必须提供“零门槛”最小示例（Hello World）与常见用法，避免要求用户先理解底层分层架构。
  - 文档需明确“先用起来，再进阶”：默认 API 路径在前，高级控制参数在后。
  - “只有源码没有文档”或“只写给架构师/机器看的文档”视为不通过。
- [x] `apps/docs-app` 必须提供 Interactive Playground：用户可在线修改 props/状态并实时预览。（`apps/docs-app/src/pages/components/pages/forms_groups_extra.rs::legend()` 已提供可交互验收面：`Controlled vs Default (Comparison)` playground 通过 `Switch` 实时切换 `controlled_required`，并把状态直接映射到 `Legend is_required=Some(...)` 预览；同时 `e2e/tests/docs_app_legend_contract.spec.mjs` 的 `runLegendControlledRequiredFlow` 覆盖键盘触发与 `reload` 后复现，满足可重复关键路径。AI Spec 子项对 `Legend` 为 N/A：该组件不是 spec/input-output 联动型组件（无 `spec.rs` 设计面），因此不强制额外 Spec 输入演示。回归锁定：`components/legend/test/legend_semantics.rs::legend_docs_app_interactive_playground_supports_live_state_and_repeatable_flow`、`components/legend/test/legend_semantics.rs::legend_e2e_key_flow_is_repeatable_with_semantic_breakpoints`。）
