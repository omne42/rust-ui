...
- 核验命令（按顺序逐项执行）：
  1. `CARGO_TARGET_DIR=target-legend-check cargo test -p ui --test legend_semantics --no-default-features --features component-legend,inject-css`（通过，7 passed）
  2. `CARGO_TARGET_DIR=target-legend-check cargo check -p ui --no-default-features --features component-legend,inject-css`（通过）
  3. `CARGO_TARGET_DIR=target-legend-check cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-legend,inject-css`（通过）
  4. `CARGO_TARGET_DIR=target-legend-check cargo clippy -p ui --no-default-features --features component-legend,inject-css -- -D warnings`（通过）
- docs smoke：
