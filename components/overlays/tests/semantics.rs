...
fn load_source(rel_path: &str) -> String {
    ...
    if mapped == "../../apps/docs-app/src/pages/components/pages/overlays.rs" {
        let parent_path = manifest_dir.join(&mapped);
        ...
        collect_paths_with_extension(
            &manifest_dir.join("../../apps/docs-app/src/pages/components/pages/overlays"),
            "rs",
            &mut child_paths,
        );
        child_paths.sort();
    ...
    if mapped == "../../apps/docs-app/src/pages/components/pages/overlays_extra.rs" {
        let parent_path = manifest_dir.join(&mapped);
        let parent = fs::read_to_string(&parent_path)
            .unwrap_or_else(|e| panic!("read_to_string failed for {parent_path:?}: {e}"));
        let mut child_paths = Vec::new();
        collect_paths_with_extension(
            &manifest_dir.join("../../apps/docs-app/src/pages/components/pages/overlays_extra"),
            "rs",
            &mut child_paths,
        );
        child_paths.sort();
    ...
    if mapped == "../../apps/docs-app/src/pages/components/pages/overlays_dialog.rs" {
        let parent_path = manifest_dir.join(&mapped);
        let parent = fs::read_to_string(&parent_path)
            .unwrap_or_else(|e| panic!("read_to_string failed for {parent_path:?}: {e}"));
        let mut child_paths = Vec::new();
        collect_paths_with_extension(
            &manifest_dir.join("../../apps/docs-app/src/pages/components/pages/overlays_dialog"),
            "rs",
            &mut child_paths,
        );
        child_paths.sort();
...
fn overlays_type_system_and_semantic_markers_define_machine_readable_contracts() {
    ...
    let overlays_module_tests = load_source("tests/semantics.rs");
...
fn overlays_semantic_contract_tests_cover_matrix_and_do_not_rely_on_snapshots_only() {
    let overlays_tests = load_source("tests/semantics.rs");
    let overlay_semantics = load_source("../../components/overlay/test/overlay_semantics.rs");
    let popover_semantics = load_source("../../components/popover/src/test/semantics.rs");
    let modal_semantics = load_source("../../components/modal/test/modal/semantics.rs");
    let modal_logic_tests = load_source("../modal/test/logic.rs");
    let nav_sheet_e2e = load_source("../../e2e/tests/docs_app_nav_sheet.spec.mjs");
...
fn overlays_semantic_test_priority_prefers_data_aria_role_and_source_contracts_over_snapshot_only_checks()
 {
    ...
    let popover_semantics = load_source("../../components/popover/src/test/semantics.rs");
    let modal_semantics = load_source("../../components/modal/test/modal/semantics.rs");
    let drawer_semantics = load_source("../../components/drawer/test/drawer/semantics.rs");
    let bottom_sheet_semantics = load_source("../../components/bottom-sheet/test/semantics.rs");
...
fn overlays_semantics_and_performance_regression_cover_aria_data_focus_and_render_count_measurement()
 {
    let local_semantics = load_source("tests/semantics.rs");
    let overlay_semantics = load_source("../../components/overlay/test/overlay_semantics.rs");
    let popover_semantics = load_source("../../components/popover/src/test/semantics.rs");
    let modal_semantics = load_source("../../components/modal/test/modal/semantics.rs");
