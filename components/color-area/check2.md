...
- [x] 性能治理：关键路径有预算（首次渲染/更新耗时/内存），回归可检测、可归因、可阻断。
  ...
  - 证据：`apps/docs-app/src/pages/components/shell.rs:34` 统一定义组件页性能预算并在 `apps/docs-app/src/pages/components/shell.rs:272` 通过 `UiPerfProbe` 挂载到包括 `color-area` 在内的组件页面（`color-area` 走 `apps/docs-app/src/pages/components/shell.rs:176` 的 `mount_only(120.0)` 默认预算）；`apps/docs-app/src/perf_probe.rs:55` 暴露稳定 `data-perf-*` 指标与 `data-perf-violation` 阈值阻断标记；`e2e/tests/docs_app_components_coverage.spec.mjs:70` 对 all-components 页面循环断言 perf probe 阈值（含 `color-area`）；基础组件预算与阻断契约由 `components/text-input/test/input_semantics.rs:327`（含 Button/Input 预算与 render_count 约束文本）和 `scripts/check-ui-performance.sh:7` 的门禁用例执行；`render_count` 自动化缺口已由 `components/accordion/test/semantics.rs:1651` + `docs/plan/TODO.md:500` 跟踪为后续必做项。
