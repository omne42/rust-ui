...
### ColorArea 本轮验收证据（2026-02-18）
- 架构下沉：新增 `crates/ui-state-primitives/src/color_area.rs` 与 `crates/ui-headless/src/color_area.rs`，并分别在 `crates/ui-state-primitives/src/lib.rs`、`crates/ui-headless/src/lib.rs` 导出。
- 组件装配重构：`crates/ui-components/src/color/area/logic.rs` 新增 `is_disabled` 兼容归一、默认值单点归一、Agent Contract 映射；`crates/ui-components/src/color/area/view.rs` 改为消费 `use_color_area` 合约并输出稳定 `data-*`/`data-ui-*` 标记。
- i18n/l10n：`crates/ui-headless/src/i18n/common.rs` 新增 `color_area_*` 文案字段；组件回退链为 `props -> UiI18n(CommonStrings) -> ui-state-primitives defaults`。
- E2E 合约：新增 `e2e/tests/docs_app_color_area_contract.spec.mjs` 与 `scripts/check-ui-components-e2e-color-area.sh`，覆盖语义选择器、稳定等待、键盘路径与 disabled 分支。
- 验证命令（最小特性）：  
  `CARGO_TARGET_DIR=target-colorarea-check2 cargo check -p ui-components --no-default-features --features component-color_area,inject-css`（通过）  
  `CARGO_TARGET_DIR=target-colorarea-check2 cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-color_area,inject-css`（通过，存在仓库已有 `observability.rs` dead_code warning）  
  `CARGO_TARGET_DIR=target-colorarea-check2 cargo test -p ui-components --test color_area_semantics --no-default-features --features component-color_area,inject-css`（17/17 通过）  
  `CARGO_TARGET_DIR=target-colorarea-check2 cargo test -p ui-headless color_area`（通过）  
  `CARGO_TARGET_DIR=target-colorarea-check2 cargo test -p ui-state-primitives color_area`（通过）  
  `./scripts/check-ui-components-e2e-color-area.sh`（通过）
