...
- 组件装配重构：`crates/ui/src/color/area/logic.rs` 新增 `is_disabled` 兼容归一、默认值单点归一、Agent Contract 映射；`crates/ui/src/color/area/view.rs` 改为消费 `use_color_area` 合约并输出稳定 `data-*`/`data-ui-*` 标记。
...
- E2E 合约：新增 `e2e/tests/docs_app_color_area_contract.spec.mjs` 与 `components/color-area/scripts/check-ui-e2e-color-area.sh`，覆盖语义选择器、稳定等待、键盘路径与 disabled 分支。
- 验证命令（最小特性）：  
  `CARGO_TARGET_DIR=target-colorarea-check2 cargo check -p ui --no-default-features --features component-color_area,inject-css`（通过）  
  `CARGO_TARGET_DIR=target-colorarea-check2 cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-color_area,inject-css`（通过，存在仓库已有 `observability.rs` dead_code warning）  
  `CARGO_TARGET_DIR=target-colorarea-check2 cargo test -p ui --test color_area_semantics --no-default-features --features component-color_area,inject-css`（17/17 通过）  
  ...
  `./components/color-area/scripts/check-ui-e2e-color-area.sh`（通过）
