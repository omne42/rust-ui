...
### 10. Code 组件核验证据（本次）
- 架构边界：`components/code/src/` 保持 `mod.rs + logic.rs + styles.rs + view.rs` 的轻量装配分层；`Code` 为展示型组件，无独立可复用状态机与交互语义契约下沉需求（`status-primitives/ui-headless/ui-motion` 对应项按 N/A 处理并已逐项确认不越层）。
...
- README：`components/code/src/README.md` 已补齐，包含展示区 / Config 区 / Code 区 / CSS Test 区说明与多场景对比示例。
