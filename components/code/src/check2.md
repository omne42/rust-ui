...
- 测试回归：`cargo test -p ui --test code_semantics` 通过（7/7）。
- 最小特性编译：`cargo check -p ui --no-default-features --features component-code,inject-css` 通过。
- wasm 路径编译：`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-code,inject-css` 通过（存在 `observability.rs` 的既有 dead_code warning，非 `code` 组件新增）。
- Clippy 门禁：`CARGO_TARGET_DIR=/tmp/code-clippy-min CARGO_INCREMENTAL=0 CARGO_BUILD_JOBS=1 cargo clippy -p ui --no-default-features --features component-code,inject-css -- -D warnings` 通过。
