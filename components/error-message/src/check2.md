...
- 本轮执行命令：
  - `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.error-message-check cargo check -p ui --no-default-features --features component-error_message,inject-css`
  - `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.error-message-check cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-error_message,inject-css`
- `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.error-message-check cargo clippy -p ui --no-default-features --features component-error_message,inject-css --tests -- -D warnings`
- `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.error-message-check cargo clippy -p ui --no-default-features --features component-error_message,inject-css --test error_message_semantics -- -D warnings`（全量 `--tests` 受其他测试文件阻塞）
- `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.error-message-check CARGO_BUILD_JOBS=1 cargo test -p ui --test error_message_semantics --no-default-features --features component-error_message,inject-css`
