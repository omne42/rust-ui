...
- 回归补齐：`components/error-message/test/error_message_semantics.rs` 增加 primitives/headless/agent-contract/check2 完整性断言；新增 `e2e/tests/docs_app_error_message_contract.spec.mjs` 覆盖语义选择器与可重复回放路径。
- docs/readme 与 playground：`components/error-message/src/README.md` 新增 `display/config/code/css test` 分区；`apps/docs-app/src/pages/components/pages/forms_extra.rs` 的 `error_message()` 新增 Display 对比与 Config+Code+CSS Test workbench。
...
- 额外约束回归：`components/error-message/test/error_message_semantics.rs` 新增 README 分区断言（display/config/code/css test）与 docs workbench 断言（`Display Comparisons` + `Config + Code + CSS Test Workbench` + `test_css_source/test_config_signal`）。
- 本轮执行命令：
