...
- 代码范围仅限 `crates/ui-components/src/field_form/fieldset/*`、`crates/ui-components/tests/fieldset_semantics.rs`、`apps/docs-app/src/pages/components/pages/forms_extra.rs`、`crates/ui-state-primitives/src/fieldset.rs`、`e2e/tests/docs_app_fieldset_contract.spec.mjs`，未改动其他组件实现，避免干扰并行协作。
