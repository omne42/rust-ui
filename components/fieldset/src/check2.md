...
**Fieldset 本轮逐项检查执行记录（对应本文件全部勾选项）**
- 代码范围仅限 `crates/ui/src/field_form/fieldset/*`、`components/fieldset/test/fieldset_semantics.rs`、`apps/docs-app/src/pages/components/pages/forms_extra.rs`、`crates/ui-state-primitives/src/fieldset.rs`、`e2e/tests/docs_app_fieldset_contract.spec.mjs`，未改动其他组件实现，避免干扰并行协作。
- 本轮补齐项：将 `Fieldset` 状态原语下沉到 `ui-state-primitives`；组件侧新增 `lang/dir` 接入、`is_*` 命名轴兼容归一、`data-required-source/data-disabled-source/data-invalid-source`、`data-ui-*` 机器可读契约；`motion.rs` 输出通过根节点 `style` 实际挂载；docs 新增 `Hello World` playground；新增 fieldset docs e2e 合同用例。
- 增量补齐（本轮继续）：docs 增加 `Fieldset Workbench (Display + Config + Code + CSS Test)`，覆盖展示区/配置区/代码区/CSS 测试区，并补充多场景对比；`README.md` 增加 `展示/config/code/css test` 分区与对比说明。
- N/A 声明：`Fieldset` 为非受控静态分组组件，无异步加载/重试协议与正文流式渲染职责；相关条目按 N/A 审核通过，稳定性由语义契约断言与 docs-e2e 保障。
- 已执行验证：
  - `cargo check -p ui --no-default-features --features component-fieldset,inject-css`
  - `cargo test -p ui --no-default-features --features component-fieldset,inject-css --test fieldset_semantics`
  - `cargo clippy -p ui --no-default-features --features component-fieldset,inject-css --test fieldset_semantics -- -D warnings`
  - `node --check e2e/tests/docs_app_fieldset_contract.spec.mjs`
