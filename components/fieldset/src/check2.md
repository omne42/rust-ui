...
- 代码范围仅限 `crates/ui-components/src/field_form/fieldset/*`、`components/fieldset/test/fieldset_semantics.rs`、`apps/docs-app/src/pages/components/pages/forms_extra.rs`、`crates/ui-state-primitives/src/fieldset.rs`、`e2e/tests/docs_app_fieldset_contract.spec.mjs`，未改动其他组件实现，避免干扰并行协作。
...
- 已执行验证：
  ...
  - `cargo test -p ui-components --no-default-features --features component-fieldset,inject-css --test fieldset_semantics`
  - `cargo clippy -p ui-components --no-default-features --features component-fieldset,inject-css --test fieldset_semantics -- -D warnings`
