...
fn selection_indicator_check2_includes_semantic_contract_testing_rules() {
    ...
    for needle in [
        ...
        "回归测试集中在 `components/selection-indicator/test/semantics.rs`",
...
fn selection_indicator_semantic_contract_checks_cover_matrix_without_snapshot_dependency() {
    let selection_module_test_source = load_source("tests/semantics.rs");
    let menu_item_semantics_test_source = load_source("tests/menu_item_semantics.rs");
    let list_module_semantics_test_source = load_source("tests/list_module/semantics.rs");
    let list_view = load_source("src/list/view.rs");
    let menu_item_view = load_source("src/menu/item/view.rs");
...
fn selection_indicator_check2_includes_machine_readable_type_marker_rules() {
    ...
    for needle in [
        ...
        "并由 `components/selection-indicator/test/semantics.rs` 持续断言。",
    ] {
        assert!(
            check2.contains(needle),
...
fn selection_indicator_check2_includes_inner_html_safety_rules() {
    ...
    for needle in [
        ...
        "语义回归由 `components/selection-indicator/test/semantics.rs` 持续断言",
