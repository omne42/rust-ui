...
fn load_source(rel_path: &str) -> String {
    if rel_path == "../../apps/docs-app/src/pages/components/pages/display_extra.rs" {
        return docs_display_extra_source().to_string();
    }
...
fn docs_display_extra_source() -> &'static str {
    ...
    SOURCE
        .get_or_init(|| {
            ...
            let workspace_dir = resolve_workspace_dir(manifest_dir);
            let parent_path =
                workspace_dir.join("apps/docs-app/src/pages/components/pages/display_extra.rs");
            let child_path = workspace_dir
                .join("apps/docs-app/src/pages/components/pages/display_extra/labeled_value.rs");
...
fn labeled_value_check2_marks_core_sections_complete() {
    ...
    for needle in [
        ...
        "components/labeled-value/tests/semantics.rs",
    ] {
        assert!(
            source.contains(needle),
