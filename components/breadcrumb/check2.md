...
- [x] API 命名契约统一：公共 props/回调严格使用 `is_*`、`on_*`、`default_*` 前缀；同语义在全库同名，禁止别名漂移。
  - 验证记录（2026-02-20）：`Breadcrumb` 对外 API 仅暴露 `items/aria_label/class_name/lang/dir`，未引入布尔状态轴；因此 `is_*` 规则在本组件按 `N/A` 处理。组件无受控/非受控状态与事件回调，`on_*`、`default_*` 规则在本组件按 `N/A` 处理。回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_api_naming_contract_is_consistent_and_state_free` 与 `components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_api_naming_contract_has_no_state_alias_drift`。
...
- [x] 受控/非受控必须成对：每个可控状态轴都提供 `value + on_value_change + default_value`（如 `open/on_open_change/default_open`）；缺一项即不通过。
  - 验证记录（2026-02-20）：`Breadcrumb` 为无内部可控状态轴的展示组件（仅消费 `items` 渲染导航语义），因此本项按组件职责标注 `N/A`：不存在 `value/on_value_change/default_value` 或 `open/on_open_change/default_open` 公共 API，也不存在“半受控”切换路径。回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_has_no_controlled_or_uncontrolled_state_axes` 与 `components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_has_no_controlled_or_uncontrolled_state_axes`。
...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。
  - 验证记录（2026-02-20）：`breadcrumb` 为纯展示导航组件，无远程请求、无异步状态轴与重试语义；`view.rs` 不暴露 `is_loading/error/retry/disabled/aria-busy` 契约，`logic.rs` 不实现 `use_async_action` 或自定义异步协议，因此本项按组件职责标注 `N/A`。回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_has_no_async_interaction_contract`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_has_no_async_interaction_contract`。
...
- [x] API 易用性验收标准（DX Paradox）：把复杂性留在内部，把简单留给用户。
  - 验证记录（2026-02-20）：`Breadcrumb` 基础 API 仅需 `items`，不要求用户手动接线 `ui-state-primitives/ui-headless` 或传入内部 `state` 对象；高级能力通过可选参数（如 `aria_label`）按需开启。docs 最小路径已在 `apps/docs-app/src/pages/components/pages/collections_breadcrumb.rs` 提供 `Hello World` 示例（`let items = vec![...];` + `<Breadcrumb items=items />`，不超过 5 行）。回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_docs_exposes_minimal_hello_world_path`、`components/breadcrumb/test/breadcrumb_semantics.rs::docs_page_exposes_minimal_breadcrumb_hello_world_path`。
...
- [x] 集合注册协议（Registration Protocol）：`Accordion/Tabs/Menu` 动态子项必须通过 `RegistrationContext` 上报 `Register/Unregister`，逻辑层维护 `items_order`，禁止依赖 `HashSet` 迭代顺序做导航。
  - 验证记录（2026-02-20）：`breadcrumb` 当前 API 直接消费 `items: Vec<BreadcrumbItem>`，不提供动态子项注册生命周期，因此本项按组件职责标注 `N/A`；`logic.rs` 顺序来源为输入 `Vec` 迭代映射，未引入 `HashSet` 迭代顺序依赖或 `RegistrationContext`。回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_has_no_registration_protocol_path`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_has_no_registration_protocol_path`。
- [x] 插槽投影策略（Slot Projection）：容器组件明确 `Lazy/KeepAlive/Eager`；`KeepAlive` 隐藏时必须通过生命周期通知（如 `NotifyHidden`）暂停轮询/动画等高耗能副作用。
  - 验证记录（2026-02-20）：`breadcrumb` 为静态线性导航装配，不存在容器投影策略轴（`Lazy/KeepAlive/Eager`）与隐藏生命周期通知（`NotifyHidden`），因此本项按组件职责标注 `N/A`。回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_has_no_slot_projection_policy_path`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_has_no_slot_projection_policy_path`。
- [x] 环境订阅流（Env Streams）：`Resize/Theme/Intersection` 等环境变化在 `view.rs` 采样、防抖后转化为高层语义 `Action`（如 `BreakpointChanged`）推送到 `logic`；禁止原始事件洪泛。
  - 验证记录（2026-02-20）：`breadcrumb` 无环境订阅语义轴（无 `Resize/Theme/Intersection` 驱动状态），因此本项按组件职责标注 `N/A`；`view.rs` 未实现采样/防抖入口，`logic.rs` 未实现 `BreakpointChanged` 等高层 env action 回流。回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_has_no_env_stream_subscription_path`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_has_no_env_stream_subscription_path`。
- [x] 事件光锥（Event Light Cone）：`Table/Grid` 等大型集合批量操作必须走 `Context Bus + Selector` 与状态压缩表达（如 `SelectionState::All`），禁止 O(N) 级向下 prop drilling。
  - 验证记录（2026-02-20）：`breadcrumb` 不是 `Table/Grid` 大型集合组件，不存在批量选择与全选状态压缩语义轴，因此本项按组件职责标注 `N/A`；`view.rs`/`logic.rs` 均未引入 `Context Bus`、`Selector`、`SelectionState::All`。回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_has_no_event_light_cone_batch_selection_path`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_has_no_event_light_cone_batch_selection_path`。
- [x] 统一因果总线（Causality Bus）：复杂派生总线操作必须支持透传 `TraceId`，确保“用户触发 -> 派生命令 -> 总线广播 -> 订阅者”因果链不断裂。
  - 验证记录（2026-02-20）：`breadcrumb` 为线性导航渲染组件，不包含复杂派生总线与跨订阅者广播语义轴，因此本项按组件职责标注 `N/A`；`view.rs`/`logic.rs` 均未引入 `TraceId`、因果总线透传或发布订阅链路。回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_has_no_unified_causality_bus_path`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_has_no_unified_causality_bus_path`。
- [x] 存在 A11y 实现、国际化与本地化实现（至少具备接入点，不硬编码用户可见文本）。
  - 验证记录（2026-02-20）：`breadcrumb` 的语义契约由 `ui_headless::navigation_attrs` 挂载（`aria-label/lang/dir`），并新增 `props -> UiRoot i18n(CommonStrings) -> fallback` 的文本来源链：`aria_label` 回退到 `CommonStrings::breadcrumb_aria_label`，分隔符回退到 `CommonStrings::breadcrumb_separator`，最终才使用组件默认值；`view.rs` 不再硬编码 `"/"`。回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_a11y_and_i18n_contract_is_headless_backed`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_a11y_and_i18n_contract_is_headless_backed`、`components/breadcrumb/test/logic.rs::resolve_root_state_uses_i18n_aria_label_when_prop_missing`、`components/breadcrumb/test/logic.rs::resolve_separator_tracks_source_priority`。
...
- [x] `spec.rs` 只用于少数复杂组件（如 button），避免泛滥。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_spec_rs_is_not_introduced_for_simple_component`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_spec_rs_is_not_introduced_for_simple_component`。
...
- [x] 默认主题美学质量达标（Visual Desire）：以 HeroUI 现代审美为学习对标，默认主题不仅“可用”，还必须“第一眼可信”。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_visual_desire_reuses_theme_visual_baseline_and_heroui_contracts`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_visual_desire_reuses_theme_visual_baseline_and_heroui_contracts`。
...
- [x] 类型系统 + 语义标记共同提供机器可读状态；关键输入空间受类型约束。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_type_system_and_semantic_markers_keep_machine_readable_contract`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_type_system_and_semantic_markers_keep_machine_readable_contract`、`components/breadcrumb/test/logic.rs::resolve_state_ignores_blank_and_last_item_links`、`components/breadcrumb/test/logic.rs::state_source_attrs_are_closed_enumerations`。
...
- [x] 焦点全局栈（Focus Stack & GC）：层叠 `Overlay` 禁止私存 `NodeRef` 作为恢复目标；必须依赖全局 Focus Manager（如 `FallbackTo/Selector`）防止焦点坠落到 `document.body`。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_has_no_focus_stack_or_overlay_restore_path`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_has_no_focus_stack_or_overlay_restore_path`。
- [x] 受控外交特区（Escape Hatches）：集成 ECharts/Map 等命令式第三方库时必须处于 `Foreign Zone`（`YieldControl/CleanupForeign`）；第三方实例不得暴露为组件公共 API 或反向污染状态机。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_has_no_foreign_zone_escape_hatch_path`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_has_no_foreign_zone_escape_hatch_path`。
...
- [x] 组件实现覆盖 `reduced-motion` / SSR / wasm 分支。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_reduced_motion_ssr_wasm_branches_keep_semantics_consistent`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_reduced_motion_ssr_wasm_branches_keep_semantics_consistent`。
...
- [x] `view!` 宏复杂度受控：单个 `view!` 块不得承载超长深嵌套结构；复杂布局按语义分块，避免一次性宏展开导致编译与 wasm 体积劣化。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_view_macro_complexity_is_split_into_local_render_helpers`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_view_macro_complexity_is_split_into_local_render_helpers`。
...
- [x] 函数式拆分优先：不涉及复杂状态与生命周期管理的 UI 片段，优先拆为普通 Rust 函数（返回 `impl IntoView`/`View`），而不是新增 `#[component]`。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_prefers_functional_splitting_without_local_component_noise`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_prefers_functional_splitting_without_local_component_noise`。
...
- [x] 静态片段常量化：复杂 SVG、页脚、长说明文本等纯静态内容优先常量化/模板化，减少重复 `view!` 渲染指令生成。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_static_fragments_are_constantized_or_marked_not_applicable`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_static_fragments_are_constantized_or_marked_not_applicable`。
  - 可判定为纯静态的片段应避免重复动态构造。
...
- [x] `inner_html` 使用约束：仅允许注入受信任静态常量，禁止拼接用户输入；使用处必须补充语义与安全回归测试。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_inner_html_contract_is_n_a_and_docs_shell_is_static_whitelist_only`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_inner_html_contract_is_n_a_and_docs_shell_is_static_whitelist_only`。
...
- [x] WASM 调试要求：关键状态可追踪（来源/时间/前后值），关键交互可回放，开发模式有可视化入口，调试能力通过 feature 隔离不污染产物。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_wasm_debug_contract_is_n_a_and_playground_surface_is_stable`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_wasm_debug_contract_is_n_a_and_playground_surface_is_stable`。
...
- [x] DX 要求：样式热重载优先无需重编 wasm；组件热开发尽量保持上下文；提供可选状态保留；有 Workbench 隔离画布。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_dx_contract_uses_scoped_playground_for_hot_style_and_context_retention`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_dx_contract_uses_scoped_playground_for_hot_style_and_context_retention`。
...
- [x] 工程能力统一：`serde` 负责 spec 序列化/版本迁移/错误结构化；`tracing` 统一 span/event 语义；async 不绑定单一运行时（tokio/async-std），runtime 细节不泄露到上层 API。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_engineering_contract_keeps_serde_protocol_and_runtime_agnostic_api`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_engineering_contract_keeps_serde_protocol_and_runtime_agnostic_api`。
...
- [x] 上下文压缩协议（Manifest + RBI）：新增/大改组件必须同步维护组件目录下 `Component.toml`（能力清单）和 `.rbi`（接口签名投影），避免 AI 检索工具箱过时。
  ...
  - 回归锁定：`components/breadcrumb/test/semantics.rs::breadcrumb_context_compression_manifest_and_rbi_projection_are_present_and_current`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_context_compression_manifest_and_rbi_projection_are_present_and_current`。
- [x] 语义标记统一升级为 Agent Contract（Schema 化），让 Agent 不依赖 DOM 猜测理解组件状态与意图。
  ...
  - 回归锁定：`components/breadcrumb/test/logic.rs::resolve_agent_contract_is_schema_typed_and_traceable`、`components/breadcrumb/test/logic.rs::resolve_agent_source_uses_whitelist_mapping`、`components/breadcrumb/test/semantics.rs::breadcrumb_agent_contract_schema_is_typed_traceable_and_whitelisted`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_agent_contract_schema_is_typed_traceable_and_whitelisted`。
...
- [x] `Snapshot` 是所有组件的基础能力（默认必须支持）。
  ...
  - 回归锁定：`components/breadcrumb/test/logic.rs::snapshot_baseline_consumes_complete_configuration_and_stays_stable`、`components/breadcrumb/test/semantics.rs::breadcrumb_snapshot_is_foundational_and_complete_config_path_is_stable`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_snapshot_is_foundational_and_complete_config_path_is_stable`。
...
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。
  ...
  - 回归锁定：`components/breadcrumb/test/logic.rs::resolve_streaming_policy_is_optional_with_snapshot_fallback_and_verified_output`、`components/breadcrumb/test/semantics.rs::breadcrumb_streaming_policy_is_optional_with_snapshot_fallback_and_explicit_output_status`、`components/breadcrumb/test/breadcrumb_semantics.rs::breadcrumb_streaming_policy_is_optional_with_snapshot_fallback_and_explicit_output_status`。
