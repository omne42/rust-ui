...
fn component_source(rel_path: &str) -> String {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let path = manifest_dir.join(rel_path);
    fs::read_to_string(&path).unwrap_or_else(|e| panic!("read_to_string failed for {path:?}: {e}"))
}

fn docs_source(rel_path: &str) -> String {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let path = manifest_dir.join(rel_path);
    fs::read_to_string(&path).unwrap_or_else(|e| panic!("read_to_string failed for {path:?}: {e}"))
}
...
#[test]
fn breadcrumb_directory_layout_matches_ui_components_contract() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    ...
    for rel_path in [
        ...
        assert!(
            ...
            "breadcrumb component should keep `{rel_path}` in the expected location."
        );
    }

    assert!(
        !manifest_dir.join("src/render.rs").exists(),
        "breadcrumb should not drift to `render.rs`; view code must stay in `view.rs`."
    );
    assert!(
        !manifest_dir.join("src/motion.rs").exists(),
        "breadcrumb currently has no semantic state-driven animation axis, so `motion.rs` stays N/A."
    );
}

#[test]
fn breadcrumb_mod_exports_minimal_public_api_and_wires_local_semantics_test() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "breadcrumb mod boundary should include `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "breadcrumb module should keep internals private and avoid DOM-type leakage via `{forbidden}`."
        );
    }
}

#[test]
fn breadcrumb_layer_boundaries_remain_assembly_only() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "breadcrumb logic should consume status primitives via `{needle}`."
        );
    }
    assert!(
        !logic_source.contains("use leptos::"),
        "logic.rs should stay framework-agnostic and not bind Leptos view types."
    );
    ...
    for needle in [
        ...
        assert!(
            ...
            "breadcrumb view should only mount headless semantics and structure via `{needle}`."
        );
    }
    assert!(
        !view_source.contains("web_sys"),
        "breadcrumb public component path should not expose or depend on web-sys details."
    );
    ...
    for needle in [
        ...
        assert!(
            ...
            "styles.rs should stay token-first and consume theme variables via `{needle}`."
        );
    }
    assert!(
        !styles_source.contains("--ui-breadcrumb-"),
        "breadcrumb styles should not define a parallel private token namespace."
    );
}
...
fn breadcrumb_spec_rs_is_not_introduced_for_simple_component() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let module_source = component_source("src/mod.rs");
    let protocol_source = component_source("src/protocol.rs");

    assert!(
        !manifest_dir.join("src/spec.rs").exists(),
        "breadcrumb is a simple navigation component; `src/spec.rs` should not be introduced without complex external schema needs."
    );
...
#[test]
fn breadcrumb_api_naming_contract_is_consistent_and_state_free() {
    let view_source = component_source("src/view.rs");
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "breadcrumb public API should include `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "breadcrumb should remain naming-consistent and avoid uncontrolled alias drift via `{forbidden}`."
        );
    }
}
...
fn breadcrumb_has_no_controlled_or_uncontrolled_state_axes() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "breadcrumb should remain a stateless display component and must not expose controlled/uncontrolled state axis token `{forbidden}`."
        );
    }
}
...
fn breadcrumb_default_values_have_single_source_in_logic() {
    ...
    for needle in [
        ...
        assert!(
            logic_source.contains(needle),
            "default normalization should live in logic.rs via `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "view.rs should consume pre-normalized root state via `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "view.rs must not re-run default fallback logic token `{forbidden}`."
        );
    }
}
...
fn breadcrumb_a11y_and_i18n_contract_is_headless_backed() {
    let view_source = component_source("src/view.rs");
    let logic_source = component_source("src/logic.rs");
    let a11y_source = component_source("../../crates/ui-headless/src/a11y.rs");
    let i18n_source = component_source("../../crates/ui-headless/src/i18n/common.rs");
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "view.rs should mount a11y/i18n contract marker `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-headless a11y shared contract should include `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-headless i18n common strings should include `{needle}` for breadcrumb."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            logic_source.contains(needle),
            "logic.rs should centralize a11y/i18n fallback and source markers via `{needle}`."
        );
    }

    assert!(
        !view_source.contains("\"/\""),
        "view.rs should not hardcode user-visible separator text; it must come from props/i18n/default chain."
    );
}
...
fn breadcrumb_state_markers_are_observable_queryable_and_stable() {
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "view.rs should expose stable observability marker `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            logic_source.contains(needle),
            "logic.rs should keep source marker as closed enumerable set token `{needle}`."
        );
    }
...
fn breadcrumb_styles_depend_on_explicit_semantic_selectors() {
    ...
    for needle in [
        ...
        assert!(
            styles_source.contains(needle),
            "styles.rs should keep explicit stable selector `{needle}`."
        );
    }
    ...
    for forbidden in [":nth-child", ":nth-of-type", "> .", " + .", " ~ ."] {
        assert!(
            !styles_source.contains(forbidden),
            "styles.rs must not rely on fragile structural selector token `{forbidden}`."
        );
    }
    ...
    for forbidden in ["style=", "style:", "set_property(", "cssText", "css_text"] {
        assert!(
            !view_source.contains(forbidden),
            "view.rs must not inject runtime business style token `{forbidden}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "visual-state explanation should remain tied to explicit semantic marker `{needle}`."
        );
    }
}
...
fn breadcrumb_token_first_static_styles_contract_is_aggregated_and_injected_via_ui_root() {
    ...
    for needle in [
        ...
        assert!(
            styles_source.contains(needle),
            "styles.rs should keep token-first static contract via `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "breadcrumb component should not adopt utility-first/CSS-in-Rust default marker `{forbidden}`."
        );
    }
}
...
fn breadcrumb_visual_desire_reuses_theme_visual_baseline_and_heroui_contracts() {
    ...
    let baseline_page_source =
        docs_source("../../apps/docs-app/src/pages/components/pages/theme_visual_baseline.rs");
    let pages_source = docs_source("../../apps/docs-app/src/pages/components/pages.rs");
    let baseline_e2e_source =
        docs_source("../../e2e/tests/docs_app_theme_visual_baseline.spec.mjs");
    let heroui_doc_source = docs_source("../../docs/spec/heroui-parameter-design-strategy.md");
...
fn breadcrumb_platform_checks_cover_native_ssr_wasm_and_non_wasm_source_guard() {
    ...
    for forbidden in [
        ...
        assert!(
            !module_source.contains(forbidden),
            "mod.rs should not include non-wasm forbidden browser token `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not include non-wasm forbidden browser token `{forbidden}`."
        );
        assert!(
            !styles_source.contains(forbidden),
            "styles.rs should not include non-wasm forbidden browser token `{forbidden}`."
        );
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not include non-wasm forbidden browser token `{forbidden}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !module_source.contains(forbidden),
            "mod.rs should not include platform split token `{forbidden}` for breadcrumb."
        );
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not include platform split token `{forbidden}` for breadcrumb."
        );
        assert!(
            !styles_source.contains(forbidden),
            "styles.rs should not include platform split token `{forbidden}` for breadcrumb."
        );
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not include platform split token `{forbidden}` for breadcrumb."
        );
    }
}
...
fn breadcrumb_ui_motion_non_wasm_stub_contract_is_preserved() {
    ...
    for forbidden in ["use ui_motion", "ui_motion::", "attach_motion(", "panic!("] {
        assert!(
            !module_source.contains(forbidden),
            "mod.rs should not include motion runtime token `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not include motion runtime token `{forbidden}`."
        );
        assert!(
            !styles_source.contains(forbidden),
            "styles.rs should not include motion runtime token `{forbidden}`."
        );
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not include motion runtime token `{forbidden}`."
        );
    }
}
...
fn breadcrumb_reduced_motion_ssr_wasm_branches_keep_semantics_consistent() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not split semantic contract by platform token `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not split semantic contract by platform token `{forbidden}`."
        );
    }
...
fn breadcrumb_type_system_and_semantic_markers_keep_machine_readable_contract() {
    ...
    let view_source = component_source("src/view.rs");
    let protocol_source = component_source("src/protocol.rs");
    let primitive_source = docs_source("../../crates/ui-state-primitives/src/breadcrumbs.rs");
    let logic_test_source = component_source("test/logic.rs");
...
fn breadcrumb_semantics_contract_tests_prioritize_contracts_over_snapshots() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "local semantic test matrix should include `{needle}`."
        );
        assert!(
...
fn breadcrumb_check2_documents_semantics_first_testing_rules() {
    let check2_source = component_source("check2.md");
...
fn breadcrumb_state_normalization_is_centralized_in_logic() {
    ...
    for needle in [
        ...
        assert!(
            logic_source.contains(needle),
            "state normalization and derivation should live in logic.rs via `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "view.rs should consume normalized state only via `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "view.rs must not rebuild state derivation details token `{forbidden}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !styles_source.contains(forbidden),
            "styles.rs must not participate in state-machine derivation via `{forbidden}`."
        );
    }
}
...
fn breadcrumb_consumes_state_primitives_without_business_store_binding() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not own primitive derivation or business-store binding token `{forbidden}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs must not directly bind app business store token `{forbidden}`."
        );
    }
}
...
#[test]
fn breadcrumb_docs_exposes_minimal_hello_world_path() {
    let source =
    ...
    for needle in [
        ...
        assert!(
            ...
            "breadcrumb docs should provide minimal hello-world path `{needle}`."
        );
    }
...
fn breadcrumb_has_no_macro_micro_drag_state_machine() {
    ...
    assert!(
        !manifest_dir.join("src/motion.rs").exists(),
        "breadcrumb has no drag physics axis, so macro/micro drag loop should remain N/A."
    );
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not implement drag micro-loop token `{forbidden}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not host drag macro-state token `{forbidden}`."
        );
    }
}
...
fn breadcrumb_has_no_two_pass_geometry_rendering_pipeline() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not implement geometry measure stage token `{forbidden}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not host geometry rectification stage token `{forbidden}`."
        );
    }
}
...
fn breadcrumb_has_no_registration_protocol_path() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not contain registration protocol token `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not contain registration protocol token `{forbidden}`."
        );
    }
...
fn breadcrumb_has_no_slot_projection_policy_path() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not include slot projection lifecycle token `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not include slot projection lifecycle token `{forbidden}`."
        );
    }
}
...
fn breadcrumb_has_no_env_stream_subscription_path() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not include env-stream sampling token `{forbidden}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not include env-stream projection token `{forbidden}`."
        );
    }
}
...
fn breadcrumb_has_no_event_light_cone_batch_selection_path() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not implement event-light-cone batch selection token `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not implement event-light-cone batch selection token `{forbidden}`."
        );
    }
...
fn breadcrumb_has_no_unified_causality_bus_path() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not include unified causality-bus token `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not include unified causality-bus token `{forbidden}`."
        );
    }
...
fn breadcrumb_has_no_focus_stack_or_overlay_restore_path() {
    ...
    for forbidden in [
        ...
        assert!(
            !module_source.contains(forbidden),
            "mod.rs should not include overlay focus-stack token `{forbidden}`."
        );
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not include overlay focus-stack token `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not include overlay focus-stack token `{forbidden}`."
        );
    }
...
fn breadcrumb_has_no_foreign_zone_escape_hatch_path() {
    ...
    for forbidden in [
        ...
        assert!(
            !module_source.contains(forbidden),
            "mod.rs should not include foreign-zone escape-hatch token `{forbidden}`."
        );
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not include foreign-zone escape-hatch token `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not include foreign-zone escape-hatch token `{forbidden}`."
        );
    }
...
fn breadcrumb_has_no_hydration_discontinuity_time_or_random_id_path() {
    ...
    for forbidden in [
        ...
        assert!(
            !module_source.contains(forbidden),
            "mod.rs should not include hydration-unstable token `{forbidden}`."
        );
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not include hydration-unstable token `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not include hydration-unstable token `{forbidden}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !module_source.contains(forbidden),
            "mod.rs should not wire id-provider token `{forbidden}` when breadcrumb has no id axis."
        );
        assert!(
            !view_source.contains(forbidden),
            "view.rs should not wire id-provider token `{forbidden}` when breadcrumb has no id axis."
        );
        assert!(
            !logic_source.contains(forbidden),
            "logic.rs should not wire id-provider token `{forbidden}` when breadcrumb has no id axis."
        );
    }
...
fn breadcrumb_performance_governance_contract_is_budgeted_traceable_and_blocking() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "breadcrumb docs route should stay on shared component-page perf pipeline via `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "docs shell should keep perf budget/probe wiring token `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "performance gate script should block regressions via `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "shared governance should keep explicit render_count follow-up token `{needle}`."
        );
    }
}
...
fn breadcrumb_view_macro_complexity_is_split_into_local_render_helpers() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "breadcrumb view should not regress to inlined nested branch token `{forbidden}`."
        );
    }
}
...
fn breadcrumb_inner_html_contract_is_n_a_and_docs_shell_is_static_whitelist_only() {
    ...
    let docs_page_source =
        docs_source("../../apps/docs-app/src/pages/components/pages/collections_breadcrumb.rs");
    let docs_shell_source = docs_source("../../apps/docs-app/src/pages/components/shell.rs");
...
fn breadcrumb_dx_contract_uses_scoped_playground_for_hot_style_and_context_retention() {
    let docs_page_source =
        docs_source("../../apps/docs-app/src/pages/components/pages/collections_breadcrumb.rs");
    let playground_source = docs_source("../../apps/docs-app/src/playground.rs");
...
fn breadcrumb_defensive_variables_use_two_level_theme_fallback_chain() {
    let styles_source = component_source("src/styles.rs");
    let theme_css_source = docs_source("../../crates/ui-theme/src/css.rs");
...
fn breadcrumb_context_compression_manifest_and_rbi_projection_are_present_and_current() {
    let manifest_source = component_source("src/Component.toml");
    let rbi_source = component_source("src/breadcrumb.rbi");
    let view_source = component_source("src/view.rs");
    let protocol_source = component_source("src/protocol.rs");
...
fn breadcrumb_documentation_entry_exists_with_beginner_first_progression() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let has_readme = manifest_dir.join("src/README.md").exists();
    let docs_page_source =
        docs_source("../../apps/docs-app/src/pages/components/pages/collections_breadcrumb.rs");
    let catalog_source = docs_source(
    ...
    breadcrumb_docs_exposes_minimal_hello_world_path();
}
...
fn breadcrumb_interactive_playground_reuses_repeatable_semantic_e2e_flow() {
    let e2e_source = docs_source("../../e2e/tests/docs_app_breadcrumb_contract.spec.mjs");
    let docs_page_source =
        docs_source("../../apps/docs-app/src/pages/components/pages/collections_breadcrumb.rs");
...
fn breadcrumb_heroui_strategy_and_component_docs_are_synchronized_and_indexable() {
    let strategy_source = docs_source("../../docs/spec/heroui-parameter-design-strategy.md");
    let pages_source = docs_source("../../apps/docs-app/src/pages/components/pages.rs");
    let catalog_source = docs_source(
        "../../apps/docs-app/src/pages/components/pages/collections_breadcrumb_catalog.rs",
    );
...
fn breadcrumb_semantics_and_performance_regression_cover_aria_data_focus_and_render_count_measurement()
 {
    ...
    let styles_source = component_source("src/styles.rs");
    let perf_probe_source = docs_source("../../apps/docs-app/src/perf_probe.rs");
    let todo_source = docs_source("../../docs/plan/TODO.md");
    ...
    for marker in [
        ...
        assert!(
            ...
            "perf probe should expose stable perf observability marker `{marker}`."
        );
    }
...
fn breadcrumb_version_deprecation_migration_is_na_without_major_breaking_upgrade() {
    let manifest_source = component_source("src/Component.toml");
    let rbi_source = component_source("src/breadcrumb.rbi");
    let mod_source = component_source("src/mod.rs");
    let logic_source = component_source("src/logic.rs");
    let view_source = component_source("src/view.rs");
    let styles_source = component_source("src/styles.rs");
    let protocol_source = component_source("src/protocol.rs");
