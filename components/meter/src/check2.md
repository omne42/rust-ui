...
- [x] `ui` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（`components/meter/src/logic.rs` 已变为 primitives 的薄装配层，`view.rs` 仅消费归一化状态。）
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（验证命令：`cargo test -p ui-state-primitives meter`、`cargo test -p ui --test meter_semantics --no-default-features --features component-meter,inject-css`、`cargo check -p ui --no-default-features --features component-meter,inject-css`、`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-meter,inject-css`、`cargo clippy -p ui --no-default-features --features component-meter,inject-css -- -D warnings`、`cargo clippy -p ui-state-primitives -- -D warnings`。）
