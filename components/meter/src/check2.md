...
- [x] `ui-motion` 定义：动效能力与契约执行层（spring、keyframes、WAAPI/RAF backend），只负责时间函数、插值与运行时驱动，不承载组件业务语义与状态决策。（`components/meter/src/motion.rs` 仅封装 spring contract 与 wasm/non-wasm attach。）
...
- [x] `ui-components` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（`components/meter/src/logic.rs` 已变为 primitives 的薄装配层，`view.rs` 仅消费归一化状态。）
