...
- 本轮执行命令：
  - `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.coachmark-check cargo check -p ui --no-default-features --features component-coachmark,inject-css`
  - `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.coachmark-check cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-coachmark,inject-css`
- `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.coachmark-check cargo clippy -p ui --no-default-features --features component-coachmark,inject-css --tests -- -D warnings`
- `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.coachmark-check cargo clippy -p ui --no-default-features --features component-coachmark,inject-css --test coachmark_semantics -- -D warnings`（全量 `--tests` 受其他测试文件阻塞）
- `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.coachmark-check cargo test -p ui --test coachmark_semantics --no-default-features --features component-coachmark,inject-css`
