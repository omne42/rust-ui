...
- 语义回归：`components/coachmark/test/coachmark_semantics.rs` 已补充 Agent Contract、feature 依赖链、checklist 完整性断言。
...
- 额外约束回归：`components/coachmark/test/coachmark_semantics.rs` 新增 README 分区断言（display/config/code/css test）与 docs workbench 断言（`Display Comparisons` + `Config + Code + CSS Test Workbench` + `test_css_source/test_config_signal`）。
- 本轮执行命令：
