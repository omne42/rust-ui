...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - `kbd` 组件最小门禁已验证：`cargo test -p ui --test kbd_semantics --no-default-features --features component-kbd,inject-css`、`cargo check -p ui --no-default-features --features component-kbd,inject-css`、`cargo check -p ui --target wasm32-unknown-unknown --no-default-features --features component-kbd,inject-css`、`cargo clippy -p ui --no-default-features --features component-kbd,inject-css --tests -- -D warnings`。
