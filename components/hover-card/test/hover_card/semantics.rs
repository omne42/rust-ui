...
#[test]
fn hover_card_does_not_introduce_spec_rs_for_simple_component_contract() {
    ...
    assert!(
        resolve_source_path("src/hover_card/spec.rs").is_none(),
        "HoverCard should not add `spec.rs` unless a stable schema/config contract requires it.",
    );

    for forbidden in ["mod spec;", "pub mod spec;", "pub use spec::"] {
        assert!(
            !mod_source.contains(forbidden),
            "HoverCard module should not expose spec layer marker `{forbidden}`.",
        );
    }
    ...
    for forbidden in ["HoverCardSpec", "spec::", ".render()"] {
        assert!(
            ...
            "HoverCard docs should not force spec-builder usage via `{forbidden}`.",
        );
    }
}

#[test]
fn hover_card_file_responsibilities_are_enforced_across_component_modules() {
    let mod_source = load_source("src/hover_card/mod.rs");
    let logic_source = load_source("src/hover_card/logic.rs");
    let styles_source = load_source("src/hover_card/styles.rs");
    ...
    for needle in [
        ...
        assert!(
            ...
            "HoverCard mod.rs should keep minimal export boundary marker `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !mod_source.contains(forbidden),
            "HoverCard mod.rs should not carry implementation detail `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "HoverCard logic.rs should expose normalization marker `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "HoverCard logic.rs should not contain view/headless/dom detail `{forbidden}`.",
        );
    }
    ...
    for needle in ["pub const CSS: &str", ".ui-hover-card", "var(--ui-"] {
        assert!(
            ...
            "HoverCard styles.rs should keep static token-first css marker `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "HoverCard styles.rs should not contain runtime logic marker `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "HoverCard view.rs should keep render/headless assembly marker `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "HoverCard view.rs should not inline engine/state-primitive internals `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "HoverCard motion.rs should keep motion contract marker `{needle}`.",
        );
    }
    ...
    for forbidden in ["view! {", "use_hover_card_trigger", "data-slot="] {
        assert!(
            ...
            "HoverCard motion.rs should not contain render/semantic mounting detail `{forbidden}`.",
        );
    }
}

#[test]
fn hover_card_component_file_responsibilities_remain_scoped() {
    let mod_source = load_source("src/hover_card/mod.rs");
    let logic_source = load_source("src/hover_card/logic.rs");
    let styles_source = load_source("src/hover_card/styles.rs");
    let view_source = load_source("src/hover_card/view.rs");
    ...
    for required in [
        "src/hover_card/mod.rs",
        "src/hover_card/logic.rs",
        "src/hover_card/styles.rs",
        "src/hover_card/view.rs",
        "src/hover_card/motion.rs",
    ] {
        assert!(
    ...
    for forbidden in ["src/hover_card/render.rs", "src/hover_card/spec.rs"] {
        assert!(
    ...
    for needle in [
        ...
        assert!(
            mod_source.contains(needle),
            "HoverCard mod.rs should keep minimal stable export marker `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !mod_source.contains(forbidden),
            "HoverCard mod.rs should not leak implementation marker `{forbidden}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            logic_source.contains(needle),
            "HoverCard logic.rs should keep normalization marker `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden),
            "HoverCard logic.rs should avoid non-logic concern `{forbidden}`."
        );
    }
    ...
    for needle in ["pub const CSS: &str", ".ui-hover-card", "var(--ui-"] {
        assert!(
            styles_source.contains(needle),
            "HoverCard styles.rs should keep static token-first marker `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !styles_source.contains(forbidden),
            "HoverCard styles.rs should avoid runtime/business marker `{forbidden}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "HoverCard view.rs should avoid engine/state primitive internals `{forbidden}`."
        );
    }
    ...
    for forbidden in ["view! {", "use_hover_card_trigger", "data-slot="] {
        assert!(
            !motion_source.contains(forbidden),
            "HoverCard motion.rs should avoid render/semantic mounting marker `{forbidden}`."
        );
    }
}
...
#[test]
fn hover_card_check2_marks_component_file_responsibility_complete() {
    ...
    for needle in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_component_file_responsibilities_remain_scoped",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_component_files_check_script_covers_scoped_responsibility_contract",
...
#[test]
fn hover_card_file_placement_discipline_is_strict_for_component_scope() {
    for required in [
        "src/hover_card/mod.rs",
        "src/hover_card/logic.rs",
        "src/hover_card/styles.rs",
    ...
    let forbidden = "src/hover_card/render.rs";
    assert!(
        !path_exists(forbidden),
    ...
    assert!(
        !path_exists("src/hover_card/spec.rs"),
        "hover-card is a simple component; `src/hover_card/spec.rs` should remain absent in this scope.",
    );

    assert!(
        path_exists("src/hover_card/protocol.rs"),
        "hover-card keeps `src/hover_card/protocol.rs` as sidecar contract file and should not regress.",
    );
}
...
#[test]
fn hover_card_check2_marks_file_placement_discipline_complete() {
    let source = load_source("../../components/hover-card/check2.md");
    ...
    for needle in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_file_placement_discipline_is_strict_for_component_scope",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_component_files_script_covers_file_placement_discipline",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_check2_marks_file_placement_discipline_complete",
        "scripts/check-ui-component-files.sh",
...
#[test]
fn hover_card_hyper_structure_builder_spec_is_not_applicable_for_simple_component() {
    let mod_source = load_source("src/hover_card/mod.rs");
    let logic_source = load_source("src/hover_card/logic.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let docs_source =
    ...
    assert!(
        !path_exists("src/hover_card/spec.rs"),
        "hover-card is a simple component; `src/hover_card/spec.rs` should remain absent for Hyper-Structure Builder gate.",
    );
...
#[test]
fn hover_card_check2_marks_hyper_structure_builder_contract_complete() {
    let source = load_source("../../components/hover-card/check2.md");
    ...
    for needle in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_check2_marks_hyper_structure_builder_contract_complete",
        "scripts/check-ui-component-files.sh",
...
#[test]
fn hover_card_context_compression_manifest_and_rbi_projection_are_present_and_current() {
    ...
    let manifest_source = load_source("../../components/hover-card/src/Component.toml");
    let rbi_source = load_source("../../components/hover-card/src/hover_card.rbi");
...
#[test]
fn hover_card_check2_marks_context_compression_manifest_and_rbi_contract_complete() {
    let source = load_source("../../components/hover-card/check2.md");
    ...
    for needle in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_component_files_check_script_covers_context_compression_manifest_contract",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_check2_marks_context_compression_manifest_and_rbi_contract_complete",
        "scripts/check-ui-component-files.sh",
...
#[test]
fn hover_card_check2_documents_agent_contract_schema_governance_rules() {
    let checklist_source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_agent_contract_is_schema_typed_and_machine_readable() {
...
#[test]
fn hover_card_agent_contract_fields_are_type_derived_without_free_form_schema_string_splicing() {
    let logic_source = load_source("../../components/hover-card/src/logic.rs");
    let view_source = load_source("../../components/hover-card/src/view.rs");
...
#[test]
fn hover_card_agent_contract_render_path_is_whitelist_safe_and_script_injection_free() {
    let view_source = load_source("../../components/hover-card/src/view.rs");
    let logic_source = load_source("../../components/hover-card/src/logic.rs");
    let styles_source = load_source("../../components/hover-card/src/styles.rs");
...
#[test]
fn hover_card_check2_marks_agent_contract_schema_governance_complete() {
...
#[test]
fn hover_card_check2_documents_streaming_definition_is_llm_output_only_with_two_modes() {
    let check2_source = load_source("../../components/hover-card/check2.md");
    let view_source = load_source("../../components/hover-card/src/view.rs");
    let logic_source = load_source("../../components/hover-card/src/logic.rs");
    let mod_source = load_source("../../components/hover-card/src/mod.rs");
...
#[test]
fn hover_card_check2_marks_streaming_two_mode_definition_complete() {
    let source = load_source("../../components/hover-card/check2.md");
    ...
    for needle in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_check2_documents_streaming_definition_is_llm_output_only_with_two_modes",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_streaming_script_covers_two_mode_definition_contract",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_check2_marks_streaming_two_mode_definition_complete",
...
#[test]
fn hover_card_check2_documents_snapshot_as_default_baseline_capability() {
    let check2_source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_snapshot_baseline_consumes_complete_result_and_renders_stably() {
    let view_source = load_source("../../components/hover-card/src/view.rs");
    let logic_source = load_source("../../components/hover-card/src/logic.rs");
    let check2_source = load_source("../../components/hover-card/check2.md");
    let script_source = load_source("../../scripts/check-ui-streaming.sh");
...
#[test]
fn hover_card_check2_marks_snapshot_baseline_capability_complete() {
    let source = load_source("../../components/hover-card/check2.md");
    ...
    for needle in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_check2_documents_snapshot_as_default_baseline_capability",
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_streaming_script_covers_snapshot_baseline_contract",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_check2_marks_snapshot_baseline_capability_complete",
        "scripts/check-ui-streaming.sh",
        "cargo test -p ui --test hover_card_semantics --no-default-features --features component-hover_card,inject-css hover_card_check2_documents_snapshot_as_default_baseline_capability",
        "cargo test -p ui --test hover_card_semantics --no-default-features --features component-hover_card,inject-css hover_card_snapshot_baseline_consumes_complete_result_and_renders_stably",
        "Invalid cross-device link (os error 18)",
...
#[test]
fn hover_card_streaming_optional_scope_keeps_role_aria_and_data_markers_continuous() {
    let view_source = load_source("../../components/hover-card/src/view.rs");
    let logic_source = load_source("../../components/hover-card/src/logic.rs");
...
#[test]
fn hover_card_streaming_validation_retry_resilience_boundaries_stay_outside_component_layer() {
    let view_source = load_source("../../components/hover-card/src/view.rs");
    let logic_source = load_source("../../components/hover-card/src/logic.rs");
    let mod_source = load_source("../../components/hover-card/src/mod.rs");
    let motion_source = load_source("../../components/hover-card/src/motion.rs");
...
#[test]
fn hover_card_check2_marks_streaming_required_optional_classification_complete() {
    let source = load_source("../../components/hover-card/check2.md");
    ...
    for needle in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_check2_documents_streaming_required_optional_classification_rules",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_streaming_optional_scope_keeps_role_aria_and_data_markers_continuous",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_streaming_validation_retry_resilience_boundaries_stay_outside_component_layer",
...
#[test]
fn hover_card_tree_shaking_contract_stays_feature_gated_in_package_and_demo_modes() {
    let cargo_source = load_source("Cargo.toml");
...
#[test]
fn hover_card_check2_marks_tree_shaking_feature_pruning_contract_complete() {
    let source = load_source("../../components/hover-card/check2.md");
    ...
    for needle in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_tree_shaking_contract_stays_feature_gated_in_package_and_demo_modes",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_tree_shaking_script_enforces_component_minimal_feature_tree_and_budget",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_check2_marks_tree_shaking_feature_pruning_contract_complete",
...
fn hover_card_logic_exposes_state_helpers() {
    ...
    for needle in [
        ...
        "pub fn resolve_id(custom_id: Option<String>, fallback_id: Cow<'static, str>)",
...
#[test]
fn hover_card_state_primitives_source_is_locked() {
    let source = load_source("src/hover_card/logic.rs");
    ...
    for needle in [
        ...
        assert!(
            source.contains(needle),
            "HoverCard logic should consume ui-state-primitives via `{needle}`."
        );
    }
    ...
    for forbidden in ["store::", "app_state::", "global_state::", "use_context::<"] {
        assert!(
            ...
            "HoverCard logic should not depend on business store via `{forbidden}`."
        );
    }
}

#[test]
fn hover_card_async_interaction_contract_is_not_applicable() {
    let logic_source = load_source("src/hover_card/logic.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let motion_source = load_source("src/hover_card/motion.rs");
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden)
                && !view_source.contains(forbidden)
                && !motion_source.contains(forbidden),
            "HoverCard should keep async contract as N/A; found `{forbidden}`."
        );
    }
}

#[test]
fn hover_card_api_keeps_basic_usage_simple_without_state_object_wiring() {
    let view_source = load_source("src/hover_card/view.rs");
    ...
    for needle in ["#[prop(into)] content: ViewFn", "children: Children"] {
        assert!(
            ...
            "HoverCard simple API should keep `{needle}` as baseline inputs."
        );
    }
    ...
    for forbidden in ["state:", "state =", "state="] {
        assert!(
            ...
            "HoverCard should not require internal state object wiring via `{forbidden}`."
        );
    }
}
...
fn hover_card_view_uses_logic_state_contracts() {
    ...
    for forbidden in [
        ...
        assert!(
            !source.contains(forbidden),
            "HoverCard view should not keep delay defaults in view via `{forbidden}`."
        );
    }
}

#[test]
fn hover_card_discrete_state_contracts_are_enum_backed() {
    let view_source = load_source("src/hover_card/view.rs");
    let logic_source = load_source("src/hover_card/logic.rs");
    ...
    for needle in [
        ...
        assert!(
            ...
            "HoverCard should keep enum-backed discrete states via `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "HoverCard should not introduce stringly discrete states via `{forbidden}`."
        );
    }
}

#[test]
fn hover_card_type_system_and_semantic_markers_form_machine_readable_contract() {
    let view_source = load_source("src/hover_card/view.rs");
    let logic_source = load_source("src/hover_card/logic.rs");
    let component_logic_test_source = load_source("../../components/hover-card/test/logic.rs");
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle) || logic_source.contains(needle),
            "hover-card machine-readable typing contract should include `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "hover-card machine-readable marker contract should include `{needle}`.",
        );
    }

    for needle in [
        "open_state_normalization_prefers_is_open_and_marks_controlled_source",
        "machine_readable_state_attr_set_is_closed_and_enumerable",
    ] {
        assert!(
            component_logic_test_source.contains(needle),
            "hover-card logic tests should pin normalization/closed-set feedback via `{needle}`.",
        );
    }
}

#[test]
fn hover_card_focus_stack_gc_is_not_applicable_for_non_modal_overlay_contract() {
    let view_source = load_source("src/hover_card/view.rs");
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "HoverCard should keep focus-stack restore protocol as N/A; found `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "HoverCard should keep focus handling scoped to aria-describedby wiring via `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "Global overlay focus manager should stay implemented in ui-headless focus_trap via `{needle}`.",
        );
    }
}

#[test]
fn hover_card_escape_hatches_foreign_zone_is_not_applicable() {
    let mod_source = load_source("src/hover_card/mod.rs");
    let logic_source = load_source("src/hover_card/logic.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let motion_source = load_source("src/hover_card/motion.rs");
    let headless_source = load_source("../ui-headless/src/hover_card.rs");
    let state_source = load_source("../ui-state-primitives/src/hover_card.rs");
    let docs_source =
        load_source("../../apps/docs-app/src/pages/components/pages/overlays_hover_card.rs");
    ...
    for forbidden in [
        ...
        assert!(
            !mod_source.contains(forbidden)
                ...
                && !docs_source.contains(forbidden),
            "HoverCard should keep foreign-zone escape hatch contract as N/A; found `{forbidden}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "HoverCard public api should not expose imperative foreign-instance handle `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "HoverCard should keep API/state contracts pure without foreign-instance pollution via `{needle}`.",
        );
    }
}

#[test]
fn hover_card_hydration_discontinuity_uses_seeded_id_provider_contract() {
    let view_source = load_source("src/hover_card/view.rs");
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "HoverCard hydration id contract should include `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "HoverCard should not use non-deterministic id source `{forbidden}` in view.",
        );
    }
...
#[test]
fn hover_card_ssr_cross_platform_contract_covers_web_ssr_wasm_and_non_wasm_guards() {
    let logic_source = load_source("src/hover_card/logic.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let motion_source = load_source("src/hover_card/motion.rs");
    let headless_hover_source = load_source("../ui-headless/src/hover_card.rs");
    ...
    for needle in [
        ...
        assert!(
            motion_source.contains(needle),
            "HoverCard motion should keep explicit platform cfg branch `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden) && !view_source.contains(forbidden),
            "HoverCard non-wasm component path should not reference browser object `{forbidden}`.",
        );
    }
...
fn hover_card_ui_headless_web_ssr_mutex_contract_is_enforced() {
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "HoverCard should consume ui-headless contract via `{needle}` without bypassing feature boundaries.",
        );
    }
}
...
fn hover_card_ui_motion_non_wasm_noop_stub_contract_is_enforced() {
    ...
    let non_wasm_start = motion_source
        ...
        .unwrap_or_else(|| panic!("HoverCard motion should define non-wasm attach branch."));
    let non_wasm_end = motion_source[non_wasm_start..]
        .find("\n#[cfg(test)]")
        .map(|offset| non_wasm_start + offset)
        .unwrap_or_else(|| panic!("HoverCard motion non-wasm branch should end before cfg(test)."));
    ...
    for needle in [
        ...
        assert!(
            ...
            "HoverCard non-wasm motion fallback should include `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "HoverCard non-wasm motion fallback should avoid runtime-dependent marker `{forbidden}`.",
        );
    }
}

#[test]
fn hover_card_reduced_motion_ssr_wasm_branches_keep_semantics_consistent() {
    let view_source = load_source("src/hover_card/view.rs");
    let motion_source = load_source("src/hover_card/motion.rs");
    let headless_hover_source = load_source("../ui-headless/src/hover_card.rs");
    let platforms_script_source = load_source("../../scripts/check-ui-platforms.sh");
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "HoverCard semantic output should keep stable marker `{needle}` across platform branches.",
        );
    }

    assert!(
        !view_source.contains("#[cfg("),
        "HoverCard view should not split semantic markup by target cfg.",
    );
    ...
    for needle in [
        ...
        assert!(
            motion_source.contains(needle),
            "HoverCard motion should include reduced-motion/wasm/non-wasm branch marker `{needle}`.",
        );
    }
...
#[test]
fn hover_card_motion_contract_is_component_scoped_reduced_motion_aware_and_non_wasm_safe() {
    let motion_source = load_source("src/hover_card/motion.rs");
    let motion_test_source = load_source("../../components/hover-card/test/motion.rs");
    let view_source = load_source("src/hover_card/view.rs");
...
#[test]
fn hover_card_check2_marks_motion_contractualization_complete() {
    let source = load_source("../../components/hover-card/check2.md");
    ...
    for needle in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_motion_contract_is_component_scoped_reduced_motion_aware_and_non_wasm_safe",
        "Invalid cross-device link (os error 18)",
    ] {
        assert!(
            source.contains(needle),
...
#[test]
fn hover_card_ui_components_fixed_entry_files_follow_layered_boundaries() {
    let lib_source = load_source("src/lib.rs");
    let css_source = load_source("src/css.rs");
    let root_source = load_source("src/root.rs");
...
#[test]
fn hover_card_check2_marks_ui_components_fixed_entry_files_complete() {
    let source = load_source("../../components/hover-card/check2.md");
    ...
    for needle in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_check2_marks_ui_components_fixed_entry_files_complete",
...
fn hover_card_performance_governance_contract_is_budgeted_traceable_and_blocking() {
    ...
    let hover_card_view_source = load_source("src/hover_card/view.rs");
    ...
    for needle in [
        ...
        assert!(
            ...
            "docs shell should keep hover-card performance baseline marker `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "hover-card docs page should remain in coverage traversal via `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            hover_card_view_source.contains(needle),
            "hover-card view should keep performance attribution marker `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !hover_card_view_source.contains(forbidden),
            "hover-card view should avoid high-frequency perf flood marker `{forbidden}`."
        );
    }
...
fn hover_card_semantics_and_performance_regression_cover_aria_data_focus_and_render_count_measurement()
 {
    let local_semantics = load_source("../../components/hover-card/test/semantics.rs");
    let aggregated_semantics = load_source("tests/hover_card/semantics.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let headless_hover_source = load_source("../ui-headless/src/hover_card.rs");
    let todo_source = load_source("../../docs/plan/TODO.md");
...
#[test]
fn hover_card_check2_marks_semantics_and_performance_regression_contract_complete() {
    let check2_source = load_source("../../components/hover-card/check2.md");
    ...
    for marker in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_semantics_and_performance_regression_cover_aria_data_focus_and_render_count_measurement",
...
fn hover_card_view_macro_complexity_is_split_into_semantic_subrenders() {
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "HoverCard view should keep semantic split marker `{needle}` to bound macro expansion.",
        );
    }
    ...
    assert!(
        view_macro_count >= 3,
        "HoverCard view macro count should reflect semantic split (expected >=3, found {view_macro_count}).",
    );

    for needle in [
        "`view!` 宏复杂度受控",
        "hover_card_view_macro_complexity_is_split_into_semantic_subrenders_locally",
        "hover_card_view_macro_complexity_is_split_into_semantic_subrenders",
    ] {
        assert!(
            check2_source.contains(needle),
            "HoverCard checklist should keep view-macro complexity governance marker `{needle}`.",
        );
    }
...
fn hover_card_view_functional_split_prefers_plain_functions_over_local_components() {
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "HoverCard view should keep function-first split marker `{needle}`.",
        );
    }
    ...
    assert_eq!(
        ...
        "HoverCard should keep only root `#[component]`; found {component_attr_count}.",
    );
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "HoverCard view should avoid non-function split marker `{forbidden}`.",
        );
    }

    for needle in [
        "函数式拆分优先",
        "hover_card_view_functional_split_prefers_plain_functions_over_local_components",
        "components/hover-card/test/semantics.rs::hover_card_view_functional_split_prefers_plain_functions_over_local_components",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_view_functional_split_prefers_plain_functions_over_local_components",
    ] {
        assert!(
            check2_source.contains(needle),
            "HoverCard checklist should keep function-first governance marker `{needle}`.",
        );
    }
...
fn hover_card_static_fragments_are_constantized_or_absent_for_simple_layout() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "HoverCard simple layout should avoid heavyweight inline static fragment marker `{forbidden}`.",
        );
    }

    assert!(
        view_source.contains("{move || content.with_value(|content| content.run())}"),
        "HoverCard panel should keep external content slot instead of embedding large static fragments.",
    );

    for needle in [
        "静态片段常量化",
        "hover_card_static_fragments_are_constantized_or_absent_for_simple_layout_locally",
        "hover_card_static_fragments_are_constantized_or_absent_for_simple_layout",
    ] {
        assert!(
            check2_source.contains(needle),
            "HoverCard checklist should keep static-fragment governance marker `{needle}`.",
        );
    }
...
#[test]
fn hover_card_inner_html_usage_is_explicitly_na_and_guarded() {
    for rel_path in [
        "src/hover_card/mod.rs",
        "src/hover_card/logic.rs",
        "src/hover_card/styles.rs",
        ...
        for forbidden in [
            ...
            assert!(
                !source.contains(forbidden),
                "HoverCard source `{rel_path}` must not contain raw-html injection token `{forbidden}`.",
            );
        }
    }
    ...
    for forbidden in [
        ...
        assert!(
            !docs_source.contains(forbidden),
            "HoverCard docs examples must not contain raw-html injection token `{forbidden}`.",
        );
    }
    ...
    for needle in [
        "`inner_html` 使用约束",
        "零注入面",
        "hover_card_inner_html_usage_is_explicitly_na_and_guarded_locally",
        "hover_card_inner_html_usage_is_explicitly_na_and_guarded",
    ] {
        assert!(
            check2_source.contains(needle),
            "HoverCard checklist should keep inner_html security evidence `{needle}`.",
        );
    }
...
#[test]
fn hover_card_wasm_debug_contract_reuses_global_trace_and_stays_feature_isolated() {
    let cargo_source = load_source("Cargo.toml");
    ...
    assert!(
        ...
        "HoverCard should not define a component-local wasm-debug feature that leaks into production API surface.",
    );
    ...
    for needle in [
        ...
        assert!(
            ...
            "HoverCard should keep replayable state/source marker `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "HoverCard docs playground should keep minimal replay path marker `{needle}`.",
        );
    }

    for needle in [
        "WASM 调试要求",
        "hover_card_wasm_debug_contract_reuses_global_trace_and_stays_feature_isolated_locally",
        "hover_card_wasm_debug_contract_reuses_global_trace_and_stays_feature_isolated",
    ] {
        assert!(
            check2_source.contains(needle),
            "HoverCard checklist should keep wasm-debug governance marker `{needle}`.",
        );
    }
...
#[test]
fn hover_card_dx_playground_supports_css_hot_reload_without_wasm_rebuild() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "HoverCard docs should mount reusable Playground hot-reload path via `{needle}`.",
        );
    }
}

#[test]
fn hover_card_dx_interactive_scope_keeps_isolated_canvas_and_context_visible_with_optional_persist_na()
 {
    let playground_source = load_source("../../apps/docs-app/src/playground.rs");
    let docs_source =
        load_source("../../apps/docs-app/src/pages/components/pages/overlays_hover_card.rs");
    ...
    for needle in [
        ...
        assert!(
            docs_source.contains(needle),
            "HoverCard docs should keep context-preserving interactive marker `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !docs_source.contains(forbidden),
            "HoverCard keeps optional persisted workbench state as N/A for current scope; `{forbidden}` should remain absent.",
        );
    }
    ...
    for required in [
        ...
        "hover_card_dx_playground_supports_css_hot_reload_without_wasm_rebuild",
        "hover_card_dx_interactive_scope_keeps_isolated_canvas_and_context_visible_with_optional_persist_na",
    ] {
        assert!(
            check2_source.contains(required),
            "HoverCard checklist should keep DX governance marker `{required}`.",
        );
    }
}
...
#[test]
fn hover_card_docs_are_copy_paste_ready_with_imports_and_streaming_snapshot_contract() {
    let docs_source =
        load_source("../../apps/docs-app/src/pages/components/pages/overlays_hover_card.rs");
    let playground_source = load_source("../../apps/docs-app/src/playground.rs");
...
#[test]
fn hover_card_check2_marks_docs_product_copy_paste_ready_contract_complete() {
    let source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_check2_documents_interactive_playground_rules() {
    let check2_source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_docs_app_provides_interactive_playground_for_props_state_and_preview() {
    let docs_source =
        load_source("../../apps/docs-app/src/pages/components/pages/overlays_hover_card.rs");
...
#[test]
fn hover_card_interactive_playground_reuses_repeatable_semantic_e2e_flow() {
...
#[test]
fn hover_card_check2_marks_interactive_playground_item_complete() {
    let check2_source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_delegates_escape_and_describedby_to_headless() {
    let view_source = load_source("src/hover_card/view.rs");
    let headless_source = load_source("../ui-headless/src/hover_card.rs");

    assert!(
        view_source.contains("use_hover_card_focus_a11y")
            && view_source.contains("use_hover_card_dismiss"),
        "HoverCard view should delegate focus/keyboard semantics to headless contracts."
    );
    assert!(
        headless_source.contains("set_attribute(\"aria-describedby\"")
            && headless_source.contains("remove_attribute(\"aria-describedby\""),
        "HoverCard headless contract should manage focused-element `aria-describedby`."
    );
    assert!(
        headless_source.contains("should_dismiss_on_escape")
            && headless_source.contains("ev.stop_propagation();")
            && headless_source.contains("ev.prevent_default();"),
        "HoverCard headless dismiss contract should own Escape normalization and dismissal side effects."
    );
}
...
#[test]
fn hover_card_styles_depend_on_explicit_markers_and_css_var_runtime_payload_only() {
    let styles_source = load_source("src/hover_card/styles.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let logic_source = load_source("src/hover_card/logic.rs");
    ...
    for needle in [
        ...
        assert!(
            styles_source.contains(needle),
            "HoverCard styles should key state branches with explicit marker `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !styles_source.contains(forbidden),
            "HoverCard styles should not guess state from brittle structure selector `{forbidden}`.",
        );
    }

    assert!(
        view_source.contains("style=panel_vars"),
        "HoverCard runtime style should stay on css-var payload via `style=panel_vars`.",
    );
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "HoverCard view should not inline business style branch via `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            logic_source.contains(needle),
            "HoverCard runtime style contract should stay css-variable-only via `{needle}`.",
        );
    }
}

#[test]
fn hover_card_styles_consume_theme_overlay_tokens() {
    let source = load_source("src/hover_card/styles.rs");
    ...
    for needle in [
        ...
        assert!(
            source.contains(needle),
            "HoverCard styles should consume theme overlay token `{needle}`."
        );
    }

    assert!(
        !source.contains("z-index: 1000;"),
        "HoverCard styles should not hardcode overlay z-index when theme tokens exist."
    );
}

#[test]
fn hover_card_cascade_layer_and_runtime_style_contract_is_enforced() {
    ...
    assert!(
        view_source.contains("style=panel_vars"),
        "HoverCard runtime style should stay css-var-only via `style=panel_vars`.",
    );
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "HoverCard view should not use plain inline style branch `{forbidden}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            logic_source.contains(needle),
            "HoverCard runtime style payload should stay custom-property only via `{needle}`."
        );
    }
}
...
#[test]
fn hover_card_check2_marks_cascade_layer_runtime_style_contract_complete() {
    let source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_styles_use_defensive_variable_fallback_chain() {
    let styles_source = load_source("src/hover_card/styles.rs");
...
#[test]
fn hover_card_check2_marks_defensive_variables_contract_complete() {
    let source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_token_first_styles_flow_through_css_aggregation_and_ui_root_injection() {
    let css_source = load_source("src/css.rs");
    let root_source = load_source("src/root.rs");
    let component_styles_source = load_source("src/hover_card/styles.rs");
    ...
    for needle in [
        ...
        assert!(
            root_source.contains(needle),
            "UiRoot should keep css injection hook `{needle}` for component styles.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "hover-card styles should stay token-first via `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !hover_card_view_source.contains(forbidden),
            "hover-card component contract should avoid utility/css-in-rust default marker `{forbidden}`.",
        );
    }
}
...
#[test]
fn hover_card_docs_include_short_hello_world_default_path() {
    let source =
        load_source("../../apps/docs-app/src/pages/components/pages/overlays_hover_card.rs");

    assert!(
        source.contains("<Playground title=\"HoverCard\" code_signal=code>"),
    ...
    let snippet_end = source[snippet_start..]
        .find("</HoverCard>")
...
#[test]
fn hover_card_default_theme_visual_desire_contract_is_documented_with_baseline_anchor() {
    let docs_source =
        load_source("../../apps/docs-app/src/pages/components/pages/overlays_hover_card.rs");
    let styles_source = load_source("src/hover_card/styles.rs");
...
#[test]
fn hover_card_composite_parent_item_api_is_not_applicable() {
    let view_source = load_source("src/hover_card/view.rs");
    let docs_source =
        load_source("../../apps/docs-app/src/pages/components/pages/overlays_hover_card.rs");
    let signature_start = view_source
    ...
    assert!(
        ...
        "HoverCard should expose explicit child composition entrypoint."
    );

    for forbidden in ["labels:", "titles:", "panels:", "item_specs:", "ItemSpec"] {
        assert!(
            !signature.contains(forbidden),
            "HoverCard should not expose collection sugar via `{forbidden}`."
        );
    }
    ...
    for forbidden in ["labels=", "titles=", "panels=", "item_specs="] {
        assert!(
            !docs_source.contains(forbidden),
            "HoverCard docs should not recommend collection array binding via `{forbidden}`."
        );
    }
}

#[test]
fn hover_card_macro_micro_drag_state_machine_is_not_applicable() {
    let logic_source = load_source("src/hover_card/logic.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let motion_source = load_source("src/hover_card/motion.rs");
    ...
    for forbidden in ["Dragging", "Action::DragEnd", "on:pointermove", "on:drag"] {
        assert!(
            !logic_source.contains(forbidden)
                && !view_source.contains(forbidden)
                && !motion_source.contains(forbidden),
            "HoverCard should keep macro/micro drag state machine as N/A; found `{forbidden}`."
        );
    }
}

#[test]
fn hover_card_two_pass_geometry_rendering_contract_is_wired_with_idempotent_guard() {
    let view_source = load_source("src/hover_card/view.rs");
    let headless_source = load_source("../ui-headless/src/popover_position.rs");
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "HoverCard view should keep two-pass geometry wiring marker `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "headless popover position contract should keep rectification/idempotent guard marker `{needle}`.",
        );
    }
}

#[test]
fn hover_card_registration_protocol_is_not_applicable() {
    let logic_source = load_source("src/hover_card/logic.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let motion_source = load_source("src/hover_card/motion.rs");
    let headless_source = load_source("../ui-headless/src/hover_card.rs");
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden)
                ...
                && !motion_source.contains(forbidden)
                && !headless_source.contains(forbidden),
            "HoverCard should keep collection registration protocol as N/A; found `{forbidden}`.",
        );
    }
}

#[test]
fn hover_card_slot_projection_strategy_is_not_applicable() {
    let logic_source = load_source("src/hover_card/logic.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let motion_source = load_source("src/hover_card/motion.rs");
    let headless_source = load_source("../ui-headless/src/hover_card.rs");
    ...
    for forbidden in ["Lazy", "KeepAlive", "Eager", "NotifyHidden"] {
        assert!(
            !logic_source.contains(forbidden)
                ...
                && !motion_source.contains(forbidden)
                && !headless_source.contains(forbidden),
            "HoverCard should keep slot projection strategy as N/A; found `{forbidden}`.",
        );
    }
}

#[test]
fn hover_card_env_streams_are_sampled_in_headless_and_not_flooded_in_view() {
    let view_source = load_source("src/hover_card/view.rs");
    let headless_source = load_source("../ui-headless/src/popover_position.rs");
    ...
    for needle in [
        ...
        assert!(
            ...
            "HoverCard env stream contract should include sampled headless marker `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "HoverCard view should not flood raw env events via `{forbidden}`.",
        );
    }
}

#[test]
fn hover_card_event_light_cone_is_not_applicable() {
    let logic_source = load_source("src/hover_card/logic.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let motion_source = load_source("src/hover_card/motion.rs");
    let headless_source = load_source("../ui-headless/src/hover_card.rs");
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden)
                ...
                && !motion_source.contains(forbidden)
                && !headless_source.contains(forbidden),
            "HoverCard should keep event light cone as N/A; found `{forbidden}`.",
        );
    }
}

#[test]
fn hover_card_causality_bus_is_not_applicable() {
    let logic_source = load_source("src/hover_card/logic.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let motion_source = load_source("src/hover_card/motion.rs");
    let headless_source = load_source("../ui-headless/src/hover_card.rs");
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden)
                ...
                && !motion_source.contains(forbidden)
                && !headless_source.contains(forbidden),
            "HoverCard should keep causality bus as N/A; found `{forbidden}`.",
        );
    }
}

#[test]
fn hover_card_a11y_i18n_l10n_contract_is_wired_without_hardcoded_copy() {
    let view_source = load_source("src/hover_card/view.rs");
    let headless_source = load_source("../ui-headless/src/hover_card.rs");
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "HoverCard a11y/i18n contract should include `{needle}`.",
        );
    }
    ...
    for forbidden in ["view! { \"", "view!{\""] {
        assert!(
            !view_source.contains(forbidden),
            "HoverCard view should not hardcode user-visible copy via `{forbidden}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            headless_source.contains(needle),
            "HoverCard should delegate shared a11y helpers to headless via `{needle}`.",
        );
    }
}

#[test]
fn hover_card_state_markers_are_observable_queryable_and_enum_bounded() {
    let view_source = load_source("src/hover_card/view.rs");
    let logic_source = load_source("src/hover_card/logic.rs");
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "HoverCard observable state marker contract should include `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            logic_source.contains(needle),
            "HoverCard source marker values should stay enum-bounded via `{needle}`.",
        );
    }
}
...
fn hover_card_semantic_contract_tests_cover_state_and_interaction_matrix_without_snapshot_dependency()
 {
    ...
    let motion_source = load_source("src/hover_card/motion.rs");
    let headless_source = load_source("../ui-headless/src/hover_card.rs");
    let semantics_source = load_source("tests/hover_card/semantics.rs");
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "HoverCard semantics should expose role/aria/state/source marker `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle) || logic_source.contains(needle),
            "HoverCard semantics matrix should include controlled/uncontrolled branch marker `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "HoverCard semantics matrix should include disabled branch marker `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "HoverCard semantics matrix should include keyboard path marker `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "HoverCard semantics matrix should include pointer path marker `{needle}`.",
        );
    }
    ...
    for needle in [
        ...
        assert!(
            motion_source.contains(needle),
            "HoverCard semantics matrix should include wasm/ssr branch marker `{needle}`.",
        );
    }
    ...
    assert!(
        ...
        "HoverCard semantic tests should not be replaced by visual snapshot-only assertions.",
    );
}

#[test]
fn hover_card_check2_documents_semantics_first_testing_rules() {
    let check2_source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_semantics_suite_is_contract_first_not_snapshot_only() {
    let view_source = load_source("src/hover_card/view.rs");
    let local_semantics_source = load_source("../../components/hover-card/test/semantics.rs");
    let aggregated_source = load_source("tests/hover_card/semantics.rs");
    ...
    for needle in [
        ...
        "fn hover_card_semantic_contract_tests_cover_state_and_interaction_matrix_without_snapshot_dependency()",
        "fn hover_card_semantics_suite_is_contract_first_not_snapshot_only()",
        "fn hover_card_semantic_markers_changed_in_view_must_be_covered_by_semantics_checks()",
    ] {
        assert!(
            local_semantics_source.contains(needle) || aggregated_source.contains(needle),
            "hover-card semantics-first suites should include `{needle}`.",
        );
    }
    ...
    let insta_snapshot = ["insta::assert", "_snapshot"].concat();
    assert!(
        !aggregated_source.contains(&snapshot_macro)
            && !aggregated_source.contains(&insta_snapshot),
...
fn hover_card_semantic_markers_changed_in_view_must_be_covered_by_semantics_checks() {
    ...
    for marker in [
        ...
        assert!(
            local_semantics_source.contains(marker) || aggregated_source.contains(marker),
            "hover-card semantics suites should cover changed semantic marker `{marker}`.",
        );
    }
}
...
#[test]
fn hover_card_check2_marks_semantics_first_testing_complete() {
    let check2_source = load_source("../../components/hover-card/check2.md");
    ...
    for needle in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_check2_documents_semantics_first_testing_rules",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_semantics_suite_is_contract_first_not_snapshot_only",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_semantic_markers_changed_in_view_must_be_covered_by_semantics_checks",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_contract_hygiene_script_covers_semantics_first_contract_guards",
        "scripts/check-ui-contract-hygiene.sh",
        "Invalid cross-device link (os error 18)",
    ] {
        assert!(
...
#[test]
fn hover_card_motion_uses_theme_overlay_tokens_and_reduced_motion_guard() {
    let source = load_source("src/hover_card/motion.rs");
    ...
    for needle in [
        ...
        assert!(
            source.contains(needle),
            "hover card motion should include `{needle}` for token-first and reduced-motion/no-op contracts."
        );
    }
}
...
#[test]
fn hover_card_engineering_contract_marks_spec_serde_path_as_na_for_simple_component_scope() {
    let mod_source = load_source("src/hover_card/mod.rs");
    let logic_source = load_source("src/hover_card/logic.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let styles_source = load_source("src/hover_card/styles.rs");
...
#[test]
fn hover_card_engineering_contract_keeps_tracing_semantics_unified_without_component_local_events()
{
    let cargo_source = load_source("Cargo.toml");
    ...
    let combined = [
        load_source("src/hover_card/mod.rs"),
        load_source("src/hover_card/logic.rs"),
        load_source("src/hover_card/view.rs"),
        load_source("src/hover_card/styles.rs"),
        load_source("src/hover_card/motion.rs"),
    ]
    .join("\n");
...
#[test]
fn hover_card_engineering_contract_avoids_runtime_leaks_in_public_api_surface() {
    let mod_source = load_source("src/hover_card/mod.rs");
    let logic_source = load_source("src/hover_card/logic.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let styles_source = load_source("src/hover_card/styles.rs");
...
#[test]
fn hover_card_version_deprecation_migration_is_na_without_major_breaking_upgrade() {
    let manifest_source = load_source("../../components/hover-card/src/Component.toml");
    let rbi_source = load_source("../../components/hover-card/src/hover_card.rbi");
    let mod_source = load_source("src/hover_card/mod.rs");
    let logic_source = load_source("src/hover_card/logic.rs");
    ...
    let styles_source = load_source("src/hover_card/styles.rs");
    let motion_source = load_source("src/hover_card/motion.rs");
    let protocol_source = load_source("../../components/hover-card/src/protocol.rs");
    let check2_source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_rust_hygiene_non_test_source_forbids_unwrap_expect_and_ignored_bindings() {
    let combined = [
        load_source("src/hover_card/mod.rs"),
        load_source("src/hover_card/logic.rs"),
        load_source("src/hover_card/styles.rs"),
        load_source("src/hover_card/view.rs"),
        load_source("src/hover_card/motion.rs"),
        load_source("src/hover_card/protocol.rs"),
    ]
    .join("\n");
...
#[test]
fn hover_card_rust_hygiene_id_fallback_path_converges_to_cow() {
    let logic_source = load_source("src/hover_card/logic.rs");
    let view_source = load_source("src/hover_card/view.rs");
    let state_source = load_source("../ui-state-primitives/src/hover_card.rs");
...
#[test]
fn hover_card_check2_marks_rust_hygiene_gate_complete() {
    let check2_source = load_source("../../components/hover-card/check2.md");
    ...
    for needle in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_rust_hygiene_non_test_source_forbids_unwrap_expect_and_ignored_bindings",
        "components/hover-card/test/hover_card/semantics.rs::hover_card_rust_hygiene_id_fallback_path_converges_to_cow",
...
#[test]
fn hover_card_check2_documents_e2e_selector_and_stable_wait_rules() {
    let check2_source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_e2e_selector_contract_uses_semantic_markers_and_settled_waits() {
    let e2e_source = load_source("../../e2e/tests/docs_app_hover_card_contract.spec.mjs");
    let docs_source =
        load_source("../../apps/docs-app/src/pages/components/pages/overlays_hover_card.rs");
...
#[test]
fn hover_card_e2e_contract_covers_ready_and_settled_conditions_for_overlay_paths() {
    let e2e_source = load_source("../../e2e/tests/docs_app_hover_card_contract.spec.mjs");
...
#[test]
fn hover_card_check2_marks_e2e_selector_stability_item_complete() {
    let check2_source = load_source("../../components/hover-card/check2.md");
    ...
    for marker in [
        ...
        "components/hover-card/test/hover_card/semantics.rs::hover_card_e2e_selector_contract_uses_semantic_markers_and_settled_waits",
...
#[test]
fn hover_card_check2_documents_e2e_repeatable_key_flow_rules() {
    let check2_source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_e2e_key_flow_is_repeatable_and_failure_points_are_semantic() {
    let e2e_source = load_source("../../e2e/tests/docs_app_hover_card_contract.spec.mjs");
...
#[test]
fn hover_card_e2e_high_risk_paths_cover_focus_keyboard_and_settled_semantic_breakpoints() {
    let e2e_source = load_source("../../e2e/tests/docs_app_hover_card_contract.spec.mjs");
...
#[test]
fn hover_card_check2_marks_replayable_e2e_critical_flow_item_complete() {
    let check2_source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_check2_documents_docs_sync_and_state_matrix_rules() {
    let check2_source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_docs_examples_and_state_matrix_sync_with_logic_api_names_and_defaults() {
    let docs_source =
        load_source("../../apps/docs-app/src/pages/components/pages/overlays_hover_card.rs");
    let logic_source = load_source("../../components/hover-card/src/logic.rs");
    let view_source = load_source("../../components/hover-card/src/view.rs");
...
#[test]
fn hover_card_check2_marks_docs_sync_and_state_matrix_item_complete() {
    let source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_check2_documents_documentation_as_product_rules() {
    let check2_source = load_source("../../components/hover-card/check2.md");
...
#[test]
fn hover_card_documentation_entry_exists_with_beginner_first_progression() {
    let readme_source = load_source("../../components/hover-card/src/README.md");
    let pages_source = load_source("../../apps/docs-app/src/pages/components/pages.rs");
    let docs_source =
        load_source("../../apps/docs-app/src/pages/components/pages/overlays_hover_card.rs");
...
#[test]
fn hover_card_check2_marks_documentation_as_product_item_complete() {
    let check2_source = load_source("../../components/hover-card/check2.md");
