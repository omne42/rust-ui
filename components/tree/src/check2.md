...
- [x] `ui` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（`Tree` 目录已形成 `logic.rs`（装配映射）+ `view.rs`（结构/契约挂载）+ `styles.rs`（静态 token-first）+ `motion.rs`（attach）；公共导出保持 `Tree/TreeNode/TreeTone/TreeDensity/TreeMotion`，未暴露 DOM 细节类型。）
...
- [x] 组件层遵循 token-first 静态样式契约：样式通过 `styles.rs` 聚合注入；运行时仅传必要 CSS 变量；不把 Utility-First/CSS-in-Rust 当组件库默认范式。（`tree/styles.rs` 使用 `var(--ui-*)` token；`ui/src/css.rs` 通过 `component-tree` feature 条件聚合；运行时仅 `--ui-tree-motion-*`。）
...
- [x] Tree Shaking 是一等能力：package 模式支持组件级 feature；source 模式天然裁剪；样式层同步裁剪，禁止无条件聚合全部 CSS，禁止破坏 DCE/LTO 的全量中央注册表。（`ui` 已有 `component-tree` feature；`src/lib.rs` 与 `src/css.rs` 均按 `#[cfg(feature = "component-tree")]` 条件导出/聚合，`tree_semantics` 增加静态断言。）
