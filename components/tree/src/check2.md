...
### 1. 大骨架（架构边界与层职责）
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（`Tree` 状态原语已下沉到 `crates/ui-state-primitives/src/tree.rs` 并在 `crates/ui-state-primitives/src/lib.rs` 导出；组件侧 `components/tree/src/logic.rs` 仅消费原语并做 tone/density 映射。）
...
- [x] `ui-headless` 定义：交互与 A11y 原语层（press/focus/hover/roving/listbox/menu/tooltip 等），把输入设备事件与状态语义标准化为可复用契约；输出必须是类型化 `attrs + handlers + state`。不做样式、不写组件 CSS、不做组件级动效编排。（`Tree` 交互/A11y 契约已下沉到 `crates/ui-headless/src/tree.rs`，组件 `view.rs` 仅挂载 `tree_root_attrs/use_tree_item` 输出。）
...
- [x] `ui-motion` 定义：动效能力与契约执行层（spring、keyframes、WAAPI/RAF backend），只负责时间函数、插值与运行时驱动，不承载组件业务语义与状态决策。（`Tree` 新增 `components/tree/src/motion.rs`，仅做语义状态到 `ui-motion` spring driver 的映射与 attach，non-wasm 提供 noop。）
...
- [x] `ui-theme` 定义：唯一设计 token 与主题上下文层（system/color/scale + Light/Dark/OLED），负责 token 分类、主题映射与 CSS 变量生成。（`Tree` 样式仍集中在 `components/tree/src/styles.rs` 并消费 `var(--ui-*)` 变量，无组件私有 token 体系。）
...
- [x] `ui-components` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（`Tree` 目录已形成 `logic.rs`（装配映射）+ `view.rs`（结构/契约挂载）+ `styles.rs`（静态 token-first）+ `motion.rs`（attach）；公共导出保持 `Tree/TreeNode/TreeTone/TreeDensity/TreeMotion`，未暴露 DOM 细节类型。）
...
- [x] API 命名契约统一：公共 props/回调严格使用 `is_*`、`on_*`、`default_*` 前缀；同语义在全库同名，禁止别名漂移。（`Tree` 新增 `is_disabled` 作为主命名，保留 `disabled` 兼容别名并在 `logic.rs::normalize_is_disabled` 集中归一；展开/选择轴主命名为 `expanded_ids + on_expanded_ids_change + default_expanded_ids`、`selected_id + on_selected_id_change + default_selected_id`，兼容别名 `on_expanded_change`、`on_selected_change`。）
...
- [x] 受控/非受控必须成对：每个可控状态轴都提供 `value + on_value_change + default_value`（如 `open/on_open_change/default_open`）；缺一项即不通过。（`Tree` 的展开轴为 `expanded_ids + on_expanded_ids_change + default_expanded_ids`，选择轴为 `selected_id + on_selected_id_change + default_selected_id`；兼容别名回调在 `logic.rs` 通过 `normalize_*_change_handler` 统一归一，优先新命名，组件侧统一经 `ui_headless::use_controllable_state` 驱动，未绕过单一事实来源。）
  - 受控模式：外部值是单一事实来源，内部不得偷偷写回本地状态。
  - 非受控模式：仅由默认值初始化一次，后续状态由内部原语管理。
  - 受控/非受控切换语义需稳定可测，避免“半受控”隐式行为。
- [x] 默认值单一来源：默认值与优先级只在 `logic.rs` 归一化；`view.rs` 禁止二次兜底或隐式改写。（`Tree` 默认值入口统一到 `logic.rs::normalize_expanded_axis` 与 `logic.rs::normalize_selected_axis`；`view.rs` 仅消费归一结果，不再出现 `unwrap_or_default` / `normalize_optional_text(default_selected_id)` 之类二次兜底。）
  - 默认值优先级必须可读且可测试（显式规则而非分散 `unwrap_or`）。
  - `view.rs` 不允许再做默认值分支；仅消费 `logic.rs` 的归一化输出。
  - 一旦发现多处默认值来源，直接判不通过并回收至 `logic.rs`。
- [x] 状态归一化集中：状态输入先类型化，再在 `logic.rs` 统一派生；禁止在 `view.rs`、事件回调、样式分支中分散拼状态机。（`Tree` 已由 `logic.rs::derive_state` 统一派生 `expanded/selected/visible`，事件回调经 `logic.rs::resolve_expanded_toggle_request` 生成下一状态；`view.rs` 仅消费派生结果并挂载。）
  - 输入边界统一进入 `logic.rs`，输出统一为可渲染语义状态与来源标记。
  - 事件处理器只触发状态变更，不重建状态机规则。
  - 样式层只消费状态标记，不承担状态判定职责。
- [x] 离散状态必须类型约束：`variant/size/mode/status` 等离散输入使用 `enum`；禁止用多个 `Option<bool>`/字符串自由组合表达互斥状态。（`TreeTone`、`TreeDensity`、`TreeControlMode`、`TreeDefaultSource`、`TreeChangeSource` 已类型化；互斥来源以枚举闭包建模。）
  - 互斥状态优先用 `enum` 建模，利用编译器封住无效组合。
  - 字符串输入若需兼容外部配置，必须先映射到类型化枚举再进入逻辑层。
  - 布尔爆炸（多个 bool 表达一个状态机）应在设计评审阶段直接拦截。
- [x] 状态原语来源正确：组件层只消费 `status-primitives`（当前 `ui-state-primitives`）能力，不直接绑定业务 store；应用级全局状态必须经桥接层适配后再接入组件。（`TreeNode/TreeVisibleNode/normalize*/sanitize*/flatten*` 与 `resolve_state_core` 全部来自 `ui-state-primitives::tree`，组件未绑定业务 store 类型。）
  - 组件中出现可复用状态机实现（受控/非受控、展开规则、选择归一）即判应下沉。
  - 组件与业务全局状态之间必须有适配边界，禁止组件直接依赖业务 store 类型。
  - `logic.rs` 仅做装配与映射，不重新实现状态原语。
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。（N/A：`Tree` 组件无远程请求与异步状态轴，不引入加载/错误协议。）
...
- [x] API 易用性验收标准（DX Paradox）：把复杂性留在内部，把简单留给用户。（默认调用仅需 `id_base + nodes` 与可选 `default_*`，无需用户手动接线 primitives/headless；`apps/docs-app` `collections_extra.rs` 提供最小可运行默认示例与进阶示例。）
...
- [x] 组合型组件主 API 必须“显示优于约定”：优先使用显式组合 `<Parent><Item ... /></Parent>`。（`Tree` 采用类型化 `TreeNode` 语义树输入，同一节点维度绑定 `id/label/children/disabled`，未使用并行数组或索引配对式隐式约定。）
...
- [x] 状态可观测、可检索、可验证：使用稳定 `data-*` 与 `aria-*` 标记表达状态和来源。（根节点输出 `data-expanded-mode/data-selected-mode/data-default-*-source/data-*-change-source/data-state`；项节点输出 `data-expanded/data-selected/data-disabled` 与 `aria-*`。）
...
- [x] 样式依赖显式状态（`data-*`/class），而非脆弱 DOM 结构猜测。（`styles.rs` 状态分支基于稳定 class 与 `data-*`，无 `:nth-child` 与深层结构猜测；运行时仅传递 motion CSS 变量。）
...
- [x] 测试验证“语义契约”而不只验证视觉快照。（`crates/ui-components/tests/tree_semantics.rs` 覆盖状态原语来源、headless 挂载、受控/非受控、来源标记、跨平台约束与 feature gate；未依赖视觉快照作为唯一证据。）
...
- [x] 组件文件职责正确：`mod.rs`（导出边界）、`logic.rs`（归一/派生/来源标记）、`styles.rs`（静态 token-first CSS）、`view.rs`（Leptos 结构 + headless 挂载）、`motion.rs`（动效契约 + attach）。（`Tree` 已拆分为上述 5 文件，语义挂载仅在 `view.rs`，动效 attach 仅在 `motion.rs`。）
...
- [x] `spec.rs` 只用于少数复杂组件（如 button），避免泛滥。（`Tree` 未新增 `spec.rs`，契约说明保留在本 `check2.md` 与语义测试。）
  - 仅当组件存在稳定外部规范/Schema 契约或复杂配置固化需求时才引入 `spec.rs`。
  - 简单组件不得为了“形式统一”新增 `spec.rs`；说明文档应留在 `check2.md`/组件文档。
  - 新增 `spec.rs` 必须同步给出契约测试与版本演进说明。
- [x] 组件层遵循 token-first 静态样式契约：样式通过 `styles.rs` 聚合注入；运行时仅传必要 CSS 变量；不把 Utility-First/CSS-in-Rust 当组件库默认范式。（`tree/styles.rs` 使用 `var(--ui-*)` token；`ui-components/src/css.rs` 通过 `component-tree` feature 条件聚合；运行时仅 `--ui-tree-motion-*`。）
...
- [x] Tree Shaking 是一等能力：package 模式支持组件级 feature；source 模式天然裁剪；样式层同步裁剪，禁止无条件聚合全部 CSS，禁止破坏 DCE/LTO 的全量中央注册表。（`ui-components` 已有 `component-tree` feature；`src/lib.rs` 与 `src/css.rs` 均按 `#[cfg(feature = "component-tree")]` 条件导出/聚合，`tree_semantics` 增加静态断言。）
...
- [x] 类型系统 + 语义标记共同提供机器可读状态；关键输入空间受类型约束。（离散状态由 `enum` 约束，关键状态由稳定 `data-*` 与 `aria-*` 输出，破坏点在 `tree_semantics` 可直接定位。）
...
- [x] SSR 与跨平台检查：覆盖 web/ssr/wasm 分支，不破坏 non-wasm 编译路径。（`tree/motion.rs` wasm 与 non-wasm `cfg` 分支齐备；`tree_semantics` 对 `ui-headless/ui-motion/tree` 跨平台契约与 no-op 路径做静态回归。）
...
- [x] `ui-headless` web/ssr feature 互斥受 `compile_error!` 保护（`crates/ui-headless/src/lib.rs`）。（`ui-headless/src/lib.rs` 保持 `#[cfg(all(feature = "web", feature = "ssr"))] compile_error!`，并由 `tree_semantics` 断言。）
  - 组件依赖 `ui-headless` 能力时，不得破坏其 web/ssr 互斥约束。
  - 组件若新增 headless 功能接入，需验证两条 feature 路径都可编译。
  - 发现“同时启用 web+ssr 仍可过编译”视为契约回归。
- [x] `ui-motion` 非 wasm 提供 no-op/stub（`crates/ui-motion/src/lib.rs`），保证 SSR/tooling 可编译。（`ui-motion/src/lib.rs` non-wasm `web` 模块提供 `prefers_reduced_motion + animate` no-op，`tree_semantics` 断言该契约。）
  - `motion.rs` 调用必须可在 non-wasm 下安全降级，不触发 panic。
  - 组件不得假设动画句柄一定存在；no-op 分支行为需可预测。
  - toolchain 场景（测试/文档/静态分析）不得因 motion 依赖阻塞编译。
- [x] 组件实现覆盖 `reduced-motion` / SSR / wasm 分支。（`tree/motion.rs` 在 wasm 分支检测 `ui_motion::web::prefers_reduced_motion()` 并即时落点，non-wasm 分支提供稳定 no-op attach；`view.rs` 通过 `resolve_motion_css_vars` 输出初始 `--ui-tree-motion-*` 变量以保持 SSR 首帧与 hydration 一致。）
...
- [x] `view!` 宏复杂度受控：单个 `view!` 块不得承载超长深嵌套结构；复杂布局按语义分块，避免一次性宏展开导致编译与 wasm 体积劣化。（`Tree` 已把节点列表渲染拆到 `render_tree_list/render_tree_node`，主组件 `view!` 仅保留 root+list 壳层；`tree_semantics` 增加 `tree_view_macro_complexity_is_split_into_semantic_blocks` 断言 `view!` 块数量与分块调用路径。）
  - 复杂结构按语义子块拆分（header/body/item 等），避免巨型单块 `view!`。
  - `view.rs` 中若出现多层嵌套重复片段，应优先提取局部渲染函数。
  - 编译时间/产物体积异常增长时，优先排查宏展开体量。
- [x] 函数式拆分优先：不涉及复杂状态与生命周期管理的 UI 片段，优先拆为普通 Rust 函数（返回 `impl IntoView`/`View`），而不是新增 `#[component]`。（`Tree` 新增普通函数 `tree_item_row_class/tree_item_chevron/render_tree_node/render_tree_list`，未引入额外 `#[component]` 子组件；回归由 `tree_view_prefers_functional_split_without_extra_component_abstraction` 锁定。）
  - 纯静态或轻逻辑片段优先函数化；仅在需要独立 props 语义时升级为组件。
  - 禁止把所有局部片段都升格为 `#[component]` 导致抽象噪音。
  - 拆分后语义标记与测试定位仍需稳定。
- [x] 静态片段常量化：复杂 SVG、页脚、长说明文本等纯静态内容优先常量化/模板化，减少重复 `view!` 渲染指令生成。（`Tree` 将重复静态片段上收为常量：`TREE_CHEVRON_*`、`TREE_*_SLOT`、`TREE_ITEM_CLASS_*`，并通过 `tree_item_chevron`/`tree_item_row_class` 统一消费；`tree_static_fragments_are_constantized_for_repeated_render_paths` 防止回退到散落硬编码。）
  - 可判定为纯静态的片段应避免重复动态构造。
