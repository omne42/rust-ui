...
- 已修复：Tree Shaking 最小特性依赖链（`crates/ui/Cargo.toml` 的 `component-color_editor` 增补 `color_area/color_field/color_slider/color_swatch/slider` 依赖）。
- 已修复：A11y locale 接入（`ColorEditor` 新增 `lang/dir` 并挂载 `ui_headless::locale_attrs`）。
- 已验证：`CARGO_TARGET_DIR=target-codex-color-editor cargo check -p ui --no-default-features --features component-color_editor,inject-css`。
- 已验证：`CARGO_TARGET_DIR=target-codex-color-editor cargo test -p ui --test color_editor_semantics --no-default-features --features component-color_editor,inject-css`（8/8 通过）。
