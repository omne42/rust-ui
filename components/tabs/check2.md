...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。（N/A：Tabs 当前无远程请求与异步状态，不存在 loading/error/retry/aria-busy 语义轴。）
...
- [x] 宏观/微观双状态机（Macro/Micro Duality）：拖拽等高频交互在 `Dragging` 期间由 `view/motion` 本地循环执行；禁止每帧穿越回 `logic.rs`，必须在结束时通过 `Action::DragEnd` 回流收敛。（N/A：Tabs 当前不包含拖拽交互与 Dragging 状态机，无每帧 drag-loop/DragEnd 收敛链路。）
- [x] 几何两段式渲染（Two-Pass Rendering）：`Tooltip/Popover/Menu` 等依赖 DOM 测量的组件必须走 `Intent -> Measure(view) -> Rectification(logic)`，并具备幂等收敛保护防死循环。（N/A：Tabs 不承载 overlay 几何定位纠偏语义；当前仅在 `motion.rs` 做指示器局部测量与动画同步，不存在需要回流 `logic.rs` 的两段式定位闭环。）
- [x] 集合注册协议（Registration Protocol）：`Accordion/Tabs/Menu` 动态子项必须通过 `RegistrationContext` 上报 `Register/Unregister`，逻辑层维护 `items_order`，禁止依赖 `HashSet` 迭代顺序做导航。（Tabs 已在 `view.rs` 建立 `RegistrationContext` 并记录 `TabsRegistrationAction::{Register,Unregister}`，由 `logic.rs::resolve_registered_items_order` 归并输出 `items_order`。）
