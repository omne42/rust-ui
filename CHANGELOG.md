...
- `ui-components` + `ui-asset` (**breaking**): migrates `asset` from `crates/ui-components/src/asset/*` to a dedicated `components/asset` crate (`ui-asset`), switches `ui-components` to feature-gated re-export (`pub use ui_asset as asset`), and updates semantic contract tests to the new canonical source path (`components/asset/src/*`).
- `ui-components` + `ui-chip` (**breaking**): migrates `chip` from `crates/ui-components/src/chip/*` to a dedicated `components/chip` crate (`ui-chip`), switches `ui-components` to feature-gated re-export (`pub use ui_chip as chip`), and updates docs/semantics/style-rule contracts to the new canonical source path (`components/chip/src/styles.rs`).
- `ui-components` + `ui-code` (**breaking**): migrates `code` from `crates/ui-components/src/code/*` to a dedicated `components/code` crate (`ui-code`), switches `ui-components` to feature-gated re-export (`pub use ui_code as code`), and updates docs/semantics/style-rule contracts to the new canonical source path (`components/code/src/styles.rs`).
- `ui-components` + `ui-help-text` (**breaking**): migrates `help_text` from `crates/ui-components/src/field_form/help_text/*` to a dedicated `components/help-text` crate (`ui-help-text`), switches `ui-components` to field-form scoped re-export (`pub use ui_help_text as help_text`), and updates docs/semantics/style-rule contracts to the new canonical source path (`components/help-text/src/styles.rs`).
- `ui-components` + `ui-kbd` (**breaking**): migrates `kbd` from `crates/ui-components/src/kbd/*` to a dedicated `components/kbd` crate (`ui-kbd`), switches `ui-components` to feature-gated re-export (`pub use ui_kbd as kbd`), and updates docs/semantics/style-rule contracts to the new canonical source path (`components/kbd/src/*`).
- `ui-components` + `ui-pressable-feedback` (**breaking**): migrates `pressable_feedback` from `crates/ui-components/src/pressable_feedback/*` to a dedicated `components/pressable-feedback` crate (`ui-pressable-feedback`), switches `ui-components` to feature-gated re-export (`pub use ui_pressable_feedback as pressable_feedback`), and updates semantic contract tests to the new canonical source path (`components/pressable-feedback/src/*`).
- `ui-components` (**breaking**): migrates `field_form/form` and `field_form/form_field` source trees from `crates/ui-components/src/field_form/*` to `components/form/*` and `components/form-field/*`, keeps `ui-components::field_form::{form, form_field}` API stable via path-based domain re-exports, and updates docs/semantics/style-rule source-path contracts to the new canonical locations.
- `ui-components` (**breaking**): migrates `sonner` out of `crates/ui-components/src/sonner/*` to the canonical `components/toast/src/sonner/*` implementation, removes legacy duplicate sources, and updates semantic contract tests/docs path assertions to the new location.
- `ui-components` + `ui-tree` (**breaking**): migrates `tree` from `crates/ui-components/src/tree/*` to a dedicated `components/tree` crate (`ui-tree`), switches `ui-components` to feature-gated re-export (`pub use ui_tree as tree`), and keeps docs/semantics contracts aligned through the migrated source boundary.
- `ui-components` + `ui-tag` (**breaking**): migrates `tag` (including `tag/group`) from `crates/ui-components/src/tag/*` to a dedicated `components/tag` crate (`ui-tag`), switches `ui-components` to feature-gated re-export (`pub use ui_tag as tag`) with subgroup feature forwarding (`component-tag_group -> ui-tag/tag-group`), and updates docs/semantics/style-rule/platform-check paths to `components/tag/src/*`.
- `ui-components` + `ui-autocomplete` (**breaking**): migrates `autocomplete` from `crates/ui-components/src/autocomplete/*` to a dedicated `components/autocomplete` crate (`ui-autocomplete`), switches `ui-components` to feature-gated re-export (`pub use ui_autocomplete as autocomplete`), and keeps semantics/style-rule/source-first contracts aligned to `components/autocomplete/src/*`.
- `ui-components` + `ui-keyboard` (**breaking**): migrates `keyboard` from `crates/ui-components/src/keyboard/*` to a dedicated `components/keyboard` crate (`ui-keyboard`), switches `ui-components` to feature-gated re-export (`pub use ui_keyboard as keyboard`), and updates docs/semantics source-path contracts to `components/keyboard/src/*`.
- `ui-components` + `ui-preview-card` (**breaking**): migrates `preview_card` from `crates/ui-components/src/preview_card/*` to a dedicated `components/preview-card` crate (`ui-preview-card`), switches `ui-components` to feature-gated re-export (`pub use ui_preview_card as preview_card`), and updates semantics/style-rule source-path contracts to `components/preview-card/src/*`.
- `ui-components` + `ui-drawer` (**breaking**): migrates `drawer` from `crates/ui-components/src/drawer/*` to a dedicated `components/drawer` crate (`ui-drawer`), switches `ui-components` to feature-gated re-export (`pub use ui_drawer as drawer`), and updates semantics/style-rule source-path contracts to `components/drawer/src/*`.
- `ui-components` + `ui-popover` (**breaking**): migrates `popover` from `crates/ui-components/src/popover/*` to a dedicated `components/popover` crate (`ui-popover`), switches `ui-components` to feature-gated re-export (`pub use ui_popover as popover`), and updates semantics/style-rule source-path contracts to `components/popover/src/*`.
...
- `ui-components` + `docs-app` (**breaking**): migrates `checkbox` and `checkbox-group` into dedicated component crates (`components/checkbox`, `components/checkbox-group`), switches `ui-components` to feature-gated re-exports (`pub use ui_checkbox as checkbox`, `pub use ui_checkbox_group as checkbox_group`), removes legacy `crates/ui-components/src/checkbox/*`, and updates docs/semantics/css-contract paths to the new canonical sources.
- `ui-components` (**breaking**): migrates `visually_hidden` from `crates/ui-components/src/visually_hidden/*` into a dedicated `components/visually-hidden` crate (`ui-visually-hidden`), switches to feature-gated re-export (`pub use ui_visually_hidden as visually_hidden`), and updates semantics contracts to the new canonical source paths.
