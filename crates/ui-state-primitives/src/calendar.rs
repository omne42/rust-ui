...
pub struct CalendarSelectedDayAxis {
    pub mode: CalendarSelectedDayMode,
    pub selected_day: Option<u8>,
    pub source: CalendarSelectedDaySource,
}
...
impl CalendarSelectedDaySource {
    pub const fn as_attr(self) -> &'static str {
        match self {
            Self::SelectedDay => "selected_day",
            Self::DefaultSelectedDay => "default_selected_day",
...
pub fn normalize_selected_day_axis(
    ...
    if selected_day.is_some() {
        return CalendarSelectedDayAxis {
            mode: CalendarSelectedDayMode::Controlled,
            selected_day: normalize_selected_day(selected_day, year, month),
            source: CalendarSelectedDaySource::SelectedDay,
        };
    }

    let has_default_selected_day = default_selected_day.is_some();
    let selected_day = normalize_selected_day(default_selected_day, year, month);
    CalendarSelectedDayAxis {
        mode: CalendarSelectedDayMode::Uncontrolled,
        selected_day,
...
pub fn resolve_effective_selected_day(
    axis: CalendarSelectedDayAxis,
    uncontrolled_selected_day: Option<u8>,
) -> Option<u8> {
    match axis.mode {
        CalendarSelectedDayMode::Controlled => axis.selected_day,
        CalendarSelectedDayMode::Uncontrolled => uncontrolled_selected_day,
    }
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct CalendarSelectedDayPressUpdate {
    pub next_uncontrolled_selected_day: Option<Option<u8>>,
    pub next_source: CalendarSelectedDaySource,
}

pub fn resolve_selected_day_press_update(
    mode: CalendarSelectedDayMode,
    day: u8,
) -> CalendarSelectedDayPressUpdate {
    match mode {
        CalendarSelectedDayMode::Controlled => CalendarSelectedDayPressUpdate {
            next_uncontrolled_selected_day: None,
            next_source: CalendarSelectedDaySource::SelectedDay,
        },
        CalendarSelectedDayMode::Uncontrolled => CalendarSelectedDayPressUpdate {
            next_uncontrolled_selected_day: Some(Some(day)),
            next_source: CalendarSelectedDaySource::Interaction,
        },
    }
}
