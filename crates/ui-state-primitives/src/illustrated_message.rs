...
pub fn resolve_view_state(
    ...
    let show_title = title.is_some_and(|value| !value.trim().is_empty());
    let show_description = description.is_some_and(|value| !value.trim().is_empty());
...
#[cfg(test)]
#[path = "test/illustrated_message.rs"]
mod tests;
