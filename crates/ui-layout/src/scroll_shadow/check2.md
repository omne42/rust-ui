...
- [x] 组合型组件主 API 必须“显示优于约定”：优先使用显式组合 `<Parent><Item ... /></Parent>`。（N/A：`ScrollShadow` 为单容器滚动阴影组件，不提供 `Item` 子结构与多槽位配对语义，默认 API 仅消费 `children`；因此不存在 `labels + children` / `titles + panels` 并行数组约定。回归：`crates/ui-layout/tests/scroll_shadow_semantics.rs` 的 `scroll_shadow_composite_api_contract_is_explicitly_not_applicable`。）
