...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（组件范围已执行 `cargo test -p ui-layout --test auto_height_semantics --no-default-features --features component-auto_height,inject-css` 与 `cargo check -p ui-layout --target wasm32-unknown-unknown --no-default-features --features component-auto_height,inject-css`。）
