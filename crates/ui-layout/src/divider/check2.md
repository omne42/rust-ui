...
### 10. Divider 本轮验收证据
- [x] 状态原语下沉并由组件桥接消费：`crates/ui-state-primitives/src/divider.rs` + `crates/ui-layout/src/divider/logic.rs`。
- [x] A11y 契约由 headless 层统一输出：`crates/ui-headless/src/divider.rs`，组件 `view.rs` 仅挂载 `attrs`。
- [x] 语义回归入口：`crates/ui-layout/tests/divider_semantics.rs`，覆盖分层边界、稳定 `data-*`/`aria-*` 标记、docs 示例契约与清单完整性。
- [x] 定向门禁命令：
  ...
  - `CARGO_TARGET_DIR=target/divider_check cargo test -p ui-layout --test divider_semantics --no-default-features --features component-divider,inject-css`
