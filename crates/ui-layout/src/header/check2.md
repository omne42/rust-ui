...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（已执行 header 定向门禁：`cargo test -p ui-layout --test header_semantics --no-default-features --features component-header,component-view,component-content,inject-css`、`cargo check -p ui-layout --no-default-features --features component-header,inject-css`、`cargo check -p ui-layout --target wasm32-unknown-unknown --no-default-features --features component-header,inject-css`、`cargo clippy -p ui-layout --no-default-features --features component-header,inject-css -- -D warnings`。）
