...
- [x] 状态原语已下沉并由组件桥接消费：`crates/ui-state-primitives/src/aspect_ratio.rs` + `crates/ui-layout/src/aspect_ratio/logic.rs`。
- [x] A11y 语义契约已通过 headless 层统一输出：`crates/ui-headless/src/aspect_ratio.rs`，组件 `view.rs` 只挂载 `attrs`。
- [x] 语义回归入口：`crates/ui-layout/tests/aspect_ratio_semantics.rs`，覆盖分层边界、稳定 `data-*`/`aria-*` 标记、docs 示例契约与清单完整性。
- [x] 定向门禁命令：
  ...
  - `CARGO_TARGET_DIR=target/aspect_ratio_check cargo test -p ui-layout --test aspect_ratio_semantics --no-default-features --features component-aspect_ratio,inject-css`
