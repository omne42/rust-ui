...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - 已验证命令：`cargo test -p ui-state-primitives footer`、`cargo test -p ui-layout --test footer_semantics --no-default-features --features component-footer,inject-css`、`cargo clippy -p ui-layout --test footer_semantics --no-default-features --features component-footer,inject-css -- -D warnings`、`cargo check -p ui-layout --target wasm32-unknown-unknown --no-default-features --features component-footer,inject-css`。
