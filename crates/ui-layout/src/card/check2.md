...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - `card` 组件最小门禁已验证：`cargo test -p ui-layout --test card_semantics`、`cargo check -p ui-layout --no-default-features --features component-card,inject-css`、`cargo check -p ui-layout --target wasm32-unknown-unknown --no-default-features --features component-card,inject-css`。
