...
fn load_source(rel_path: &str) -> String {
    if rel_path == "../../apps/docs-app/src/pages/components/pages/layout.rs" {
        ...
        let child_path =
            manifest_dir.join("../../apps/docs-app/src/pages/components/pages/layout/separator.rs");
...
fn separator_token_first_static_styles_are_aggregated_and_injected_via_uiroot() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-layout css aggregator should feature-gate separator styles via `{needle}`."
        );
    }
...
fn separator_tree_shaking_contract_is_feature_gated_and_budget_guarded() {
    ...
    let tree_script_source = load_source("../../scripts/check-ui-layout-tree-shaking.sh");
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-layout Cargo feature graph should include `{needle}`."
        );
    }
    ...
    assert!(
        ...
        "web-demo should consume ui-layout without default all-components."
    );
    assert!(
    ...
    for needle in [
        ...
        "cargo check -p ui-layout --target wasm32-unknown-unknown --no-default-features --features component-separator,inject-css",
...
fn separator_type_system_and_semantic_markers_form_machine_readable_contract() {
    ...
    let logic_source = load_source("src/separator/logic.rs");
    let view_source = load_source("src/separator/view.rs");
    let tests_source = load_source("tests/separator/semantics.rs");
...
fn separator_cross_platform_compile_contract_has_explicit_cfg_and_no_non_wasm_web_sys_usage() {
    ...
    for needle in [
        "cargo check -p ui-layout --no-default-features --features component-separator,inject-css",
        "cargo check -p ui-layout --target wasm32-unknown-unknown --no-default-features --features component-separator,inject-css",
...
fn separator_ui_motion_non_wasm_stub_contract_is_preserved() {
    ...
    for needle in [
        ...
        "cargo test -p ui-layout --test separator_semantics separator_motion_stays_ui_motion_driven_and_semantic_free --no-default-features --features component-separator,inject-css",
...
fn separator_wasm_debug_capability_reuses_global_trace_and_stays_feature_isolated() {
    ...
    for needle in [
        ...
        "cargo check -p ui-layout --target wasm32-unknown-unknown --no-default-features --features inject-css,button-wasm-debug",
        "cargo test -p ui-layout --test separator_semantics --no-default-features --features component-separator,inject-css separator_wasm_debug_capability_reuses_global_trace_and_stays_feature_isolated",
...
#[test]
fn separator_ui_layout_layer_stays_assembly_only_and_platform_agnostic() {
    let mod_source = load_source("src/separator/mod.rs");
    let logic_source = load_source("src/separator/logic.rs");
    let view_source = load_source("src/separator/view.rs");
    let motion_source = load_source("src/separator/motion.rs");
...
fn separator_component_files_follow_responsibility_boundaries() {
    ...
    for needle in [
        ...
        assert!(
            lib_source.contains(needle),
            "ui-layout lib entry should gate separator module via `{needle}`."
        );
    }
...
#[test]
fn separator_ui_layout_fixed_entry_files_follow_layered_boundaries() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let lib_source = load_source("src/lib.rs");
    let css_source = load_source("src/css.rs");
    let root_source = load_source("src/root.rs");
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "ui-layout lib entry should not expose internal platform/details marker `{forbidden}`."
        );
    }
...
fn separator_e2e_check_script_covers_selector_and_key_flow_contracts() {
    let script_source = load_source("../../scripts/check-ui-layout-e2e-separator.sh");
...
fn separator_forbidden_antipatterns_checklist_items_are_individually_enforced() {
    ...
    let styles_source = load_source("src/separator/styles.rs");
    let motion_source = load_source("src/separator/motion.rs");
    let test_source = load_source("tests/separator/semantics.rs");
