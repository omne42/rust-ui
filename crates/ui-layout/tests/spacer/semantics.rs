...
fn load_source(rel_path: &str) -> String {
    if rel_path == "../../apps/docs-app/src/pages/components/pages/layout.rs" {
        ...
        let parent_path = manifest_dir.join("../../apps/docs-app/src/pages/components/pages/layout.rs");
        let child_path =
            manifest_dir.join("../../apps/docs-app/src/pages/components/pages/layout/spacer.rs");
...
fn spacer_docs_page_covers_primary_playgrounds() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_docs_page_covers_primary_playgrounds`",
...
fn spacer_docs_interactive_playground_contract_is_stable() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_docs_page_covers_primary_playgrounds` + `spacer_docs_interactive_playground_contract_is_stable`",
...
fn spacer_docs_playgrounds_lock_state_matrix_contract_values() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_docs_playgrounds_lock_state_matrix_contract_values` + `spacer_docs_are_synced_and_copy_paste_ready`",
...
fn spacer_semantics_checks_prioritize_contract_assertions() {
    let check2_source = load_source("src/spacer/check2.md");
    let test_source = load_source("tests/spacer/semantics.rs");
    let view_source = load_source("src/spacer/view.rs");
    let semantics_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/spacer/semantics.rs");

    for needle in [
        "- [x] 语义测试优先：验证 `data-*` / `aria-*` / role / 状态来源契约，不只视觉快照。",
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_semantics_checks_prioritize_contract_assertions`",
...
fn spacer_e2e_selectors_and_wasm_wait_strategy_are_semantic_and_stable() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_e2e_selectors_and_wasm_wait_strategy_are_semantic_and_stable`",
    ] {
        assert!(
            check2_source.contains(needle),
...
fn spacer_critical_flow_regression_scope_is_explicit_and_traceable() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_critical_flow_regression_scope_is_explicit_and_traceable`",
    ] {
        assert!(
            check2_source.contains(needle),
...
fn spacer_dx_fast_style_feedback_and_workbench_contract_is_stable() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_dx_fast_style_feedback_and_workbench_contract_is_stable`",
    ] {
        assert!(
            check2_source.contains(needle),
...
fn spacer_agent_contract_markers_are_machine_readable() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_agent_contract_markers_are_machine_readable`",
    ] {
        assert!(
            check2_source.contains(needle),
...
fn spacer_streaming_scope_is_explicitly_snapshot_only() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_streaming_scope_is_explicitly_snapshot_only`",
...
fn spacer_docs_are_synced_and_copy_paste_ready() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_docs_are_synced_and_copy_paste_ready`",
...
fn spacer_heroui_benchmark_docs_sync_contract_is_explicit() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_heroui_benchmark_docs_sync_contract_is_explicit`",
...
fn spacer_forbidden_antipatterns_contract_is_locked() {
    ...
    for needle in [
        ...
        "- [x] 公共 API 泄露底层实现细节类型。",
        "- [x] 用临时补丁破坏跨组件一致性。",
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_forbidden_antipatterns_contract_is_locked`",
...
fn spacer_final_merge_gate_contracts_are_satisfied() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_final_merge_gate_contracts_are_satisfied`",
        "该条暂不做 smoke",
    ] {
        assert!(
            check2_source.contains(needle),
...
fn spacer_engineering_capabilities_stay_unified_and_runtime_agnostic() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-layout/tests/spacer/semantics.rs::spacer_engineering_capabilities_stay_unified_and_runtime_agnostic`",
    ] {
        assert!(
            check2_source.contains(needle),
