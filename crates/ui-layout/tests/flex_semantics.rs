...
fn load_source(rel_path: &str) -> String {
    if rel_path == "../../apps/docs-app/src/pages/components/pages/layout.rs" {
        ...
        let child_path =
            manifest_dir.join("../../apps/docs-app/src/pages/components/pages/layout/flex.rs");
...
fn flex_uses_logic_state_model() {
    ...
    let render_source = load_source("src/flex/view.rs");
...
#[test]
fn flex_emits_baseline_style_state_data_attributes() {
    let source = load_source("src/flex/view.rs");
    ...
    for attr in [
        ...
        assert!(
            ...
            "Flex should expose `{attr}` for baseline-style styling and state inspection."
        );
    }
}
