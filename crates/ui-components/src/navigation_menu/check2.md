...
1. 最小特性根因修复：`component-navigation_menu` 单独启用时缺少 `active_highlight` 依赖，导致 `navigation_menu` 的 motion 与 re-export 无法解析。
2. 修复动作：在 `crates/ui-components/Cargo.toml` 将 `component-navigation_menu` 调整为依赖 `component-active_highlight`，保持最小特性链可编译。
3. 语义契约回归：`crates/ui-components/tests/navigation_menu_semantics.rs` 新增 `check2` 无未勾选项断言，防止后续回退为未勾选清单状态。
4. API 与状态语义：`navigation_menu` 已覆盖受控/非受控选择态（`selected_id` / `default_selected_id` / `on_selected_id_change`）；异步加载语义在本组件范围为 N/A（无远程请求与 loading/retry 状态轴）。
5. 流式职责：`navigation_menu` 非正文阅读面，按 `Streaming Optional` 执行，当前以 `snapshot` 路径稳定渲染。
6. 本轮执行门禁（定向）：
   - `CARGO_TARGET_DIR=target-check2-navigation-menu cargo test -p ui-components --no-default-features --features component-navigation_menu,inject-css --test navigation_menu_semantics`
   - `CARGO_TARGET_DIR=target-check2-navigation-menu cargo clippy -p ui-components --no-default-features --features component-navigation_menu,inject-css -- -D warnings`
   - `CARGO_TARGET_DIR=target-check2-navigation-menu cargo check -p ui-components --no-default-features --features component-navigation_menu,inject-css`
   - `CARGO_TARGET_DIR=target-check2-navigation-menu cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-navigation_menu,inject-css`
   - `cargo tree -e features -p ui-components --no-default-features --features component-navigation_menu,inject-css -f "{p} {f}" | head -n 1`
