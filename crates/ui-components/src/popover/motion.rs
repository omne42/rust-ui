...
pub fn attach_motion(
    ...
    Effect::new(move |_| {
        ...
        let _ = style.set_property("--ui-popover-y", &format!("{y_initial}px"));

        let style_for_opacity = style.clone();
        let opacity = ui_motion::spring::SpringAnimator::new(opacity_initial, config, move |v| {
            let v = v.clamp(0.0, 1.0);
            let _ = style_for_opacity.set_property("--ui-popover-opacity", &format!("{v}"));
        });
