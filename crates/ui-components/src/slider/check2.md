...
**核心原则：单向依赖，职责分离，环境隔离**
- [ ] **Core 层纯净性**：`crates/ui-core` 仅包含纯状态原语（Toggle/Selection/List 等），**零依赖** `Leptos/DOM/web-sys`，确保逻辑可单测、可移植。
- [ ] **Headless 层抽象**：`crates/ui-headless` 封装交互与 A11y（Press/Focus/Hover/Menu 等），输出 `attrs + handlers + state`。**严禁**包含样式或动效。
- [ ] **Motion 层独立**：`crates/ui-motion` 实现动效引擎（Spring/WAAPI）。非 WASM 环境提供 no-op 实现，确保 SSR 可编译。
- [ ] **Theme 层解耦**：`crates/ui-theme` 仅负责 Token 定义与 CSS 变量生成（Light/Dark/OLED），不包含组件具体 CSS。
...
- [x] **ARIA 完备**：正确暴露 `aria-*` 属性，`role` 定义准确。
...
- [x] **Agent Contract**：语义标记 Schema 化，使 Agent 无需猜测 DOM 结构即可理解组件意图。
