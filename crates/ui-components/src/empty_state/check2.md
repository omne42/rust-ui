...
> 本轮执行记录（empty_state）  
> 1) 状态原语下沉到 `crates/ui-state-primitives/src/empty_state.rs`，组件 `logic.rs` 改为纯消费导出。  
> 2) `view.rs` 接入 `lang/dir` locale attrs，并把 `motion.rs` attach 到根节点，暴露 `data-motion-source/data-custom-motion`。  
> 3) `i18n.rs` 默认文案改为复用 `DEFAULT_TITLE/DEFAULT_DESCRIPTION/DEFAULT_ARIA_LABEL`，确保默认值单一来源。  
> 4) `styles.rs` 新增 enter 动效变量消费与 reduced-motion 降级。  
> 5) 语义回归同步到 `crates/ui-components/tests/empty_state_semantics.rs`；组件无受控状态轴、无异步请求、无 `inner_html`，对应条目按 N/A 通过。
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）：`rustfmt --edition 2024 crates/ui-state-primitives/src/empty_state.rs crates/ui-state-primitives/src/lib.rs crates/ui-components/src/empty_state/logic.rs crates/ui-components/src/empty_state/mod.rs crates/ui-components/src/empty_state/i18n.rs crates/ui-components/src/empty_state/view.rs crates/ui-components/src/empty_state/styles.rs crates/ui-components/tests/empty_state_semantics.rs`、`CARGO_TARGET_DIR=target/es-check2 cargo test -p ui-state-primitives empty_state`、`CARGO_TARGET_DIR=target/es-check2 cargo test -p ui-components --no-default-features --features component-empty_state,inject-css --test empty_state_semantics`、`CARGO_TARGET_DIR=target/es-check2 cargo clippy -p ui-components --no-default-features --features component-empty_state,inject-css -- -D warnings`、`CARGO_TARGET_DIR=target/es-check2 cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-empty_state,inject-css`、`CARGO_TARGET_DIR=target/es-check2 cargo check -p ui-headless --no-default-features --features web`、`CARGO_TARGET_DIR=target/es-check2 cargo check -p ui-headless --no-default-features --features ssr`、`CARGO_TARGET_DIR=target/es-check2 cargo check -p ui-headless --no-default-features --features web,ssr`（按预期触发 `compile_error!`）、`CARGO_TARGET_DIR=target/es-check2 cargo check -p ui-motion`、`CARGO_TARGET_DIR=target/es-check2 cargo check -p ui-motion --target wasm32-unknown-unknown`。
