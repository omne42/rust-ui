...
- [x] 组件文档必须对新手友好（Documentation as Product）：组件 README 或等价文档入口必须存在。（`TimeField` 已保留等价 docs-app 入口 `apps/docs-app/src/pages/components/pages/forms_extra.rs`，并在同页提供“Hello World -> Controlled + Step 15 -> Strong Tone + Custom Placeholder”的默认到进阶顺序；`Hello World` 片段保持零门槛（<= 5 行、无需底层分层接线）。回归：`time_field_docs_entry_exists_as_readme_or_equivalent_docs_app_page` + `time_field_docs_are_beginner_friendly_with_default_then_advanced_path` + `time_field_docs_hello_world_snippet_is_zero_threshold_and_not_architecture_wiring` + `time_field_check2_marks_documentation_as_product_complete`。）
  - 每个基础组件必须提供“零门槛”最小示例（Hello World）与常见用法，避免要求用户先理解底层分层架构。
  - 文档需明确“先用起来，再进阶”：默认 API 路径在前，高级控制参数在后。
  - “只有源码没有文档”或“只写给架构师/机器看的文档”视为不通过。
- [x] `apps/docs-app` 必须提供 Interactive Playground：用户可在线修改 props/状态并实时预览。（`apps/docs-app/src/pages/components/pages/forms_extra.rs` 的 `TimeField` 已提供 `Interactive Playground (State + Source Markers)`：可实时切换 disabled / minute_step / tone、编辑与重置 value，并通过状态摘要观察结果；关键流程的可重复语义断点由 `e2e/tests/docs_app_time_field_contract.spec.mjs`（含 reload 复现）覆盖。`TimeField` 非 AI Spec 组件，Spec 输入联动条目对本组件记为 N/A。回归：`time_field_check2_documents_interactive_playground_rules` + `time_field_docs_app_provides_interactive_playground_for_props_state_and_preview` + `time_field_interactive_playground_reuses_repeatable_semantic_e2e_flow`。）
  - Playground 至少支持基础 props 调整、状态切换、交互反馈观察。
  - 对 AI Spec 相关组件，至少提供一组 Spec 输入与预览输出的联动示例。
  - Playground 作为验收面，需可重复复现关键交互路径。
- [x] Source-first 文档必须 Copy-Paste Ready：提供一键复制组件源码或最小可用片段能力。（`TimeField` docs 已补 `Source-first / Copy-Paste Ready` 区块：提供 `Snippet(copyable=true)` 的最小可运行片段、真实源码落点（`time_field/{mod,logic,view,styles,motion}.rs`）与 feature 前提（`component-time_field` + `inject-css`）；Playground 示例继续复用 `apps/docs-app/src/playground.rs::compose_copy_ready_code` + `CodeBlock` 复制路径，保证输出 import-ready 且与实现同步。回归：`time_field_check2_documents_source_first_copy_paste_ready_rules` + `time_field_docs_are_copy_paste_ready_with_imports_copy_button_and_sync` + `time_field_contract_hygiene_script_covers_source_first_copy_paste_ready_contract`。）
  - docs-app 页面应提供复制按钮，输出代码默认可直接运行（含必要 imports/依赖提示）。
  - 若为 source-first 组件，文档需指向真实源码落点并说明依赖前提，避免“复制即报错”。
  - 文档代码与当前实现必须同步，防止示例漂移。
- [x] HeroUI 对标文档与组件文档同步：参数模型变更需同步 `docs/spec/heroui-parameter-design-strategy.md`（必要时补充 `docs/research/spectrum-heroui-style-interface-study.md`），并保证组件文档可访问。（已在 `docs/spec/heroui-parameter-design-strategy.md` 新增 `TimeField 同步记录（2026-02-17）`，同步参数模型、docs 入口、示例矩阵与 Source-first 复制链路；`TimeField` docs 入口由 `component_doc!("TimeField", "time-field", "Forms", forms_extra::time_field)` 索引。回归：`time_field_heroui_strategy_and_component_docs_are_synced_for_parameter_model_changes` + `time_field_check2_marks_heroui_strategy_and_component_docs_sync_complete` + `time_field_contract_hygiene_script_covers_heroui_strategy_doc_sync_contract`。）
  - 若参数语义发生变化，需同步更新对标策略文档，不允许实现先漂移文档后补。
  - 组件文档入口必须存在（docs-app 页面或等价文档），且可被索引定位。
  - “仅代码更新无文档更新”在接口变更场景下直接判不通过。

### 8. 明确禁止的反模式
- [x] 在 `status-primitives`（当前 `ui-state-primitives`）写 DOM/样式逻辑。（`ui-state-primitives/src/time_field.rs` 保持纯 Rust 状态与归一化函数，不引入 Leptos/DOM/CSS 依赖；`TimeField` 组件仅消费其输出。回归：`time_field_anti_pattern_status_primitives_remains_dom_and_style_free`。）
  - 发现 `ui-state-primitives` 引入 DOM/样式依赖即判架构越层，必须回滚并迁移到正确层。
- [x] 在 `ui-headless` 写视觉和动画编排。（`ui-headless/src/time_field.rs` 仅输出语义 attrs/handlers/state 契约，不承载 class/CSS/动效时间线；动效映射保持在组件 `motion.rs`。回归：`time_field_anti_pattern_ui_headless_remains_visual_and_motion_free`。）
  - headless 只输出交互/A11y 契约；出现 class/CSS/动效时间线即判职责污染。
- [x] 在 `view` 层隐藏关键状态决策。（`time_field/view.rs` 只消费 `logic::normalize_disabled_state`、`logic::normalize_value_state` 与 `logic::resolve_state` 输出，关键默认值/状态判定集中在 `logic.rs` + primitives。回归：`time_field_anti_pattern_view_keeps_decisions_in_logic_layer`。）
  - `view.rs` 只消费归一化结果；关键业务分支若散落在 view，必须回收至 `logic.rs`。
- [x] 新增参数但不纳入统一命名与契约。（`TimeField` 参数遵循统一命名与受控三元组：`is_*`/`default_*`/`on_*`，并经类型约束、默认值归一与语义回归覆盖。回归：`time_field_anti_pattern_new_params_follow_naming_type_default_and_semantic_contract`。）
  - 新参数必须进入命名体系、类型约束、默认值归一和语义测试；缺任一项不得合并。
- [x] 用并行数组/隐式约定替代显式语义结构（如 `labels + children`）。（`TimeField` 采用显式 props + 结构渲染，不存在并行数组/索引配对式 API。回归：`time_field_anti_pattern_parallel_array_api_is_absent_for_time_field_scope`。）
  - 标题、语义、内容必须显式绑定在同一 item 结构；依赖位置索引配对视为反模式。
  - 发现“少写几行但语义变弱”的接口设计，默认拒绝合入。
- [x] 公共 API 泄露底层实现细节类型。（`time_field/mod.rs` 对外仅暴露稳定组件入口与类型，不暴露 `web-sys`/运行时私有类型；平台细节封装在内部模块。回归：`time_field_anti_pattern_public_api_does_not_leak_platform_or_runtime_types`。）
  - 公共接口不得暴露 `web-sys`/运行时私有类型；平台细节只允许存在于内部模块。
- [x] 用临时补丁破坏跨组件一致性。（`TimeField` 代码路径未引入临时 patch/hack 分叉，命名与状态来源契约沿用统一实现路径。回归：`time_field_anti_pattern_no_temporary_patch_contract_drift_tokens_in_time_field_scope`。）
  - 临时 patch 若绕开统一契约（命名/状态/语义），必须在同 PR 里修正或显式回退计划。
- [x] 明明是跨组件可复用状态原语，却长期留在某个组件 `logic.rs` 不下沉。（`TimeField` 可复用状态不变量位于 `ui-state-primitives/src/time_field.rs` 与 `ui-headless/src/time_field.rs`，组件 `logic.rs` 仅做装配映射。回归：`time_field_anti_pattern_reusable_state_invariants_are_sunk_to_primitives_or_headless` + `time_field_check2_marks_forbidden_anti_patterns_complete`。）
  - 一旦确认具备可复用状态不变量，应下沉至 `ui-state-primitives`/`ui-headless`，组件层仅保留装配映射。

### 9. 合并门禁（最终裁决）
- [x] 架构正确（边界不破）。（分层边界由 `time_field_uses_logic_state_model` + `time_field_anti_pattern_status_primitives_remains_dom_and_style_free` + `time_field_anti_pattern_ui_headless_remains_visual_and_motion_free` 约束。）
- [x] 行为正确（状态与交互语义成立）。（状态与交互路径由 `time_field_interaction_matrix_covers_controlled_uncontrolled_disabled_keyboard_pointer_and_platform_split` + `time_field_streaming_optional_scope_keeps_role_aria_and_data_markers_continuous` 覆盖。）
- [x] 可访问性达标（默认可用）。（A11y 与 locale 接线由 `time_field_a11y_i18n_and_locale_contract_is_wired` + `time_field_e2e_high_risk_paths_cover_focus_keyboard_and_settled_semantic_breakpoints` 回归。）
- [x] 默认主题美学质量达标（与可访问性同级门禁）。（视觉基线由 `time_field_visual_desire_reuses_default_theme_baseline_and_visual_regression_gates` 守护。）
- [x] 可测试（契约可断言）。（契约断言优先由 `time_field_semantics_suite_prioritizes_contract_assertions_over_snapshots` + `time_field_semantic_markers_changed_in_view_must_be_covered_by_semantics_tests` 保证。）
- [x] 可维护（命名和模式一致）。（命名/默认值/契约一致性由 `time_field_anti_pattern_new_params_follow_naming_type_default_and_semantic_contract` + `time_field_docs_examples_sync_with_logic_api_names_and_state_matrix` 约束。）
- [x] 可解释（人和自动化都能读懂）。（机器可读语义与 Agent 合约由 `time_field_type_system_and_semantic_markers_form_machine_readable_contract` + `time_field_agent_contract_is_schema_typed_and_machine_readable` 提供。）
- [x] 改动在正确层。（职责落位由 `time_field_component_file_responsibilities_remain_scoped` + `time_field_anti_pattern_reusable_state_invariants_are_sunk_to_primitives_or_headless` 守卫。）
- [x] 命名与全库一致。（命名契约由 `time_field_api_keeps_is_prefixed_boolean_and_value_triplet_contract` + `time_field_docs_examples_sync_with_logic_api_names_and_state_matrix` 覆盖。）
- [x] 无效状态被限制或归一化。（归一化与类型约束由 `time_field_default_value_priority_is_centralized_in_logic` + `time_field_type_system_and_semantic_markers_form_machine_readable_contract` 保证。）
- [x] 暴露必要语义标记。（关键语义标记由 `time_field_emits_baseline_style_state_data_attributes` + `time_field_agent_contract_is_schema_typed_and_machine_readable` 回归。）
- [x] 覆盖 reduced-motion / SSR / wasm 分支。（平台分支与降级策略由 `time_field_reduced_motion_ssr_wasm_branches_keep_semantics_consistent` + `time_field_platform_guards_keep_cfg_split_and_non_wasm_web_sys_free` + `time_field_ui_headless_feature_mutex_contract_is_guarded` 覆盖。）
- [x] 文档与示例同步更新。（文档矩阵与示例同步由 `time_field_docs_examples_sync_with_logic_api_names_and_state_matrix` + `time_field_check2_marks_documentation_as_product_complete` 守卫。）
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（已执行定向门禁链路并通过；回归：`time_field_check2_marks_final_merge_gates_complete`。）
