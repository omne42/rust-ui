...
use crate::color_swatch_picker::{ColorSwatchPickerState, ColorSwatchPickerStateInput};
...
pub fn normalize_items(items: Vec<ColorSwatchPickerItem>) -> Vec<ColorSwatchPickerItem> {
    ...
    for item in items {
        let Some(color) = color_swatch::sanitize_color_value(Some(item.color)) else {
            continue;
        };
...
pub fn sanitize_selected_color(selected_color: Option<String>) -> Option<String> {
    color_swatch::sanitize_color_value(selected_color)
}
