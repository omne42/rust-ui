...
已核验（kbd，2026-02-18）：本组件是静态按键标签展示，核心输入轴为 `size/keys/class_name`，不承载受控 value 轴、远程异步流程、overlay 焦点链路与流式正文渲染职责；相关条目按 N/A 语义核验并保持契约可追溯。
...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - `kbd` 组件最小门禁已验证：`cargo test -p ui-components --test kbd_semantics --no-default-features --features component-kbd,inject-css`、`cargo check -p ui-components --no-default-features --features component-kbd,inject-css`、`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-kbd,inject-css`、`cargo clippy -p ui-components --no-default-features --features component-kbd,inject-css --tests -- -D warnings`。
