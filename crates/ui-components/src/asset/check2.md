...
- [x] 本次发现并修复的阻断项：
  - `component-asset` 最小特性无法单独编译，原因是缺少对 `component-thumbnail` 的特性依赖；已修复 `crates/ui-components/Cargo.toml`：`component-asset = ["component-thumbnail"]`。
  - `asset/view.rs` 依赖根导出 `crate::Thumbnail`，在最小特性下不可见；已改为直接依赖模块路径 `crate::thumbnail::Thumbnail`。
- [x] 结构与职责结论：`asset` 保持 `mod/logic/styles/view/motion` 分层；`logic.rs` 仅做归一化与来源标记，`view.rs` 仅挂载语义与结构，`motion.rs` 仅复用 `thumbnail` motion contract。
- [x] 状态与类型结论：离散状态使用 `AssetVariant` / `AssetSize`，无字符串协议；`data-state/data-*-source` 输出封闭集合，契约可断言。
- [x] A11y/i18n 与样式结论：内建图标具备 `role="img"` + `aria-label`；样式依赖稳定 `data-*` / class，不依赖脆弱 DOM 结构。
- [x] Async/Streaming/E2E 适用性结论：
  - Async：N/A（`Asset` 为同步展示组件，无远程请求与加载状态机）。
  - Streaming：`Asset` 不是正文流式阅读面，按 `Streaming Optional` 执行，默认 `snapshot` 渲染路径成立。
  - E2E：当前组件为轻量展示包装，关键契约由 `asset_semantics` + docs Playground 覆盖；无新增独立 E2E 用例。
- [x] 文档与示例结论：`apps/docs-app/src/pages/components/pages/display_extra_asset.rs` 已提供 docs 页面与三组 playground（variant/custom/state-source），可复制代码路径存在。
- [x] Tree-shaking 结论：最小特性链路已验证 `component-asset -> component-thumbnail`，未出现 `all-components` 隐式拉起。

#### 10.1 门禁与验证命令（逐条执行）
- [x] `rustfmt --edition 2024 --check crates/ui-components/src/asset/view.rs crates/ui-components/src/asset/logic.rs crates/ui-components/src/asset/mod.rs crates/ui-components/src/asset/motion.rs crates/ui-components/src/asset/styles.rs`
- [x] `CARGO_TARGET_DIR=target/asset-check cargo check -p ui-components --no-default-features --features component-asset,inject-css`
- [x] `CARGO_TARGET_DIR=target/asset-check cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-asset,inject-css`
- [x] `CARGO_TARGET_DIR=target/asset-check cargo clippy -p ui-components --no-default-features --features component-asset,inject-css --test asset_semantics --no-deps -- -D warnings`
- [x] `CARGO_TARGET_DIR=target/asset-check cargo test -p ui-components --no-default-features --features component-asset,inject-css --test asset_semantics`
- [x] `CARGO_TARGET_DIR=target/asset-check cargo test -p ui-components --no-default-features --features component-asset,inject-css --test asset_semantics <single-test> -- --exact`（8 条语义测试逐条单独执行，全部通过）
- [x] `cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-asset,inject-css`
