...
### 1. 大骨架（架构边界与层职责）
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（`ColorLoupe` 状态原语已下沉至 `ui-state-primitives/src/color_loupe.rs`，并在 `ui-state-primitives/src/lib.rs` 导出；组件侧 `crates/ui-components/src/color_loupe/logic.rs` 仅桥接消费，回归覆盖 `crates/ui-components/tests/color_loupe_semantics.rs`）
...
- [x] `ui-headless` 定义：交互与 A11y 原语层（press/focus/hover/roving/listbox/menu/tooltip 等），把输入设备事件与状态语义标准化为可复用契约；输出必须是类型化 `attrs + handlers + state`。不做样式、不写组件 CSS、不做组件级动效编排。（`ColorLoupe` 为纯展示覆盖层，无键盘/指针状态机；A11y 语义收敛为 `role="img"` + `aria-label` + 稳定 `data-*` 标记，未在组件层重复发明 headless 交互契约）
...
- [x] `ui-motion` 定义：动效能力与契约执行层（spring、keyframes、WAAPI/RAF backend），只负责时间函数、插值与运行时驱动，不承载组件业务语义与状态决策。（`ColorLoupe` 当前仅使用静态 CSS keyframes，不引入组件级 motion runtime 依赖，保持 SSR/non-wasm 编译路径无额外执行器耦合）
...
- [x] `ui-theme` 定义：唯一设计 token 与主题上下文层（system/color/scale + Light/Dark/OLED），负责 token 分类、主题映射与 CSS 变量生成。（`crates/ui-components/src/color_loupe/styles.rs` 仅消费 `var(--ui-*)` 与 `color-mix(...)`，未引入组件私有 token 体系）
...
- [x] `ui-components` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（`mod.rs / logic.rs / styles.rs / view.rs` 分工清晰；`view.rs` 只做结构与语义挂载，`logic.rs` 只做归一和 class 组装）
...
- [x] 测试验证“语义契约”而不只验证视觉快照。（覆盖：`crates/ui-components/tests/color_loupe_semantics.rs`，断言 role/aria/data-state/slot/class 契约与 docs 状态矩阵）
...
### 7. 测试与文档（验证闭环）
- [x] 语义测试优先：验证 `data-*` / `aria-*` / role / 状态来源契约，不只视觉快照。（`color_loupe_semantics` 已补充 `check2` 完成度守卫与状态原语来源守卫）
  - 每个交互组件至少有对应 `*_semantics.rs` 测试覆盖关键状态轴与动作语义。
  - 断言应聚焦语义契约（状态来源/可访问性/键盘路径），快照仅作补充。
  - 新增/变更语义字段必须同步补测试，否则不得打勾。
- [x] E2E 选择器稳定：使用语义标记，WASM 场景有稳定等待策略。（`ColorLoupe` 当前作为纯展示 primitive，关键契约由语义测试稳定覆盖；WASM 路径通过最小特性编译验证）
  - E2E 选择器优先 `data-*` 语义标记，禁止依赖脆弱 DOM 层级或文本定位。
  - WASM 场景必须使用稳定等待策略（语义状态就绪而非固定 sleep）。
  - 若组件涉及异步/动画，E2E 需显式覆盖 ready/settled 条件。
- [x] 关键流程纳入可重复回归集合（Playwright/Cypress）。（N/A：无键盘/指针流程；重复回归焦点在 docs 状态矩阵与语义契约断言）
...
### 8. 明确禁止的反模式
- [x] 在 `status-primitives`（当前 `ui-state-primitives`）写 DOM/样式逻辑。（已核对 `ui-state-primitives/src/color_loupe.rs` 仅包含纯 Rust 数据与归一函数）
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（本次执行：`cargo test -p ui-components --test color_loupe_semantics --no-default-features --features component-color_loupe,inject-css`、`cargo test -p ui-state-primitives color_loupe --lib`、`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-color_loupe,inject-css`、`cargo tree -e features -p ui-components --no-default-features --features component-color_loupe,inject-css`）
