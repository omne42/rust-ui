...
### 1. 大骨架（架构边界与层职责）
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（`FooterStateInput/FooterState/resolve_state` 已下沉到 `crates/ui-state-primitives/src/footer.rs`，组件 `logic.rs` 仅做装配映射。）
...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。
  - N/A：`Footer` 无远程请求与异步状态轴，状态仅由同步 props 归一与语义标记派生驱动。
...
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。
  - `Footer` 归类为 `Streaming Optional`，当前组件按 `fallback=snapshot` 稳定渲染。
...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - 已验证命令：`cargo test -p ui-state-primitives footer`、`cargo test -p ui-components --test footer_semantics --no-default-features --features component-footer,inject-css`、`cargo clippy -p ui-components --test footer_semantics --no-default-features --features component-footer,inject-css -- -D warnings`、`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-footer,inject-css`。
