...
### 本次逐项核验证据（text_area）
- 代码分层与 API：`crates/ui-components/src/text_input/text_area/mod.rs`、`crates/ui-components/src/text_input/text_area/logic.rs`、`crates/ui-components/src/text_input/text_area/view.rs`、`crates/ui-components/src/text_input/text_area/motion.rs`、`crates/ui-state-primitives/src/text_area.rs`。
- 样式与 token-first：`crates/ui-components/src/text_input/text_area/styles.rs`（仅消费 `var(--ui-*)` 与 motion css var）。
- 语义与文档回归：`crates/ui-components/tests/text_area_semantics.rs`、`apps/docs-app/src/pages/components/pages/forms.rs`、`apps/web-demo/src/demos/text_area_demo.rs`、`e2e/tests/docs_app_text_area_contract.spec.mjs`。
- 受控/非受控与命名兼容：`value + on_value_change + default_value` 为主路径，`set_value/disabled/read_only/required/invalid` 作为兼容别名并在 `logic.rs` 归一。
- N/A（按组件职责判定）：异步协议、LLM 流式正文显示、`spec.rs` 引入、业务级 runtime 暴露等条目对 TextArea 叶子输入组件不适用，已按“无该能力即 N/A”逐条核验。
- 本地链路命令（`/root/.cargo/bin/cargo`，2026-02-18）：
  - 已通过：
    - `cargo fmt --all`
    - `cargo test -p ui-state-primitives text_area`
    - `cargo test -p ui-components --no-default-features --features component-text_area,inject-css --test text_area_semantics`
    - `cargo check -p ui-components --no-default-features --features component-text_area,inject-css`
    - `cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-text_area,inject-css --offline`
    - `cargo check -p docs-app --offline`
    - `cargo check -p web-demo --offline`
    - `cargo clippy -p ui-state-primitives --all-targets -- -D warnings -A clippy::too_many_arguments`
    - `cargo clippy -p ui-components --no-default-features --features component-text_area,inject-css --test text_area_semantics -- -D warnings -A clippy::too_many_arguments -A clippy::clone_on_copy -A unused_macros -A unused_imports -A dead_code`
    - `apps/docs-app/dist` 静态产物 smoke：`python3 -m http.server + playwright screenshot --wait-for-selector 'body:not(:has(#boot))'`，结果 `smoke-docs-app(dist): ok`（`/tmp/rust-ui-smoke-docs-dist.6fquRJ.png`）。
    - `apps/web-demo/dist` 静态产物 smoke：`python3 -m http.server + playwright screenshot --wait-for-selector 'body:not(:has(#boot))'`，结果 `smoke-web-demo(dist): ok`（`/tmp/rust-ui-smoke-web-dist.vpeEi6.png`）。
- 本轮复验通过的外部阻断点：
  - `crates/ui-headless/src/slider.rs`：`is_focus_visible` 类型已与 `use_focus_ring` 对齐（`Memo<bool>`），不再阻断 wasm/docs/web-demo 链路。
  - `crates/ui-components/src/button/copy/motion.rs`：`attach_motion` 参数改为 `Signal<bool>`，与 snippet logic 输出一致。
  - `apps/docs-app/src/pages/components/pages/collections.rs`：修复 `TagGroup tags` 的 `ReadSignal<Vec<Tag>>` 传参，以及 `Tabs labels` move 语义错误。
...
- [x] `ui-headless` 定义：交互与 A11y 原语层（press/focus/hover/roving/listbox/menu/tooltip 等），把输入设备事件与状态语义标准化为可复用契约；输出必须是类型化 `attrs + handlers + state`。不做样式、不写组件 CSS、不做组件级动效编排。（`TextArea` 在 `view.rs` 通过 `use_text_field + use_focus_ring + locale_attrs` 挂载 typed `aria-* + handlers + focus state + lang/dir` 语义契约；组件层未重写 headless A11y 工具，相关回归见 `crates/ui-components/tests/text_area_semantics.rs` 的 `text_area_view_uses_logic_state_and_a11y_contracts`。）
...
- [x] SSR 与跨平台检查：覆盖 web/ssr/wasm 分支，不破坏 non-wasm 编译路径。（native + wasm compile-only 已复验通过）
...
- [x] `ui-headless` web/ssr feature 互斥受 `compile_error!` 保护（`crates/ui-headless/src/lib.rs`）。（本轮复验未破坏互斥约束）
...
- [x] 组件实现覆盖 `reduced-motion` / SSR / wasm 分支。（`reduced-motion`/SSR 语义测试 + wasm compile-only 均通过）
