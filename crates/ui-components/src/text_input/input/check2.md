...
[x] 本轮推进结论：`Input/InputGroup/InputOtp` 语义测试与 docs 覆盖已落实；API 命名统一、受控/非受控成对、Input 专项 E2E、`reduced-motion` 与渲染预算治理均已完成并复验通过。

### 1. 大骨架（架构边界与层职责）
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（`Input` 状态原语下沉至 `crates/ui-state-primitives/src/input.rs`，`InputGroup` 状态原语下沉至 `crates/ui-state-primitives/src/input_group.rs`，组件侧 `logic.rs` 改为消费 `ui_state_primitives::*` 并只保留装配逻辑）
...
- [x] `ui-headless` 定义：交互与 A11y 原语层（press/focus/hover/roving/listbox/menu/tooltip 等），把输入设备事件与状态语义标准化为可复用契约；输出必须是类型化 `attrs + handlers + state`。不做样式、不写组件 CSS、不做组件级动效编排。（已完成：`Input` Escape 清空键盘模型下沉到 `crates/ui-headless/src/clearable_text_field.rs`，`Input/InputGroup/InputOtp` 接入 `lang/dir` locale attrs，新增 `e2e/tests/docs_app_input_contract.spec.mjs` 并实跑通过）
...
Input/InputGroup/InputOtp 无远程请求与异步状态轴，异步交互语义项为 `N/A`（仅同步输入与状态派生）。
...
Input/InputGroup/InputOtp 归类为 `Streaming Optional`，当前实现按 `fallback=snapshot` 消费完整配置并持续输出 `role/aria/data-*` 语义标记。
...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - 说明：本项按 input 负责范围执行（feature 编译 + `input/input_group/input_otp` 语义测试 + clippy）；仓库级 smoke 属于整仓门禁，在并行开发环境下标记为 `N/A`，不作为本组件阻断。
