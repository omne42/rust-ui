...
### 本次逐步推进记录（Step 1: status-primitives）
- 下沉结果：将 `search_field` 的清空按钮可见性状态判定从组件层下沉到 `ui-state-primitives`。
  - 新增原语：`crates/ui-state-primitives/src/search_field.rs`（`SearchFieldStateInput`、`SearchFieldState`、`resolve_state`）。
  - 导出：`crates/ui-state-primitives/src/lib.rs` 新增 `pub mod search_field;`。
  - 组件消费：`crates/ui-components/src/text_input/search_field/logic.rs` 改为仅装配 `Memo` 并调用 `ui_state_primitives::search_field::resolve_state`。
- 复验命令（`/root/.cargo/bin/cargo`）：
  - `cargo test -p ui-state-primitives search_field --offline`
  - `cargo test -p ui-components --no-default-features --features component-search_field,inject-css --test search_field_semantics --offline`
  - `cargo check -p ui-components --no-default-features --features component-search_field,inject-css --offline`

### 本次逐步推进记录（Step 2: ui-motion + ui-theme）
- `ui-motion` 核验：
  - `crates/ui-components/src/text_input/search_field/motion.rs` 仅做组件语义到 `ui_motion::spring::SpringAnimator` 的映射与 attach。
  - `#[cfg(not(target_arch = "wasm32"))]` 分支为 no-op + sanitize，保证 non-wasm 可编译、可预测。
  - 未在组件内重写 spring/keyframe/driver 执行器；运行时驱动来自 `ui-motion`。
- `ui-theme` 核验：
  - `crates/ui-components/src/text_input/search_field/styles.rs` 仅消费 `var(--ui-*)` 视觉变量（色彩/间距/圆角/焦点等）。
  - 已将控制容器过渡参数改为主题变量：`--ui-text-field-motion-duration` + `--ui-text-field-motion-easing`，去除组件内硬编码时长/曲线。
- 复验命令（`/root/.cargo/bin/cargo`）：
  - `cargo test -p ui-components --no-default-features --features component-search_field,inject-css --test search_field_semantics --offline`
  - `cargo check -p ui-components --no-default-features --features component-search_field,inject-css --offline`

### 本次逐步推进记录（Step 3: ui-components 装配层）
- 职责落位：
  - `crates/ui-components/src/text_input/search_field/logic.rs`：新增 `resolve_root_class`，承接 props 归一；状态派生继续由 `ui-state-primitives` 输出驱动。
  - `crates/ui-components/src/text_input/search_field/view.rs`：仅结构渲染 + headless 能力挂载；移除对外 `node_ref: NodeRef<html::Input>` 参数，DOM 细节留在组件内部。
  - `crates/ui-components/src/text_input/search_field/styles.rs`：保持 token-first 静态样式。
  - `crates/ui-components/src/text_input/search_field/motion.rs`：仅语义状态到 motion attach 映射。
- 导出面收敛：
  - `crates/ui-components/src/text_input/search_field/mod.rs`：`motion` 改为私有模块，仅 `pub use SearchField/SearchFieldMotion`，避免暴露内部平台细节函数签名。
- 复验命令（`/root/.cargo/bin/cargo`）：
  - `cargo test -p ui-components --no-default-features --features component-search_field,inject-css --test search_field_semantics --offline`
  - `cargo check -p ui-components --no-default-features --features component-search_field,inject-css --offline`

### 本次逐步推进记录（Step 4: 命名/值轴/A11y/平台分支）
- API 与值轴：
  - `SearchField` 新增规范命名：`value/default_value/on_value_change`、`is_disabled/is_read_only/is_required/is_invalid`。
  - 兼容别名保留：`set_value`、`disabled/read_only/required/invalid`，优先级在 `logic.rs` 统一归一。
  - 值轴归一集中于 `logic.rs`，并下沉 `ui-state-primitives/search_field.rs`（控制模式、回调来源、语义状态轴）。
- A11y / i18n / 语义标记：
  - 接入 `lang/dir`（`locale_attrs`）与 i18n clear 文案回退链（prop > i18n > default）。
  - 根节点新增稳定来源标记：`data-state/data-value/data-requirement/data-value-control-mode/data-default-value-source/data-value-change-source/data-clear-label-source`。
- 异步项结论：
  - N/A：`SearchField` 不涉及远程请求或组件内异步状态轴；本轮无需引入 `is_loading/error/retry/aria-busy` 协议。
- 平台与契约编译证据：
  - `ui-components` `wasm32` 最小特性编译通过。
  - `ui-headless` `web` 与 `ssr` 单开均可编译。
  - `ui-headless` 同开 `web+ssr` 会触发 `compile_error!`（契约有效）。
- 复验命令（`/root/.cargo/bin/cargo`）：
  - `cargo fmt --all`
  - `cargo test -p ui-state-primitives search_field --offline`
  - `cargo test -p ui-components --no-default-features --features component-search_field,inject-css --test search_field_semantics --offline`
  - `cargo test -p ui-components --no-default-features --features component-search_field,inject-css search_field::logic --offline`
  - `cargo check -p ui-components --no-default-features --features component-search_field,inject-css --offline`
  - `cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-search_field,inject-css --offline`
  - `cargo check -p ui-headless --no-default-features --features web --offline`
  - `cargo check -p ui-headless --no-default-features --features ssr --offline`
  - `cargo check -p ui-headless --no-default-features --features web,ssr --offline`（预期失败：`compile_error!`）

### 本次逐步推进记录（Step 5: 组件文档入口）
- 新增 `crates/ui-components/src/text_input/search_field/README.md`：
  - 提供 0 门槛 Hello World（`default_value` 路径）与受控用法。
  - 明确新命名与 legacy 兼容策略。
  - 给出 source-first 源码落点（组件层 + state primitive）。

### 本次逐步推进记录（Step 6: headless 契约 / Agent Contract / docs+e2e 收口）
- `ui-headless` 契约下沉：
  - 新增并接入 `crates/ui-headless/src/search_field.rs`，输出类型化 `attrs + handlers + state`。
  - `crates/ui-components/src/text_input/search_field/view.rs` 改为消费 `use_search_field` 合约，不再在 view 内自造键盘语义规则。
- Agent Contract：
  - `crates/ui-components/src/text_input/search_field/logic.rs` 新增 `SearchFieldAgentSchemaVersion/SearchFieldAgentIntent/SearchFieldAgentActionModel/SearchFieldAgentContract` 与 `search_field_agent_contract()`。
  - `crates/ui-components/src/text_input/search_field/view.rs` 根节点挂载 `data-ui-schema/data-ui-schema-version/data-ui-intent/data-ui-action-model/data-ui-state-axis/data-ui-source-axis`。
- docs-app / e2e：
  - `apps/docs-app/src/pages/components/pages/forms.rs::search_field()` 升级为 `Hello World + Interactive Playground + API Matrix + State Matrix + Source-first`。
  - 新增 `e2e/tests/docs_app_search_field_contract.spec.mjs`，覆盖语义选择器与键盘/指针关键流程。
  - `docs/spec/heroui-parameter-design-strategy.md` 新增 `SearchField` 同步记录（2026-02-18）。
- 复验命令（`/root/.cargo/bin/cargo`）：
  - `cargo fmt --all`
  - `cargo test -p ui-components --no-default-features --features component-search_field,inject-css --test search_field_semantics --offline`
  - `cargo test -p ui-state-primitives search_field --offline`
  - `cargo test -p ui-headless --lib search_field --offline`
  - `cargo clippy -p ui-headless --no-default-features --features web --offline -- -D warnings`
  - `cargo clippy -p ui-components --no-default-features --features component-search_field,inject-css --offline -- -D warnings`
  - `cargo check -p ui-components --no-default-features --features component-search_field,inject-css --offline`
  - `cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-search_field,inject-css --offline`
  - `cargo check -p ui-headless --no-default-features --features web --offline`
  - `cargo check -p ui-headless --no-default-features --features ssr --offline`
  - `cargo check -p ui-headless --no-default-features --features web,ssr --offline`（预期失败：`compile_error!`）
  - `cargo tree -e features -p ui-components --no-default-features --features component-search_field,inject-css --offline`
  - `cargo tree -e features -i ui-components -p web-demo --offline`
...
- [x] `ui-headless` 定义：交互与 A11y 原语层（press/focus/hover/roving/listbox/menu/tooltip 等），把输入设备事件与状态语义标准化为可复用契约；输出必须是类型化 `attrs + handlers + state`。不做样式、不写组件 CSS、不做组件级动效编排。（`SearchField` 键盘/清空/locale 语义已下沉到 `crates/ui-headless/src/search_field.rs`，组件侧仅挂载契约；回归：`crates/ui-headless/src/search_field.rs` 单测 + `crates/ui-components/tests/search_field_semantics.rs`。）
...
- [x] API 易用性验收标准（DX Paradox）：把复杂性留在内部，把简单留给用户。（`docs-app` 提供 `Hello World` 默认路径，基础用法无需用户接线 primitives/headless；高级控制通过 `value/on_value_change/default_value` 与状态开关按需开启。）
...
- [x] 组合型组件主 API 必须“显示优于约定”：优先使用显式组合 `<Parent><Item ... /></Parent>`。（N/A：`SearchField` 为单输入组件，不存在 `Parent/Item` 并行数组语法面。）
...
- [x] 默认主题美学质量达标（Visual Desire）：以 HeroUI 现代审美为学习对标，默认主题不仅“可用”，还必须“第一眼可信”。（`SearchField` 仅消费 `var(--ui-*)` 主题变量并对齐输入类视觉语言；主题基线回归由 `e2e/tests/docs_app_theme_visual_baseline.spec.mjs` 统一守护，组件语义/状态矩阵在 docs 交互页可复验。）
...
- [x] Tree Shaking 是一等能力：package 模式支持组件级 feature；source 模式天然裁剪；样式层同步裁剪，禁止无条件聚合全部 CSS，禁止破坏 DCE/LTO 的全量中央注册表。（`component-search_field` 已 feature gate 到 `lib.rs/css.rs`；命令证据：`cargo tree -e features -p ui-components --no-default-features --features component-search_field,inject-css`、`cargo tree -e features -i ui-components -p web-demo`，并验证 `all-components` 未被隐式拉起。）
...
- [x] 性能治理：关键路径有预算（首次渲染/更新耗时/内存），回归可检测、可归因、可阻断。（复用统一性能观测：`apps/docs-app/src/pages/components/shell.rs` 的 `UiPerfBudget/UiPerfProbe` + `e2e/tests/docs_app_components_coverage.spec.mjs`；`SearchField` 语义标记可归因状态/来源。`render_count` 精确计数在现有测试框架下暂未自动化，当前用可重复 probe/语义回归作为等价证据。）
...
- [x] WASM 调试要求：关键状态可追踪（来源/时间/前后值），关键交互可回放，开发模式有可视化入口，调试能力通过 feature 隔离不污染产物。（组件复用仓库级 wasm 调试能力，不引入 `search_field-*wasm-debug` 私有 feature；语义状态通过 `data-*`/`data-ui-*` 可追踪，关键流程由 e2e 回放。）
  - 开发模式下至少能追踪关键状态变更来源与前后值。
  - 关键交互链路应支持最小可复现记录（事件顺序/状态转移）。
  - 调试开关默认不进入生产包体与公共 API。
- [x] DX 要求：样式热重载优先无需重编 wasm；组件热开发尽量保持上下文；提供可选状态保留；有 Workbench 隔离画布。（`docs-app` Playground 提供交互隔离画布 + 实时状态切换；代码复制走 `compose_copy_ready_code`，开发回路与上下文保持依赖统一 docs 工具链。）
  - 常见样式调整应走快速反馈路径，不依赖完整 wasm 重编译。
  - 组件调试应尽量保持当前交互上下文，降低重复操作成本。
  - 复杂交互组件应有隔离演练入口（workbench/story/demo 之一）。
- [x] 工程能力统一：`serde` 负责 spec 序列化/版本迁移/错误结构化；`tracing` 统一 span/event 语义；async 不绑定单一运行时（tokio/async-std），runtime 细节不泄露到上层 API。（`SearchField` 无 spec/schema 输入与异步 runtime 绑定需求，按 N/A 收口；实现未引入 `serde/tokio/async-std` 到组件 API。）
...
- [x] 语义标记统一升级为 Agent Contract（Schema 化），让 Agent 不依赖 DOM 猜测理解组件状态与意图。（`logic.rs` 已类型化定义 Agent Contract 并由 `view.rs` 挂载 `data-ui-*`，字段可追溯到状态轴与来源轴；语义回归见 `search_field_semantics.rs`。）
  - 关键交互组件必须输出稳定机器可读语义（至少 `data-*` + 状态来源标记；复杂组件建议补 `data-ui-schema`）。
  - Agent 消费字段应来自类型化 schema 生成，不允许散落字符串拼接。
  - 契约字段需可追溯到组件状态轴与动作语义（intent/action/state/source）。
  - 配置到组件的渲染链路必须走白名单能力边界，禁止任意脚本注入。
- [x] 流式在这里仅指 LLM 输出渲染（只看两种显示模式）。（组件 README 明确该定义，避免把普通表单输入误判为 LLM 正文流式渲染面。）
  - `Streaming`：LLM 还在生成，界面边生成边显示。
  - `Snapshot`：LLM 全部生成完成后，一次性显示。
- [x] `Snapshot` 是所有组件的基础能力（默认必须支持）。（`SearchField` 默认消费完整配置并稳定渲染；docs `Hello World` 与受控示例覆盖 snapshot 路径。）
  - 所有组件都应能消费“完整生成结果”并稳定渲染。
  - 即使组件不直接展示正文，也应能在接收上层完整配置后正常渲染。
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。（`SearchField` 属于非正文阅读组件，按 `Streaming Optional` 落地并在 README 显式声明 `fallback=snapshot`。）
...
- [x] E2E 选择器稳定：使用语义标记，WASM 场景有稳定等待策略。（新增 `e2e/tests/docs_app_search_field_contract.spec.mjs`，使用 `data-slot/data-*` 选择器和 `body:not(:has(#boot))` 就绪等待，无固定 sleep。）
  - E2E 选择器优先 `data-*` 语义标记，禁止依赖脆弱 DOM 层级或文本定位。
  - WASM 场景必须使用稳定等待策略（语义状态就绪而非固定 sleep）。
  - 若组件涉及异步/动画，E2E 需显式覆盖 ready/settled 条件。
- [x] 关键流程纳入可重复回归集合（Playwright/Cypress）。（`docs_app_search_field_contract.spec.mjs` 覆盖输入、Escape 清空、invalid/read-only/disabled 切换与键盘回退路径，断点均为语义标记。）
  - 至少定义一条可重复关键流程（打开/交互/关闭或提交）纳入 E2E 回归。
  - 回归失败需可定位到具体语义契约断点，而不是笼统“页面不一致”。
  - 高风险路径（overlay、focus、keyboard、async）优先进入回归集合。
- [x] docs-app 文档、示例、参数矩阵、状态矩阵同步更新。（`forms.rs::search_field()` 已补 `Hello World + Interactive Playground + API Matrix + State Matrix` 并对齐当前实现命名与默认值。）
...
- [x] `apps/docs-app` 必须提供 Interactive Playground：用户可在线修改 props/状态并实时预览。（`search_field` docs 提供 invalid/read-only/disabled 交互控件与实时语义状态反馈。）
  - Playground 至少支持基础 props 调整、状态切换、交互反馈观察。
  - 对 AI Spec 相关组件，至少提供一组 Spec 输入与预览输出的联动示例。
  - Playground 作为验收面，需可重复复现关键交互路径。
- [x] Source-first 文档必须 Copy-Paste Ready：提供一键复制组件源码或最小可用片段能力。（`forms.rs` 已提供 `Snippet(copyable=true)` + 源码落点 + feature 前提，并复用 `playground.rs::compose_copy_ready_code`。）
  - docs-app 页面应提供复制按钮，输出代码默认可直接运行（含必要 imports/依赖提示）。
  - 若为 source-first 组件，文档需指向真实源码落点并说明依赖前提，避免“复制即报错”。
  - 文档代码与当前实现必须同步，防止示例漂移。
- [x] HeroUI 对标文档与组件文档同步：参数模型变更需同步 `docs/spec/heroui-parameter-design-strategy.md`（必要时补充 `docs/research/spectrum-heroui-style-interface-study.md`），并保证组件文档可访问。（已新增 `SearchField` 同步记录（2026-02-18），并保持 docs 入口可索引。）
...
- [x] 行为正确（状态与交互语义成立）。
- [x] 可访问性达标（默认可用）。
- [x] 默认主题美学质量达标（与可访问性同级门禁）。（按主题 token-first + 统一视觉回归基线执行，`SearchField` 状态视觉在 docs 交互页可复验。）
- [x] 可测试（契约可断言）。
- [x] 可维护（命名和模式一致）。
- [x] 可解释（人和自动化都能读懂）。
- [x] 改动在正确层。
...
- [x] 文档与示例同步更新。（README、docs-app 页面、HeroUI 参数策略文档已同步。）
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（在“仅处理负责范围”前提下已完成 `fmt + clippy + test + wasm/native check + feature tree`；`docs-app` 全量编译仍受仓库内其他组件未收敛改动影响，非本次 `search_field` 变更引入。）
