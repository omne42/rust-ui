...
use crate::text_input::time_field::{
...
use ui_headless::{
    A11yDirection, HoverOptions, HoverState, PressOptions, PressState, TimeFieldOptions, use_hover,
    use_press, use_time_field,
};
...
fn render_clear_button(
    ...
    clear_press: PressState,
    on_clear: Callback<()>,
    clear_aria_label: StoredValue<String>,
    clear_label: StoredValue<String>,
    clear_ref: NodeRef<html::Button>,
) -> impl IntoView {
    let is_visible = Signal::derive(move || state.get().has_value);
    let is_disabled_signal = Signal::derive(move || !state.get().has_value);
    let on_pointer_enter = clear_hover.handlers.on_pointer_enter;
    let on_pointer_leave = clear_hover.handlers.on_pointer_leave;
    let on_pointer_down = clear_press.handlers.on_pointer_down;
    let on_pointer_up = clear_press.handlers.on_pointer_up;
    let on_pointer_cancel = clear_press.handlers.on_pointer_cancel;
    let on_click = clear_press.handlers.on_click;
    let on_key_down = clear_press.handlers.on_key_down;
    let on_key_up = clear_press.handlers.on_key_up;
    let on_blur = clear_press.handlers.on_blur;
    let clear_aria_label_text = clear_aria_label.get_value();

    view! {
        <ClearButton
            slot_name=SLOT_TIME_FIELD_CLEAR
            aria_label=clear_aria_label_text
            class_name=CLASS_TIME_FIELD_CLEAR.to_string()
            node_ref=clear_ref
            is_visible=is_visible
            is_disabled_signal=is_disabled_signal
            ...
            on_pointer_cancel=Callback::new(move |_| on_pointer_cancel.run(()))
            on_pointer_enter=Callback::new(move |_| on_pointer_enter.run(()))
            on_pointer_leave=Callback::new(move |_| on_pointer_leave.run(()))
            ...
            on_blur=Callback::new(move |_| on_blur.run(()))
        >
...
pub fn TimeField(
    ...
    let on_clear_press = Callback::new(move |_: ()| {
