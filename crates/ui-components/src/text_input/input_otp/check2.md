...
### 本轮执行证据（InputOtp / check2）
- 回归项：`crates/ui-components/tests/input_otp_semantics.rs` 新增 `input_otp_e2e_contract_uses_semantic_selectors`，锁定 docs-app OTP 用例的语义选择器契约。
- 文档与工作台补齐：`apps/docs-app/src/pages/components/pages/forms.rs` 新增 `Interactive Playground`（展示/Config/Code/CSS Test）与 `State Comparison`（多场景对比）。
- README 补齐：`crates/ui-components/src/text_input/input_otp/README.md` 已覆盖 `展示区（Display）`、`Config 区`、`Code 区`、`CSS Test 区`、`多种情况对比显示`。
- 已存在语义覆盖：`crates/ui-components/tests/input_otp_semantics.rs` 覆盖 hidden input、headless hooks、i18n/locale、样式与 docs playground 契约。
- E2E 扩展：`e2e/tests/docs_app_input_contract.spec.mjs` 新增 comparison 回归（default/disabled/invalid 三场景语义契约）。
- 已存在 E2E 覆盖：`e2e/tests/docs_app_input_contract.spec.mjs` 包含 `docs-app input-otp normalizes digits and preserves slot contracts`，使用 `data-slot` 语义选择器与稳定等待。
- 2026-02-18 补充稳态锚点：`apps/docs-app/src/pages/components/pages/forms.rs` 为 InputOtp Interactive/State Comparison 增加 `data-slot="input-otp-workbench"` 与 `data-slot="input-otp-state-compare"`。
- 2026-02-18 E2E 选择器加固：`e2e/tests/docs_app_input_contract.spec.mjs` comparison 用例改为先定位 `input-otp-state-compare` 区块，再断言 default/disabled/invalid 三状态，避免跨区块误匹配。
- 已执行验证：
  - `cargo tree -e features -p ui-components --no-default-features --features component-input_otp,inject-css --depth 3`（通过）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo check -p ui-components --no-default-features --features component-input_otp,inject-css`（通过）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-input_otp,inject-css`（通过，存在仓库既有 `observability.rs` dead_code warning，不阻断 check）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo test -p ui-components --test input_otp_semantics --no-default-features --features component-input_otp,inject-css`（通过，10/10）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo clippy -p ui-components --no-default-features --features component-input_otp,inject-css --all-targets -- -D warnings`（通过）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo test -p ui-motion --test non_wasm_stub`（通过，2/2）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo check -p ui-headless --no-default-features --features web`（通过）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo check -p ui-headless --no-default-features --features ssr`（通过）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo check -p ui-headless --no-default-features --features web,ssr`（按预期失败，命中 `compile_error!`）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo test -p ui-components --test input_otp_semantics --no-default-features --features component-input_otp,inject-css`（通过，10/10，复跑）
  - `node --check e2e/tests/docs_app_input_contract.spec.mjs`（通过）
  - `trunk build`（阻塞：`apps/docs-app` 存在仓库并行改动导致的既有编译错误，首个失败位于 `apps/docs-app/src/pages/components/pages/display_extra_icons_workflow.rs:347`，非本组件修改引入）
...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。（N/A：`InputOtp` 仅本地输入归一与同步回调，无异步加载协议）
...
- [x] 流式在这里仅指 LLM 输出渲染（只看两种显示模式）。（N/A：`InputOtp` 非 LLM 正文渲染组件）
  - `Streaming`：LLM 还在生成，界面边生成边显示。
  - `Snapshot`：LLM 全部生成完成后，一次性显示。
- [x] `Snapshot` 是所有组件的基础能力（默认必须支持）。（`InputOtp` 支持完整值快照渲染）
  - 所有组件都应能消费“完整生成结果”并稳定渲染。
  - 即使组件不直接展示正文，也应能在接收上层完整配置后正常渲染。
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。（判定：`Streaming Optional`，`fallback=snapshot`）
