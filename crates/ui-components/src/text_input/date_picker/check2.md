...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（DatePicker scoped gates：primitives test + semantics test + minimal check + wasm check + headless 互斥校验）。

### 10. 本次核验证据（DatePicker / check2）
- 代码分层修复：`DatePicker` 状态原语已下沉到 `crates/ui-logic-calendar/src/date_picker.rs`，并在 `crates/ui-state-primitives/src/lib.rs` 导出；`crates/ui-components/src/text_input/date_picker/logic.rs` 改为仅 re-export + 组件装配。
- A11y/i18n 修复：`crates/ui-components/src/text_input/date_picker/view.rs` 已接入 `lang/dir`（`ui_headless::a11y::locale_attrs`），不再假设单语言/单方向。
- 语义回归：`crates/ui-components/tests/date_picker_semantics.rs` 新增/更新为 13 条契约测试（含 primitives 来源、feature 依赖、locale 接入、样式/状态标记、docs 覆盖、README 白名单与 workbench/comparison 文档约束）。
- docs shell 白名单：`apps/docs-app/src/pages/components/shell.rs` 已将 `"date-picker"` 映射到 `crates/ui-components/src/text_input/date_picker/README.md`，沿用静态 `include_str!` + `_ => None` 安全边界。
- docs 展示能力：`apps/docs-app/src/pages/components/pages/forms_extra.rs::date_picker()` 已提供 `展示 / Config / Code / CSS Test` workbench 与 `Comparison Matrix (Default / Quiet / Strong / Disabled)` 对比区。
- 组件文档：`crates/ui-components/src/text_input/date_picker/README.md` 已补充 docs-playground 四区说明与多场景对比说明。
- primitives 回归：`cargo test -p ui-state-primitives date_picker` 通过（4 tests）。
- 组件语义回归：`cargo test -p ui-components --test date_picker_semantics --no-default-features --features component-date_picker,inject-css` 通过（13 tests）。
- 最小特性编译：`cargo check -p ui-components --no-default-features --features component-date_picker,inject-css` 通过。
- wasm 最小特性编译：`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-date_picker,inject-css` 通过。
- 最小特性静态检查：`cargo clippy -p ui-components --no-default-features --features component-date_picker,inject-css -- -D warnings` 通过。
- 特性树证据：`cargo tree -e features -p ui-components --no-default-features --features component-date_picker,inject-css -f "{p} {f}"` 显示 `component-date_picker` 正确拉起 `component-button/component-calendar/component-popover`。
- headless 互斥证据：
  - `cargo check -p ui-headless --no-default-features --features web` 通过；
  - `cargo check -p ui-headless --no-default-features --features ssr` 通过；
  - `cargo check -p ui-headless --no-default-features --features web,ssr` 按预期触发 `compile_error!` 失败。
- 限定说明：
  - 本次只处理 `date_picker` 相关文件，未改动其他组件实现；
  - 由于协作分支存在非本任务编译问题，本次验证均采用 DatePicker 最小特性路径，避免干扰他人工作树。
