...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（本轮按组件最小范围执行并通过：`cargo test -p ui-components --test command_dialog_semantics --no-default-features --features component-command_dialog,inject-css`、`cargo check -p ui-components --no-default-features --features component-command_dialog,inject-css`、`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-command_dialog,inject-css`；全仓默认特性构建存在他人模块既有错误，不在本任务改动范围。）

### 10. 本轮验证证据
- 代码落点：
  - `crates/ui-components/src/command_dialog/motion.rs`（新增，`CommandDialogMotion + attach_motion`）
  - `crates/ui-components/src/command_dialog/view.rs`（接入 motion，补 `data-ui-*` / `data-stream-*` / `data-output-status`）
  - `crates/ui-components/src/command_dialog/mod.rs`（导出 `motion` 与 `CommandDialogMotion`）
  - `crates/ui-components/src/command_dialog/README.md`（新增新手友好文档）
  - `e2e/tests/docs_app_command_dialog.spec.mjs`（新增关键流程 E2E 用例）
  - `crates/ui-components/tests/command_dialog_semantics.rs`（新增 README / E2E / motion / Agent 标记语义守卫）
- 关键命令：
  - `cargo test -p ui-components --test command_dialog_semantics --no-default-features --features component-command_dialog,inject-css`
  - `cargo check -p ui-components --no-default-features --features component-command_dialog,inject-css`
  - `cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-command_dialog,inject-css`
  - `cargo tree -e features -p ui-components --no-default-features --features component-command_dialog,inject-css`
