...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（本次按组件范围执行：`component-illustrated_message` 最小特性 `check + clippy + illustrated_message_semantics`。）

### 10. 本轮验收记录（IllustratedMessage）
- 语义契约回归：`cargo test -p ui-components --test illustrated_message_semantics --no-default-features --features component-illustrated_message,inject-css` 通过，8/8 用例通过。
- 最小特性编译（native）：`cargo check -p ui-components --no-default-features --features component-illustrated_message,inject-css` 通过。
- 最小特性编译（wasm）：`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-illustrated_message,inject-css` 通过。
- 静态检查：`cargo clippy -p ui-components --no-default-features --features component-illustrated_message,inject-css -- -D warnings` 通过。
- Tree Shaking 约束：`component-illustrated_message` 为独立 feature（`crates/ui-components/Cargo.toml`），`lib.rs` 与 `css.rs` 均按 feature 条件导出/聚合。
- N/A 说明：组件无异步请求链路（`is_loading`/retry/`aria-busy` 不适用）；非组合型 item 组件（并行数组约束不适用）；无 `inner_html` 注入路径。
