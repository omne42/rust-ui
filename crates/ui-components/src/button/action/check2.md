...
- [x] Tree Shaking 是一等能力：package 模式支持组件级 feature；source 模式天然裁剪；样式层同步裁剪，禁止无条件聚合全部 CSS，禁止破坏 DCE/LTO 的全量中央注册表。（手工验证：`cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-action_group,inject-css` 仅包含 `component-action_group -> component-button` 与 `inject-css`；`cargo tree -e features -i ui-components -p web-demo` 未出现 `all-components` 且由 `web-demo-components` 拉起；`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-action_group,inject-css` 通过）
...
- [x] SSR 与跨平台检查：覆盖 web/ssr/wasm 分支，不破坏 non-wasm 编译路径。（`ActionButton` 复用 `Button` 与 `ui-motion/ui-headless` 的平台分支契约：native/ssr/wasm compile-only 由 `scripts/check-ui-components-platforms.sh` 门禁覆盖；组件自身不引入 `web-sys/js_sys` 浏览器依赖）
...
- [x] DX 要求：样式热重载优先无需重编 wasm；组件热开发尽量保持上下文；提供可选状态保留；有 Workbench 隔离画布。（新增 `ActionButton Workbench`：`apps/docs-app/src/pages/components/pages/actions.rs`，包含 `data-slot=\"action-button-workbench(-controls|-canvas)\"`、可选状态保留（`localStorage`）与 scoped CSS live-edit；样式直接复用 `ui_components::button::styles::CSS`（借助 button 能力简化）；回归：`action_button_dx_playground_supports_css_hot_reload_without_wasm_rebuild`、`action_button_dx_workbench_supports_optional_state_persistence_and_isolated_canvas`，并纳入 `scripts/check-ui-components-dx.sh`）
...
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo test -p ui-components --test action_button_semantics --no-default-features --features component-button,component-action_button_group,component-action_group,inject-css`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo test -p ui-components --test action_button_group_semantics --no-default-features --features component-button,component-action_button_group,inject-css`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo test -p ui-components --test action_group_semantics --no-default-features --features component-button,component-action_group,inject-css`
