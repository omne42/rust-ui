...
2. 组件职责落位：已补 `crates/ui-components/src/color_handle/motion.rs`，并在 `view.rs` 挂载 `motion::attach_motion` 与 `data-motion-source`；`styles.rs` 改为消费 `--ui-color-handle-motion-duration`。
