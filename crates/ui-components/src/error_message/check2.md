...
### 本轮核验记录（error_message）
- 状态原语下沉：`ErrorMessageTone/ErrorMessageElement/ErrorMessageStateInput/ErrorMessageState` 与归一化函数已下沉到 `crates/ui-state-primitives/src/error_message.rs`，组件 `logic.rs` 改为 bridge re-export。
- Headless 语义契约：新增 `crates/ui-headless/src/error_message.rs`，统一输出 `role/aria-live/lang/dir` 与 `data-*` 状态来源标记，`view.rs` 改为挂载契约。
- Agent Contract + Snapshot：`error_message/view.rs` 新增 `data-ui-schema/data-ui-intent/data-ui-action/data-ui-state/data-ui-source` 与 `snapshot` 标记。
- 回归补齐：`crates/ui-components/tests/error_message_semantics.rs` 增加 primitives/headless/agent-contract/check2 完整性断言；新增 `e2e/tests/docs_app_error_message_contract.spec.mjs` 覆盖语义选择器与可重复回放路径。
- docs/readme 与 playground：`crates/ui-components/src/error_message/README.md` 新增 `display/config/code/css test` 分区；`apps/docs-app/src/pages/components/pages/forms_extra.rs` 的 `error_message()` 新增 Display 对比与 Config+Code+CSS Test workbench。
- E2E 本机执行状态：本机已可启动 Playwright；`semantic markers` 与 `repeatable` 两条通过，但 `playground code path` 受 docs-app 构建失败覆盖层拦截点击而超时（构建错误来自其他页面，非 error_message 范围）。
- 额外约束回归：`crates/ui-components/tests/error_message_semantics.rs` 新增 README 分区断言（display/config/code/css test）与 docs workbench 断言（`Display Comparisons` + `Config + Code + CSS Test Workbench` + `test_css_source/test_config_signal`）。
- 本轮执行命令：
  - `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.error-message-check cargo check -p ui-components --no-default-features --features component-error_message,inject-css`
  - `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.error-message-check cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-error_message,inject-css`
- `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.error-message-check cargo clippy -p ui-components --no-default-features --features component-error_message,inject-css --tests -- -D warnings`
- `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.error-message-check cargo clippy -p ui-components --no-default-features --features component-error_message,inject-css --test error_message_semantics -- -D warnings`（全量 `--tests` 受其他测试文件阻塞）
- `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.error-message-check CARGO_BUILD_JOBS=1 cargo test -p ui-components --test error_message_semantics --no-default-features --features component-error_message,inject-css`
  - `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.error-message-check cargo test -p ui-state-primitives error_message`
  - `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.error-message-check cargo test -p ui-headless --lib error_message`
  - `node --check e2e/tests/docs_app_error_message_contract.spec.mjs`
  - `cd e2e && npx playwright test tests/docs_app_error_message_contract.spec.mjs --project=chromium --reporter=list`（当前环境缺少浏览器二进制，执行失败）
