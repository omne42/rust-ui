...
### 本轮执行证据（AlertDialog / check2）
- 修复项 1：`crates/ui-components/Cargo.toml` 将 `component-alert_dialog` 从空特性修正为 `["component-overlay", "component-button"]`，修复最小特性构建缺依赖问题。
- 修复项 2：`crates/ui-components/src/alert_dialog/view.rs` 导入改为 `crate::button::{Button, ButtonVariant}`，避免最小特性下根导出缺失导致编译失败。
- 回归项：`crates/ui-components/tests/alert_dialog_semantics.rs` 新增 `alert_dialog_feature_gate_declares_required_component_dependencies` 与 `alert_dialog_e2e_contract_uses_semantic_selectors`，锁定 feature 依赖与 E2E 语义选择器契约。
- E2E 补齐：新增 `e2e/tests/docs_app_alert_dialog_contract.spec.mjs`，覆盖打开、语义来源标记、Esc 关闭与自动聚焦路径。
- 已执行验证：
  - `cargo tree -e features -p ui-components --no-default-features --features component-alert_dialog,inject-css --depth 2`（通过）
  - `cargo check -p ui-components --no-default-features --features component-alert_dialog,inject-css`（通过）
  - `cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-alert_dialog,inject-css`（通过）
  - `cargo test -p ui-components --test alert_dialog_semantics --no-default-features --features component-alert_dialog,inject-css`（通过，15/15）
  - `cargo clippy -p ui-components --no-default-features --features component-alert_dialog,inject-css --all-targets -- -D warnings`（通过）
  ...
  - `cargo check -p ui-headless --no-default-features --features web,ssr`（按预期失败，命中 `compile_error!`）
...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。（N/A：`AlertDialog` 仅本地开关与同步回调，无异步加载协议）
...
- [x] 流式在这里仅指 LLM 输出渲染（只看两种显示模式）。（N/A：`AlertDialog` 非 LLM 正文渲染组件）
  - `Streaming`：LLM 还在生成，界面边生成边显示。
  - `Snapshot`：LLM 全部生成完成后，一次性显示。
- [x] `Snapshot` 是所有组件的基础能力（默认必须支持）。（`AlertDialog` 支持完整内容快照渲染）
...
- [x] E2E 选择器稳定：使用语义标记，WASM 场景有稳定等待策略。（新增：`e2e/tests/docs_app_alert_dialog_contract.spec.mjs`）
  - E2E 选择器优先 `data-*` 语义标记，禁止依赖脆弱 DOM 层级或文本定位。
  - WASM 场景必须使用稳定等待策略（语义状态就绪而非固定 sleep）。
  - 若组件涉及异步/动画，E2E 需显式覆盖 ready/settled 条件。
- [x] 关键流程纳入可重复回归集合（Playwright/Cypress）。（新增流程：打开 -> 语义标记断言 -> Esc 关闭 -> 自动聚焦断言）
