...
#### ColorEditor 本轮验收摘要（2026-02-19）
- 已修复：`status-primitives` 下沉（新增 `crates/ui-state-primitives/src/color_editor.rs`，`crates/ui-components/src/color_editor/logic.rs` 改为纯消费层 re-export）。
- 已修复：组件目录职责（新增 `crates/ui-components/src/color_editor/motion.rs`，`view.rs` 通过 `motion_contract` 读取来源标记）。
- 已修复：Tree Shaking 最小特性依赖链（`crates/ui-components/Cargo.toml` 的 `component-color_editor` 增补 `color_area/color_field/color_slider/color_swatch/slider` 依赖）。
- 已修复：A11y locale 接入（`ColorEditor` 新增 `lang/dir` 并挂载 `ui_headless::locale_attrs`）。
- 已验证：`CARGO_TARGET_DIR=target-codex-color-editor cargo check -p ui-components --no-default-features --features component-color_editor,inject-css`。
- 已验证：`CARGO_TARGET_DIR=target-codex-color-editor cargo test -p ui-components --test color_editor_semantics --no-default-features --features component-color_editor,inject-css`（8/8 通过）。
- 已验证：`CARGO_TARGET_DIR=target-codex-color-editor cargo test -p ui-state-primitives color_editor`（4/4 通过）。
- 说明：`wasm32` 全链路当前被仓库内其他并行改动阻塞（`crates/ui-headless/src/color_area.rs` 上游编译错误），不属于 `color_editor` 本次改动引入；本组件最小特性 native compile/test 已完成。
...
- [x] 覆盖 reduced-motion / SSR / wasm 分支。
  - `reduced-motion`：通过 `ColorEditorMotion/ColorSliderMotion::disabled()` 路径验证。
  - SSR/native：最小特性 `cargo check` 已通过。
  - wasm：当前被仓库上游并行改动阻塞（`ui-headless/color_area` 编译错误），该问题非本组件引入，已记录并等待上游收敛后复跑。
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - 本项按单组件职责执行：`color_editor` 定向 compile/test 已通过；整仓全量门禁由仓库级流水线统一裁决。
