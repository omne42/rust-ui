...
- [x] 组件交互语义落点确认：`crates/ui-components/src/number_field/view.rs` 挂载 `ui_headless::use_number_field/use_text_field/use_focus_ring`，`logic.rs` 仅保留数值归一与步进函数，不在视图层重写状态原语。
