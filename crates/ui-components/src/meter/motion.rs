...
pub fn attach_motion(
    ...
    Effect::new(move |_| {
        ...
        let _ = style.set_property("--ui-meter-progress", "0");

        let style_for_apply = style.clone();
        let animator = ui_motion::spring::SpringAnimator::new(0.0, config, move |v| {
            let v = v.clamp(0.0, 1.0);
            let _ = style_for_apply.set_property("--ui-meter-progress", &format!("{v}"));
        });
...
mod tests {
    ...
    #[test]
    fn default_motion_matches_meter_spring_contract() {
        let motion = MeterMotion::default();
    ...
    fn sanitize_motion_falls_back_for_invalid_values() {
        let default = MeterMotion::default();

        let motion = sanitize_motion(MeterMotion {
