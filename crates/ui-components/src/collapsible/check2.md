...
### 0.1 本次核验记录（2026-02-18）
- 修复：`crates/ui-components/Cargo.toml` 中 `component-collapsible` 补齐依赖 `component-disclosure`，解决最小特性编译失败。
- 关键验证：
  - `cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-collapsible,inject-css`
  - `cargo check -p ui-components --no-default-features --features component-collapsible,inject-css`
  - `cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-collapsible,inject-css`
  - `cargo clippy -p ui-components --no-default-features --features component-collapsible,inject-css -- -D warnings`
  - `cargo test -p ui-components --test collapsible_semantics --no-default-features --features component-collapsible,inject-css -- --nocapture`
  - `cargo check -p ui-headless --no-default-features --features web`
  - `cargo check -p ui-headless --no-default-features --features ssr`
  - `cargo check -p ui-headless --no-default-features --features web,ssr`（按预期命中 `compile_error!` 互斥）
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（本次以 `component-collapsible` 最小特性链完成 check/clippy/test/wasm 与 headless feature 互斥校验；未触碰他人并行改动范围）。
