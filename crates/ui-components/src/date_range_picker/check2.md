...
### 1. 大骨架（架构边界与层职责）
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（`DateRangePickerStateInput/DateRangePickerState` 与归一化规则已下沉至 `crates/ui-state-primitives/src/date_range_picker.rs`，组件层仅消费。）
...
- [x] `ui-headless` 定义：交互与 A11y 原语层（press/focus/hover/roving/listbox/menu/tooltip 等），把输入设备事件与状态语义标准化为可复用契约；输出必须是类型化 `attrs + handlers + state`。不做样式、不写组件 CSS、不做组件级动效编排。（`view.rs` 通过 `ui_headless::use_controllable_state` 统一受控/非受控日期轴，不在组件层重写该原语。）
...
- [x] `ui-motion` 定义：动效能力与契约执行层（spring、keyframes、WAAPI/RAF backend），只负责时间函数、插值与运行时驱动，不承载组件业务语义与状态决策。（N/A：`date_range_picker` 本体无独立 `motion.rs`，动效由内部 `DatePicker` 子组件按其契约承担。）
...
- [x] `ui-components` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（`logic.rs` 负责文本/输入归一与 primitives 映射，`view.rs` 仅负责组合两个 `DatePicker` 与挂载语义标记。）
...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。（N/A：组件仅本地同步日期选择与区间有效性判定，无异步请求轴。）
...
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。（`DateRangePicker` 归类 `Streaming Optional`，默认走 `snapshot` 渲染路径。）
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（已执行：`rustfmt --edition 2024`（仅本次改动文件）；`cargo clippy -p ui-state-primitives -- -D warnings`；`cargo clippy -p ui-components --no-default-features --features component-date_range_picker,inject-css -- -D warnings`；`cargo test -p ui-state-primitives date_range_picker`；`cargo test -p ui-components --test date_range_picker_semantics --no-default-features --features component-date_range_picker,inject-css`；`cargo check` native/wasm 最小特性链。）
