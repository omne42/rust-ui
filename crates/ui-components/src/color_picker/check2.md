...
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。
  - `ColorPicker` 归类为 `Streaming Optional`，当前实现为 `fallback=snapshot`。
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（组件范围已执行 `rustfmt --edition 2024 crates/ui-components/src/color_picker/view.rs crates/ui-components/tests/color_picker_semantics.rs`、`cargo clippy -p ui-components --no-default-features --features component-color_picker,inject-css -- -D warnings`、`cargo test -p ui-components --test color_picker_semantics --no-default-features --features component-color_picker,inject-css`、`cargo check -p ui-components --no-default-features --features component-color_picker,inject-css`、`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-color_picker,inject-css`。）
