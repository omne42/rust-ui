...
### 执行记录（2026-02-18）
- `component-chart` 最小特性链已修复为 `component-chart = ["component-active_highlight"]`（`crates/ui-components/Cargo.toml`）。
- 状态原语已下沉：`crates/ui-state-primitives/src/chart.rs`，组件 `logic.rs` 改为 re-export 消费。
- 交互/A11y 契约已下沉：`crates/ui-headless/src/chart.rs`，`view.rs` 通过 `use_chart(ChartOptions)` 消费 `attrs + handlers + state`。
- 组件动效分层已补齐：新增 `crates/ui-components/src/chart/motion.rs`，`view.rs` 仅调用 `sanitize_motion/attach_motion`。
- 验证命令（逐项）：
  - `rustfmt --edition 2024 --check crates/ui-state-primitives/src/chart.rs crates/ui-headless/src/chart.rs crates/ui-components/src/chart/logic.rs crates/ui-components/src/chart/motion.rs crates/ui-components/src/chart/view.rs crates/ui-components/tests/chart_semantics.rs` ✅
  - `cargo clippy -p ui-state-primitives -- -D warnings` ✅
  - `cargo clippy -p ui-headless -- -D warnings` ✅
  - `cargo clippy -p ui-components --no-default-features --features component-chart,inject-css --tests -- -D warnings` ✅
  - `cargo test -p ui-state-primitives chart` ✅
  - `cargo test -p ui-headless chart` ✅
  ...
  - `cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-chart,inject-css` ✅（显示 `component-chart -> component-active_highlight`）
  - `cargo tree -e features -i ui-components -p web-demo` ✅（`all-components: ABSENT`，仅 `web-demo-components` 链路）
