...
`Chart` 是一个基于 `ui-state-primitives` + `ui-headless` + `ui-motion` 组合出来的图表组件，提供 `Bar/Line` 两种展示模式与可受控 active-index 语义。
...
- `Interactive Playground` 启用 `Scoped CSS` 测试面板：
  - Source: `crates/ui-components/src/chart/styles.rs`
...
- 常见导入：
  ...
  - `use ui_components::*;`
- 真实源码落点：
  - `crates/ui-components/src/chart/mod.rs`
  - `crates/ui-components/src/chart/logic.rs`
