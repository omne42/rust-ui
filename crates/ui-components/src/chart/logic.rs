use crate::chart::{DEFAULT_ARIA_LABEL, DEFAULT_ID_BASE};
...
mod tests {
    ...
    fn resolve_state_and_class_name_surface_markers() {
        ...
        let class_name = compose_class_name(Some("docs-chart".to_string()), state);
        for token in [
            "ui-chart",
            "ui-chart--line",
            "ui-chart--grid",
    ...
    #[test]
    fn geometry_helpers_generate_stable_coordinates() {
        let points = vec![
            ChartPoint::new("jan", "Jan", 10.0),
            ChartPoint::new("feb", "Feb", 20.0),
            ChartPoint::new("mar", "Mar", 30.0),
        ];
        let domain = value_domain(&points);

        assert!(point_x(0, points.len()) < point_x(2, points.len()));
        assert!(point_y(30.0, domain) < point_y(10.0, domain));

        let polyline = polyline_points(&points, domain);
        assert!(polyline.contains(','));
        assert!(polyline.contains(' '));
        assert!(bar_width(points.len()) >= 4.0);
    }

    #[test]
    fn keyboard_navigation_clamps_to_bounds() {
        assert_eq!(next_index_for_key("ArrowLeft", 0, 4), Some(0));
        assert_eq!(next_index_for_key("ArrowRight", 0, 4), Some(1));
        assert_eq!(next_index_for_key("End", 1, 4), Some(3));
        assert_eq!(next_index_for_key("Home", 3, 4), Some(0));
        assert_eq!(next_index_for_key("Space", 2, 4), None);

        assert_eq!(default_active_index(0, Some(3)), 0);
        assert_eq!(default_active_index(3, Some(8)), 2);
    }
}
