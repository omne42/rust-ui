...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（本次按组件范围执行：`component-carousel` 最小特性 `check + clippy + carousel_semantics`。）

### 10. 本轮验收记录（Carousel）
- 关键修复：`component-carousel` 最小特性编译原先失败（缺少 `active_highlight` 特性依赖），已修复为 `component-carousel = ["component-active_highlight"]`（`crates/ui-components/Cargo.toml`）。
- 语义契约回归：`cargo test -p ui-components --test carousel_semantics`（隔离 target 复验）通过，14/14 用例通过。
- 最小特性编译：`cargo check -p ui-components --no-default-features --features component-carousel,inject-css` 通过。
- wasm 路径编译：`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-carousel,inject-css` 通过。
- 静态检查：`cargo clippy -p ui-components --no-default-features --features component-carousel,inject-css -- -D warnings` 通过。
- Tree Shaking 约束：`component-carousel` 仅补齐必需依赖 `component-active_highlight`，未引入额外跨组件特性耦合。
- N/A 说明：Carousel 无异步请求链路（`is_loading`/retry/`aria-busy` 不适用）；Streaming 属于可选（`fallback=snapshot`），当前默认路径满足 Snapshot 基线。
