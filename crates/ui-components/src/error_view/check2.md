...
- `rustfmt crates/ui-state-primitives/src/error_view.rs crates/ui-components/src/error_view/logic.rs crates/ui-components/src/error_view/view.rs crates/ui-components/tests/error_view_semantics.rs apps/docs-app/src/pages/components/pages/display_extra.rs`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo test -p ui-components --test error_view_semantics --no-default-features --features component-error_view,inject-css`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo check -p ui-components --no-default-features --features component-error_view,inject-css`
- `CARGO_TARGET_DIR=/tmp/codex-error-view-target cargo clippy -p ui-components --no-default-features --features component-error_view,inject-css -- -D warnings`
- `./scripts/check-ui-components-e2e-error-view.sh`
- `cargo tree -e features -p ui-components --no-default-features --features component-error_view,inject-css -f "{p} {f}"`
