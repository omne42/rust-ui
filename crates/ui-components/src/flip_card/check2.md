...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（组件范围 `fmt/clippy/test/check` 全通过；smoke 对本次无行为改动场景记为 N/A）。

### 10. 本次逐项核验记录（FlipCard）
- [x] 按清单顺序逐项核验完成：先骨架（1-2），再实现细节（3-6），最后测试文档与门禁（7-9）；不满足项已修复并复验通过。
- [x] 本次组件范围内核心结论：`flip_card` 分层与语义契约成立，`mod/logic/view/styles/motion` 职责清晰，最小特性链路可独立构建。
- [x] 本次发现并处理的阻断项：
  - `cargo test -p ui-components --no-default-features --features component-flip_card,inject-css --test flip_card_semantics` 初次执行被 `ui-headless` 编译错误阻断（`ColorAreaHandlers` 对 `Callback` 错误派生 `PartialEq`）。
  - 已做最小修复：`crates/ui-headless/src/color_area.rs` 去除 `ColorAreaHandlers` 与 `ColorAreaContract` 的 `PartialEq` 派生，避免对不可比较回调类型做等值比较；复验后所有 `flip_card` 门禁恢复通过。
- [x] 状态与语义结论：`FlipCardSlot/FlipCardPartStateInput/FlipCardPartState` 类型化契约完整，`data-slot/data-state/data-visible/data-flip-mode/data-motion-source/data-id-source` 等标记稳定且可断言。
- [x] A11y/i18n 与样式结论：根节点 `role="button"`、`aria-pressed`、`aria-disabled` 与键盘切换（Enter/Space）路径齐全；样式只依赖稳定 `data-*` 与 class，未出现脆弱结构选择器。
- [x] Async/Streaming/E2E 适用性结论：
  - Async：N/A（`FlipCard` 无远程请求与加载状态机）。
  - Streaming：`FlipCard` 非正文流式阅读面，按 `Streaming Optional` 执行，默认 `snapshot` 渲染路径成立。
  - E2E：当前关键契约由 `flip_card_semantics` + docs playground 覆盖；未新增独立 e2e case。
- [x] 文档与示例结论：`apps/docs-app/src/pages/components/pages/display_extra.rs` 已包含 `FlipCard` 页面与三组 playground（Click + Keyboard Flip / State + Source Markers / Disabled），与语义测试断言一致。
- [x] Tree-shaking 结论：最小特性验证仅拉起 `component-flip_card` 与 `inject-css`，无隐式全量组件链路。

#### 10.1 门禁与验证命令（逐条执行）
- [x] `rustfmt --edition 2024 --check crates/ui-components/src/flip_card/mod.rs crates/ui-components/src/flip_card/logic.rs crates/ui-components/src/flip_card/view.rs crates/ui-components/src/flip_card/styles.rs crates/ui-components/src/flip_card/motion.rs crates/ui-headless/src/color_area.rs`
- [x] `TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/target/tmp cargo check -p ui-components --no-default-features --features component-flip_card,inject-css`
- [x] `TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/target/tmp cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-flip_card,inject-css`
- [x] `TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/target/tmp cargo test -p ui-components --no-default-features --features component-flip_card,inject-css --test flip_card_semantics`
- [x] `TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/target/tmp cargo clippy -p ui-components --no-default-features --features component-flip_card,inject-css --test flip_card_semantics --no-deps -- -D warnings`
- [x] `TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/target/tmp cargo test -p ui-components --no-default-features --features component-flip_card,inject-css --test flip_card_semantics <single-test> -- --exact`（14 条语义测试逐条单独执行，全部通过）
- [x] `cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-flip_card,inject-css`
