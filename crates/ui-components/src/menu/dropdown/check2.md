...
### 10. 本次逐项核验记录（2026-02-18）
1. 架构与职责：
   - 状态原语已下沉到 `crates/ui-state-primitives/src/dropdown.rs`，`crates/ui-components/src/menu/dropdown/logic.rs` 仅做归一化装配与 class 组合。
   - `view.rs` 仅做结构挂载与 headless 连接；`motion.rs` 只做 motion contract 映射。
2. API 与状态契约：
   - `Dropdown` 支持 `is_disabled/is_open` 前缀命名并兼容 `disabled/open` 别名。
   - open 轴按 `is_open/open + default_open + on_open_change` 三元组接入 `ui_headless::use_controllable_open_state_traced`。
3. 语义标记与可测性：
   - 暴露稳定 `data-*` / `aria-*` 标记（含 controlled/uncontrolled、state/source、motion-source）。
   - 语义测试与 E2E 选择器均改为语义锚点优先。
4. 文档与示例：
   - docs-app 新增 Hello World / Default / Controlled Playground 与稳定 `data-slot` 锚点。
   - 组件文档新增 `crates/ui-components/src/menu/dropdown/README.md`，含 Streaming 策略与 Source-first 路径。
5. 平台与工程约束：
   - 保持 `ui-headless` web/ssr 互斥契约引用。
   - 保持 `ui-motion` non-wasm no-op/stub 路径引用。
   - 本组件无异步请求语义，`is_loading/error/retry` 统一协议项记为 N/A（不适用）。

#### 验证命令与结果
- `cargo test -p ui-state-primitives dropdown` ✅
- `cargo test -p ui-components --test dropdown_semantics --no-default-features --features component-dropdown,inject-css` ✅
- `cargo clippy -p ui-components --test dropdown_semantics --no-default-features --features component-dropdown,inject-css -- -D warnings` ✅
- `node --check e2e/tests/docs_app_dropdown_contract.spec.mjs` ✅
