...
- `rustfmt crates/ui-components/src/alert_banner/view.rs crates/ui-components/tests/alert_banner_semantics.rs apps/docs-app/src/pages/components/pages/display_extra.rs`
- `CARGO_TARGET_DIR=/tmp/codex-alert-banner-target cargo test -p ui-components --test alert_banner_semantics --no-default-features --features component-alert_banner,inject-css`
- `CARGO_TARGET_DIR=/tmp/codex-alert-banner-target cargo check -p ui-components --no-default-features --features component-alert_banner,inject-css`
- `CARGO_TARGET_DIR=/tmp/codex-alert-banner-target cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-alert_banner,inject-css`
- `CARGO_TARGET_DIR=/tmp/codex-alert-banner-target cargo clippy -p ui-components --no-default-features --features component-alert_banner,inject-css -- -D warnings`
- `./scripts/check-ui-components-e2e-alert-banner.sh`
- `cargo tree -e features -p ui-components --no-default-features --features component-alert_banner,inject-css -f "{p} {f}"`
- `cargo tree -e features -i ui-components -p web-demo -f "{p} {f}"`
- `CARGO_TARGET_DIR=/tmp/codex-alert-banner-target cargo check -p ui-headless --no-default-features --features web`
- `CARGO_TARGET_DIR=/tmp/codex-alert-banner-target cargo check -p ui-headless --no-default-features --features ssr`
- `CARGO_TARGET_DIR=/tmp/codex-alert-banner-target cargo check -p ui-headless --no-default-features --features web,ssr`（按预期触发 `compile_error!`）
- `CARGO_TARGET_DIR=/tmp/codex-alert-banner-target cargo check -p ui-motion`
- `CARGO_TARGET_DIR=/tmp/codex-alert-banner-target cargo check -p ui-motion --target wasm32-unknown-unknown`
