...
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。
  - 已下沉：`SpinnerStateInput/SpinnerState/normalize_optional_text/resolve_aria_label/resolve_state/compose_class_name` 已迁入 `crates/ui-state-primitives/src/spinner.rs` 并在 `crates/ui-state-primitives/src/lib.rs` 导出；`crates/ui-components/src/spinner/logic.rs` 仅保留消费层 `pub use`。
...
- [x] `ui-theme` 定义：唯一设计 token 与主题上下文层（system/color/scale + Light/Dark/OLED），负责 token 分类、主题映射与 CSS 变量生成。
  - `Spinner` 不在组件层重建主题：`crates/ui-components/src/spinner/styles.rs` 改为仅消费 `ui-theme` 输出变量（`--ui-button-spinner-size` / `--ui-button-spinner-border` / `--ui-button-spinner-duration` / `--ui-space-*` / `--ui-accent` / `--ui-fg` / `--ui-border`）；`crates/ui-components/src/spinner/motion.rs` 默认时长来自 `ui_theme::default_button_layout_tokens().spinner_duration_ms`。回归锁定：`crates/ui-components/tests/spinner_semantics.rs::spinner_styles_include_size_and_source_markers`、`spinner_motion_contract_stays_mapping_only`。
...
- [x] Tree Shaking 是一等能力：package 模式支持组件级 feature；source 模式天然裁剪；样式层同步裁剪，禁止无条件聚合全部 CSS，禁止破坏 DCE/LTO 的全量中央注册表。
  - 已修复最小特性链断裂：`crates/ui-components/Cargo.toml` 中 `component-spinner` 显式依赖 `component-circular_progress`，避免 `--features component-spinner` 下缺失 `CircularProgress`。`lib.rs`/`css.rs` 均为 feature 条件导出/聚合。验证：
    - `cargo check -p ui-components --no-default-features --features component-spinner,inject-css` ✅
    - `cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-spinner,inject-css` ✅
    - `cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-spinner,inject-css` 显示 `component-spinner -> component-circular_progress` 且仅命令行特性生效。
    - `cargo tree -e features -i ui-components -p web-demo` 未见 `all-components`，由 `web-demo-components` 链路拉起。
...
- [x] SSR 与跨平台检查：覆盖 web/ssr/wasm 分支，不破坏 non-wasm 编译路径。
  - compile-only 证据：
    - native：`cargo check -p ui-components --no-default-features --features component-spinner,inject-css` ✅
    - wasm32：`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-spinner,inject-css` ✅
...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - 组件范围门禁实跑通过（spinner 相关）：
    - `rustfmt --edition 2024 --check crates/ui-components/src/spinner/{mod,logic,styles,view,motion}.rs crates/ui-components/tests/spinner_semantics.rs` ✅
