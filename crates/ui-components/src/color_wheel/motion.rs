...
impl Default for ColorWheelMotion {
    fn default() -> Self {
...
mod tests {
    ...
    fn sanitize_motion_falls_back_for_invalid_spring_values() {
        let motion = sanitize_motion(ColorWheelMotion {
            enabled: true,
            spring: ui_motion::spring::SpringConfig {
                stiffness: f64::NAN,
                damping: -1.0,
        ...
        let default = ColorWheelMotion::default().spring;
    ...
    fn disabled_constructor_turns_motion_off() {
        assert!(!ColorWheelMotion::disabled().enabled);
    }
}
