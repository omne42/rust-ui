...
- 代码分层修复：`DatePicker` 状态原语已下沉到 `crates/ui-state-primitives/src/date_picker.rs`，并在 `crates/ui-state-primitives/src/lib.rs` 导出；`crates/ui-components/src/date_picker/logic.rs` 改为仅 re-export + 组件装配。
- A11y/i18n 修复：`crates/ui-components/src/date_picker/view.rs` 已接入 `lang/dir`（`ui_headless::a11y::locale_attrs`），不再假设单语言/单方向。
...
- docs shell 白名单：`apps/docs-app/src/pages/components/shell.rs` 已将 `"date-picker"` 映射到 `crates/ui-components/src/date_picker/README.md`，沿用静态 `include_str!` + `_ => None` 安全边界。
...
- 组件文档：`crates/ui-components/src/date_picker/README.md` 已补充 docs-playground 四区说明与多场景对比说明。
