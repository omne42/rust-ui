...
- E2E 扩展：`e2e/tests/docs_app_dialog_contract.spec.mjs` 新增 interactive/comparison 回归（workbench 打开、compact/title-only、custom-motion 三场景）。
- 2026-02-18 补充稳态锚点：`apps/docs-app/src/pages/components/pages/overlays_dialog.rs` 为 Interactive/Comparison 增加 `data-slot="dialog-workbench"` 与 `data-slot="dialog-scenario-compare"`，避免 E2E 误选页面容器节点。
- 2026-02-18 E2E 选择器加固：`e2e/tests/docs_app_dialog_contract.spec.mjs` 改为按 `aria-labelledby="docs-dialog-*-title"` 锚定具体 overlay 实例，并在 comparison 中显式覆盖 default/compact/motion 三分支。
- 已执行验证：
  - `cargo tree -e features -p ui-components --no-default-features --features component-dialog,inject-css --depth 3`（通过）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-dialog cargo check -p ui-components --no-default-features --features component-dialog,inject-css`（通过）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-dialog cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-dialog,inject-css`（通过，存在仓库既有 `observability.rs` dead_code warning，不阻断 check）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-dialog cargo test -p ui-components --test dialog_semantics --no-default-features --features component-dialog,inject-css`（通过，18/18）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-dialog cargo clippy -p ui-components --no-default-features --features component-dialog,inject-css --all-targets -- -D warnings`（通过）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-dialog cargo test -p ui-motion --test non_wasm_stub`（通过，2/2）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-dialog cargo check -p ui-headless --no-default-features --features web`（通过）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-dialog cargo check -p ui-headless --no-default-features --features ssr`（通过）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-dialog cargo check -p ui-headless --no-default-features --features web,ssr`（按预期失败，命中 `compile_error!`）
  - `CARGO_TARGET_DIR=/root/autodl-tmp/ct-input-otp cargo test -p ui-components --test dialog_semantics --no-default-features --features component-dialog,inject-css`（通过，18/18，复跑）
  - `node --check e2e/tests/docs_app_dialog_contract.spec.mjs`（通过）
...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。（N/A：`Dialog` 仅本地开关与同步关闭回调，无异步加载协议）
...
- [x] 语义标记统一升级为 Agent Contract（Schema 化），让 Agent 不依赖 DOM 猜测理解组件状态与意图。（本组件通过稳定 `data-*` + 来源标记输出机器可读语义；复杂组件才强制 `data-ui-schema`）
  - 关键交互组件必须输出稳定机器可读语义（至少 `data-*` + 状态来源标记；复杂组件建议补 `data-ui-schema`）。
  - Agent 消费字段应来自类型化 schema 生成，不允许散落字符串拼接。
  - 契约字段需可追溯到组件状态轴与动作语义（intent/action/state/source）。
  - 配置到组件的渲染链路必须走白名单能力边界，禁止任意脚本注入。
- [x] 流式在这里仅指 LLM 输出渲染（只看两种显示模式）。（N/A：`Dialog` 非 LLM 正文渲染组件）
  - `Streaming`：LLM 还在生成，界面边生成边显示。
  - `Snapshot`：LLM 全部生成完成后，一次性显示。
- [x] `Snapshot` 是所有组件的基础能力（默认必须支持）。（`Dialog` 支持完整内容快照渲染）
...
- [x] E2E 选择器稳定：使用语义标记，WASM 场景有稳定等待策略。（新增：`e2e/tests/docs_app_dialog_contract.spec.mjs`）
  - E2E 选择器优先 `data-*` 语义标记，禁止依赖脆弱 DOM 层级或文本定位。
  - WASM 场景必须使用稳定等待策略（语义状态就绪而非固定 sleep）。
  - 若组件涉及异步/动画，E2E 需显式覆盖 ready/settled 条件。
- [x] 关键流程纳入可重复回归集合（Playwright/Cypress）。（新增流程：打开 -> 语义标记断言 -> Esc 关闭）
