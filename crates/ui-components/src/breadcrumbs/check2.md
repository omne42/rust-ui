...
### 1. 大骨架（架构边界与层职责）
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（`BreadcrumbsState` 与归一化逻辑已下沉到 `crates/ui-state-primitives/src/breadcrumbs.rs`，组件层仅消费。）
...
- [x] `ui-headless` 定义：交互与 A11y 原语层（press/focus/hover/roving/listbox/menu/tooltip 等），把输入设备事件与状态语义标准化为可复用契约；输出必须是类型化 `attrs + handlers + state`。不做样式、不写组件 CSS、不做组件级动效编排。（N/A：`Breadcrumbs` 为静态导航装配，交互语义由原生 `<nav>/<a>` 与 `aria-current` 完成，无额外可复用输入归一需求。）
...
- [x] `ui-motion` 定义：动效能力与契约执行层（spring、keyframes、WAAPI/RAF backend），只负责时间函数、插值与运行时驱动，不承载组件业务语义与状态决策。（N/A：组件无进入/退出或手势动效状态轴，不引入 `motion.rs`。）
...
- [x] `ui-theme` 定义：唯一设计 token 与主题上下文层（system/color/scale + Light/Dark/OLED），负责 token 分类、主题映射与 CSS 变量生成。（`styles.rs` 仅消费 `var(--ui-*)`，未引入私有 token 体系。）
...
- [x] `ui-components` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。（`logic.rs` 负责归一与映射，`view.rs` 仅渲染语义结构并挂载 `data-*`/`aria-*`。）
...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。（N/A：`Breadcrumbs` 无异步状态与远程请求链路。）
...
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。（`Breadcrumbs` 归类为 `Streaming Optional`，默认按 `fallback=snapshot` 稳定渲染。）
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（已执行 `cargo fmt --all`、`cargo clippy -p ui-components --no-default-features --features component-breadcrumbs,inject-css -- -D warnings`、`cargo test -p ui-state-primitives breadcrumbs`、`cargo test -p ui-components --test breadcrumbs_semantics --no-default-features --features component-breadcrumbs,inject-css`、`cargo check` native/wasm 最小特性链。）
