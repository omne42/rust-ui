...
### 1. 大骨架（架构边界与层职责）
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（`AutoHeightStateInput/AutoHeightState/resolve_state` 已下沉到 `crates/ui-state-primitives/src/auto_height.rs`，组件 `logic.rs` 仅保留装配。）
...
- [x] `ui-headless` 定义：交互与 A11y 原语层（press/focus/hover/roving/listbox/menu/tooltip 等），把输入设备事件与状态语义标准化为可复用契约；输出必须是类型化 `attrs + handlers + state`。不做样式、不写组件 CSS、不做组件级动效编排。（N/A：`AutoHeight` 为布局容器，不引入可复用 keyboard/pointer 语义契约。）
...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。
  - N/A：`AutoHeight` 无远程请求与异步状态轴，仅同步消费 children 高度变化。
...
- [x] `Streaming` 是否强制，按组件职责判断（不能一刀切）。
  - `AutoHeight` 归类为 `Streaming Optional`，当前按 `fallback=snapshot` 稳定渲染。
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（组件范围已执行 `cargo test -p ui-components --test auto_height_semantics --no-default-features --features component-auto_height,inject-css` 与 `cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-auto_height,inject-css`。）
