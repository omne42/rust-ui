...
### 10. 本次逐项核验记录（CodeBlock）
- [x] 按清单顺序逐项核验完成：先骨架（1-2），再实现细节（3-6），最后测试文档与门禁（7-9）；不满足项已修复并复验通过。
- [x] 本次发现并修复的阻断项：
  - `component-code_block` 最小特性无法单独编译，原因是缺少 `button` 与 `snippet` 依赖链；已修复 `crates/ui-components/Cargo.toml`：`component-code_block = ["component-button", "component-snippet"]`。
  - `code_block/view.rs` 误用根导出 `crate::{Button, ...}`，在最小特性下不可见；已改为模块路径 `crate::button::{Button, ButtonSize, ButtonVariant}`。
- [x] 结构与职责结论：`code_block` 保持 `mod/logic/styles/view/motion/i18n` 分层；状态归一委托 `ui-state-primitives`，组件层仅做装配映射与语义挂载。
- [x] 状态与类型结论：`CodeBlockStateInput/CodeBlockViewState` 由 `ui-state-primitives` 提供；`data-state/data-header/data-motion-source` 等标记为封闭集合，契约可断言。
- [x] A11y/i18n 与样式结论：复制按钮有可读 `aria-label`，`aria-live` 状态播报存在；样式分支依赖稳定 `data-*` 与 class，不依赖脆弱 DOM 结构。
- [x] Async/Streaming/E2E 适用性结论：
  - Async：N/A（`CodeBlock` 组件无远程请求与加载状态机）。
  - Streaming：`CodeBlock` 不是正文流式阅读面，按 `Streaming Optional` 执行，默认 `snapshot` 渲染路径成立。
  - E2E：当前组件关键契约由 `code_block_semantics` + docs Playground 覆盖；未新增独立 e2e case。
- [x] 文档与示例结论：`apps/docs-app/src/pages/components/pages/display.rs` 已提供 `CodeBlock` 页面与两组 playground（`Header + Copy Motion`、`Compact + No Copy`），示例与当前 API 一致。
- [x] Tree-shaking 结论：最小特性链路已验证 `component-code_block -> component-button/component-snippet`，无 `all-components` 隐式拉起。

#### 10.1 门禁与验证命令（逐条执行）
- [x] `rustfmt --edition 2024 --check crates/ui-components/src/code_block/mod.rs crates/ui-components/src/code_block/logic.rs crates/ui-components/src/code_block/view.rs crates/ui-components/src/code_block/styles.rs crates/ui-components/src/code_block/motion.rs crates/ui-components/src/code_block/i18n.rs`
- [x] `CARGO_TARGET_DIR=target/code-block-check cargo check -p ui-components --no-default-features --features component-code_block,inject-css`
- [x] `CARGO_TARGET_DIR=target/code-block-check cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-code_block,inject-css`
- [x] `CARGO_TARGET_DIR=target/code-block-check cargo clippy -p ui-components --no-default-features --features component-code_block,inject-css --test code_block_semantics --no-deps -- -D warnings`
- [x] `CARGO_TARGET_DIR=target/code-block-check cargo test -p ui-components --no-default-features --features component-code_block,inject-css --test code_block_semantics`
- [x] `CARGO_TARGET_DIR=target/code-block-check cargo test -p ui-components --no-default-features --features component-code_block,inject-css --test code_block_semantics <single-test> -- --exact`（11 条语义测试逐条单独执行，全部通过）
- [x] `cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-code_block,inject-css`
