...
- 架构边界：`crates/ui-components/src/help_text/` 维持 `mod.rs + logic.rs + styles.rs + view.rs + motion.rs` 分层；状态归一与来源标记集中在 `logic.rs`，`view.rs` 只负责渲染与挂载，未越层实现可复用状态原语。
...
- README：`crates/ui-components/src/help_text/README.md` 已补齐，包含展示区 / Config 区 / Code 区 / CSS Test 区结构与状态对比说明。
...
- 说明：默认特性全量编译当前被 `crates/ui-components/src/dropdown/view.rs` 的既有错误阻塞（与本组件无关），本次按“最小特性链 + 组件边界”完成核验，避免干扰并行协作改动。
