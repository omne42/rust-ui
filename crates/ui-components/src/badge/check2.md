...
**Badge 本轮逐项检查执行记录（对应本文件全部勾选项）**
- 代码范围仅限 `crates/ui-components/src/badge/*`、`crates/ui-components/tests/badge_semantics.rs`、`apps/docs-app/src/pages/components/pages/display.rs`、`e2e/tests/docs_app_badge_contract.spec.mjs`，未修改其他组件实现，避免干扰并行协作。
- 本轮补齐项：`Badge` 增加 `lang/dir` 接入、`data-class-source` 与 `data-ui-*` 机器可读契约；docs 新增 `Hello World` playground；新增 badge docs e2e 合同用例。
- 增量补齐（本轮继续）：docs 增加 `Badge Workbench (Display + Config + Code + CSS Test)`，覆盖展示区/配置区/代码区/CSS 测试区，并补充多场景对比；新增 `crates/ui-components/src/badge/README.md`，明确 `展示/config/code/css test` 分区。
- N/A 声明：`Badge` 为非交互、无异步、无业务 runtime 绑定的显示组件，涉及异步协议/高风险键盘流/运行时 tracing 的条目按 N/A 审核通过，语义稳定性由静态契约与 docs-e2e 断言覆盖。
- 已执行验证：
  - `cargo test -p ui-components --test badge_semantics`
  - `cargo check -p ui-components --no-default-features --features component-badge,inject-css`
  - `cargo test -p ui-components --no-default-features --features component-badge,inject-css --test badge_semantics`
  - `cargo clippy -p ui-components --no-default-features --features component-badge,inject-css --test badge_semantics -- -D warnings`
