...
已核验（card，2026-02-18）：本组件为静态展示容器，核心输入轴为 `variant/padded/class_name`，不承载可控状态机、异步流程、overlay 焦点链路与流式正文渲染职责；相关条目按 N/A 语义核验并保持契约可追溯。
...
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。  
  检查结论（card）：通过。`card` 仅做 props 归一与语义/样式映射，未实现受控/非受控状态机、展开规则或选择归一化原语。
...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - `card` 组件最小门禁已验证：`cargo test -p ui-components --test card_semantics`、`cargo check -p ui-components --no-default-features --features component-card,inject-css`、`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-card,inject-css`。
  - 仓库级 `clippy -D warnings` 当前存在外部阻塞：`crates/ui-state-primitives/src/lib.rs` 声明 `pub mod divider;` 但对应文件缺失（`E0583`），非 `card` 改动引入。
