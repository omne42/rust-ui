...
### 10. Code 组件核验证据（本次）
- 架构边界：`crates/ui-components/src/code/` 保持 `mod.rs + logic.rs + styles.rs + view.rs` 的轻量装配分层；`Code` 为展示型组件，无独立可复用状态机与交互语义契约下沉需求（`status-primitives/ui-headless/ui-motion` 对应项按 N/A 处理并已逐项确认不越层）。
- 状态模型：`CodeVariant`（`Inline/Block`）在 `logic.rs` 类型化；`resolve_state` 与 `compose_class_name` 集中派生，`view.rs` 只消费派生结果并挂载语义标记。
- 语义标记：`view.rs` 暴露 `data-slot/data-variant/data-state/data-inline/data-block/data-custom-class`；`styles.rs` 仅基于稳定 class/data 选择器消费状态，无 DOM 猜测式选择器。
- 文档入口：`apps/docs-app/src/pages/components/pages/display.rs` 提供 `Code` 页面三个 Playground（`Variant Matrix`、`Custom Class + Block`、`Interactive Playground`）；其中 Interactive 覆盖展示区 / Config 区 / Code 区 / CSS Test 区，并包含 `Show compare matrix` 多场景对比。
- README：`crates/ui-components/src/code/README.md` 已补齐，包含展示区 / Config 区 / Code 区 / CSS Test 区说明与多场景对比示例。
- 测试回归：`cargo test -p ui-components --test code_semantics` 通过（7/7）。
- 最小特性编译：`cargo check -p ui-components --no-default-features --features component-code,inject-css` 通过。
- wasm 路径编译：`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-code,inject-css` 通过（存在 `observability.rs` 的既有 dead_code warning，非 `code` 组件新增）。
- Clippy 门禁：`CARGO_TARGET_DIR=/tmp/code-clippy-min CARGO_INCREMENTAL=0 CARGO_BUILD_JOBS=1 cargo clippy -p ui-components --no-default-features --features component-code,inject-css -- -D warnings` 通过。
- N/A 明确项：受控/非受控轴、异步协议、交互型 E2E 流程、`inner_html` 注入、组件级 motion 驱动、`Streaming Required`（`Code` 仅消费稳定 Snapshot 渲染）。
