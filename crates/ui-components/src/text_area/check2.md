...
### 本次逐项核验证据（text_area）
- 代码分层与 API：`crates/ui-components/src/text_area/mod.rs`、`crates/ui-components/src/text_area/logic.rs`、`crates/ui-components/src/text_area/view.rs`、`crates/ui-components/src/text_area/motion.rs`、`crates/ui-state-primitives/src/text_area.rs`。
- 样式与 token-first：`crates/ui-components/src/text_area/styles.rs`（仅消费 `var(--ui-*)` 与 motion css var）。
