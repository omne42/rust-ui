...
- 组件装配重构：`crates/ui-components/src/color_area/logic.rs` 新增 `is_disabled` 兼容归一、默认值单点归一、Agent Contract 映射；`crates/ui-components/src/color_area/view.rs` 改为消费 `use_color_area` 合约并输出稳定 `data-*`/`data-ui-*` 标记。
