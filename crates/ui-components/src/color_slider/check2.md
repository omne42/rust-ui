...
1. 特性解耦修复（阻断项）：
   - 新增 `crates/ui-components/src/color_slider/motion.rs`，`ColorSliderMotion` 与 `attach_motion` 下沉到组件内，移除对 `crate::slider` 的隐式依赖。
   - `crates/ui-components/src/color_slider/logic.rs::sanitize_track_color` 改为 `ui_state_primitives::swatch::sanitize_color_value(...)`，移除对 `crate::color_swatch` 的 feature 依赖。
2. API/可访问性契约补齐：
   - `crates/ui-components/src/color_slider/view.rs` 新增 `is_disabled`（兼容 `disabled`）、`lang`、`dir` 接口。
   - disabled 归一由 `logic::normalize_accessibility_state` 统一处理，输出 `data-disabled-source`。
3. Headless 与状态语义：
   - `view.rs` 改用 `ui_headless::use_slider(SliderOptions { ... })`，输入事件与交互态（hover/focus/press）使用 headless contract。
   - 根节点新增稳定标记：`data-control-mode`、`data-value-source`、`data-default-value-source`、`data-value-change-source`、`data-disabled-source`。
4. Agent Contract（机器可读）补齐：
   - `logic.rs` 新增类型化 contract：`ColorSliderAgentSchema/StreamSupport/StreamFallback/StreamMode/OutputStatus/Intent/UiAction`。
   - `view.rs` 挂载 `data-ui-schema`、`data-ui-schema-version`、`data-ui-stream-*`、`data-ui-output-status`、`data-ui-intent`、`data-ui-action`、`data-ui-source`、`data-ui-state`。
5. 文件落点修复：
   - 组件目录补齐标准文件：`crates/ui-components/src/color_slider/motion.rs`。
   - 新增新手文档入口：`crates/ui-components/src/color_slider/README.md`（Hello World + 受控示例 + API 约定 + source-first 路径）。
6. 回归测试更新：
   - `crates/ui-components/tests/color_slider_semantics.rs` 新增/更新：
...
7. 组件范围验证命令（通过）：
   - `CARGO_TARGET_DIR=target-color-slider-check cargo test -p ui-components --test color_slider_semantics --no-default-features --features component-color_slider,inject-css`
   - `CARGO_TARGET_DIR=target-color-slider-check cargo check -p ui-components --no-default-features --features component-color_slider,inject-css`
   - `CARGO_TARGET_DIR=target-color-slider-check cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-color_slider,inject-css`
   - `CARGO_TARGET_DIR=target-color-slider-check cargo clippy -p ui-components --no-default-features --features component-color_slider,inject-css -- -D warnings`
8. 并行协作阻断说明（不跨线修改）：
   - 复跑阶段出现外部错误：`crates/ui-headless/src/color_area.rs` 的 `#[derive(PartialEq)]` 与 `Callback` 类型不兼容（非 `color_slider` 改动引入）。
   - 为避免干扰他人，本次未改动该文件；`color_slider` 组件范围验证已独立给出通过证据。
