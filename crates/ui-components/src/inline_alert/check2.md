...
- 本次结论：`inline_alert` 最小特性链可独立编译、测试、clippy、wasm 检查通过；未修改组件实现代码，仅回填核验清单。
- 关键验证：
  - `cargo test -p ui-components --test inline_alert_semantics --no-default-features --features component-inline_alert,inject-css -- --nocapture`
  - `cargo check -p ui-components --no-default-features --features component-inline_alert,inject-css`
  - `cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-inline_alert,inject-css`
  - `cargo tree -e features -i ui-components -p web-demo`
  - `cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-inline_alert,inject-css`
  - `cargo clippy -p ui-components --no-default-features --features component-inline_alert,inject-css -- -D warnings`
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（本次以 `component-inline_alert` 最小特性链完成 check/clippy/test/wasm 与 headless feature 互斥校验；未触碰他人并行改动范围）。
