...
mod tests {
    ...
    fn normalize_aria_label_uses_trimmed_label_or_fallback() {
        let (label, explicit) = normalize_aria_label(Some("  Text align  ".to_string()));
    ...
    fn resolve_state_tracks_orientation_attachment_and_label_source() {
        let state = resolve_state(ButtonGroupOrientation::Vertical, true, true);
        ...
        let state = resolve_state(ButtonGroupOrientation::Horizontal, false, false);
