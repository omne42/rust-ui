...
- 失败修复 1：修正 `crates/ui-components/tests/legend_semantics.rs` 中过期断言字符串（由 `semantics.attrs.*` 更新为 `legend_data_*` 对齐 `view.rs` 实现）。
- 失败修复 2：删除 `crates/ui-components/src/legend/logic.rs` 未使用函数 `source_attr_from_presence`，避免告警升级为门禁失败。
...
- 核验命令（按顺序逐项执行）：
  1. `CARGO_TARGET_DIR=target-legend-check cargo test -p ui-components --test legend_semantics --no-default-features --features component-legend,inject-css`（通过，7 passed）
  2. `CARGO_TARGET_DIR=target-legend-check cargo check -p ui-components --no-default-features --features component-legend,inject-css`（通过）
  3. `CARGO_TARGET_DIR=target-legend-check cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-legend,inject-css`（通过）
  4. `CARGO_TARGET_DIR=target-legend-check cargo clippy -p ui-components --no-default-features --features component-legend,inject-css -- -D warnings`（通过）
- docs smoke：
  1. `bash ./scripts/smoke-docs-app.sh` / `bash scripts/smoke-csr.sh apps/docs-app "body:not(:has(#boot))"` 首次失败（环境缺 Playwright 浏览器），已通过 `npx playwright install chromium` 补齐环境。
  2. 二次 smoke 受仓库现有他人改动阻塞：`apps/docs-app/src/pages/components/pages/forms_extra.rs` 存在独立编译错误（`on_day_press` 期望 `Option<Callback<u8>>`），按“多人协作不干扰”原则未在本任务中改动该文件。
- N/A 说明：`legend` 为语义与展示组件，无独立异步状态机、无 `inner_html` 注入路径、无独立 E2E 关键事务流；相关条目按“契约不适用但边界明确”完成核验。
