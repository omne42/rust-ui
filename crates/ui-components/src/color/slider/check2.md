...
1. 特性解耦修复（阻断项）：
   - 新增 `crates/ui-components/src/color/slider/motion.rs`，`ColorSliderMotion` 与 `attach_motion` 下沉到组件内，移除对 `crate::slider` 的隐式依赖。
   - `crates/ui-components/src/color/slider/logic.rs::sanitize_track_color` 改为 `ui_state_primitives::swatch::sanitize_color_value(...)`，移除对 `crate::color_swatch` 的 feature 依赖。
2. API/可访问性契约补齐：
   - `crates/ui-components/src/color/slider/view.rs` 新增 `is_disabled`（兼容 `disabled`）、`lang`、`dir` 接口。
...
5. 文件落点修复：
   - 组件目录补齐标准文件：`crates/ui-components/src/color/slider/motion.rs`。
   - 新增新手文档入口：`crates/ui-components/src/color/slider/README.md`（Hello World + 受控示例 + API 约定 + source-first 路径）。
6. 回归测试更新：
