...
1. 最小特性根因修复：`component-color_handle` 依赖链已修复为 `component-color_handle -> component-color_thumb -> component-color_swatch`，避免最小特性构建缺失 `ColorThumb/ColorSwatch` 导致编译失败。
2. 组件职责落位：已补 `crates/ui-components/src/color/handle/motion.rs`，并在 `view.rs` 挂载 `motion::attach_motion` 与 `data-motion-source`；`styles.rs` 改为消费 `--ui-color-handle-motion-duration`。
3. 语义契约回归：`crates/ui-components/tests/color_handle_semantics.rs` 新增 motion 契约挂载断言与 `check2` 无未勾选项断言，确保后续改动不回退。
4. API 与状态语义：`color_handle` 当前无受控/非受控状态轴与异步请求路径，相关条目按组件职责标记为 N/A（静态状态输入组件）。
5. 流式职责：`color_handle` 非正文阅读面，按 `Streaming Optional` 执行，当前以 `snapshot` 渲染链路为主。
6. 本轮执行门禁（定向）：
   - `cargo test -p ui-components --no-default-features --features component-color_handle,inject-css --test color_handle_semantics`
   - `cargo clippy -p ui-components --no-default-features --features component-color_handle,inject-css -- -D warnings`
   - `cargo check -p ui-components --no-default-features --features component-color_handle,inject-css`
   - `cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-color_handle,inject-css`
   - `cargo tree -e features -p ui-components --no-default-features --features component-color_handle,inject-css`
