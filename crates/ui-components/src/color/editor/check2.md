...
- 已修复：`status-primitives` 下沉（新增 `crates/ui-state-primitives/src/color_editor.rs`，`crates/ui-components/src/color/editor/logic.rs` 改为纯消费层 re-export）。
- 已修复：组件目录职责（新增 `crates/ui-components/src/color/editor/motion.rs`，`view.rs` 通过 `motion_contract` 读取来源标记）。
