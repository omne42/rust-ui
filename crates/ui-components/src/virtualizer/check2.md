...
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），组件层无自造状态原语实现。
...
- [x] `ui-headless` 定义：交互与 A11y 原语层（press/focus/hover/roving/listbox/menu/tooltip 等），把输入设备事件与状态语义标准化为可复用契约；输出必须是类型化 `attrs + handlers + state`。不做样式、不写组件 CSS、不做组件级动效编排。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），组件层无自实现交互/A11y 语义契约逻辑。
...
- [x] `ui-motion` 定义：动效能力与契约执行层（spring、keyframes、WAAPI/RAF backend），只负责时间函数、插值与运行时驱动，不承载组件业务语义与状态决策。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），组件层无自实现 motion 引擎或组件动效状态机。
...
- [x] `ui-theme` 定义：唯一设计 token 与主题上下文层（system/color/scale + Light/Dark/OLED），负责 token 分类、主题映射与 CSS 变量生成。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），组件层无私有 token 体系与主题映射实现漂移。
...
- [x] `ui-components` 定义：最终 Leptos 组件装配层，组合 `status-primitives + ui-headless + ui-motion + ui-theme` 并暴露稳定公共 API。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），不存在组件装配实现与对外 API 暴露面。
...
- [x] API 命名契约统一：公共 props/回调严格使用 `is_*`、`on_*`、`default_*` 前缀；同语义在全库同名，禁止别名漂移。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无公共组件 props/回调 API，命名契约无漂移面。
...
- [x] 受控/非受控必须成对：每个可控状态轴都提供 `value + on_value_change + default_value`（如 `open/on_open_change/default_open`）；缺一项即不通过。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无受控/非受控状态轴 API，不存在“半受控”隐式行为。
...
- [x] 默认值单一来源：默认值与优先级只在 `logic.rs` 归一化；`view.rs` 禁止二次兜底或隐式改写。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无 `logic.rs/view.rs` 默认值归一化实现，不存在多来源默认值分叉。
...
- [x] 状态归一化集中：状态输入先类型化，再在 `logic.rs` 统一派生；禁止在 `view.rs`、事件回调、样式分支中分散拼状态机。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无状态输入归一化与派生实现，不存在分散拼接状态机路径。
...
- [x] 离散状态必须类型约束：`variant/size/mode/status` 等离散输入使用 `enum`；禁止用多个 `Option<bool>`/字符串自由组合表达互斥状态。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无离散状态输入 API，不存在字符串协议或布尔爆炸实现。
...
- [x] 状态原语来源正确：组件层只消费 `status-primitives`（当前 `ui-state-primitives`）能力，不直接绑定业务 store；应用级全局状态必须经桥接层适配后再接入组件。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），组件层无状态原语实现与业务 store 直接绑定面。
...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。
  - N/A：当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无远程请求与异步状态实现。
...
- [x] API 易用性验收标准（DX Paradox）：把复杂性留在内部，把简单留给用户。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无对外组件调用路径与示例接入面。
...
- [x] 组合型组件主 API 必须“显示优于约定”：优先使用显式组合 `<Parent><Item ... /></Parent>`。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无组合型组件主 API 设计与暴露面。
...
- [x] 存在 A11y 实现、国际化与本地化实现（至少具备接入点，不硬编码用户可见文本）。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无组件侧 A11y/i18n/l10n 语义挂载与文案实现面。
...
- [x] 状态可观测、可检索、可验证：使用稳定 `data-*` 与 `aria-*` 标记表达状态和来源。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无状态语义 marker 挂载实现，不存在标记契约漂移面。
...
- [x] 样式依赖显式状态（`data-*`/class），而非脆弱 DOM 结构猜测。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无 `styles.rs` 状态分支实现，不存在 DOM 结构猜测式选择器漂移面。
...
- [x] 测试验证“语义契约”而不只验证视觉快照。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无组件语义契约测试目标面。
...
- [x] 组件文件职责正确：`mod.rs`（导出边界）、`logic.rs`（归一/派生/来源标记）、`styles.rs`（静态 token-first CSS）、`view.rs`（Leptos 结构 + headless 挂载）、`motion.rs`（动效契约 + attach）。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无组件目录职责拆分实现面。
...
- [x] `spec.rs` 只用于少数复杂组件（如 button），避免泛滥。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无新增 `spec.rs` 的组件实现需求。
...
- [x] 组件层遵循 token-first 静态样式契约：样式通过 `styles.rs` 聚合注入；运行时仅传必要 CSS 变量；不把 Utility-First/CSS-in-Rust 当组件库默认范式。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无组件样式实现与 token 消费面。
...
- [x] 默认主题美学质量达标（Visual Desire）：以 HeroUI 现代审美为学习对标，默认主题不仅“可用”，还必须“第一眼可信”。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无默认主题视觉实现与回归基线接入面。
...
- [x] Tree Shaking 是一等能力：package 模式支持组件级 feature；source 模式天然裁剪；样式层同步裁剪，禁止无条件聚合全部 CSS，禁止破坏 DCE/LTO 的全量中央注册表。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无组件 feature 与样式聚合导出面可导致可达性回归。
...
- [x] 类型系统 + 语义标记共同提供机器可读状态；关键输入空间受类型约束。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无状态输入类型建模与语义标记输出实现面。
...
- [x] SSR 与跨平台检查：覆盖 web/ssr/wasm 分支，不破坏 non-wasm 编译路径。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无 web/ssr/wasm 平台分支实现面。
...
- [x] `ui-headless` web/ssr feature 互斥受 `compile_error!` 保护（`crates/ui-headless/src/lib.rs`）。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无组件侧 `ui-headless` 功能接入路径。
...
- [x] `ui-motion` 非 wasm 提供 no-op/stub（`crates/ui-motion/src/lib.rs`），保证 SSR/tooling 可编译。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无组件侧 `motion.rs` 调用与 non-wasm 降级路径实现面。
...
- [x] 组件实现覆盖 `reduced-motion` / SSR / wasm 分支。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无组件运行时分支实现与 hydration 行为接入面。
  - `reduced-motion` 下动画应跳过或降级为最小必要反馈。
  - SSR 输出必须与客户端 hydration 兼容，避免首帧语义错位。
  - wasm 分支允许增强交互，但语义契约不得与 SSR 分支分裂。
- [x] 性能治理：关键路径有预算（首次渲染/更新耗时/内存），回归可检测、可归因、可阻断。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无组件性能预算定义、render_count 测试与回归归因接入面。
...
- [x] `view!` 宏复杂度受控：单个 `view!` 块不得承载超长深嵌套结构；复杂布局按语义分块，避免一次性宏展开导致编译与 wasm 体积劣化。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无 `view!` 宏展开实现与体积劣化风险接入面。
...
- [x] 函数式拆分优先：不涉及复杂状态与生命周期管理的 UI 片段，优先拆为普通 Rust 函数（返回 `impl IntoView`/`View`），而不是新增 `#[component]`。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无组件片段拆分与 `#[component]` 升格实现面。
...
- [x] 静态片段常量化：复杂 SVG、页脚、长说明文本等纯静态内容优先常量化/模板化，减少重复 `view!` 渲染指令生成。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无静态片段渲染实现与常量化路径接入面。
  - 可判定为纯静态的片段应避免重复动态构造。
...
- [x] `inner_html` 使用约束：仅允许注入受信任静态常量，禁止拼接用户输入；使用处必须补充语义与安全回归测试。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无 `inner_html` 注入路径与语义/安全测试接入面。
  - 仅允许编译期常量或明确白名单内容进入 `inner_html`。
  - 严禁直接或间接注入用户输入、远端返回或未清洗模板字符串。
  - 使用 `inner_html` 的节点必须补语义测试与安全回归说明。
- [x] WASM 调试要求：关键状态可追踪（来源/时间/前后值），关键交互可回放，开发模式有可视化入口，调试能力通过 feature 隔离不污染产物。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无关键状态追踪与交互回放调试接入面。
  - 开发模式下至少能追踪关键状态变更来源与前后值。
  - 关键交互链路应支持最小可复现记录（事件顺序/状态转移）。
  - 调试开关默认不进入生产包体与公共 API。
- [x] DX 要求：样式热重载优先无需重编 wasm；组件热开发尽量保持上下文；提供可选状态保留；有 Workbench 隔离画布。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无样式热重载与组件热开发调试接入面。
  - 常见样式调整应走快速反馈路径，不依赖完整 wasm 重编译。
  - 组件调试应尽量保持当前交互上下文，降低重复操作成本。
  - 复杂交互组件应有隔离演练入口（workbench/story/demo 之一）。
- [x] 工程能力统一：`serde` 负责 spec 序列化/版本迁移/错误结构化；`tracing` 统一 span/event 语义；async 不绑定单一运行时（tokio/async-std），runtime 细节不泄露到上层 API。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无 `serde`/`tracing`/async 工程能力接入面。
...
- [x] `ui-components` 固定入口文件落点正确。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无组件入口注册与固定文件落点接入面。
...
- [x] 组件目录标准文件落点正确。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无 `<component>/mod.rs|logic.rs|styles.rs|view.rs|motion.rs|spec.rs` 目录落点接入面。
...
- [x] 语义标记统一升级为 Agent Contract（Schema 化），让 Agent 不依赖 DOM 猜测理解组件状态与意图。
  - 当前 `virtualizer` 兼容模块已移除（`src/virtualizer/mod.rs` 不存在，且 crate root 未注册该模块），无 `data-ui-schema` 与 Agent 语义契约输出链路接入面。
...
### 10. 本次真实核验记录（2026-02-17）
- [x] `src/virtualizer/mod.rs` 不存在（`test -e crates/ui-components/src/virtualizer/mod.rs` 返回不存在）。
- [x] `crates/ui-components/src/lib.rs` 未注册 `pub mod virtualizer;`（`grep` 未命中）。
- [x] `crates/ui-components/tests/virtualizer_module_semantics.rs` 存在且最小特性测试通过：`cargo test -p ui-components --no-default-features --test virtualizer_module_semantics`（2 passed）。
- [ ] 全量门禁暂未通过：`cargo fmt --all -- --check`、`cargo clippy -p ui-components --no-default-features --all-targets -- -D warnings` 受当前并行改动影响失败（失败点在 `swatch`/`text_field`/`tray`/`tree` 等非 `virtualizer` 文件）。
