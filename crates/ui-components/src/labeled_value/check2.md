...
### 1. 大骨架（架构边界与层职责）
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（`LabeledValue` 状态原语已下沉到 `ui-state-primitives/src/labeled_value.rs`，并在 `ui-state-primitives/src/lib.rs` 导出）
...
- [x] `ui-headless` 定义：交互与 A11y 原语层（press/focus/hover/roving/listbox/menu/tooltip 等），把输入设备事件与状态语义标准化为可复用契约；输出必须是类型化 `attrs + handlers + state`。不做样式、不写组件 CSS、不做组件级动效编排。（已新增 `ui-headless/src/labeled_value.rs`，组件 `view.rs` 仅挂载 `use_labeled_value` 输出）
...
### 8. 明确禁止的反模式
- [x] 在 `status-primitives`（当前 `ui-state-primitives`）写 DOM/样式逻辑。（已核对 `ui-state-primitives/src/labeled_value.rs` 仅含纯 Rust 状态与归一逻辑）
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（本次执行：`cargo test -p ui-state-primitives labeled_value --lib`、`cargo test -p ui-headless labeled_value --lib`、`cargo test -p ui-components --test labeled_value_semantics --no-default-features --features component-labeled_value,inject-css`、`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-labeled_value,inject-css`；语义回归文件：`crates/ui-components/tests/labeled_value_semantics.rs`）
