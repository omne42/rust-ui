...
### 10. Checkbox / CheckboxGroup 本次迁移验收记录（2026-02-18）
- [x] `group` 目录并入 `checkbox` 主域：不再保留 `src/checkbox_field/checkbox/group/*`；`CheckboxGroup` 逻辑/样式/视图/动效契约已并入 `src/checkbox/{logic,motion,styles,view}.rs`。
- [x] `field` 与 `checkbox` 域拆分完成：`checkbox` 迁移到顶层 `src/checkbox/*`；`checkbox_field` 保持独立目录，仅负责字段包装。
- [x] API 改名与导出迁移完成：`checkbox::group::CheckboxGroup` 改为 `checkbox::CheckboxGroup`；CSS 聚合从 `checkbox::group::styles::CSS` 改为 `checkbox::styles::CHECKBOX_GROUP_CSS`。
- [x] docs-app 补齐 interactive playground：`checkbox` 与 `checkbox-group` 页面均包含 Display / Config / Code / CSS Test 区域，并保留多状态对比展示。
- [x] README 已补齐：新增 `src/checkbox/README.md`，并在 docs shell 注册 `checkbox` / `checkbox-group` 文档入口。
- [x] 语义测试与最小特性门禁通过：
  ...
  - `CARGO_TARGET_DIR=target/checkbox-refactor cargo test -p ui-components --lib --no-default-features --features component-checkbox,component-checkbox_group,component-checkbox_field,inject-css`
  - `CARGO_TARGET_DIR=target/checkbox-refactor cargo clippy -p ui-components --no-default-features --features component-checkbox,component-checkbox_group,component-checkbox_field,inject-css --tests -- -D warnings`
  - `CARGO_TARGET_DIR=target/checkbox-refactor cargo check -p docs-app`
  - `CARGO_TARGET_DIR=target/checkbox-refactor cargo test -p ui-components --test css --features all-components,inject-css`
  - `CARGO_TARGET_DIR=target/checkbox-refactor cargo test -p ui-components --no-default-features --features component-checkbox,inject-css --test upstream_name_parity_semantics`
