...
pub fn SearchField(
    ...
    let clear_label_state = logic::resolve_clear_button_label(logic::ClearButtonLabelInput {
        ...
        i18n_clear_aria_label: Some(common.clear_aria_label.to_string()),
    });
    ...
    let clear_press = use_press(PressOptions {
        ...
        on_press: Some(Callback::new(move |_| {
            search_field_contract.handlers.on_clear.run(())
        })),
        ..Default::default()
    });
        ...
        >
            ...
            >
                ...
                <button
                    ...
                    data-visible=move || search_field_contract.state.can_clear.get().then_some("true")
                    aria-hidden=move || (!search_field_contract.state.can_clear.get()).then_some("true")
                    type="button"
                    tabindex="-1"
                    aria-label=clear_button_aria_label
                    node_ref=clear_button_ref
                    disabled=move || is_disabled || is_read_only || !search_field_contract.state.can_clear.get()
