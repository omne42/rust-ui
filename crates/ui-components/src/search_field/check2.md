...
- 下沉结果：将 `search_field` 的清空按钮可见性状态判定从组件层下沉到 `ui-state-primitives`。
  ...
  - 组件消费：`crates/ui-components/src/search_field/logic.rs` 改为仅装配 `Memo` 并调用 `ui_state_primitives::search_field::resolve_state`。
...
- `ui-motion` 核验：
  - `crates/ui-components/src/search_field/motion.rs` 仅做组件语义到 `ui_motion::spring::SpringAnimator` 的映射与 attach。
  - `#[cfg(not(target_arch = "wasm32"))]` 分支为 no-op + sanitize，保证 non-wasm 可编译、可预测。
  - 未在组件内重写 spring/keyframe/driver 执行器；运行时驱动来自 `ui-motion`。
- `ui-theme` 核验：
  - `crates/ui-components/src/search_field/styles.rs` 仅消费 `var(--ui-*)` 视觉变量（色彩/间距/圆角/焦点等）。
...
- 职责落位：
  - `crates/ui-components/src/search_field/logic.rs`：新增 `resolve_root_class`，承接 props 归一；状态派生继续由 `ui-state-primitives` 输出驱动。
  - `crates/ui-components/src/search_field/view.rs`：仅结构渲染 + headless 能力挂载；移除对外 `node_ref: NodeRef<html::Input>` 参数，DOM 细节留在组件内部。
  - `crates/ui-components/src/search_field/styles.rs`：保持 token-first 静态样式。
  - `crates/ui-components/src/search_field/motion.rs`：仅语义状态到 motion attach 映射。
- 导出面收敛：
  - `crates/ui-components/src/search_field/mod.rs`：`motion` 改为私有模块，仅 `pub use SearchField/SearchFieldMotion`，避免暴露内部平台细节函数签名。
- 复验命令（`/root/.cargo/bin/cargo`）：
  - `cargo test -p ui-components --no-default-features --features component-search_field,inject-css --test search_field_semantics --offline`
  - `cargo check -p ui-components --no-default-features --features component-search_field,inject-css --offline`
...
- 新增 `crates/ui-components/src/search_field/README.md`：
...
- `ui-headless` 契约下沉：
  ...
  - `crates/ui-components/src/search_field/view.rs` 改为消费 `use_search_field` 合约，不再在 view 内自造键盘语义规则。
- Agent Contract：
  - `crates/ui-components/src/search_field/logic.rs` 新增 `SearchFieldAgentSchemaVersion/SearchFieldAgentIntent/SearchFieldAgentActionModel/SearchFieldAgentContract` 与 `search_field_agent_contract()`。
  - `crates/ui-components/src/search_field/view.rs` 根节点挂载 `data-ui-schema/data-ui-schema-version/data-ui-intent/data-ui-action-model/data-ui-state-axis/data-ui-source-axis`。
- docs-app / e2e：
