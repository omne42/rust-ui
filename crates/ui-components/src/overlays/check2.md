...
Overlays（`OverlaysRoot` 及 Overlay/Popover/Modal/Tray 聚合导出）无远程请求与组件内异步状态轴，本项按 `N/A` 验收。
...
Overlays 归类为 `Streaming Optional`，当前按 `fallback=snapshot` 稳定渲染，并持续暴露 `role/aria/data-*` 语义状态。
...
本轮门禁记录（组件范围）：`cargo check/test/clippy -p ui-components --no-default-features --features component-overlays,inject-css` 已通过；仓库级全量 smoke 由并行任务统一执行。
