...
> 本轮执行记录（circular_progress）  
> 1) 状态原语下沉到 `crates/ui-state-primitives/src/circular_progress.rs`，组件 `logic.rs` 改为纯消费导出。  
> 2) `view.rs` 接入 `ui-headless` i18n 与 `lang/dir` locale attrs，去除硬编码默认文案来源。  
> 3) `styles.rs` 改为 token-first 回退链（`--ui-button-spinner-*`）并补齐 reduced-motion 降级。  
> 4) 语义回归更新在 `crates/ui-components/tests/circular_progress_semantics.rs`。  
> 5) 组件为纯展示型，不存在受控/非受控状态轴、异步协议、组合型 Parent/Item、`inner_html`，上述项按 N/A 通过并保持契约约束。
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）：`rustfmt --edition 2024 crates/ui-state-primitives/src/circular_progress.rs crates/ui-state-primitives/src/lib.rs crates/ui-components/src/circular_progress/logic.rs crates/ui-components/src/circular_progress/mod.rs crates/ui-components/src/circular_progress/view.rs crates/ui-components/tests/circular_progress_semantics.rs`、`CARGO_TARGET_DIR=target/cp-check2 cargo test -p ui-state-primitives circular_progress`、`CARGO_TARGET_DIR=target/cp-check2 cargo test -p ui-components --no-default-features --features component-circular_progress,component-button,inject-css --test circular_progress_semantics`、`CARGO_TARGET_DIR=target/cp-check2 cargo clippy -p ui-components --no-default-features --features component-circular_progress,inject-css -- -D warnings`、`CARGO_TARGET_DIR=target/cp-check2 cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-circular_progress,inject-css`、`CARGO_TARGET_DIR=target/cp-check2 cargo check -p ui-headless --no-default-features --features web`、`CARGO_TARGET_DIR=target/cp-check2 cargo check -p ui-headless --no-default-features --features ssr`、`CARGO_TARGET_DIR=target/cp-check2 cargo check -p ui-headless --no-default-features --features web,ssr`（按预期触发 `compile_error!`）。
