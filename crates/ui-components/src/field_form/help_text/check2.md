...
### 10. HelpText 组件核验证据（本次）
- 架构边界：`crates/ui-components/src/field_form/help_text/` 维持 `mod.rs + logic.rs + styles.rs + view.rs + motion.rs` 分层；状态归一与来源标记集中在 `logic.rs`，`view.rs` 只负责渲染与挂载，未越层实现可复用状态原语。
- 类型与状态契约：`HelpTextTone` 枚举约束离散输入；`HelpTextStateInput/HelpTextState` 建模消息类型（`description/error/none`）、来源标记（`aria/error/class source`）与 `data-state` 封闭集合。
- 语义标记：`view.rs` 输出稳定 `data-*`（`data-tone/data-state/data-message-kind/data-aria-source/data-error-source/...`）与 `aria-*`（`aria-invalid/aria-disabled/aria-label`），满足可观测/可检索契约。
- 样式契约：`styles.rs` 仅基于稳定 class + `data-*` 选择器消费状态；视觉值使用主题变量，不依赖 DOM 层级猜测。
- 文档与示例：`apps/docs-app/src/pages/components/pages/forms_extra.rs` 提供 `HelpText` 页面三个 Playground（`Description (Neutral)`、`Invalid + Error Icon`、`Interactive Playground`）；Interactive 覆盖展示区 / Config 区 / Code 区 / CSS Test 区，并提供 `Show compare matrix` 多场景对比。
- README：`crates/ui-components/src/field_form/help_text/README.md` 已补齐，包含展示区 / Config 区 / Code 区 / CSS Test 区结构与状态对比说明。
- 语义测试：`cargo test -p ui-components --test help_text_semantics --no-default-features --features component-help_text,inject-css` 通过（6/6）。
- 最小特性编译：`cargo check -p ui-components --no-default-features --features component-help_text,inject-css` 通过。
- wasm 路径编译：`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-help_text,inject-css` 通过（存在 `observability.rs` 的既有 dead_code warning，非 `help_text` 组件新增）。
- Clippy 门禁：`cargo clippy -p ui-components --no-default-features --features component-help_text,inject-css -- -D warnings` 通过。
- 说明：默认特性全量编译当前被 `crates/ui-components/src/menu/dropdown/view.rs` 的既有错误阻塞（与本组件无关），本次按“最小特性链 + 组件边界”完成核验，避免干扰并行协作改动。
- N/A 明确项：受控/非受控状态轴、异步协议、`inner_html` 注入、`Streaming Required`（`HelpText` 为表单辅助展示组件，按 Snapshot 稳定渲染）。
