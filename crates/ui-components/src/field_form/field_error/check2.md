...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（本次按组件范围执行：`component-field_error` 最小特性 `check + clippy + field_error_semantics`。）

### 10. 本轮验收记录（FieldError）
- 语义契约回归：`cargo test -p ui-components --test field_error_semantics --no-default-features --features component-field_error,inject-css` 通过，6/6 用例通过。
- 最小特性编译（native）：`cargo check -p ui-components --no-default-features --features component-field_error,inject-css` 通过。
- 最小特性编译（wasm）：`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-field_error,inject-css` 通过。
- 静态检查：`cargo clippy -p ui-components --no-default-features --features component-field_error,inject-css -- -D warnings` 通过。
- Tree Shaking 约束：`component-field_error` 为独立 feature（`crates/ui-components/Cargo.toml`），`lib.rs` 与 `css.rs` 均按 feature 条件导出/聚合。
- N/A 说明：`FieldError` 无异步请求链路（`is_loading`/retry/`aria-busy` 不适用）；非组合型 item 组件（并行数组约束不适用）；无 `inner_html` 注入路径。
- 范围说明：默认全特性测试当前受其他组件（如 `dropdown`/`color_slider`）既有编译问题影响；本次按“单组件 check2”要求完成 `field_error` 目标特性链路验收。
