...
pub fn Fieldset(
    ...
    let motion = crate::field_form::fieldset::motion::sanitize_motion(motion);
    let has_custom_motion = motion != FieldsetMotion::default();
    let motion_style = StoredValue::new(Some(crate::field_form::fieldset::motion::attach_motion(
        motion,
    )));
