...
已核验（field，2026-02-18）：本组件是表单字段容器，核心职责是标签/描述/错误文案与 `required/disabled/invalid` 语义映射，不承载受控 value 轴、远程异步流程、overlay 焦点链路与流式正文渲染职责；相关条目按 N/A 语义核验并保持契约可追溯。
...
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。
  - `field` 组件最小门禁已验证：`cargo test -p ui-components --test field_semantics --no-default-features --features component-field,inject-css`、`cargo check -p ui-components --no-default-features --features component-field,inject-css`、`cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-field,inject-css`、`cargo clippy -p ui-components --no-default-features --features component-field,inject-css --tests -- -D warnings`。
  - 仓库当前存在与本任务无关的全量构建阻塞（`crates/ui-components/src/color/slider/view.rs` 缺失）；本任务按组件最小特性集完成并验证通过，不干扰并行协作分支。
