...
### 本轮核验记录（coachmark）
- 最小特性依赖修复：`component-coachmark` 现在显式依赖 `component-asset`、`component-button`、`component-contextual_help`，并补齐 `component-contextual_help -> component-button + component-popover`。
- Agent Contract 与流式标记：`coachmark/view.rs` 增加 `data-ui-schema/data-ui-intent/data-ui-action/data-ui-state/data-ui-source` 及 `snapshot` 模式标记。
- 语义回归：`crates/ui-components/tests/coachmark_semantics.rs` 已补充 Agent Contract、feature 依赖链、checklist 完整性断言。
- E2E 回归：新增 `e2e/tests/docs_app_coachmark_contract.spec.mjs`，覆盖语义选择器、受控开关流程、reload 可重复路径。
- docs/readme 与 playground：`crates/ui-components/src/coachmark/README.md` 新增 `display/config/code/css test` 分区；`apps/docs-app/src/pages/components/pages/overlays_extra_coachmark.rs` 增补 Display 对比与 Config+Code+CSS Test workbench。
- E2E 本机执行状态：当前环境已具备浏览器二进制，但 `docs-app` 构建被其他页面错误拦截（非 coachmark 范围），导致页面出现 `Build failure` 覆盖层并使点击路径失败。
- 额外约束回归：`crates/ui-components/tests/coachmark_semantics.rs` 新增 README 分区断言（display/config/code/css test）与 docs workbench 断言（`Display Comparisons` + `Config + Code + CSS Test Workbench` + `test_css_source/test_config_signal`）。
- 本轮执行命令：
  - `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.coachmark-check cargo check -p ui-components --no-default-features --features component-coachmark,inject-css`
  - `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.coachmark-check cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-coachmark,inject-css`
- `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.coachmark-check cargo clippy -p ui-components --no-default-features --features component-coachmark,inject-css --tests -- -D warnings`
- `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.coachmark-check cargo clippy -p ui-components --no-default-features --features component-coachmark,inject-css --test coachmark_semantics -- -D warnings`（全量 `--tests` 受其他测试文件阻塞）
- `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.coachmark-check cargo test -p ui-components --test coachmark_semantics --no-default-features --features component-coachmark,inject-css`
- `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=target/.coachmark-check cargo test -p ui-state-primitives coachmark`
- `node --check e2e/tests/docs_app_coachmark_contract.spec.mjs`
- `cd e2e && npx playwright test tests/docs_app_coachmark_contract.spec.mjs --project=chromium --reporter=list`（当前被 docs-app 其他页面构建失败阻塞）
