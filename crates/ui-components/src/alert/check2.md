...
- [x] 受控/非受控必须成对：每个可控状态轴都提供 `value + on_value_change + default_value`（如 `open/on_open_change/default_open`）；缺一项即不通过。
  - `Alert` 当前无可控状态轴（纯展示组件），该项按 N/A 通过；若未来引入状态轴，必须一次性提供完整三元组。
...
- [x] 默认值单一来源：默认值与优先级只在 `logic.rs` 归一化；`view.rs` 禁止二次兜底或隐式改写。
  - `Alert` 的 `variant` 改为 `Option<AlertVariant>` 输入，并在 `logic.rs::normalize_variant` 统一默认到 `AlertVariant::Default`；`view.rs` 仅消费归一化输出。
...
- [x] 状态归一化集中：状态输入先类型化，再在 `logic.rs` 统一派生；禁止在 `view.rs`、事件回调、样式分支中分散拼状态机。
  - `AlertStateInput -> resolve_state` 为唯一状态派生路径；`view.rs` 不直接使用 `resolve_state_core`/`live_region_attrs`，`styles.rs` 只消费 `data-*` 标记。
...
- [x] 离散状态必须类型约束：`variant/size/mode/status` 等离散输入使用 `enum`；禁止用多个 `Option<bool>`/字符串自由组合表达互斥状态。
  - `AlertVariant` 使用 `enum` 建模离散状态，`view` API 为 `variant: Option<AlertVariant>`；未出现字符串/布尔爆炸表达。
...
- [x] 状态原语来源正确：组件层只消费 `status-primitives`（当前 `ui-state-primitives`）能力，不直接绑定业务 store；应用级全局状态必须经桥接层适配后再接入组件。
  - `logic.rs` 仅通过 `ui_state_primitives::alert::resolve_state_core` 消费状态原语；未直接依赖业务 store。
...
- [x] 如果无异步相关，直接打勾。异步交互语义统一：`is_loading`、error/retry、disabled、`aria-busy` 映射一致；优先复用统一 async action 原语（如 `use_async_action`），禁止每组件自定义一套加载/错误协议。
  - N/A：`Alert` 为同步展示组件，无远程请求与异步状态轴；不引入 `is_loading`/`aria-busy`/retry 协议。
...
- [x] API 易用性验收标准（DX Paradox）：把复杂性留在内部，把简单留给用户。
  - docs 新增 `Hello World` playground，最小调用路径仅 `Alert + title + children`，无需手动接线 `ui-state-primitives/ui-headless`。
...
- [x] 组合型组件主 API 必须“显示优于约定”：优先使用显式组合 `<Parent><Item ... /></Parent>`。
  - `Alert` 采用显式 `<Alert>...</Alert>` 组合方式承载 actions slot，不使用并行数组/并行槽位隐式配对。
