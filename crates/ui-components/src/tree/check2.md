...
### 1. 大骨架（架构边界与层职责）
- [x] `status-primitives` 定义：纯状态原语层（受控/非受控、toggle、selection、list、overlay open state、expansion 等）。不依赖 Leptos/DOM/web-sys；只包含 Rust 数据结构和方法，不含视图与事件绑定。（`Tree` 状态原语已下沉到 `crates/ui-state-primitives/src/tree.rs` 并在 `crates/ui-state-primitives/src/lib.rs` 导出；组件侧 `crates/ui-components/src/tree/logic.rs` 仅消费原语并做 tone/density 映射。）
...
- [x] `ui-motion` 定义：动效能力与契约执行层（spring、keyframes、WAAPI/RAF backend），只负责时间函数、插值与运行时驱动，不承载组件业务语义与状态决策。（`Tree` 新增 `crates/ui-components/src/tree/motion.rs`，仅做语义状态到 `ui-motion` spring driver 的映射与 attach，non-wasm 提供 noop。）
...
- [x] `ui-theme` 定义：唯一设计 token 与主题上下文层（system/color/scale + Light/Dark/OLED），负责 token 分类、主题映射与 CSS 变量生成。（`Tree` 样式仍集中在 `crates/ui-components/src/tree/styles.rs` 并消费 `var(--ui-*)` 变量，无组件私有 token 体系。）
