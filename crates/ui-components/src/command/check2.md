...
### 10. Command 本轮验收证据
- [x] Tree-shaking 依赖链修复：`crates/ui-components/Cargo.toml` 中补齐 `component-command -> component-active_highlight`，并补齐 `component-command_dialog -> component-command + component-modal` 与 `component-modal -> component-overlay`。
- [x] 最小特性编译通过（native + wasm）：
  - `CARGO_TARGET_DIR=target/command-check2 cargo check -p ui-components --no-default-features --features component-command,inject-css`
  - `CARGO_TARGET_DIR=target/command-check2 cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-command,inject-css`
  - `CARGO_TARGET_DIR=target/command-check2 cargo check -p ui-components --no-default-features --features component-command_dialog,inject-css`
  - `CARGO_TARGET_DIR=target/command-check2 cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-command_dialog,inject-css`
- [x] 语义回归通过：
  - `CARGO_TARGET_DIR=target/command-check2 cargo test -p ui-components --test command_semantics --no-default-features --features component-command,inject-css`
  - `CARGO_TARGET_DIR=target/command-check2 cargo test -p ui-components --test command_dialog_semantics --no-default-features --features component-command_dialog,inject-css`
- [x] 最小特性树确认（未被 `all-components` 隐式拉起）：
  - `cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-command,inject-css`
  - `cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-command_dialog,inject-css`
- [x] 组件内导入路径去根重导出耦合：`crates/ui-components/src/command_dialog/view.rs` 与 `crates/ui-components/src/modal/view.rs` 改为直接引用 `crate::modal` / `crate::overlay`，避免最小 feature 下因 root re-export 缺失而构建失败。
