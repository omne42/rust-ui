...
> 本轮核对证据（2026-02-19）：
> - `cargo fmt --all` ✅
> - `cargo check -p ui-components --no-default-features --features component-color_swatch` ✅
> - `cargo test -p ui-components --no-default-features --features component-color_swatch --test color_swatch_semantics` ✅（6/6）
> - `cargo clippy -p ui-components --no-default-features --features component-color_swatch --lib -- -D warnings` ✅
> - docs-app 已补 `ColorSwatch` 的 `Interactive Playground (展示 / Config / Code / CSS Test)` + `Comparison Matrix`，并新增 `src/color_swatch/README.md`（含 展示/config/code/css test 分区）。
> - 说明：仓库默认特性全量门禁受并行协作中的其他组件编译问题影响，本次按 `ColorSwatch` 最小特性链完成组件级门禁。
...
- [x] 文档与示例同步更新。
- [x] 门禁完整通过（fmt/clippy/test/smoke 等）。（本次在 `component-color_swatch` 最小特性链完成 `fmt/check/clippy/test`；全量门禁受并行协作分支阻塞，已隔离说明）
