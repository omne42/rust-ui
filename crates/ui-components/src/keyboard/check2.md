...
### 10. 本次逐项核验记录（Keyboard）
- [x] 按清单顺序逐项核验完成：先骨架（1-2），再实现细节（3-6），最后测试文档与门禁（7-9）；发现并修复 1 项 API 命名阻断。
- [x] 本次发现并修复的阻断项：
  - `Keyboard` 公共布尔参数由 `compact` 统一更正为 `is_compact`（不保留兼容别名），同步更新 `view.rs`、docs playground 与 `keyboard_semantics` 断言。
- [x] 结构与职责结论：`keyboard` 维持 `mod/logic/styles/view` 分层；`logic.rs` 负责 tone/compact/source 归一与 class 组装，`view.rs` 仅负责 Leptos 结构与语义挂载。
- [x] 状态与类型结论：离散输入由 `KeyboardTone` 建模，关键状态通过 `KeyboardStateInput -> KeyboardState` 派生并稳定暴露 `data-tone/data-state/data-compact/data-*-source`。
- [x] A11y/i18n 与样式结论：`aria_label` 支持自定义与默认回退；样式依赖稳定 `data-*` 与 `ui-keyboard--*` 状态 class，不依赖脆弱 DOM 结构。
- [x] Async/Streaming/E2E 适用性结论：
  - Async：N/A（`Keyboard` 为静态展示组件，无远程请求与加载/重试状态轴）。
  - Streaming：`Keyboard` 非正文流式阅读面，按 `Streaming Optional` 执行，默认 `snapshot` 渲染路径成立。
  - E2E：当前由 `keyboard_semantics` + docs playground 覆盖语义契约，未新增独立 E2E 用例。
- [x] 文档与示例结论：`apps/docs-app/src/pages/components/pages/display_extra.rs` 包含 Keyboard 页面与两组 playground（Interactive Playground 展示/Config/Code/CSS Test + Comparison Matrix 多场景对比）。
- [x] Tree-shaking 结论：`component-keyboard` 最小特性链路仅拉起 `keyboard` 自身 + `inject-css`，未被 `all-components` 隐式全量拉起。

#### 10.1 门禁与验证命令（逐条执行）
- [x] `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=/root/autodl-tmp/cargo-target/keyboard-check cargo check -p ui-components --no-default-features --features component-keyboard,inject-css`
- [x] `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=/root/autodl-tmp/cargo-target/keyboard-check cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-keyboard,inject-css`
- [x] `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=/root/autodl-tmp/cargo-target/keyboard-check cargo clippy -p ui-components --no-default-features --features component-keyboard,inject-css --test keyboard_semantics --no-deps -- -D warnings`
- [x] `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=/root/autodl-tmp/cargo-target/keyboard-check cargo test -p ui-components --no-default-features --features component-keyboard,inject-css --test keyboard_semantics`
- [x] `TMPDIR=/root/autodl-tmp/tmp CARGO_TARGET_DIR=/root/autodl-tmp/cargo-target/keyboard-check cargo test -p ui-components --no-default-features --features component-keyboard,inject-css --test keyboard_semantics <single-test> -- --exact`（6 条语义测试逐条执行，全部通过）
- [x] `cargo tree -e features -i ui-components -p ui-components --no-default-features --features component-keyboard,inject-css`
- [x] `cargo tree -e features -i ui-components -p web-demo`
- [x] smoke：N/A（本次未改运行时代码路径，仅处理 API 命名一致性与 check2 核验证据）。
