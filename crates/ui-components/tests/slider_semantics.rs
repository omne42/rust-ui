...
fn slider_docs_are_copy_paste_ready_with_imports_copy_button_and_sync() {
    ...
    for needle in [
        ...
        "\"crates/ui-components/src/slider/mod.rs\"",
        "\"crates/ui-components/src/slider/logic.rs\"",
        "\"crates/ui-components/src/slider/view.rs\"",
        "\"crates/ui-components/src/slider/styles.rs\"",
        "\"crates/ui-components/src/slider/motion.rs\"",
...
#[test]
fn slider_final_merge_gate_capabilities_are_backed_by_contract_tests() {
...
fn slider_component_file_responsibilities_remain_scoped() {
    ...
    assert!(
        !manifest_dir.join("src/slider/render.rs").exists(),
        "Slider should keep render implementation in `view.rs` without `render.rs` drift."
    );
    assert!(
        !manifest_dir.join("src/slider/spec.rs").exists(),
...
fn slider_engineering_contract_marks_spec_serde_path_as_na_for_simple_component_scope() {
    ...
    assert!(
        cargo_source.contains("component-slider = []"),
        "Slider feature should stay lightweight without serde/spec dependency fan-out."
    );
    assert!(
...
fn slider_semantics_suite_is_contract_first_not_snapshot_only() {
    ...
    for required in [
        ...
        "slider_semantic_markers_changed_in_view_must_be_covered_by_semantics_tests",
        "data-state=move || state.get().phase_attr",
        "data-control-mode=control_mode_attr",
        "data-value-source=value_source_attr",
        "data-value-change-source=value_change_source_attr",
...
#[test]
fn slider_semantic_markers_changed_in_view_must_be_covered_by_semantics_tests() {
    let view_source = load_source("src/slider/view.rs");
    let semantics_source = load_source("tests/slider_semantics.rs");
