...
#[path = "../../../components/color-swatch-picker/test/semantics.rs"]
mod color_swatch_picker_local_semantics;
...
fn color_swatch_picker_does_not_expose_logic_or_view_modules() {
    let source = load_source("../../components/color-swatch-picker/src/mod.rs");
...
fn color_swatch_picker_uses_logic_state_model() {
    ...
    assert!(
        !view_source.contains("unwrap_or_else(|| \"ui-color-swatch-picker\".to_string())"),
        "ColorSwatchPicker view should not hold id default fallback; keep default normalization in logic.rs."
    );
    for forbidden in [
        ...
        assert!(
            ...
            "ColorSwatchPicker view should not rebuild state derivation via `{forbidden}`."
        );
    }
}
...
fn color_swatch_picker_state_primitive_source_boundary_is_enforced() {
    ...
    for needle in [
        ...
        assert!(
            logic_source.contains(needle),
            "ColorSwatchPicker logic should source state primitives via `{needle}`."
        );
    }

    assert!(
        view_source.contains("overlay_open::use_controllable_state("),
        "ColorSwatchPicker view should consume controllable state through bridge adapter."
    );

    for forbidden in ["use leptos::", "Signal<", "RwSignal<", "WriteSignal<"] {
        assert!(
            !logic_source.contains(forbidden),
            "ColorSwatchPicker logic should stay framework-agnostic; found `{forbidden}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden),
            "ColorSwatchPicker logic should not depend on business store `{forbidden}`."
        );
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker view should not depend on business store `{forbidden}`."
        );
    }
}
...
fn color_swatch_picker_has_no_async_interaction_contract() {
    ...
    for forbidden in [
        ...
        assert!(
            !logic_source.contains(forbidden),
            "ColorSwatchPicker logic should not define async interaction protocol `{forbidden}`."
        );
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker view should not define async interaction protocol `{forbidden}`."
        );
    }
}
...
fn color_swatch_picker_dx_paradox_keeps_hello_world_simple() {
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "ColorSwatchPicker view should keep simple-by-default API via `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker should not expose internal state as required public API `{forbidden}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "ColorSwatchPicker docs hello-world path should include `{needle}`.",
        );
    }
}
...
fn color_swatch_picker_composite_api_uses_typed_item_spec_only() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "ColorSwatchPicker composite API should be backed by typed item spec evidence `{needle}`.",
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker should not expose parallel-array or implicit-slot API `{forbidden}`."
        );
    }
}
...
fn color_swatch_picker_macro_micro_duality_is_not_applicable_without_dragging() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker view should not introduce drag-loop mechanics `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "ColorSwatchPicker logic should not model drag-loop convergence `{forbidden}`."
        );
        assert!(
            !motion_source.contains(forbidden),
            "ColorSwatchPicker motion should not model drag physics `{forbidden}`."
        );
    }

    assert!(
        view_source.contains("on:click=move |_| aria.handlers.on_radio_click.run(index)"),
        "ColorSwatchPicker should keep discrete click-based selection interaction."
    );
}
...
fn color_swatch_picker_two_pass_geometry_is_not_applicable_without_dom_measurement() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker view should not include DOM measurement two-pass artifact `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "ColorSwatchPicker logic should not include geometry rectification artifact `{forbidden}`."
        );
    }
}
...
fn color_swatch_picker_registration_protocol_is_not_applicable_without_dynamic_items() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker view should not include registration protocol artifact `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "ColorSwatchPicker logic should not include registration protocol artifact `{forbidden}`."
        );
    }

    assert!(
        view_source.contains("swatches: ReadSignal<Vec<ColorSwatchPickerItem>>"),
        "ColorSwatchPicker should consume typed swatches list as ordered source."
    );
    assert!(
        view_source.contains("items")
            && view_source.contains(".into_iter()")
            && view_source.contains(".enumerate()"),
        "ColorSwatchPicker should render options by list order, not set iteration order."
    );
}
...
fn color_swatch_picker_slot_projection_is_not_applicable_without_container_slots() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker view should not include slot projection lifecycle artifact `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "ColorSwatchPicker logic should not include slot projection lifecycle artifact `{forbidden}`."
        );
        assert!(
            !motion_source.contains(forbidden),
            "ColorSwatchPicker motion should not include slot projection lifecycle artifact `{forbidden}`."
        );
    }
    ...
    assert!(
        view_source.contains("items")
            && view_source.contains(".into_iter()")
            && view_source.contains(".enumerate()"),
        "ColorSwatchPicker should render swatches eagerly from ordered list input."
    );
}
...
fn color_swatch_picker_env_streams_are_not_applicable_without_environment_subscriptions() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker view should not include env stream sampling artifact `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "ColorSwatchPicker logic should not include env stream action artifact `{forbidden}`."
        );
    }

    assert!(
        view_source.contains("on:click=move |_| aria.handlers.on_radio_click.run(index)"),
        "ColorSwatchPicker should keep explicit user-triggered selection without env stream side-channel."
    );
}
...
fn color_swatch_picker_event_light_cone_is_not_applicable_without_bulk_collection_ops() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker view should not include large-collection event-light-cone artifact `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "ColorSwatchPicker logic should not include large-collection event-light-cone artifact `{forbidden}`."
        );
    }

    assert!(
        view_source.contains("swatches: ReadSignal<Vec<ColorSwatchPickerItem>>"),
        "ColorSwatchPicker should keep direct typed swatch list input instead of context-bus fanout."
    );
    assert!(
        view_source.contains("items")
            && view_source.contains(".into_iter()")
            && view_source.contains(".enumerate()"),
        "ColorSwatchPicker should render by direct ordered enumeration without selector compression protocol."
    );
}
...
fn color_swatch_picker_causality_bus_is_not_applicable_without_derived_bus_workflow() {
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker view should not include causality-bus artifact `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "ColorSwatchPicker logic should not include causality-bus artifact `{forbidden}`."
        );
    }

    assert!(
        view_source.contains("on:click=move |_| aria.handlers.on_radio_click.run(index)"),
        "ColorSwatchPicker should keep direct user-event to local-state convergence path."
    );
    assert!(
        logic_source.contains("resolve_state(ColorSwatchPickerStateInput {"),
        "ColorSwatchPicker logic should converge state locally through primitive resolution, not derived bus broadcast."
    );
}
...
fn color_swatch_picker_a11y_i18n_contract_is_headless_driven_and_no_view_text_hardcode() {
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "ColorSwatchPicker view should mount headless/i18n contract `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            logic_source.contains(needle),
            "ColorSwatchPicker logic should source text fallback primitive from ui-state-primitives `{needle}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker view should not hardcode user-facing text or duplicate a11y text helpers `{forbidden}`."
        );
    }
}
...
fn color_swatch_picker_exposes_baseline_a11y_and_state_markers() {
    let source = load_source("../../components/color-swatch-picker/src/view.rs");
...
#[test]
fn color_swatch_picker_state_source_marker_values_use_closed_enums() {
    let source = load_source("../../components/color-swatch-picker/src/logic.rs");
    ...
    for enum_value in [
        ...
        assert!(
            source.contains(enum_value),
            "ColorSwatchPicker logic source marker values should stay within closed enum set and include {enum_value}."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            source.contains(needle),
            "ColorSwatchPicker logic should centralize source marker derivation helper `{needle}`."
        );
    }
}
...
fn color_swatch_picker_styles_include_selected_focus_and_disabled_contracts() {
    let source = load_source("../../components/color-swatch-picker/src/styles.rs");
    ...
    for selector in [
        ...
        "--ui-color-swatch-picker-transition-easing: var(--ui-text-field-motion-easing",
...
fn color_swatch_picker_styles_depend_on_explicit_markers_and_runtime_vars_only() {
    ...
    for selector in [
        ...
        assert!(
            ...
            "ColorSwatchPicker styles should branch visual states via explicit marker selector `{selector}`."
        );
    }
    ...
    for forbidden in [":nth-child", ":nth-of-type"] {
        assert!(
            ...
            "ColorSwatchPicker styles should not depend on fragile structure selector `{forbidden}`."
        );
    }

    assert!(
        view_source.contains("style=move || style_vars.get_value()"),
        "ColorSwatchPicker view runtime style should attach CSS variable payload only."
    );

    let style_attr_count = view_source.match_indices("style=").count();
    assert_eq!(
        style_attr_count, 1,
        "ColorSwatchPicker view should keep a single style entrypoint for CSS variable injection."
    );
    ...
    for needle in [
        ...
        assert!(
            ...
            "ColorSwatchPicker motion should only expose runtime CSS variables like `{needle}`."
        );
    }
    ...
    for forbidden in ["background:", "color-mix(", "box-shadow:", "border:"] {
        assert!(
            !motion_source.contains(forbidden),
            "ColorSwatchPicker motion should not inject business style rule `{forbidden}` at runtime."
        );
    }
}
...
fn color_swatch_picker_token_first_style_pipeline_is_wired_through_css_and_ui_root() {
    ...
    assert!(
        ...
        "ColorSwatchPicker styles.rs should remain the static CSS aggregation point."
    );
    assert!(
        styles_source.contains("var(--ui-"),
        "ColorSwatchPicker styles.rs should consume ui-theme token variables via `var(--ui-*)`."
    );
    assert!(
        ...
        "ui-components css registry should aggregate ColorSwatchPicker CSS through feature-gated styles.rs export."
    );
    assert!(
    ...
    assert!(
        view_source.contains("style=move || style_vars.get_value()"),
        "ColorSwatchPicker view runtime style should only carry CSS variable payload."
    );
    ...
    for forbidden in [
        ...
        assert!(
            !styles_source.contains(forbidden),
            "ColorSwatchPicker styles.rs should not depend on utility-first/CSS-in-Rust marker `{forbidden}`."
        );
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker view.rs should not depend on utility-first/CSS-in-Rust marker `{forbidden}`."
        );
    }
}
...
fn color_swatch_picker_styles_use_defensive_variable_fallback_chain() {
    ...
    for required in [
        ...
        assert!(
            ...
            "ColorSwatchPicker styles should keep defensive double-fallback token `{required}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            !styles_source.contains(forbidden),
            "ColorSwatchPicker styles should avoid hard-coded terminal size `{forbidden}`."
        );
    }
    assert!(
        !styles_source.contains("px;"),
        "ColorSwatchPicker styles should avoid raw px terminal values; route terminals through ui-theme fallbacks."
    );
    assert!(
        !contains_hex_color_literal(&styles_source),
        "ColorSwatchPicker styles should avoid hard-coded hex literals."
    );
...
fn color_swatch_picker_visual_desire_gate_reuses_theme_visual_baseline_and_heroui_contracts() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "ColorSwatchPicker docs should keep visual-baseline entry `{needle}`."
        );
    }
...
fn color_swatch_picker_semantics_and_performance_regression_cover_aria_data_focus_and_render_count_measurement()
 {
    let local_semantics = include_str!("../../../components/color-swatch-picker/test/semantics.rs");
...
fn color_swatch_picker_engineering_contract_marks_spec_serde_path_as_na_and_keeps_tracing_runtime_boundaries()
 {
    ...
    let spec_path = workspace_dir.join("components/color-swatch-picker/src/spec.rs");
    assert!(
        !spec_path.exists(),
...
fn color_swatch_picker_semantic_test_matrix_covers_key_paths_without_snapshot_reliance() {
    ...
    let logic_source = load_source("../../components/color-swatch-picker/src/logic.rs");
    let mod_source = load_source("../../components/color-swatch-picker/src/mod.rs");
    let local_semantics_source = include_str!("color_swatch_picker_semantics.rs");
    ...
    for needle in [
        ...
        assert!(
            view_source.contains(needle),
            "ColorSwatchPicker semantic matrix should cover branch contract `{needle}`."
        );
    }
    ...
    for forbidden in ["web_sys::", "wasm_bindgen", "JsValue"] {
        assert!(
            !view_source.contains(forbidden),
            "ColorSwatchPicker view should keep SSR/non-wasm compatible surface without `{forbidden}`."
        );
        assert!(
            !logic_source.contains(forbidden),
            "ColorSwatchPicker logic should keep SSR/non-wasm compatible surface without `{forbidden}`."
        );
        assert!(
            !mod_source.contains(forbidden),
            "ColorSwatchPicker module should keep SSR/non-wasm compatible export surface without `{forbidden}`."
        );
    }
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "ColorSwatchPicker semantics test suite should not rely on visual snapshot assertion `{forbidden}`.",
        );
    }
}
...
fn color_swatch_picker_semantics_priority_contract_prefers_semantic_assertions_over_snapshot_only()
{
    ...
    let local_semantics_source =
        include_str!("../../../components/color-swatch-picker/test/semantics.rs");
    let legacy_semantics_source = include_str!("color_swatch_picker_semantics.rs");
    let check2_source = load_source("../../components/color-swatch-picker/check2.md");
    let check2_source_mirror = load_source("../../components/color-swatch-picker/src/check2.md");
    let script_source = load_source("../../scripts/check-ui-components-contract-hygiene.sh");
    ...
    for required in [
        ...
        assert!(
            view_source.contains(required),
            "ColorSwatchPicker semantic contract must keep role/aria/data/action marker `{required}`."
        );
    }
...
fn color_swatch_picker_file_placement_discipline_is_strict_for_component_scope() {
    ...
    assert!(
        !path_exists("../../components/color-swatch-picker/src/spec.rs"),
...
fn color_swatch_picker_context_compression_manifest_and_rbi_projection_are_present_and_current() {
    ...
    assert!(
        path_exists("../../components/color-swatch-picker/src/Component.toml"),
        "color-swatch-picker context-compression file should exist: `Component.toml`."
    );
    assert!(
        path_exists("../../components/color-swatch-picker/src/color_swatch_picker.rbi"),
...
fn color_swatch_picker_final_merge_gate_capabilities_are_backed_by_contract_checks() {
    ...
    color_swatch_picker_uses_logic_state_model();
    color_swatch_picker_type_system_and_semantic_markers_keep_machine_readable_contract();
...
fn color_swatch_picker_state_markers_are_observable_and_source_enums_are_closed() {
    color_swatch_picker_exposes_baseline_a11y_and_state_markers();
    color_swatch_picker_state_source_marker_values_use_closed_enums();
}
...
fn color_swatch_picker_check2_has_no_unchecked_checklist_items() {
    ...
    for source in [check2_source.as_str(), check2_source_mirror.as_str()] {
        assert!(
            !source.contains("- [ ]"),
