...
fn resizable_docs_page_includes_api_state_and_source_first_sections() {
    ...
    for needle in [
        ...
        "Copy starter",
        "component-resizable",
        "crates/ui-components/src/resizable/view.rs",
    ] {
        assert!(
            source.contains(needle),
...
#[test]
fn resizable_ui_components_entrypoint_files_follow_boundary_contract() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-components/lib.rs entrypoint contract should include `{needle}`."
        );
    }
    for forbidden in ["pub use web_sys::", "pub use leptos::web_sys::"] {
        assert!(
            !lib_source.contains(forbidden),
            "ui-components/lib.rs should not expose platform-detail type `{forbidden}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-components/css.rs aggregation contract should include `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-components/root.rs contract should include `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-components/active_highlight.rs contract should include `{needle}`."
        );
    }
    for forbidden in [
        "data-slot=\"resizable\"",
    ...
    assert!(
        ...
        "ui-components/src/overlay_open.rs should not exist."
    );
    assert!(
        !manifest_dir.join("src/presence.rs").exists(),
        "ui-components/src/presence.rs should not exist."
    );
    assert!(
        !manifest_dir.join("src/a11y.rs").exists(),
        "ui-components/src/a11y.rs should not exist."
    );
...
fn resizable_platform_tree_shaking_and_cross_layer_guards_hold() {
    ...
    for needle in [
        ...
        assert!(
            lib_source.contains(needle),
            "ui-components lib export contract should include `{needle}`."
        );
    }
    ...
    assert!(
        css_source.contains("#[cfg(feature = \"component-resizable\")]")
            && css_source.contains("out.push_str(crate::resizable::styles::CSS);"),
        "ui-components css aggregation should stay feature-gated for resizable."
    );
    assert!(
        cargo_source.contains("component-resizable = []"),
        "ui-components Cargo features should expose component-resizable."
    );
