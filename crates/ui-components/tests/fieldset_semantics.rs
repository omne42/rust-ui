...
fn fieldset_tree_shaking_keeps_component_feature_and_css_boundaries() {
    ...
    let field_form_source = load_source("src/field_form.rs");
    let css_source = load_source("src/css.rs");
    ...
    assert!(
        field_form_source.contains("#[cfg(feature = \"component-fieldset\")]")
            && field_form_source.contains("pub use ui_fieldset as fieldset;"),
        "field_form.rs should feature-gate fieldset export for tree-shaking."
    );
    assert!(
...
fn fieldset_ui_components_fixed_entry_files_follow_layered_boundaries() {
    ...
    for required in [
        ...
        assert!(
            field_form_source.contains(required),
            "fieldset module export should stay feature-gated via `{required}`."
        );
    }
