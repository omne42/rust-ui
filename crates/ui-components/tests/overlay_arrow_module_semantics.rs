...
#[test]
fn overlay_arrow_compat_module_is_removed() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let path = manifest_dir.join("src/overlay_arrow/mod.rs");
    assert!(!path.exists(), "compat module  should not exist.",);
}

#[test]
fn crate_root_does_not_register_overlay_arrow_compat_module() {
    let source = load_source("src/lib.rs");

    assert!(
        !source.contains("pub mod overlay_arrow;"),
