...
fn menu_section_does_not_expose_logic_or_view_modules() {
    ...
    for needle in ["pub mod logic", "pub mod view"] {
        assert!(
            ...
            "MenuSection internals should stay private; found `{needle}`."
        );
    }
}
...
fn menu_section_uses_logic_state_model() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "MenuSection logic should include `{needle}` for centralized state derivation."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "MenuSection view should derive state via logic helpers; missing `{needle}`."
        );
    }
}
...
fn menu_section_emits_spectrum_style_state_data_attributes() {
    ...
    for attr in [
        ...
        "data-slot=\"menu-section-items\"",
        "data-slot=\"menu-section-divider\"",
...
fn menu_section_styles_include_tone_and_state_markers() {
    ...
    for selector in [
        ...
        assert!(
            ...
            "MenuSection styles should include `{selector}` as stable state-marker contracts."
        );
    }
}
...
fn menu_section_supports_group_accessibility_and_items_layout() {
    ...
    for needle in [
        ...
        assert!(
            source.contains(needle),
            "MenuSection should include `{needle}` for accessibility and content composition."
        );
    }
}
