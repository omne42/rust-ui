...
fn well_consumes_state_primitives_and_keeps_component_assembly_local() {
    ...
    for needle in [
        ...
        "logic::normalize_props(logic::WellNormalizeInput {",
        "fallback_aria_label: strings.aria_label.as_ref().to_string(),",
        "let region = region_attrs(normalized.aria_label, lang, dir);",
...
#[test]
fn well_check2_documents_ui_components_entrypoint_rules() {
    let checklist_source = load_source("src/well/check2.md");
    ...
    for required in [
        ...
        assert!(
            checklist_source.contains(required),
            "Well checklist should keep ui-components entrypoint governance rule `{required}`."
        );
    }
}

#[test]
fn well_ui_components_entry_files_keep_feature_gated_public_surface_and_no_platform_leaks() {
...
#[test]
fn well_ui_components_css_registry_remains_feature_gated_and_non_global() {
...
#[test]
fn well_ui_components_forbidden_entrypoint_files_are_absent_and_headless_paths_are_present() {
...
fn well_entrypoints_check_script_covers_fixed_entrypoint_contract() {
    ...
    let needle = "cargo test -p ui-components --test well_semantics --no-default-features --features component-well,inject-css well_ui_components_forbidden_entrypoint_files_are_absent_and_headless_paths_are_present";
    assert!(
        script_source.contains(needle),
...
fn well_view_macro_complexity_is_small_and_does_not_require_semantic_subrenders() {
    ...
    let script_needle = "cargo test -p ui-components --test well_semantics --no-default-features --features component-well,inject-css well_view_macro_complexity_is_small_and_does_not_require_semantic_subrenders";
    assert!(
...
fn well_view_functional_split_prefers_no_extra_local_components_for_simple_layout() {
    ...
    let script_needle = "cargo test -p ui-components --test well_semantics --no-default-features --features component-well,inject-css well_view_functional_split_prefers_no_extra_local_components_for_simple_layout";
    assert!(
        script_source.contains(script_needle),
        "view-macro gate script should include `{script_needle}`."
    );
