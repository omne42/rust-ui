...
fn spacer_docs_page_covers_primary_playgrounds() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_docs_page_covers_primary_playgrounds`",
...
fn spacer_docs_interactive_playground_contract_is_stable() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_docs_page_covers_primary_playgrounds` + `spacer_docs_interactive_playground_contract_is_stable`",
...
fn spacer_docs_playgrounds_lock_state_matrix_contract_values() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_docs_playgrounds_lock_state_matrix_contract_values` + `spacer_docs_are_synced_and_copy_paste_ready`",
...
fn spacer_semantics_tests_prioritize_contract_assertions() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_semantics_tests_prioritize_contract_assertions`",
...
fn spacer_e2e_selectors_and_wasm_wait_strategy_are_semantic_and_stable() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_e2e_selectors_and_wasm_wait_strategy_are_semantic_and_stable`",
    ] {
        assert!(
            check2_source.contains(needle),
...
fn spacer_critical_flow_regression_scope_is_explicit_and_traceable() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_critical_flow_regression_scope_is_explicit_and_traceable`",
    ] {
        assert!(
            check2_source.contains(needle),
...
fn spacer_tree_shaking_feature_chain_is_component_scoped() {
    ...
    assert!(
        ...
        "ui-components feature table should define an independent `component-spacer` gate."
    );
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-components public surface should keep spacer behind feature gate via `{needle}`."
        );
    }
...
fn spacer_dx_fast_style_feedback_and_workbench_contract_is_stable() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_dx_fast_style_feedback_and_workbench_contract_is_stable`",
    ] {
        assert!(
            check2_source.contains(needle),
...
#[test]
fn spacer_ui_components_entry_files_are_wired_correctly() {
    let lib_source = load_source("src/lib.rs");
    let css_source = load_source("src/css.rs");
    ...
    for needle in [
        ...
        assert!(
            lib_source.contains(needle),
            "ui-components lib entry should expose spacer through feature-gated API via `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            css_source.contains(needle),
            "ui-components css entry should feature-gate spacer CSS via `{needle}`."
        );
    }
    ...
    for absent in ["overlay_open.rs", "presence.rs", "a11y.rs"] {
        assert!(
            ...
            "ui-components root should not contain deprecated shared primitive shim `{absent}`."
        );
    }
...
fn spacer_agent_contract_markers_are_machine_readable() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_agent_contract_markers_are_machine_readable`",
    ] {
        assert!(
            check2_source.contains(needle),
...
fn spacer_streaming_scope_is_explicitly_snapshot_only() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_streaming_scope_is_explicitly_snapshot_only`",
...
fn spacer_docs_are_synced_and_copy_paste_ready() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_docs_are_synced_and_copy_paste_ready`",
...
fn spacer_heroui_benchmark_docs_sync_contract_is_explicit() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_heroui_benchmark_docs_sync_contract_is_explicit`",
...
fn spacer_forbidden_antipatterns_contract_is_locked() {
    ...
    for needle in [
        ...
        "- [x] 公共 API 泄露底层实现细节类型。",
        "- [x] 用临时补丁破坏跨组件一致性。",
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_forbidden_antipatterns_contract_is_locked`",
...
fn spacer_final_merge_gate_contracts_are_satisfied() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_final_merge_gate_contracts_are_satisfied`",
        "该条暂不做 smoke",
    ] {
        assert!(
            check2_source.contains(needle),
    ...
    for needle in [
        "fn spacer_ui_components_layering_contract_is_enforced()",
    ...
    for needle in [
        "cargo clippy -p ui-components --no-default-features --features component-spacer --test spacer_semantics -- -D warnings",
        "cargo test -p ui-components --no-default-features --features component-spacer --test spacer_semantics",
        "cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-spacer",
    ] {
        assert!(
            check2_source.contains(needle),
...
fn spacer_ui_components_layering_contract_is_enforced() {
    ...
    for forbidden in ["web_sys::", "wasm_bindgen", "HtmlElement", "JsValue"] {
        ...
        assert!(
            ...
            "ui-components public re-export should not include platform detail `{forbidden}`."
        );
    }
}
...
fn spacer_engineering_capabilities_stay_unified_and_runtime_agnostic() {
    ...
    for needle in [
        ...
        "回归：`crates/ui-components/tests/spacer_semantics.rs::spacer_engineering_capabilities_stay_unified_and_runtime_agnostic`",
    ] {
        assert!(
            check2_source.contains(needle),
