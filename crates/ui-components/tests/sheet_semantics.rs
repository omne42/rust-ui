...
fn sheet_is_exported_and_exposes_state_contracts() {
    ...
    let logic_source = load_source("src/sheet/logic.rs");
...
fn sheet_api_naming_and_control_contract_are_consistent_for_component_scope() {
    let view = load_source("src/sheet/view.rs");
...
fn sheet_state_markers_are_observable_queryable_and_closed_set() {
    let view = load_source("src/sheet/view.rs");
    let logic = load_source("src/sheet/logic.rs");
...
fn sheet_styles_consume_ui_theme_overlay_tokens() {
    let styles = load_source("src/sheet/styles.rs");
...
#[test]
fn sheet_semantics_contract_tests_prioritize_semantics_over_snapshots() {
...
fn sheet_component_files_respect_layered_responsibilities() {
    let module = load_source("src/sheet/mod.rs");
    let logic = load_source("src/sheet/logic.rs");
    let styles = load_source("src/sheet/styles.rs");
    let view = load_source("src/sheet/view.rs");
    let motion = load_source("src/sheet/motion.rs");
...
fn sheet_does_not_define_spec_module_for_simple_component_scope() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let module_source = load_source("src/sheet/mod.rs");
    let sheet_spec = manifest_dir.join("src/sheet/spec.rs");
    let button_spec = manifest_dir.join("src/button/spec.rs");

    assert!(
        !sheet_spec.exists(),
        "Sheet is a simple component and should not define `src/sheet/spec.rs` without explicit complex schema-contract need."
    );
    assert!(
...
fn sheet_source_first_docs_are_copy_paste_ready_and_traceable() {
    ...
    for needle in [
        ...
        "crates/ui-components/src/sheet/mod.rs",
        "crates/ui-components/src/sheet/logic.rs",
        "crates/ui-components/src/sheet/view.rs",
        "crates/ui-components/src/sheet/styles.rs",
        "crates/ui-components/src/sheet/motion.rs",
...
fn sheet_tree_shaking_contract_is_feature_gated_and_budgeted() {
    ...
    assert!(
        ...
        "sheet module export should stay feature-gated in lib.rs."
    );
    assert!(
...
fn sheet_cross_platform_compile_contract_has_explicit_cfg_and_no_non_wasm_web_sys_usage() {
    let mod_source = load_source("src/sheet/mod.rs");
    let logic_source = load_source("src/sheet/logic.rs");
    let styles_source = load_source("src/sheet/styles.rs");
    let view_source = load_source("src/sheet/view.rs");
    let motion_source = load_source("src/sheet/motion.rs");
...
fn sheet_headless_web_ssr_mutex_guard_is_preserved() {
    let sheet_view = load_source("src/sheet/view.rs");
...
fn sheet_ui_motion_non_wasm_stub_contract_is_predictable_and_tooling_safe() {
    ...
    let sheet_motion = load_source("src/sheet/motion.rs");
    let platform_script = load_source("../../scripts/check-ui-components-platforms.sh");
...
fn sheet_performance_governance_contract_is_budgeted_traceable_and_blocking() {
    ...
    let perf_source = load_source("../ui-headless/src/perf.rs");
    let coverage_source = load_source("../../e2e/tests/docs_app_components_coverage.spec.mjs");
    let script_source = load_source("../../scripts/check-ui-components-performance.sh");
    let todo_source = load_source("../../docs/plan/TODO.md");
    let check2_source = load_source("src/sheet/check2.md");
    let view_source = load_source("src/sheet/view.rs");
    let motion_source = load_source("src/sheet/motion.rs");
...
fn sheet_engineering_and_entry_boundaries_stay_consistent() {
    ...
    for needle in ["#[cfg(feature = \"component-sheet\")]", "pub mod sheet;"] {
        assert!(
            lib.contains(needle),
...
fn sheet_check2_marks_final_merge_gates_complete_including_full_gate() {
    ...
    for needle in [
        ...
        "sheet_semantics_contract_tests_prioritize_semantics_over_snapshots",
        "sheet_anti_pattern_new_params_follow_naming_type_default_and_semantic_contract",
