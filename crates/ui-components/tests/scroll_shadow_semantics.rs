...
fn scroll_shadow_token_first_static_style_contract_is_satisfied() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-components/css.rs should aggregate ScrollShadow css via feature gate `{needle}`."
        );
    }
...
fn scroll_shadow_tree_shaking_contract_is_feature_gated_and_css_scoped() {
    ...
    for needle in [
        ...
        "cargo tree -e features -p ui-components --no-default-features --features component-scroll_shadow,inject-css",
        "cargo tree -e features -i ui-components -p web-demo",
        "cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-scroll_shadow,inject-css",
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-components/Cargo.toml should keep scroll-shadow feature token `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-components/lib.rs should keep feature-gated scroll-shadow module token `{needle}`."
        );
    }
    ...
    for (idx, _) in lib_source.match_indices("pub mod scroll_shadow;") {
        ...
        assert_eq!(
            ...
            "ui-components/lib.rs must gate `pub mod scroll_shadow;` with feature cfg."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            css_source.contains(needle),
            "ui-components/css.rs should keep feature-gated css aggregation token `{needle}`."
        );
    }
    ...
    for (idx, _) in css_source.match_indices("out.push_str(crate::scroll_shadow::styles::CSS);") {
        ...
        assert_eq!(
            prev_line, "#[cfg(feature = \"component-scroll_shadow\")]",
            "ui-components/css.rs must gate scroll-shadow css aggregation with feature cfg."
        );
    }
}
...
fn scroll_shadow_platform_contract_uses_explicit_cfg_and_keeps_non_wasm_paths_websys_free() {
    ...
    for needle in [
        ...
        "cargo check -p ui-components --target wasm32-unknown-unknown --no-default-features --features component-scroll_shadow,inject-css",
        "cargo check -p ui-components --no-default-features --features component-scroll_shadow,inject-css",
...
fn scroll_shadow_headless_web_ssr_mutex_contract_is_preserved() {
    ...
    assert!(
        ...
        "ui-components should depend on ui-headless via centralized crate dependency."
    );
    assert!(
        !components_cargo_source
            .contains("ui-headless = { path = \"../ui-headless\", features = [\"web\", \"ssr\"] }"),
        "ui-components must not force-enable both ui-headless web+ssr features."
    );
...
fn scroll_shadow_check2_marks_source_first_copy_paste_ready_complete() {
    ...
    for needle in [
        ...
        "component-scroll_shadow",
        "crates/ui-components/src/scroll_shadow/{mod,logic,view,styles}.rs",
...
fn scroll_shadow_wasm_debug_capability_reuses_global_trace_and_stays_feature_isolated() {
    ...
    for needle in [
        ...
        assert!(
            cargo_source.contains(needle),
            "ui-components Cargo features should keep global wasm-debug opt-in token `{needle}`."
        );
    }
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-components crate root should keep shared wasm-debug isolation marker `{needle}`."
        );
    }
...
fn scroll_shadow_engineering_contract_stays_spec_free_tracing_aligned_and_runtime_agnostic() {
    ...
    for needle in [
        ...
        "`crates/ui-components/Cargo.toml` 保持 `component-scroll_shadow = []` 且不引入 `dep:serde/dep:serde_json`",
...
#[test]
fn scroll_shadow_ui_components_fixed_entry_files_follow_layered_boundaries() {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let lib_source = load_source("src/lib.rs");
    let css_source = load_source("src/css.rs");
    let root_source = load_source("src/root.rs");
...
fn scroll_shadow_check2_marks_final_merge_gates_complete_with_full_gate_done() {
    ...
    for needle in [
        ...
        "scroll_shadow_reduced_motion_ssr_wasm_contract_is_consistent",
        "scroll_shadow_docs_source_first_copy_paste_ready_with_imports_source_paths_and_sync",
        "$HOME/.cargo/bin/rustfmt crates/ui-components/tests/scroll_shadow_semantics.rs",
        "$HOME/.cargo/bin/cargo test -p ui-components --test scroll_shadow_semantics --no-default-features --features component-scroll_shadow,inject-css",
        "$HOME/.cargo/bin/cargo clippy -p ui-components --test scroll_shadow_semantics --no-default-features --features component-scroll_shadow,inject-css -- -D warnings",
