use std::fs;
use std::path::Path;

fn load_source(rel_path: &str) -> String {
    let manifest_dir = Path::new(env!("CARGO_MANIFEST_DIR"));
    let path = manifest_dir.join(rel_path);
    fs::read_to_string(&path).unwrap_or_else(|e| panic!("read_to_string failed for {path:?}: {e}"))
}

#[test]
fn modal_does_not_expose_logic_or_view_modules() {
    let source = load_source("src/modal/mod.rs");

    for needle in ["pub mod logic", "pub mod view"] {
        assert!(
            !source.contains(needle),
            "Modal internals should stay private; found `{needle}`."
        );
    }
}

#[test]
fn modal_uses_logic_state_model() {
    let view_source = load_source("src/modal/view.rs");
    let logic_source = load_source("src/modal/logic.rs");

    for needle in [
        "pub struct ModalStateInput",
        "pub struct ModalState",
        "pub fn normalize_optional_text(",
        "pub fn normalize_required_text(",
        "pub fn normalize_id_base(",
        "pub fn resolve_state(",
        "pub fn compose_class_name(",
    ] {
        assert!(
            logic_source.contains(needle),
            "Modal logic should include `{needle}` for centralized state derivation."
        );
    }

    for needle in [
        "logic::normalize_id_base(id_base)",
        "logic::normalize_required_text(title, \"Modal\")",
        "logic::normalize_optional_text(description)",
        "logic::resolve_state(ModalStateInput {",
        "logic::compose_class_name(class_name, state)",
    ] {
        assert!(
            view_source.contains(needle),
            "Modal view should derive state via logic helpers; missing `{needle}`."
        );
    }
}

#[test]
fn modal_composes_overlay_with_stable_aria_ids() {
    let source = load_source("src/modal/view.rs");

    for needle in [
        "<Overlay",
        "let title_id = format!(\"{id_base}-title\")",
        "aria_labelledby=title_id.clone()",
        "id=move || title_id_attr.get()",
    ] {
        assert!(
            source.contains(needle),
            "Modal should expose stable a11y id wiring (`{needle}`)."
        );
    }
}

#[test]
fn modal_only_sets_describedby_when_description_exists() {
    let source = load_source("src/modal/view.rs");

    assert!(
        source.contains("if let Some(description) = description"),
        "Modal should branch on description presence rather than emitting empty aria-describedby."
    );

    for needle in [
        "let description_id = format!(\"{id_base}-description\")",
        "aria_describedby=description_id.clone()",
        "id=move || description_id_attr.get()",
    ] {
        assert!(
            source.contains(needle),
            "Modal should wire description ids only on described path (`{needle}`)."
        );
    }
}

#[test]
fn modal_emits_spectrum_style_state_data_attributes() {
    let source = load_source("src/modal/view.rs");

    for attr in [
        "data-slot=\"modal\"",
        "data-state=state.state_attr",
        "data-description=state.description_attr",
        "data-with-description=state.show_description.then_some(\"true\")",
        "data-custom-class=state.has_custom_class_name.then_some(\"true\")",
        "data-slot=\"modal-title\"",
        "data-slot=\"modal-body\"",
    ] {
        assert!(
            source.contains(attr),
            "Modal should expose `{attr}` for Spectrum-style state inspection and styling."
        );
    }
}

#[test]
fn modal_styles_include_state_marker_contracts() {
    let source = load_source("src/modal/styles.rs");

    for selector in [
        ".ui-modal--with-description",
        ".ui-modal[data-state=\"with-description\"]",
        ".ui-modal--title-only",
        ".ui-modal[data-description=\"present\"]",
        ".ui-modal--custom-class",
        ".ui-modal[data-custom-class=\"true\"]",
        ".ui-modal__body",
    ] {
        assert!(
            source.contains(selector),
            "Modal styles should include `{selector}` as stable state-marker contracts."
        );
    }
}

#[test]
fn modal_forwards_motion_and_exit_completion_to_overlay() {
    let source = load_source("src/modal/view.rs");

    for needle in ["motion=motion", "on_exit_complete=on_exit_complete"] {
        assert!(
            source.contains(needle),
            "Modal should forward `{needle}` into Overlay so motion/presence contracts remain intact."
        );
    }
}
