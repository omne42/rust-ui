...
fn surface_token_first_styles_flow_through_css_pipeline_and_ui_root_injection() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "ui-components public css bridge should include `{needle}`."
        );
    }
...
#[test]
fn surface_check2_marks_ui_components_entrypoint_rules_complete() {
    let check2_source = load_source("src/surface/check2.md");
    ...
    for needle in [
        ...
        "surface_ui_components_entry_files_keep_feature_gated_public_surface_and_no_platform_leaks",
        ...
        "surface_ui_components_forbidden_entrypoint_files_are_absent_and_headless_paths_are_present",
    ] {
        assert!(
            check2_source.contains(needle),
            "Surface checklist should keep ui-components entrypoint completion evidence `{needle}`."
        );
    }
}
...
#[test]
fn surface_ui_components_entry_files_keep_feature_gated_public_surface_and_no_platform_leaks() {
    let lib_source = load_source("src/lib.rs");
...
#[test]
fn surface_ui_components_css_registry_remains_feature_gated_and_non_global() {
    let css_source = load_source("src/css.rs");
...
#[test]
fn surface_ui_components_forbidden_entrypoint_files_are_absent_and_headless_paths_are_present() {
...
fn surface_check2_marks_final_merge_gates_complete_with_full_gate_done() {
    ...
    for done in [
        ...
        "/root/.cargo/bin/cargo clippy -p ui-components --no-default-features --features component-surface,inject-css -- -D warnings",
        "TMPDIR=/root/autodl-tmp/zjj/p/rust-ui/.tmp/rust-tmp /root/.cargo/bin/cargo test -p ui-components --test surface_semantics --no-default-features --features component-surface,inject-css",
