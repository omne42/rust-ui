...
fn rac_compatibility_reuses_ui_root_docs_playground() {
    ...
    for needle in ["title=\"UiRoot\"", "slug=\"ui-root\"", "Theme::dark()"] {
        assert!(
            source.contains(needle),
            "ui_root docs should contain `{needle}` for rac compatibility coverage."
        );
    }
}
...
fn rac_module_docs_page_covers_primary_playgrounds() {
    ...
    for needle in [
        ...
        assert!(
            source.contains(needle),
            "ui_root docs should include `{needle}` for rac module primary playground coverage.",
        );
    }
...
fn rac_module_docs_playgrounds_lock_state_matrix_contract_values() {
    ...
    for needle in [
        ...
        assert!(
            source.contains(needle),
            "ui_root docs playgrounds should contain `{needle}` for rac module contracts.",
        );
    }
}
