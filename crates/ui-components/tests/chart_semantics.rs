...
#[test]
fn chart_feature_depends_on_active_highlight_for_minimal_build() {
    let cargo_toml = load_source("Cargo.toml");
    assert!(
        cargo_toml.contains("component-chart = [\"component-active_highlight\"]"),
        "component-chart feature should explicitly depend on component-active_highlight."
    );
}
...
fn chart_state_primitives_are_sourced_from_ui_state_primitives() {
    ...
    let component_logic = load_source("src/chart/logic.rs");
...
fn chart_motion_contract_lives_in_chart_motion_rs() {
    let mod_source = load_source("src/chart/mod.rs");
    let motion_source = load_source("src/chart/motion.rs");
    let view_source = load_source("src/chart/view.rs");
...
fn chart_readme_exists_and_is_copy_paste_ready() {
    let source = load_source("src/chart/README.md");
    ...
    for needle in [
        ...
        "<Chart",
        "ChartPoint::new",
        "crates/ui-components/src/chart/view.rs",
...
fn chart_check2_marks_all_items_complete() {
    let source = load_source("src/chart/check2.md");
    assert!(
        !source.contains("- [ ]"),
...
fn chart_check2_records_layering_and_verification_evidence() {
    ...
    for needle in [
        "component-chart = [\"component-active_highlight\"]",
        "crates/ui-state-primitives/src/chart.rs",
        "crates/ui-headless/src/chart.rs",
        "crates/ui-components/src/chart/motion.rs",
