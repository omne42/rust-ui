...
cargo test -p ui-components --test well_semantics --no-default-features --features component-well,inject-css well_streaming_semantics_are_not_required_for_snapshot_container_scope

echo "[streaming] contract: well checklist pins two-mode streaming definition"
cargo test -p ui-components --test well_semantics --no-default-features --features component-well,inject-css well_check2_documents_streaming_definition_is_llm_output_only_with_two_modes

echo "[streaming] contract: well checklist pins snapshot as baseline capability"
cargo test -p ui-components --test well_semantics --no-default-features --features component-well,inject-css well_check2_documents_snapshot_as_default_baseline_capability

echo "[streaming] contract: well consumes complete snapshot result stably"
cargo test -p ui-components --test well_semantics --no-default-features --features component-well,inject-css well_snapshot_baseline_consumes_complete_result_and_renders_stably

echo "[streaming] contract: well checklist pins streaming required/optional classification"
cargo test -p ui-components --test well_semantics --no-default-features --features component-well,inject-css well_check2_documents_streaming_required_optional_classification_rules

echo "[streaming] contract: well optional streaming scope keeps role/aria/data markers continuous"
cargo test -p ui-components --test well_semantics --no-default-features --features component-well,inject-css well_streaming_optional_scope_keeps_role_aria_and_data_markers_continuous

echo "[streaming] contract: well keeps validation/retry/resilience policy outside component layer"
cargo test -p ui-components --test well_semantics --no-default-features --features component-well,inject-css well_streaming_validation_retry_resilience_boundaries_stay_outside_component_layer
...
cargo test -p ui-components --test scroll_area_semantics --no-default-features --features component-scroll_area,inject-css scroll_area_check2_documents_streaming_definition_is_llm_output_only_with_two_modes
