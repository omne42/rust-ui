...
if ! require_cmd cargo; then
  echo "dev-docs-app: missing cargo; install Rust (recommended: rustup)" >&2
  exit 1
fi

if ! require_cmd trunk; then
  echo "dev-docs-app: missing trunk; install: cargo install trunk" >&2
  exit 1
fi
...
if require_cmd rustup; then
  if ! rustup target list --installed | grep -qx "wasm32-unknown-unknown"; then
    echo "dev-docs-app: missing wasm32 target; install: rustup target add wasm32-unknown-unknown" >&2
    exit 1
  fi
else
  ...
  if [[ ! -d "$sysroot/lib/rustlib/wasm32-unknown-unknown" ]]; then
    cat >&2 <<'EOF'
dev-docs-app: wasm32-unknown-unknown target not installed.
